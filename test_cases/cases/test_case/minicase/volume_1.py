...
# Date: 2018-09-03
# @summary：
#    创建卷时，不指定total_bytes	QUOTA_COMPUTE卷，df显示存储池容量
# @steps:
#    1.创建一个新卷,不指定total_bytes
#    2.get_volumes,查看卷
#    3.df查看
#    4.更新卷，指定total_bytes，始终为QUOTA_COMPUTE卷
# @changelog：
#################################################################
...
def case():
    ...
    for line in stdout.splitlines():
        if FILE_NAME in line:
            str_lst = line.split()
            log.info('volume-totalbytes/1024 : %s  df :%s' % (my_volume_size_byte/1024/1024/1024,
                                                              int(str_lst[1])/1024/1024))
            if (my_volume_size_byte/1024/1024/1024) != int(str_lst[1])/1024/1024:
                rc_lst[sys._getframe().f_lineno] = '3> df size != volume-totalbytes'
            break

    log.info("4> 更新卷，指定total_bytes，始终为QUOTA_COMPUTE卷")
    rc, pscli_info = ob_volume.update_volume(my_volume_id, total_bytes=TOTAL_BYTES)
    if rc == 0:
        rc_lst[sys._getframe().f_lineno] = '4> updata_volume succeed'
    rc, pscli_info = ob_volume.get_volumeinfo_by_id(my_volume_id)
    if rc != 0:
        rc_lst[sys._getframe().f_lineno] = '4> get_volumeinfo_by_id'
    if pscli_info['result']['volumes'][0]['quota_cal_type'] != 'QUOTA_COMPUTE':
        rc_lst[sys._getframe().f_lineno] = '4> quota_cal_type != QUOTA_COMPUTE'
