...
def case():
    ...
    cmd = ("/opt/ltp/runltp -p -q -l /tmp/ltp_dio.result -o /tmp/ltp_dio.out "
           "-C /tmp/ltp_dio.fail -d /mnt/parastor/ltp -f /opt/ltp/runtest/dio")
    log.info(cmd)
    ...
    if 0 != rc:
        ...
        for mem in stdout_lst:
            if mem not in syscalls:
                log.error("Execute command: LTP_dio failed,Please check /tmp/ltp_dio.fail!")
                raise Exception(
                    "Execute command: LTP_dio failed,Please check /tmp/ltp_dio.fail!")
