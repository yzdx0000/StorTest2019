...
def down_net(node_ip, eth_name_lst, ip_mask_lst):
    while True:
        for eth_name in eth_name_lst:
            make_fault.down_eth(node_ip, eth_name)
            sleep_time = random.randint(1, 50)
            log.info('wait %ss for up %s' %(sleep_time, eth_name))
            time.sleep(sleep_time)
            make_fault.up_eth(node_ip, eth_name, ip_mask_lst[0])
            time.sleep(300)
...
def case():
    ...
    vdbench = tool_use.Vdbenchrun(elapsed=3600)
    ...
    if p1.exitcode != 0:
        common.except_exit('vdbench check_write failed')

    make_fault.up_eth(CLIENT_IP[0], eth_name_lst, ip_mask_lst)
