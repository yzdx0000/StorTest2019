...
def up_down(pipein):
    ...
    while True:
        ...
        if 'pull' in line:
            break
    time.sleep(60)
    log.info("\033[1;31;40m在业务运行过程中，将某一lmos节点的全部日志盘拔出并随机删除一块数据盘；\033[0m")
    disk_name_dict, node_id_lst = this.get_disk_nodeid_uuid_usage(is_lmos_jnl_disk=True, remove_all_shared=True)
    this.pullout_disk(disk_name_dict)
    time.sleep(60)
    disk_data, node_id_lst_new = this.get_disk_nodeid_uuid_usage(is_data_disk=True)
    this.del_disk(disk_data)
    while True:
        line = os.read(pipein, 32)
        if 'add' in line:
            break
    ...
    this.del_disk(disk_name_dict, is_shared=True)
    this.add_disk(disk_name_dict, is_shared=True)
    this.add_disk(disk_data, is_shared=False)
