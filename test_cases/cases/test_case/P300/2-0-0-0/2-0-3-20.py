...
def up_down(pipein):
    ...
    while True:
        ...
        if 'pull' in line:
            break
    time.sleep(60)
    log.info("\033[1;31;40m在业务运行过程中，一节点拔出一块数据盘，修复过程中另一节点拔出两块数据盘；\033[0m")
    disk_name_dict, node_id_lst = this.get_disk_nodeid_uuid_usage(is_data_disk=True, disk_num_each_node=1)
    disk_name, node_id_lst_new = this.get_disk_nodeid_uuid_usage(is_data_disk=True, disk_num_each_node=2, node_id_del=node_id_lst)
    this.pullout_disk(disk_name_dict)

    time.sleep(70)

    this.pullout_disk(disk_name)
    while True:
        line = os.read(pipein, 32)
        if 'add' in line:
            break
    ...
    this.insert_disk(disk_name)
    this.del_disk(disk_name)
    this.add_disk(disk_name)
