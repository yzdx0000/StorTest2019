...
#           3、format结束后，执行读写操作，客户端数据网断掉，观察业务情况，过程中ifup客户端数据网；
#           4、使用vdbench检查数据一致性./vdbench -f test -jro；
# @changelog：
#

# testlink case: 3.0-41742
# 当前脚本名称
...
def up_down():
    ...
    log.info("\033[1;31;40m在业务运行过程中，将随机一个独立私有客户端数据网断开；\033[0m")
...
def vdb_jn():
    ...
    this.vdbench_run(VOLUME, run_write_jn=True, ip_list=client_ip_list, is_fault_exit=False)
