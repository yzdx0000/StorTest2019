...
def executing_case():
    """
        1、设置home目录，其他参数默认；
        pscli --command=update_smb_global_config --id=1 --home_dir="parastor:/homes"
    :return:
    """
    ...
    # 本脚本会增加（1）home_dir目录删除检查，（2）update使用不存在的家目录的检查；
    dir_name_1 = "homes-nas_16-0-4-62_dir"
    dir_name_2 = "not_exist_home_dir"
    create_file_path_1 = nas_common.ROOT_DIR + dir_name_1
    create_file_path_2 = nas_common.ROOT_DIR + dir_name_2

    check_result1 = nas_common.create_file(path=create_file_path_1)
    if check_result1["detail_err_msg"] != "":
        log.info("%s" % check_result1["detail_err_msg"])

    check_result2 = nas_common.get_smb_global_configs()
    if check_result2["detail_err_msg"] != "":
        log.error(("%s Failed") % FILE_NAME)
        raise Exception(("%s Failed") % FILE_NAME)
    config = check_result2["result"]["configs"][0]  # 只update第一个
    ...
    check_result3 = nas_common.update_smb_global_config(smb_global_config_id=id,
                                                        home_dir=create_file_path_1)
    if check_result3["detail_err_msg"] != "":
        log.error(("%s Failed") % FILE_NAME)
        raise Exception(("%s Failed") % FILE_NAME)

    check_result4 = nas_common.get_smb_global_configs()
    if check_result4["detail_err_msg"] != "":
        log.error(("%s Failed") % FILE_NAME)
        raise Exception(("%s Failed") % FILE_NAME)
    config = check_result4["result"]["configs"][0]  # 只update第一个
    allow_create_ntfs_acl = config["allow_create_ntfs_acl"]
    enable_alternative_datasource = config["enable_alternative_datasource"]
    enable_change_notify = config["enable_change_notify"]
    enable_dos_attributes = config["enable_dos_attributes"]
    ...
    if allow_create_ntfs_acl != True \
            ...
            or enable_os2style_ex_attrs != False \
            or enable_send_ntlmv2 != False \
            or home_dir != create_file_path_1:
        log.error(("%s Failed") % FILE_NAME)
        raise Exception(("%s Failed") % FILE_NAME)

    # （1）尝试删除create_file_path_1目录，预期删除失败
    test_path_1 = os.path.join(NAS_PATH, dir_name_1)
    cmd = "rm -rf %s" % test_path_1
    rc, stdout, stderr = shell.ssh(nas_common.RANDOM_NODE_IP, cmd)
    if rc == 0: # 如果删除成功，则失败
        log.info("rc = %s" % (rc))
    ...
    # （2）使用不存在的目录update_smb_global_config，预期update失败
    check_result5 = nas_common.update_smb_global_config(smb_global_config_id=id,
                                                        home_dir=create_file_path_2)
