...
#   3、添加vip1池，输入64个字符的域名，预期创建失败；
#   4、添加vip2池，输入63个字符的域名，预期创建成功；
#   5、通过pscli --command=get_vip_address_pools查看与配置的信息是否匹配；
...
def case():
    ...
    '''3> 添加vip1池，输入65个字符的域名，预期创建失败'''
    allocation_method_list = ['STATIC', 'DYNAMIC']
    used_protocol = 'NAS'
    used_method = random.choice(allocation_method_list)
    abnormal_name_list = []
    for i in range(37):
        abnormal_name_list.append(str(i))
        abnormal_name_str = ''.join(abnormal_name_list)
        exe_info = nas_common.add_vip_address_pool(subnet_id=subnet_id, domain_name=abnormal_name_str,
                                                   vip_addresses=nas_common.VIP_ADDRESSES, supported_protocol=used_protocol,
                                                   allocation_method='DYNAMIC')
        if exe_info["err_msg"] != "" or exe_info["detail_err_msg"] != "":
            log.error('add vip pool %s failed !!!' % abnormal_name_str)
    ...
    normal_name_list = []
    for i in range(30):
        normal_name_list.append(str(i))
    normal_name_str = ''.join(normal_name_list) + '.com'
    exe_info = nas_common.add_vip_address_pool(subnet_id, normal_name_str, nas_common.VIP_ADDRESSES,
                                               used_protocol, used_method)
    if exe_info["err_msg"] != "" or exe_info["detail_err_msg"] != "":
        common.except_exit('add vip address pool failed !!! !!!')
    ...
    if vip_pool_info['domain_name'] != normal_name_str or vip_pool_info['vip_addresses'][0] != nas_common.VIP_ADDRESSES:
