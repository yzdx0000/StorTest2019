...
def executing_case1():
    ...
    access_zone_name = "access_zone_16_0_4_104"
    node = common.Node()
    ...
    global access_zone_id_16_0_4_104
    access_zone_id_16_0_4_104 = msg1["result"]

    # 2> 启动NAS
    log.info("\t[ enable_nas ]")
    msg2 = nas_common.enable_nas(access_zone_id=access_zone_id_16_0_4_104, protocol_types="NFS")
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        common.except_exit('%s enable_nas failed!!!' % node_ip)

    # 3> 查看NAS是否按配置启动
    log.info("\t[ get_access_zones ]")
    check_result3 = nas_common.get_access_zones(ids=access_zone_id_16_0_4_104)
    name = check_result3["result"]["access_zones"][0]["name"]
    if name != access_zone_name:
        common.except_exit('%s enable_nas failed!!!' % node_ip)
    global auth_provider_id_16_0_4_104
    auth_provider_id_16_0_4_104 = check_result3["result"]["access_zones"][0]["auth_provider_id"]

    """1> 检查目录并creat file"""
    log.info("\t[ case1 create_file ]")
    global nfs_path
    nfs_path = nas_common.ROOT_DIR + "nfs_dir_16_0_4_104"
    global NAS_PATH
    NAS_PATH = get_config.get_one_nas_test_path() + "/nfs_dir_16_0_4_104"
    ...
    nfs_export_name = "nfs_exp_test_16_0_4_104"
    global description_nfs
    description_nfs = 'old_export_description'
    cmd = "create_nfs_export --access_zone_id=%s " \
          "--export_name=%s --export_path=%s --description=%s" % (access_zone_id_16_0_4_104, nfs_export_name,
                                                                  nfs_path, description_nfs)
    check_result1 = nas_common.create_nfs_export(access_zone_id=access_zone_id_16_0_4_104, export_name=nfs_export_name,
    ...
    global nfs_export_id_16_0_4_104
    nfs_export_id_16_0_4_104 = msg1["result"]
...
def executing_case2():
    ...
    cmd = "get_nfs_exports --ids=%s" % nfs_export_id_16_0_4_104
    check_result = nas_common.get_nfs_exports(ids=nfs_export_id_16_0_4_104)
    ...
    if access_zone_id != access_zone_id_16_0_4_104:
...
def executing_case3():
    ...
    cmd = "add_nfs_export_auth_clients --export_id=%s " \
          "--name=%s --permission_level=%s" % (
              nfs_export_id_16_0_4_104, auth_clients_name, auth_clients_permission_level)
    check_result = nas_common.add_nfs_export_auth_clients(export_id=nfs_export_id_16_0_4_104,
    ...
    global nfs_export_clients_id_16_0_4_104
    nfs_export_clients_id_16_0_4_104 = msg["result"][0]
...
def executing_case4():
    ...
    cmd = "get_nfs_export_auth_clients --export_ids=%s" % nfs_export_id_16_0_4_104
    check_result = nas_common.get_nfs_export_auth_clients(export_ids=nfs_export_id_16_0_4_104)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    msg = check_result
    if msg["err_msg"] != "" or msg["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    ...
    if id != nfs_export_clients_id_16_0_4_104:
        raise Exception('%s id failed!!!' % node_ip)
    if export_id != nfs_export_id_16_0_4_104:
...
def executing_case5():
    ...
    cmd = "ssh %s ls /mnt/nfs_dir_16_0_4_104" % auth_clients_name
    rc, stdout, stderr = shell.ssh(node_ip, cmd)
    if stdout == "":
        cmd = "ssh %s umount -l /mnt/nfs_dir_16_0_4_104" % auth_clients_name
        rc, stdout, stderr = shell.ssh(node_ip, cmd)
        cmd = "ssh %s rm -rf /mnt/nfs_dir_16_0_4_104" % auth_clients_name
        rc, stdout, stderr = shell.ssh(node_ip, cmd)
        if rc != 0:
            log.error("WARNING: \ncmd = %s\nstdout = %s\nstderr = %s" % (cmd, stdout, stderr))
            raise Exception('%s rm -rf client file failed!!!' % node_ip)
    cmd = "ssh %s mkdir /mnt/nfs_dir_16_0_4_104" % auth_clients_name
    rc, stdout, stderr = shell.ssh(node_ip, cmd)
    if rc != 0:
        log.info("rc = %s" % rc)
        log.info("WARNING: \ncmd = %s\nstdout = %s\nstderr = %s" % (cmd, stdout, stderr))
    ...
    rc = 1
    while rc != 0:
        cmd1 = "ssh %s mount -t nfs %s:%s /mnt/nfs_dir_16_0_4_104" % (auth_clients_name, node_ip, NAS_PATH)
        ...
        if int(during_time) >= 15:
            ...
            raise Exception('%s mount file failed and timeout 15s!!!' % node_ip)
        log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd1, stdout))
        time.sleep(5)

    """3> 测试r"""
    log.info("\t[ case5 test-r ]")
    cmd1 = "ssh %s ls /mnt/nfs_dir_16_0_4_104" % auth_clients_name
    rc, stdout, stderr = shell.ssh(node_ip, cmd1)
    print stdout
    if rc != 0 or stdout == "":
        log.info("rc = %s" % rc)
    ...
    cmd2 = "ssh %s touch /mnt/nfs_dir_16_0_4_104/test2" % auth_clients_name
    ...
    """5> 删除test-w的文件"""
    log.info("\t[ case5 删除test-w的文件 ]")
    cmd1 = "ssh %s rm -rf /mnt/nfs_dir_16_0_4_104/test2" % auth_clients_name
    rc, stdout, stderr = shell.ssh(node_ip, cmd1)
    print stdout
    if rc != 0:
        log.info("rc = %s" % rc)
    ...
    cmd1 = "ssh %s umount -fl /mnt/nfs_dir_16_0_4_104" % auth_clients_name
    rc, stdout, stderr = shell.ssh(node_ip, cmd1)
    print stdout
    if rc != 0:
        log.info("rc = %s" % rc)
    ...
    cmd1 = "ssh %s rm -rf /mnt/nfs_dir_16_0_4_104 " % auth_clients_name
    rc, stdout, stderr = shell.ssh(node_ip, cmd1)
    print stdout
    if rc != 0:
        log.info("rc = %s" % rc)
