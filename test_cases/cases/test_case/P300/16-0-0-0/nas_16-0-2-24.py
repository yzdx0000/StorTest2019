...
# date 2018-08-08
# @summary：
# 16_0_2_24     修改输入错误的访问区id
# @steps:
# 1> 创建三节点访问分区
# 2> 查看三节点访问分区信息
# 3> 启动NAS
# 4> 查看NAS服务状态
# 5> 修改三节点访问分区为不存在的访问分区
# @changelog：
#
#######################################################
import os
...
def executing_case1():
    ...
    access_zone_name = "access_zone_16_0_2_24"
    node = common.Node()
    ids = node.get_nodes_id()
    m = len(ids)
    access_zone_node_id_16_0_2_24 = ','.join(str(p) for p in ids)
    node_name_list = nas_common.get_node_name_list(ids)
    cmd = "create_access_zone "
    msg1 = nas_common.create_access_zone(node_ids=access_zone_node_id_16_0_2_24, name=access_zone_name)
    ...
    msg1 = nas_common.update_access_zone(access_zone_id=access_zone_id,
                                         node_ids=access_zone_node_id_16_0_2_24)
    if msg1["err_msg"] != "ILLEGAL_ARGUMENT" or \
       msg1["detail_err_msg"].find("Can not find the node id") == -1:
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, msg1))
