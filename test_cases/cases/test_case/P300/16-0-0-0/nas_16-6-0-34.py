...
#    7、针对目录配置inode数为200的硬阈值，100软阈值（宽限时间为150s），50建议阈值
#    8、客户端尝试上传210文件（测试硬阈值是否生效）
#    9、删除部分文件，保留120个文件，超过宽限时间后，尝试继续写入文件（测试软阈值是否生效）
#    10、删除配额及文件数据
#    11、清理环境
#
# @changelog：
...
def case():
    ...
    rc, check_result = quota_common.create_one_quota(path=quota_path,
                                                     ...
                                                     filenr_suggest_threshold=suggest_file_num,
                                                     filenr_soft_threshold=soft_file_num,
                                                     filenr_grace_time=grace_time,
                                                     filenr_hard_threshold=hard_file_num)
    ...
    """再次创建并上传120个文件，触发grace_time开始计时"""
    excess_file_num = 120
    for i in range(excess_file_num):
