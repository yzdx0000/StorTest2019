...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)              # /mnt/volume/nas/nas_16_0_0_63
...
def executing_case1():
    ...
    check_result2 = nas_common.add_auth_provider_ldap(name="nas_16_0_0_63_ldap", base_dn=LDAP_BASE_DN,
    ...
    check_result2 = nas_common.get_auth_providers_ldap(ids=id_16_0_0_63)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    msg2 = check_result2
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    ...
    check_result2 = nas_common.check_auth_provider(provider_id=id_16_0_0_63)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    msg2 = check_result2
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    ...
    # 4. 删除认证
    log.info("\t[case4 delete_auth_provider]")
    wait_time1 = random.randint(1, 3)
    time.sleep(wait_time1)
    cmd = "delete_auth_providers "
    check_result2 = nas_common.delete_auth_providers(ids=id_16_0_0_63)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    msg2 = check_result2
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
