...
nfs_path = os.path.join(nas_common.NAS_PATH, FILE_NAME)                  # /mnt/wangguanglin/nas_test_dir/nas_16_4_1_34
create_file_path = os.path.join(nas_common.ROOT_DIR, FILE_NAME)          # wangguanglin:/nas_test_dir/nas_16_4_1_34


def case():
    """若访问分区已经创建好，该步骤需要注释掉"""
    """创建访问分区"""
    access_zone_name = "nas_16_4_1_34_access_zone_name1"
    ob_node = common.Node()
    node_ids = []
    node_ids = ob_node.get_nodes_id()
    print node_ids
    access_zone_node_id_16_4_1_34 = ','.join(str(p) for p in node_ids)
    check_result1 = nas_common.create_access_zone(node_ids=access_zone_node_id_16_4_1_34, name=access_zone_name)
    if check_result1["err_msg"] != "" or check_result1["detail_err_msg"] != "":
        log.error('%s Failed ' % FILE_NAME)
        raise Exception('%s create_access_zone failed!!!' % FILE_NAME)
    id_16_4_1_34 = check_result1["result"]

    """配置共享目录、授权协议、对外IP地址、授权客户端IP"""
    """enable nas"""
    check_result2 = nas_common.enable_nas(access_zone_id=id_16_4_1_34)
    ...
    """查看NAS是否按配置启动"""
    check_result3 = nas_common.get_access_zones(ids=id_16_4_1_34)
    name = check_result3["result"]["access_zones"][0]["name"]
    if name != access_zone_name:
        raise Exception('%s enable_nas failed!!!' % FILE_NAME)
    auth_provider_id_16_4_1_34 = check_result3["result"]["access_zones"][0]["auth_provider_id"]
    ...
    """添加本地用户组/用户"""
    check_result5 = nas_common.create_auth_group(auth_provider_id=auth_provider_id_16_4_1_34,
                                                 name="group_16_4_1_34")

    if check_result5["err_msg"] != "" or check_result4["detail_err_msg"] != "":
        log.error('%s Failed' % (FILE_NAME))
        raise Exception('%s create_auth_group Failed!!!' % FILE_NAME)
    primary_group_id_16_4_1_34 = check_result5["result"]
    """添加本地用户"""
    check_result6 = nas_common.create_auth_user(auth_provider_id=auth_provider_id_16_4_1_34,
                                                name='user_16_4_1_34', password='111111',
                                                primary_group_id=primary_group_id_16_4_1_34)
    ...
    check_result7 = nas_common.create_nfs_export(access_zone_id=id_16_4_1_34,
    ...
    nfs_export_id_16_4_1_34 = check_result7["result"]
    ...
    check_result8 = nas_common.add_nfs_export_auth_clients(export_id=nfs_export_id_16_4_1_34,
    ...
    """nfs客户端以4M块大小分别写入10T文件的文件头，文件偏移60%位置，文件末尾，并验证移动文件的正确性"""
    offset1=0
    ...
    log.info('nfs客户端移动10T文件')
    cmd = "ssh %s mv %s %s" %(auth_clients_name1,nfs_file1,nfs_file2)
    rc, stdout = common.run_command(auth_clients_name1, cmd)
    common.judge_rc(rc, 0,'nfs client  %s copy  %s failed!!!' % (auth_clients_name1, offset1))

    log.info('nfs客户端以4M块大小写入10T文件的文件头,并验证移动文件的正确性')
    """获取nfs客户端上文件的md5值"""
    cmd21 = "ssh %s python %s %s %s %s %s" \
           % (auth_clients_name1,verify_file, nfs_file2,offset1,block_size,r_w)
    ...
    log.info('nfs客户端以4M块大小写入10T文件偏移60%位置,并验证移动文件的正确性')
    offset2=6597069766656 # 10T*0.6=10*1024*1024*1024*1024*0.6=10995116277760*0.6=6597069766656

    """获取nfs客户端上文件的md5值"""
    cmd21 = "ssh %s python %s %s %s %s %s" \
           % (auth_clients_name1,verify_file, nfs_file2,offset2,block_size,r_w)
    rc, stdout = common.run_command(auth_clients_name1, cmd21)
    ...
    log.info('nfs客户端以4M块大小写入10T文件末尾,并验证移动写入文件的正确性')
    offset3=10995112083456 # 10T-4M=10*1024*1024*1024*1024-4*1024*1024=10995112083456
    """获取nfs客户端上文件的md5值"""
    cmd21 = "ssh %s python %s %s %s %s %s" \
           % (auth_clients_name1,verify_file, nfs_file2,offset3,block_size,r_w)
    rc, stdout = common.run_command(auth_clients_name1, cmd21)
