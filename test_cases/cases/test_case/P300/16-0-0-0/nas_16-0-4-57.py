...
def executing_case():
    """
        1、删除正在使用的smb共享目录；
        pscli --command=delete_smb_exports --ids=x
    :return:
    """
    ...
    # 正常删除共享目录（使用中）【李阳说可以删除，页面会有风险告警，用例预期不可删除】，见jira P300-1345
    # 此处按可以删除写脚本

    create_file_path = nas_common.ROOT_DIR + "nas_16-0-4-57_dir"
    export_name = "nas_16_0_4_57_smb_export_name"
    ...
    # 删除smb exports的同时也会删除smb export auth clients
    check_result5 = nas_common.delete_smb_exports(ids=result)
    if check_result5["detail_err_msg"] != "":
        log.error(("%s Failed") % FILE_NAME)
        raise Exception(("%s Failed") % FILE_NAME)

    check_result6 = nas_common.get_smb_exports(ids=result)
    if check_result6["detail_err_msg"].find("Can not find smb export by id") == -1:
        log.error(("%s Failed") % FILE_NAME)
        raise Exception(("%s Failed") % FILE_NAME)
    else:
