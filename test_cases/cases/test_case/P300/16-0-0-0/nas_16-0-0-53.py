...
# 16-0-0-53          修改认证名称测试
# @steps:
# case 1、集群添加ldap认证，填写正确的IP地址、端口号和基准DN；
...
# case4、修改认证名称；
# pscli --command=update_auth_provider_ldap --name=ldap_test1
# case5、通过命令pscli --command=get_auth_provider_ldap查看ldap认证配置是否修改成功；
# case6、通过命令行pscli --command=check_auth_provider --ids=x查看认证配置信息是否能够正确的连接ldap认证；
# @changelog：
#
#######################################################
import os
...
def executing_case1():
    # 1、集群添加ldap认证，填写正确的IP地址、端口号和基准DN
    ...
    check_result2 = nas_common.add_auth_provider_ldap(name="nas_16_0_0_53_ldap", base_dn=LDAP_BASE_DN,
    ...
    check_result2 = nas_common.get_auth_providers_ldap(ids=id_16_0_0_53)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    msg2 = check_result2
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    ...
    time.sleep(wait_time1)
    auth_providers_ldap = nas_common.get_auth_providers_ldap(ids=id_16_0_0_53)
    id_id = auth_providers_ldap["result"]["auth_providers"][0]["id"]
    cmd = "check_auth_provider "
    check_result2 = nas_common.check_auth_provider(provider_id=id_id)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    msg2 = check_result2
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    ...
    check_result2 = nas_common.update_auth_provider_ldap(name="nas_16_0_0_53_ldap_update", provider_id=id_id)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    msg2 = check_result2
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    ...
    # 5、通过命令pscli --command=get_auth_provider_ldap查看ldap认证配置是否修改成功
    ...
    if ip_addresses != LDAP_IP:
        raise Exception('%s ip_addresses error!!!' % node_ip)
    if name != "nas_16_0_0_53_ldap_update":
        raise Exception('%s name error!!!' % node_ip)
    if port != 389:
        raise Exception('%s port error!!!' % node_ip)
    if type != "LDAP":
