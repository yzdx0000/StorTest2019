...
def executing_case1():
    ...
    msg2 = nas_common.add_auth_provider_ldap(name=ldap_name,
    ...
    ldap_id_16_0_2_9 = msg2["result"]

    """2> 使用LDAP认证创建访问分区"""
    log.info("\t[ 2.create_access_zone ]")
    access_zone_name = "access_zone_16_0_2_9"
    node = common.Node()
    ids = node.get_nodes_id()
    print ids
    access_zone_node_id_16_0_2_9 = ','.join(str(p) for p in ids)
    node_name_list = nas_common.get_node_name_list(ids)
    cmd = "create_access_zone"
    msg1 = nas_common.create_access_zone(node_ids=access_zone_node_id_16_0_2_9,
                                         name=access_zone_name,
                                         auth_provider_id=ldap_id_16_0_2_9)
    ...
    check_result3 = nas_common.get_auth_providers_ldap(ids=ldap_id_16_0_2_9)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result3))
    auth_providers_ldap_version = check_result3["result"]["auth_providers"][0]["version"]
    if check_result3["detail_err_msg"] != "" or check_result3["err_msg"] != "":
