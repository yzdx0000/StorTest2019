...
# date 2018-04-27
# @summary：
# 16-0-4-96     输入不存在的共享目录ID
# @steps:
# case1、使用不存在的ID来修改一个NFS共享；
# pscli --command=update_nfs_export --id=250 --description=new_export_description
# 注：共享ID 250不存在；
# @changelog：
#
#######################################################
...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)  # /mnt/volume1/snap/snap_16_0_4_96
...
#######################################################
# 1.executing_case1
# @function：使用不存在的ID来修改一个NFS共享；
# @parameter：
#       node_ip：执行cmd命令的节点ip
#       cmd：使用不存在的ID来修改一个NFS共享；
# @return：
...
def executing_case1():
    ...
    access_zone_name = "access_zone_16_0_4_96"
    node = common.Node()
    ...
    global access_zone_id_16_0_4_96
    access_zone_id_16_0_4_96 = msg1["result"]

    # 2> 启动NAS
    log.info("\t[ enable_nas ]")
    msg2 = nas_common.enable_nas(access_zone_id=access_zone_id_16_0_4_96, protocol_types="NFS")
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        common.except_exit('%s enable_nas failed!!!' % node_ip)

    # 3> 查看NAS是否按配置启动
    log.info("\t[ get_access_zones ]")
    check_result3 = nas_common.get_access_zones(ids=access_zone_id_16_0_4_96)
    name = check_result3["result"]["access_zones"][0]["name"]
    if name != access_zone_name:
        common.except_exit('%s enable_nas failed!!!' % node_ip)
    global auth_provider_id_16_0_4_96
    auth_provider_id_16_0_4_96 = check_result3["result"]["access_zones"][0]["auth_provider_id"]
    ...
    #     os.makedirs(prepath)
    global nfs_path
    nfs_path = nas_common.ROOT_DIR + "nfs_dir_16_0_4_96"
    global NAS_PATH
    NAS_PATH = get_config.get_one_nas_test_path() + "/nfs_dir_16_0_4_96"
    ...
    nfs_export_name = "nfs_exp_test_16_0_4_96"
    global description_nfs
    description_nfs = 'old_export_description'
    cmd = "create_nfs_export --access_zone_id=%s " \
          "--export_name=%s --export_path=%s --description=%s" % (access_zone_id_16_0_4_96, nfs_export_name,
                                                                  nfs_path, description_nfs)
    check_result1 = nas_common.create_nfs_export(access_zone_id=access_zone_id_16_0_4_96, export_name=nfs_export_name,
    ...
    global nfs_export_id_16_0_4_96
    nfs_export_id_16_0_4_96 = msg1["result"]

    """4>检查导出路径"""
    """4-1> get_ftp_exports"""
    log.info("\t[ case1 get_nfs_exports ]")
    cmd = "get_nfs_exports --ids=%s" % nfs_export_id_16_0_4_96
    check_result = nas_common.get_nfs_exports(ids=nfs_export_id_16_0_4_96)
    ...
    if access_zone_id != access_zone_id_16_0_4_96:
    ...
    """5> 修改导出路径"""
    log.info("\t[ case1 update_nfs_export ]")
    global description_nfs
    description_nfs = 'new_export_description'
    nfs_export_id_16_0_4_96_1 = nfs_export_id_16_0_4_96+10
    cmd = "update_nfs_export --id=%s --description=%s" % (nfs_export_id_16_0_4_96_1, description_nfs)
    check_result1 = nas_common.update_nfs_export(export_id=nfs_export_id_16_0_4_96_1,
...
def clearing_environment():
    ...
    '''1> 删除export'''
    log.info("\t[ 删除export ]")
    cmd = "delete_nfs_exports --ids=%s" % (nfs_export_id_16_0_4_96)
    check_result1 = nas_common.delete_nfs_exports(ids=nfs_export_id_16_0_4_96)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
    msg1 = check_result1
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
    ...
    log.info(stdout)
    # prepath1 = '/mnt/volume/nas/nfs_dir_16_0_4_96'
