...
def executing_case1():
    ...
    if len(nodes) >= 5:
        """1> 制造oJmgs服务器换主"""
        log.info("\t[1 制造oJmgs服务器换主]")
        rc, stdout = common.get_master()
        msg = common.json_loads(stdout)
        ip_master_a = str(msg["result"]["data_ips"][0]["ip_address"])       # 获取master的ip
        ip_master_new = str(msg["result"]["data_ips"][0]["ip_address"])
        while ip_master_a == ip_master_new:                                 # 排除当下执行命令的id与master的id
            cmd = "ps -ef | grep /home/parastor/bin/oJmgs | grep -v grep |awk '{print $2}'"
            rc, pid = common.run_command(ip_master_a, cmd)
            common.judge_rc(rc, 0, "get PID of oJmgs fialed")
            cmd = "kill -9 %s" % pid
            rc, stdout = common.run_command(ip_master_a, cmd)
            # common.judge_rc(rc, 0, "kill -9 oJmgs failed")
            time.sleep(15)
            rc, stdout = common.get_master()
            msg = common.json_loads(stdout)
            ip_master_new = str(msg["result"]["data_ips"][0]["ip_address"])  # 获取master的ip

        """ 2> 创建访问分区 """
        log.info("\t[2 create_access_zone ]")
        access_zone_name = "access_zone_16_0_2_20"
        node = common.Node()
        outtext = node.get_nodes()
        nodes = outtext['result']['nodes']
        ids = []
        ...
        print ids
        access_zone_node_id_16_0_2_20 = ','.join(str(p) for p in ids)
        msg1 = nas_common.create_access_zone(node_ids=access_zone_node_id_16_0_2_20, name=access_zone_name)
        if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
            common.except_exit("发生oJmgs换主时，创建访问分区与预期不符，请检查节点状态与访问分区状态")
