...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：16-0-x-x 开启home目录，NTFS ACL 打开，关闭允许创建NTFS ACL
#######################################################
...
def executing_case():
    """
        1、设置home目录后，再打开enable_ntfs_acl参数，关闭allow_create_ntfs_acl；
        pscli --command=update_smb_global_config --id=1 --home_dir="parastor:/homes" --enable_ntfs_acl=true --allow_create_ntfs_acl=false
    :return:
    """
    ...
    create_file_path = nas_common.ROOT_DIR + "homes-nas_16-0-4-67_dir"
    ...
    if allow_create_ntfs_acl != False \
            or enable_alternative_datasource != False \
            or enable_change_notify != True \
            or enable_dos_attributes != True \
            or enable_guest != False \
    ...
    # 环境恢复，allow_create_ntfs_acl="true"
