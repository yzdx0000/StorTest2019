...
def executing_case():
    """
        1、创建/export目录的FTP共享；
        pscli --command=create_ftp_export --access_zone_id=x --export_name=export1 --export_path=volume:/export
        2、创建/export/dir目录的FTP共享；
        pscli --command=create_ftp_export --access_zone_id=x --export_name=export1 --export_path=volume:/export/dir
    :return:
    """
    ...
    # create_file
    create_file_path1 = nas_common.ROOT_DIR + "nas_16-0-4-132_dir"
    create_file_path2 = create_file_path1 + "/" + "dir"
    # create_ftp_export
    export_name1 = "nas_16_0_4_132_ftp_export_name_1"
    export_name2 = "nas_16_0_4_132_ftp_export_name_2"
    ...
    check_result2 = nas_common.create_file(path=create_file_path2)
    if check_result2["detail_err_msg"] != "":
        log.info("%s" % check_result2["detail_err_msg"])

    check_result3 = nas_common.create_ftp_export(access_zone_id=access_zone_id,
                                                 user_name=nas_common.AD_USER_1,
                                                 export_path=create_file_path1)
    if check_result3["detail_err_msg"] != "":
        log.error(("%s Failed") % FILE_NAME)
        raise Exception(("%s Failed") % FILE_NAME)
    result1 = check_result3["result"]

    check_result4 = nas_common.create_ftp_export(access_zone_id=access_zone_id,
                                                 user_name=nas_common.AD_USER_1,
                                                 export_path=create_file_path2)

    check_result5 = nas_common.get_ftp_exports(ids=result1)
