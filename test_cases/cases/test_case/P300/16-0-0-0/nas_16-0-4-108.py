...
# date 2018-05-07
# @summary：
# 16-0-4-108      anonuid，非65534
# @steps:
# case1、设置NFS授权客户端的uid为65536；
# pscli --command=add_nfs_export_auth_clients --export_id=1 --name=10.20.104.43 --permission_level=rw --anonuid=65536
# case2、验证anonuid权限；
# @changelog：
#
#######################################################
...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)  # /mnt/volume1/snap/snap_16_0_4_108
...
def executing_case1():
    ...
    access_zone_name = "access_zone_16_0_4_108"
    node = common.Node()
    ...
    global access_zone_id_16_0_4_108
    access_zone_id_16_0_4_108 = msg1["result"]

    # 2> 启动NAS
    log.info("\t[ enable_nas ]")
    msg2 = nas_common.enable_nas(access_zone_id=access_zone_id_16_0_4_108, protocol_types="NFS")
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        common.except_exit('%s enable_nas failed!!!' % node_ip)

    # 3> 查看NAS是否按配置启动
    log.info("\t[ get_access_zones ]")
    check_result3 = nas_common.get_access_zones(ids=access_zone_id_16_0_4_108)
    name = check_result3["result"]["access_zones"][0]["name"]
    if name != access_zone_name:
        common.except_exit('%s enable_nas failed!!!' % node_ip)
    global auth_provider_id_16_0_4_108
    auth_provider_id_16_0_4_108 = check_result3["result"]["access_zones"][0]["auth_provider_id"]
    ...
    #     os.makedirs(prepath)
    global nfs_path
    nfs_path = nas_common.ROOT_DIR + "nfs_dir_16_0_4_108"
    global NAS_PATH_1
    NAS_PATH_1 = get_config.get_one_nas_test_path() + "/nfs_dir_16_0_4_108"
    ...
    nfs_export_name = "nfs_exp_test_16_0_4_108"
    ...
    cmd = "create_nfs_export --access_zone_id=%s " \
          ...
          % (access_zone_id_16_0_4_108,
             nfs_export_name,
             nfs_path, description_nfs)
    check_result1 = nas_common.create_nfs_export(access_zone_id=access_zone_id_16_0_4_108,
    ...
    global nfs_export_id_16_0_4_108
    nfs_export_id_16_0_4_108 = msg1["result"]

    """4> get_ftp_exports"""
    log.info("\t[ case1-4 get_nfs_exports ]")
    cmd = "get_nfs_exports --ids=%s" % nfs_export_id_16_0_4_108
    check_result = nas_common.get_nfs_exports(ids=nfs_export_id_16_0_4_108)
    ...
    if access_zone_id != access_zone_id_16_0_4_108:
    ...
    cmd = "add_nfs_export_auth_clients --export_id=%s " \
          ...
          % (nfs_export_id_16_0_4_108,
             auth_clients_name,
             auth_clients_permission_level,
             auth_clients_write_mode,
             auth_clients_anonuid)
    check_result = nas_common.add_nfs_export_auth_clients(export_id=nfs_export_id_16_0_4_108,
    ...
    global nfs_export_clients_id_16_0_4_108
    nfs_export_clients_id_16_0_4_108 = msg["result"][0]

    """7> get_nfs_export_auth_clients"""
    log.info("\t[ case1-7 get_nfs_export_auth_clients ]")
    cmd = "get_nfs_export_auth_clients --export_ids=%s" % nfs_export_id_16_0_4_108
    check_result = nas_common.get_nfs_export_auth_clients(export_ids=nfs_export_id_16_0_4_108)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    msg = check_result
    if msg["err_msg"] != "" or msg["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    ...
    if id != nfs_export_clients_id_16_0_4_108:
        raise Exception('%s id failed!!!' % node_ip)
    if export_id != nfs_export_id_16_0_4_108:
    ...
    auth_clients_mount_dir = "/mnt/nfs_dir_16_0_4_108"
    cmd = "ssh %s ls /mnt/nfs_dir_16_0_4_108" % auth_clients_name
    rc, stdout, stderr = shell.ssh(node_ip, cmd)
    if stdout == "":
        cmd = "ssh %s umount -l /mnt/nfs_dir_16_0_4_108" % auth_clients_name
        shell.ssh(node_ip, cmd)
        cmd = "ssh %s rm -rf /mnt/nfs_dir_16_0_4_108" % auth_clients_name
        rc, stdout, stderr = shell.ssh(node_ip, cmd)
        if rc != 0:
            log.error("WARNING: \ncmd = %s\nstdout = %s\nstderr = %s" % (cmd, stdout, stderr))
...
def executing_case2():
    ...
    cmd1 = "umount -fl /mnt/nfs_dir_16_0_4_108"
    ...
    cmd1 = "rm -rf /mnt/nfs_dir_16_0_4_108"
    rc, stdout, stderr = shell.ssh(auth_clients_name, cmd1)
    print stdout
    if rc != 0:
        log.info("rc = %s" % rc)
