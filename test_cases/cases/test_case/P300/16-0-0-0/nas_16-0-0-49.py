...
# date 2018-04-19
# @summary：
# 16-0-0-49         用户组DN错误连接测试
# @steps:
# case 1、集群添加ldap认证，输入错误的用户DN；
# pscli --command=add_auth_provider_ldap --name=ldap_test --ip_addresses=x.x.x.x
# --base_dn=dc=xxx,dc=com --port=389 --bind_dn=cn=root, dc=abc,dc=com
# --bind_password=xxx --user_search_path=ou=user,dc=abc,dc=com --group_search_path=ou=group,dc=abc,dc=com
...
def executing_case1():
    ...
    check_result2 = nas_common.add_auth_provider_ldap(name="nas_16_0_0_49_ldap", base_dn=LDAP_BASE_DN,
                                                      ...
                                                      group_search_path="ou=Ggroup,dc=test,dc=com")
    ...
    global id_16_0_0_49
    id_16_0_0_49 = msg2["result"]

    # 2、通过命令pscli --command=get_auth_provider_ldap查看ldap认证配置信息是否正确
    log.info("\t[case2 get_auth_providers_ldap ]")
    wait_time1 = random.randint(2, 5)
    time.sleep(wait_time1)
    cmd = "get_auth_providers_ldap --ids=%s"
    check_result2 = nas_common.get_auth_providers_ldap(ids=id_16_0_0_49)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    msg2 = check_result2
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    ...
    check_result2 = nas_common.check_auth_provider(provider_id=id_16_0_0_49)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    msg2 = check_result2
    if msg2["err_msg"] == "" or msg2["detail_err_msg"] == "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
