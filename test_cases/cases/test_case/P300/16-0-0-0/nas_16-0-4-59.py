...
def executing_case():
    """
        1、批量删除smb共享目录；
        pscli --command=delete_ftp_exports --ids=1,2,3,4,5（注：1,2,3,4,5 export id均存在）
    :return:
    """
    ...
    if smb_ids:
        smb_ids_new = ','.join(str(i) for i in smb_ids)
        check_result4 = nas_common.delete_smb_exports(ids=smb_ids_new)
        if check_result4["detail_err_msg"] != "":
            log.error(("%s Failed") % FILE_NAME)
            raise Exception(("%s Failed") % FILE_NAME)

        for smb_id in smb_ids:
            check_result5 = nas_common.get_smb_exports(ids=smb_id)
            if check_result5["detail_err_msg"].find("Can not find smb export by id") == -1:
                log.error(("%s Failed") % FILE_NAME)
                raise Exception(("%s Failed") % FILE_NAME)

        log.info(("%s Succeed") % FILE_NAME)
    else:
        log.info("%s, smb_ids is Empty.") % FILE_NAME
