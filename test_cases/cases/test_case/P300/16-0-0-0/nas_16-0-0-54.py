...
# 16-0-0-54           修改基准DN测试
# @steps:
# case1、集群添加ldap认证服务器，填写错误的基准DN号，填写正确的IP地址、端口号；
...
# case4、通过命令pscli --command=update_auth_provider_ldap --id=x --base_dn=dc=xxx,dc=com 修改到正确的基准DN号；
...
def executing_case1():
    # 1、集群添加ldap认证服务器，填写错误的基准DN号，填写正确的IP地址、端口号
    ...
    check_result2 = nas_common.add_auth_provider_ldap(name="nas_16_0_0_54_ldap", base_dn="dc=testtt,dc=com",
    ...
    check_result2 = nas_common.get_auth_providers_ldap(ids=id_16_0_0_54)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    msg2 = check_result2
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    ...
    check_result2 = nas_common.check_auth_provider(provider_id=id_16_0_0_54)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    msg2 = check_result2
    ...
    # 4、通过命令pscli --command=update_auth_provider_ldap --id=x --base_dn=dc=xxx,dc=com 修改到正确的基准DN号
    log.info("\t[case4 update_auth_provider_ldap]")
    wait_time1 = random.randint(1, 3)
    time.sleep(wait_time1)
    cmd = "update_auth_provider_ldap"
    check_result2 = nas_common.update_auth_provider_ldap(name="nas_16_0_0_54_ldap", provider_id=id_16_0_0_54,
    ...
    # 5、通过命令pscli --command=get_auth_provider_ldap查看ldap认证服务器配置信息
    ...
    # 6、通过命令行pscli --command=check_auth_provider --ids=x查看认证配置信息是否能够正确的连接ldap认证服务器
