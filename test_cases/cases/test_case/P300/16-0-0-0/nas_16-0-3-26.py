...
def case():
    ...
    '''4> 通过命令行ip addr观察SVIP绑定到哪个节点ethx上'''
    node_ip_list = [SYSTEM_IP_0, SYSTEM_IP_1, SYSTEM_IP_2]
    svip_judge = nas_common.SUBNET_SVIP + '/' + nas_common.SUBNET_MASK
    eth_judge = nas_common.SUBNET_NETWORK_INTERFACES + ':'
    for i in range(3):
        cmd = 'ip addr'
        rc, stdout = common.run_command(node_ip_list[i], cmd)
        split_list = stdout.split()
        if svip_judge in split_list and eth_judge in split_list:
            log.info('svip is on %s' % node_ip_list[i])
            break
    else:
        log.error('svip is not on any system node !!!')
        raise Exception('svip is not on any system node !!!')

    '''5> 通过pscli --command=delete_subnet --id=x删除之前的业务子网'''
    """disable nas 服务"""
    ...
    cmd = 'pscli --command=delete_subnet --id=0'  # 输入错误id，预期删除失败
    rc, stdout = common.run_command(SYSTEM_IP_2, cmd)
    common.judge_rc_unequal(rc, 0, 'result is unexpected !!!')
