...
# 16-0-4-87      共享名重复检查
# @steps:
# case1、创建一个名称为XXX的NFS共享；
# case2、再创建一个相同名称的NFS共享；
# @changelog：
#
#######################################################
import os
...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)  # /mnt/volume1/snap/snap_16_0_4_87
...
#######################################################
# 1.executing_case1
# @function：创建一个名称为XXX的NFS共享；
# @parameter：
#       node_ip：执行cmd命令的节点ip
#       cmd：创建一个名称为XXX的NFS共享；
# @return：
...
def executing_case1():
    ...
    access_zone_name = "access_zone_16_0_4_87"
    node = common.Node()
    ids = node.get_nodes_id()
    print ids
    access_zone_node_id_16_0_4_87 = ','.join(str(p) for p in ids)
    msg1 = nas_common.create_access_zone(node_ids=access_zone_node_id_16_0_4_87, name=access_zone_name)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        common.except_exit('%s create_access_zone failed!!!' % node_ip)
    global access_zone_id_16_0_4_87
    access_zone_id_16_0_4_87 = msg1["result"]

    # 2> 启动NAS
    log.info("\t[ enable_nas ]")
    msg2 = nas_common.enable_nas(access_zone_id=access_zone_id_16_0_4_87, protocol_types="NFS")
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        common.except_exit('%s enable_nas failed!!!' % node_ip)

    # 3> 查看NAS是否按配置启动
    log.info("\t[ get_access_zones ]")
    check_result3 = nas_common.get_access_zones(ids=access_zone_id_16_0_4_87)
    name = check_result3["result"]["access_zones"][0]["name"]
    if name != access_zone_name:
        common.except_exit('%s enable_nas failed!!!' % node_ip)
    global auth_provider_id_16_0_4_87
    auth_provider_id_16_0_4_87 = check_result3["result"]["access_zones"][0]["auth_provider_id"]
    ...
    #     os.makedirs(prepath)
    global nfs_path
    nfs_path = nas_common.ROOT_DIR + "nfs_dir_16_0_4_87"
    global NAS_PATH
    NAS_PATH = get_config.get_one_nas_test_path() + "/nfs_dir_16_0_4_87"
    cmd = "create_file "
    ...
    nfs_export_name = "nfs_exp_test_16_0_4_87"
    cmd = "create_nfs_export "
    check_result1 = nas_common.create_nfs_export(access_zone_id=access_zone_id_16_0_4_87, export_name=nfs_export_name,
    ...
    global nfs_export_id_16_0_4_87
    nfs_export_id_16_0_4_87 = msg1["result"]

    """4>检查导出路径"""
    log.info("\t[ case1 get_nfs_exports ]")
    cmd = "get_nfs_exports "
    check_result = nas_common.get_nfs_exports(ids=nfs_export_id_16_0_4_87)
    ...
    if access_zone_id != access_zone_id_16_0_4_87:
...
def executing_case2():
    ...
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
    msg1 = check_result1
    if msg1["err_msg"] == "" or msg1["detail_err_msg"] != "Export name:nfs_exp_test_16_0_4_87 already exist. " \
                                                          "Tips: name is not case sensitive.":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
        raise Exception('%s create_nfs_export failed!!!' % node_ip)
...
def clearing_environment():
    ...
    check_result1 = nas_common.delete_nfs_exports(ids=nfs_export_id_16_0_4_87)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
    msg1 = check_result1
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
    ...
    log.info(stdout)
    # prepath1 = '/mnt/volume/nas/nfs_dir_16_0_4_87'
