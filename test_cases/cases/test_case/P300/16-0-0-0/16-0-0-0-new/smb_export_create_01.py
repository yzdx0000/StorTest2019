...
#########################################################
# author: liyao
# 概述：本地认证，创建smb导出（默认值），授权IP不做限制（对用户授权）
# 过程：1、创建3节点访问分区，启动服务
#      2、创建本地用户组/用户
#      3、创建默认参数的smb导出并对无限制IP进行授权（用户）
...
def case():
    ...
    """1> 创建访问分区"""
    access_zone_id, local_provider_id = create_access_zone()
    log.info('waiting for 5s')
    time.sleep(5)
    ...
    auth_num = 3
    if auth_num > int(group_num):   # 控制添加授权的数目不能超过用户总数
        auth_num = group_num
    else:
        pass
