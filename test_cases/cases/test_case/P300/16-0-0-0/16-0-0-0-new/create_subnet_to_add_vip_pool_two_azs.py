...
"""
 ...
 date 2018-12-05
 @summary：
    覆盖两个访问区（访问区节点：单节点+多节点）场景，创建业务子网，添加vip池，参数为参数全集，客户端挂载
 @steps:
    1、根据是否继承的标志位决定是否清除环境，若清除环境，则生成单节点访问区和多节点访问区
    2、生成参数字典
    3、为每个访问区创建业务子网、添加vip池
...
def generate_params_vip(node_num, vip_add):
    ...
    allocation_method_lst = [nas_common.DYNAMIC]
...
def case():
    ...
    if len(node_list_string) < 3:
        common.except_exit(len(node_list_string), 3, "less 3 nodes,exit")
    """默认不继承访问区分布False，遗留（不删除）子网True"""
    flag_inherit = False
    flag_genetic = True
    ...
    if not flag_inherit:
        ...
        az_new_lone = "az_lone_" + str(time.localtime().tm_hour) + str(time.localtime().tm_min) + str(
            time.localtime().tm_sec)
        msg_crt_az = nas_common.create_access_zone(node_ids=node_list_string[0], name=az_new_lone)
        ...
        az_new_multi = "az_multi_" + str(time.localtime().tm_hour) + str(time.localtime().tm_min) + str(
            time.localtime().tm_sec)
        msg_crt_az = nas_common.create_access_zone(node_ids=",".join(node_list_string[1:]), name=az_new_multi)
        common.judge_rc(msg_crt_az['err_no'], 0, "create new access zone")
        access_zone_id = msg_crt_az["result"]
        access_zone_id_lst.append(access_zone_id)
    ...
    vip_pool_id = -2
    sub_net_id = -2
    for access_zone_id in access_zone_id_lst:
        msg_enable_nas = nas_common.enable_nas(access_zone_id=access_zone_id, print_flag=False)
        ...
        for sub_net_para in crt_sub_param_lst:
            ...
            # 循环的开头删除子网，以便跳出循环时保留了一个id
                ...
                if file_test_flag:
                    ...
                    msg_exp_nfs = nas_common.create_nfs_export(access_zone_id=access_zone_id,
                                                               export_name=nfs_export_name,
                                                               export_path=nfs_path_4crt)
                    ...
                    msg_add_nfs_client = nas_common.add_nfs_export_auth_clients(export_id=nfs_ex_id,
                ...
                log.info("===================az:{}=========NO.{} FINISH===================".format(access_zone_id, for_loop_time))
                for_loop_time += 1
