...
# date 2018-11-22
# @summary：
# x-x-x-x     用户/用户组，添加认证服务器，被disable的访问分区引用（访问分区修改认证服务器）
# @steps:
...
# 3.修改访问分区中的认证服务器，获取用户/用户组并与步骤2中获取用户/用户组的结果作对比
# @changelog：
#
#######################################################
import os
...
def executing_case1():
    ...
    for i in range(1, run_times + 1):
        ...
        auth_provider_id_list.remove(az_auth_provider_id)
        a = []
        ...
        az_auth_provider_id = random.choice(auth_provider_id_list)  # 随机选一种认证服务
        msg1 = nas_common.update_access_zone(access_zone_id=access_zone_id, name=access_zone_name,
                                             auth_provider_id=az_auth_provider_id)
        if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        ...
        if ad_auth_provider_id == az_auth_provider_id:
            get_ad_user_group(auth_provider_id=ad_auth_provider_id)
            if update_auth_user_list != get_ad_user_list:
                log.error("update_auth_user_list=%s, get_ad_user_list=%s"
                          % (update_auth_user_list, get_ad_user_list))
                common.except_exit("After create access zones, users have been changed ")
            if update_auth_group_list != get_ad_group_list:
                log.error("update_auth_group_list=%s, get_ad_group_list=%s"
                          % (update_auth_group_list, get_ad_group_list))
                common.except_exit("After create access zones, groups have been changed ")
        elif ldap_auth_provider_id == az_auth_provider_id:
            get_ldap_user_group(auth_provider_id=ldap_auth_provider_id)
            if update_auth_user_list != get_ldap_user_list:
                log.error("update_auth_user_list=%s, get_ldap_user_list=%s"
                          % (update_auth_user_list, get_ldap_user_list))
                common.except_exit("After create access zones, users have been changed ")
            if update_auth_group_list != get_ldap_group_list:
                log.error("update_auth_group_list=%s, get_ldap_group_list=%s"
                          % (update_auth_group_list, get_ldap_group_list))
                common.except_exit("After create access zones, groups have been changed ")
        elif ldap_pdc_auth_provider_id == az_auth_provider_id:
            get_ldap_pdc_user_group(auth_provider_id=ldap_pdc_auth_provider_id)
            if update_auth_user_list != get_ldap_pdc_user_list:
                log.error("update_auth_user_list=%s, get_ldap_pdc_user_list=%s"
                          % (update_auth_user_list, get_ldap_pdc_user_list))
                common.except_exit("After create access zones, users have been changed ")
            if update_auth_group_list != get_ldap_pdc_group_list:
                log.error("update_auth_group_list=%s, get_ldap_pdc_group_list=%s"
                          % (update_auth_group_list, get_ldap_pdc_group_list))
                common.except_exit("After create access zones, groups have been changed ")
        elif nis_auth_provider_id == az_auth_provider_id:
            get_nis_user_group(auth_provider_id=nis_auth_provider_id)
            if update_auth_user_list != get_nis_user_list:
                log.error("update_auth_user_list=%s, get_nis_user_list=%s"
                          % (update_auth_user_list, get_nis_user_list))
                common.except_exit("After create access zones, users have been changed ")
            if update_auth_group_list != get_nis_group_list:
                log.error("update_auth_group_list=%s, get_nis_group_list=%s"
                          % (update_auth_group_list, get_nis_group_list))
                common.except_exit("After create access zones, groups have been changed ")
