...
#########################################################
# author: liyao
# 概述：本地认证，2个访问分区，创建nfs导出，对各种IP形式授权（权限不同）
# 过程：1、创建2个访问分区，分别启动服务
#      2、修改访问分区
#      3、创建默认参数的nfs导出并对['*', '50.2.42.1/22', '50.2.42.128']分别授与['rw', 'rw_nodelsacl', 'ro']的权限
...
def case():
    ...
    # print ('******* %s %s ********' (auth_client_ip_1, type(auth_client_ip_1)))
    # print ('******* %s %s ********'(auth_client_ip_2, type(auth_client_ip_2)))

    auth_clients_lst = ['*', auth_client_ip_2, auth_client_ip_1]
    permission_level_lst = ['rw', 'rw_nodelsacl', 'ro']
    ...
    rc, stdout = common.run_command(nas_common.CLIENT_IP_1, cmd)
    common.judge_rc_unequal(rc, 0, 'the auth constain failed !!!')
