...
def executing_case():
    ...
    access_zone_name = 'nas_16_0_3_69_access_zone_name'
    # for create_subnet
    subnet_name = 'nas_16_0_3_69_subnet_name'
    ...
    """删除vip地址池"""
    msg = nas_common.delete_vip_address_pool(vip_address_pool_id=vip_address_pool_id)
    if msg['detail_err_msg'] != '':
        raise Exception('%s Failed' % FILE_NAME)

    """删除子网"""
    msg = nas_common.delete_subnet(subnet_id=subnet_id)
    if msg['detail_err_msg'] != '':
        raise Exception('%s Failed' % FILE_NAME)
    ...
    common.judge_rc(rc, -1, 'judge_vip_layinfo failed')
