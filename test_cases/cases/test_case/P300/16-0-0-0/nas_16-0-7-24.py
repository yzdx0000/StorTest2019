...
# Date: 2018-08-09
# @summary：
#   add_smb_export_auth_clients的name参数为user name，type参数为GROUP
# @steps:
...
#   2、add_smb_export_auth_clients指定name为存在的user name，type为GROUP，run_as_root为true；
#   pscli --command=add_smb_export_auth_clients --export_id=x --name=exist_user_name --type=GROUP --run_as_root=true
#   3、update_smb_global_config配置home_dir；
#   pscli --command=update_smb_global_config --id=x --home_dir=volume:/homedir
#   4、mstsc到某windows设备，使用export_name进行挂载，在弹出的登录框中输入步骤2中指定的用户名和密码；
#   5、尝试删除导出的策略；
#   6、尝试删除导出的目录；
# @changelog：
#   None
...
def executing_case():
    ...
    auth_provider_name = 'nas_16_0_7_24_ad_auth_name'
    ...
    unix_id_range = [5000, 5099]
    # for create_access_zone
    node_ids = '1,2,3'
    access_zone_name = 'nas_16_0_7_24_access_zone_name'
    # for create_file
    dir_name = 'nas_16-0-7-24_dir'
    create_file_path = nas_common.ROOT_DIR + dir_name
    create_home_dir_file_path = nas_common.ROOT_DIR + 'nas_16_0_7_24_homedir'
    # for create_smb_export，不配authorization_ip即允许所有ip
    export_name = 'nas_16_0_7_24_smb_export_name'
