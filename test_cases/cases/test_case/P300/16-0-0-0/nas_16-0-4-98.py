...
# date 2018-04-27
# @summary：
# 16-0-4-98     正常删除共享目录（使用中）
# @steps:
# case1、删除正在使用的NFS共享目录；
...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)  # /mnt/volume1/snap/snap_16_0_4_98
...
def executing_case1():
    ...
    access_zone_name = "access_zone_16_0_4_98"
    node = common.Node()
    ...
    global access_zone_id_16_0_4_98
    access_zone_id_16_0_4_98 = msg1["result"]

    # 2> 启动NAS
    log.info("\t[ enable_nas ]")
    msg2 = nas_common.enable_nas(access_zone_id=access_zone_id_16_0_4_98, protocol_types="NFS")
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        common.except_exit('%s enable_nas failed!!!' % node_ip)

    # 3> 查看NAS是否按配置启动
    log.info("\t[ get_access_zones ]")
    check_result3 = nas_common.get_access_zones(ids=access_zone_id_16_0_4_98)
    name = check_result3["result"]["access_zones"][0]["name"]
    if name != access_zone_name:
        common.except_exit('%s enable_nas failed!!!' % node_ip)
    global auth_provider_id_16_0_4_98
    auth_provider_id_16_0_4_98 = check_result3["result"]["access_zones"][0]["auth_provider_id"]
    ...
    #     os.makedirs(prepath)
    global nfs_path
    nfs_path = nas_common.ROOT_DIR + "nfs_dir_16_0_4_98"
    global NAS_PATH
    NAS_PATH = get_config.get_one_nas_test_path() + "/nfs_dir_16_0_4_98"
    ...
    nfs_export_name = "nfs_exp_test_16_0_4_98"
    ...
    cmd = "create_nfs_export --access_zone_id=%s " \
          ...
          % (access_zone_id_16_0_4_98,
             nfs_export_name,
             nfs_path, description_nfs)
    check_result1 = nas_common.create_nfs_export(access_zone_id=access_zone_id_16_0_4_98,
    ...
    global nfs_export_id_16_0_4_98
    nfs_export_id_16_0_4_98 = msg1["result"]

    """4> get_ftp_exports"""
    log.info("\t[ case1-4 get_nfs_exports ]")
    cmd = "get_nfs_exports --ids=%s" % nfs_export_id_16_0_4_98
    check_result = nas_common.get_nfs_exports(ids=nfs_export_id_16_0_4_98)
    ...
    if access_zone_id != access_zone_id_16_0_4_98:
    ...
    cmd = "add_nfs_export_auth_clients --export_id=%s " \
          ...
          "--anongid=%s " \
          % (nfs_export_id_16_0_4_98,
             auth_clients_name,
             auth_clients_permission_level,
             auth_clients_write_mode,
             auth_clients_port_constraint,
             auth_clients_permission_constraint,
             auth_clients_anongid)
    check_result = nas_common.add_nfs_export_auth_clients(export_id=nfs_export_id_16_0_4_98,
    ...
    global nfs_export_clients_id_16_0_4_98
    nfs_export_clients_id_16_0_4_98 = msg["result"][0]

    """7> get_nfs_export_auth_clients"""
    log.info("\t[ case1-7 get_nfs_export_auth_clients ]")
    cmd = "get_nfs_export_auth_clients --export_ids=%s" % nfs_export_id_16_0_4_98
    check_result = nas_common.get_nfs_export_auth_clients(export_ids=nfs_export_id_16_0_4_98)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    msg = check_result
    if msg["err_msg"] != "" or msg["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    ...
    if id != nfs_export_clients_id_16_0_4_98:
        raise Exception('%s id failed!!!' % node_ip)
    if export_id != nfs_export_id_16_0_4_98:
    ...
    auth_clients_mount_dir = "/mnt/nfs_dir_16_0_4_98"
    """检查客户端"""
    cmd = "ls /mnt/nfs_dir_16_0_4_98"
    rc, stdout, stderr = shell.ssh(auth_clients_name, cmd)
    if stdout == "":
        cmd = "ssh %s umount -l /mnt/nfs_dir_16_0_4_98" % auth_clients_name
        rc, stdout, stderr = shell.ssh(node_ip, cmd)
        cmd = "ssh %s rm -rf /mnt/nfs_dir_16_0_4_98" % auth_clients_name
        rc, stdout, stderr = shell.ssh(node_ip, cmd)
        if rc != 0:
            log.error("WARNING: \ncmd = %s\nstdout = %s\nstderr = %s" % (cmd, stdout, stderr))
    ...
    if rc != 0:
        log.info("rc = %s" % (rc))
        log.error("WARNING: \ncmd = %s\nstdout = %s\nstderr = %s" % (cmd, stdout, stderr))
        raise Exception('%s create_file failed!!!' % auth_clients_name)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (auth_clients_name, cmd, stdout))

    """10-2> 客户端mount共享路径"""
    wait_time1 = random.randint(8, 10)
    time.sleep(wait_time1)
    cmd1 = "ssh %s mount -t nfs %s:%s %s" % (auth_clients_name, node_ip, NAS_PATH, auth_clients_mount_dir)
    ...
    # cmd1 = "file1=`grep 10.2.41.143 /etc/exports | head -n 1` && file2=`echo ${file1#*root_squash,}` && " \
    #        "echo ${file2%,anonuid=65534*}"
    cmd1 = "file1=`grep %s /etc/exports | head -n 1` && file2=`echo ${file1#*root_squash,}` && " % auth_clients_name + \
           "echo ${file2%,anonuid=65534*}"
    ...
    cmd = "delete_nfs_exports --ids=%s" % (nfs_export_id_16_0_4_98)
    check_result1 = nas_common.delete_nfs_exports(ids=nfs_export_id_16_0_4_98)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
    msg1 = check_result1
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
