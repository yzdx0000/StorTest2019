...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)              # /mnt/volume/nas/nas_16_0_0_44
...
def executing_case1():
    """1、集群添加ldap认证，输入错误端口号"""
    log.info("\t[case1 add_auth_provider_ldap ]")
    msg2 = nas_common.add_auth_provider_ldap(name="nas_16_0_0_44_ldap", base_dn=LDAP_BASE_DN,
                                             ip_addresses=LDAP_IP, port=340)
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        common.except_exit('%s add_auth_provider_ldap failed!!!' % node_ip)
    id_16_0_0_44 = msg2["result"]

    """2、通过命令pscli --command=get_auth_provider_ldap查看ldap认证配置信息是否正确"""
    log.info("\t[case2 get_auth_providers_ldap ]")
    msg2 = nas_common.get_auth_providers_ldap(ids=id_16_0_0_44)
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
    ...
    auth_providers_ldap = nas_common.get_auth_providers_ldap(ids=id_16_0_0_44)
    id_id = auth_providers_ldap["result"]["auth_providers"][0]["id"]
    msg2 = nas_common.check_auth_provider(provider_id=id_id)
    if msg2["err_msg"] != "CONNECT_AUTH_PROVIDER_FAILED":
        common.except_exit('%s check_auth_provider failed and err_msg error!!!' % node_ip)
    if msg2["detail_err_msg"].find("Connect to the authentication provider failed") == -1:
        common.except_exit('%s check_auth_provider failed and detail_err_msg error!!!' % node_ip)
    return
