...
def executing_case():
    """
        1、使用不存在的ID来修改一个FTP共享；
        pscli --command=update_ftp_export --id=250 --allow_anonymous=false
        注：共享ID 250不存在；
    :return:
    """
    ...
    check_result = nas_common.update_ftp_export(export_id=not_exist_id)
