...
#   5、修改正确的SVIP地址；
#   6、get_subnet观察信息；
#   7、通过命令行ip addr观察SVIP绑定到哪个节点ethx上；
#   8、删除子网、访问分区，清理环境
#
# @changelog：
####################################################################################
...
def case():
    ...
    svip_judge = svip_before_update + '/' + nas_common.SUBNET_MASK
    ...
    '''5> 修改正确的SVIP地址'''
    svip_after_update = nas_common.SUBNET_2_SVIP
    exe_info = nas_common.update_subnet(subnet_id, svip=svip_after_update)
    if exe_info["err_msg"] != "" or exe_info["detail_err_msg"] != "":
        common.except_exit('update subnet information is wrong !!!')

    '''6> 通过pscli --command=get_subnets命令查看业务子网信息'''
    exe_info = nas_common.get_subnets(subnet_id)
    subnet = exe_info['result']['subnets'][0]
    if subnet['svip'] != svip_after_update:
        log.error('subnet information is wrong !!!')
        raise Exception('subnet information is wrong !!!')

    '''7> 通过命令行ip addr观察SVIP绑定到哪个节点ethx上；'''
    svip_judge = svip_after_update + '/' + nas_common.SUBNET_MASK
    eth_judge_str = nas_common.SUBNET_NETWORK_INTERFACES
    eth_judge_lst = eth_judge_str.split(',')
    for node_ip in node_ip_list:
        cmd = 'ip addr'
