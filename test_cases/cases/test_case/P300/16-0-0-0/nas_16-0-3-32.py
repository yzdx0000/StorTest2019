...
#   5、重启SVIP所在的节点，观察节点上SVIP状态(节点重启后，SVIP漂移到其他节点的ethx网卡上)；
#   6、再次通过命令行ip addr观察SVIP绑定到哪个节点ethx上；
#   7、删除子网、访问分区，清理环境
#
# @changelog：
####################################################################################
...
def case():
    ...
    '''5> 重启SVIP所在的节点，观察节点上SVIP状态'''
    """重启节点"""
    cmd = 'echo b > /proc/sysrq-trigger'
    common.run_command(first_node_ip, cmd)
    log.info('waiting for 10s')
    time.sleep(10)

    """重复检查，直到节点启动成功"""
    for node_ip in node_ip_list:
        if node_ip != first_node_ip:
            check_ip = node_ip

    rc = make_fault.check_node_connection(first_node_ip, check_ip)
    common.judge_rc(rc, 0, '%s is not connected well !!!' % first_node_ip)
