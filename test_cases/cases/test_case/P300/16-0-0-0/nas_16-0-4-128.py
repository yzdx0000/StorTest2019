...
# date 2018-05-09
# @summary：
# 16-0-4-128        输入错误的授权客户端ID
# @steps:
# case1、删除authorization client id错误的NFS授权客户端；
# pscli --command=remove_nfs_export_auth_clients --ids=250
# 注：authorization client id 250不存在；
# @changelog：
#
#######################################################
import os
...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)  # /mnt/volume1/snap/snap_16_0_4_128
...
#######################################################
# 1.executing_case1
# @function：删除authorization client id错误的NFS授权客户端；
# @parameter：
#       node_ip：执行cmd命令的节点ip
#       cmd：删除authorization client id错误的NFS授权客户端；
# @return：
...
def executing_case1():
    ...
    access_zone_name = "access_zone_16_0_4_128"
    node = common.Node()
    ids = node.get_nodes_id()
    print ids
    access_zone_node_id_16_0_4_128 = ','.join(str(p) for p in ids)
    msg1 = nas_common.create_access_zone(node_ids=access_zone_node_id_16_0_4_128, name=access_zone_name)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        common.except_exit('%s create_access_zone failed!!!' % node_ip)
    global access_zone_id_16_0_4_128
    access_zone_id_16_0_4_128 = msg1["result"]

    # 2> 启动NAS
    log.info("\t[ enable_nas ]")
    msg2 = nas_common.enable_nas(access_zone_id=access_zone_id_16_0_4_128, protocol_types="NFS")
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        common.except_exit('%s enable_nas failed!!!' % node_ip)

    # 3> 查看NAS是否按配置启动
    log.info("\t[ get_access_zones ]")
    check_result3 = nas_common.get_access_zones(ids=access_zone_id_16_0_4_128)
    name = check_result3["result"]["access_zones"][0]["name"]
    if name != access_zone_name:
        common.except_exit('%s enable_nas failed!!!' % node_ip)
    global auth_provider_id_16_0_4_128
    auth_provider_id_16_0_4_128 = check_result3["result"]["access_zones"][0]["auth_provider_id"]

    """1> 检查目录并creat file"""
    log.info("\t[ case1-1 create_file ]")
    global nfs_path
    nfs_path = nas_common.ROOT_DIR + "nfs_dir_16_0_4_128"
    global NAS_PATH
    NAS_PATH = get_config.get_one_nas_test_path() + "/nfs_dir_16_0_4_128"
    cmd = "create_file "
    ...
    nfs_export_name = "nfs_exp_test_16_0_4_128"
    global description_nfs
    description_nfs = 'old_export_description'
    cmd = "create_nfs_export"
    check_result1 = nas_common.create_nfs_export(access_zone_id=access_zone_id_16_0_4_128,
    ...
    global nfs_export_id_16_0_4_128
    nfs_export_id_16_0_4_128 = msg1["result"]

    """4> get_ftp_exports"""
    log.info("\t[ case1-4 get_nfs_exports ]")
    cmd = "get_nfs_exports "
    check_result = nas_common.get_nfs_exports(ids=nfs_export_id_16_0_4_128)
    ...
    if access_zone_id != access_zone_id_16_0_4_128:
    ...
    check_result = nas_common.add_nfs_export_auth_clients(export_id=nfs_export_id_16_0_4_128,
    ...
    global nfs_export_clients_id_16_0_4_128
    nfs_export_clients_id_16_0_4_128 = msg["result"][0]

    """7> get_nfs_export_auth_clients"""
    log.info("\t[ case1-7 get_nfs_export_auth_clients ]")
    cmd = "get_nfs_export_auth_clients "
    check_result = nas_common.get_nfs_export_auth_clients(export_ids=nfs_export_id_16_0_4_128)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    msg = check_result
    if msg["err_msg"] != "" or msg["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    ...
    if id != nfs_export_clients_id_16_0_4_128:
        raise Exception('%s id failed!!!' % node_ip)
    if export_id != nfs_export_id_16_0_4_128:
    ...
    auth_clients_mount_dir = "/mnt/nfs_dir_16_0_4_128"
    cmd = "ssh %s ls /mnt/nfs_dir_16_0_4_128" % auth_clients_name
    rc, stdout, stderr = shell.ssh(node_ip, cmd)
    if stdout == "":
        cmd = "ssh %s umount -l /mnt/nfs_dir_16_0_4_128" % auth_clients_name
        shell.ssh(node_ip, cmd)
        cmd = "ssh %s rm -rf /mnt/nfs_dir_16_0_4_128" % auth_clients_name
        rc, stdout, stderr = shell.ssh(node_ip, cmd)
        if rc != 0:
            log.error("WARNING: \ncmd = %s\nstdout = %s\nstderr = %s" % (cmd, stdout, stderr))
    ...
    """12> 删除nfs客户端"""
    log.info("\t[ case1-12 remove_nfs_export_auth_clients ]")
    nfs_export_clients_id_16_0_4_128_1 = nfs_export_clients_id_16_0_4_128 + 10
    cmd = "remove_nfs_export_auth_clients "
    check_result = nas_common.remove_nfs_export_auth_clients(ids=nfs_export_clients_id_16_0_4_128_1)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    msg = check_result
    ...
    """13> 客户端umount共享路径"""
    log.info("\t[ case1-13 客户端umount共享路径 ]")
    cmd1 = "umount -fl /mnt/nfs_dir_16_0_4_128"
    ...
    """14> 客户端删除mount路径"""
    log.info("\t[ case1-14 客户端删除mount路径 ]")
    cmd1 = "rm -rf /mnt/nfs_dir_16_0_4_128"
    rc, stdout, stderr = shell.ssh(auth_clients_name, cmd1)
    print stdout
    if rc != 0:
        log.info("rc = %s" % rc)
...
def clearing_environment():
    ...
    check_result1 = nas_common.delete_nfs_exports(ids=nfs_export_id_16_0_4_128)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
    msg1 = check_result1
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
