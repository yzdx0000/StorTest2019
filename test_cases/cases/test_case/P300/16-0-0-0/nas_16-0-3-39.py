...
def case():
    ...
    vip_address_single = nas_common.VIP_2_ADDRESSES.split(',')[0]
    exe_info = nas_common.add_vip_address_pool(subnet_id, nas_common.VIP_DOMAIN_NAME, vip_address_single,
    ...
    if vip_pool_info['domain_name'] != nas_common.VIP_DOMAIN_NAME or vip_pool_info['vip_addresses'][0] != vip_address_single:
        common.except_exit('vip address pool information is wrong !!!')

    '''5> 命令行通过IP addr观察VIP是否绑定到3个节点到对应的网卡上（相同网段的网卡）'''
    ip_dict, xml_ip_list, eth_ip_list = nas_common.get_vip_from_eth(vip_addresses=vip_address_single)
