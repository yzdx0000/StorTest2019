...
# date 2018-08-13
# @summary：
# 16_0_2_16     5节点，其中一个节点处于僵尸状态创建访问区
# @steps:
# 0> 检查环境是否大于等于五个节点，大于等于五节点可以执行该脚本
# 1> 修改断网时间后断网，制造一个节点处于僵尸状态
# 2> 创建访问分区
# 3> 恢复环境
# @changelog：
#
...
time_out = 720    # 判断是否进入僵尸状态的最大等待时间，超时会退出
...
def executing_case1():
    ...
    if len(nodes) >= 5:
        ...
        while "NODE_STATE_ZOMBIE" not in node_state:
            time.sleep(30)
            node = common.Node()
            ...
            if int(time_use) > time_out:
                common.except_exit("节点%s秒后仍无法进入僵尸状态，请检查节点状态" % time_out)

        """节点进入僵尸状态后，开始创建访问分区"""
        access_zone_name = "access_zone_16_0_2_16"
        node = common.Node()
        outtext = node.get_nodes()
        nodes = outtext['result']['nodes']
        ids = []
        ...
        print ids
        access_zone_node_id_16_0_2_16 = ','.join(str(p) for p in ids)
        msg1 = nas_common.create_access_zone(node_ids=access_zone_node_id_16_0_2_16, name=access_zone_name)
        if msg1["err_msg"] != "NODE_STATE_INVALID" or \
           msg1["detail_err_msg"].find("current state is NODE_STATE_ZOMBIE, not NODE_STATE_HEALTHY") == -1:
            common.except_exit("有节点在僵尸状态时，创建访问分区与预期不符，请检查节点状态与访问分区状态")
