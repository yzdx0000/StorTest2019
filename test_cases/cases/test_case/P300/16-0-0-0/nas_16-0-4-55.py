...
def executing_case():
    """
        1、修改smb共享时输入不存在的export id；
        pscli --command=update_smb_export --id=250（注：export id 250不存在）
    :return:
    """
    ...
    check_result = nas_common.update_smb_export(export_id=not_exist_id)

    print ("***************************")
    print check_result["detail_err_msg"].find("Cat not find smb export resource")

    if check_result["detail_err_msg"].find("Cat not find smb export resource") == -1:
