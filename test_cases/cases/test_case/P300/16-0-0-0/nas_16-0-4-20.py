...
def executing_case():
    ...
    check_result3 = nas_common.create_smb_export(access_zone_id=access_zone_id,
                                                 export_name="nas_16_0_4_20_smb_export_name",
    ...
    check_result4 = nas_common.add_smb_export_auth_clients(export_id=result,
                                                           name="nas_16-0-4-20_export_auth_clients_name",
                                                           user_type="USER",run_as_root="true")
    if check_result4["detail_err_msg"] != "":
        log.error(("%s Failed") % FILE_NAME)
        raise Exception(("%s Failed") % FILE_NAME)

    check_result5 = nas_common.delete_file(path=create_file_path)
    if check_result5["detail_err_msg"].find("has been exported by SMB") == -1:
        log.error(("%s Failed") % FILE_NAME)
        raise Exception(("%s Failed") % FILE_NAME)
    else:
