...
# date 2018-04-17
# @summary：
# 16-0-0-38     正常连接LDAP认证服务器（bind_dn）
# @steps:
# 1> 集群添加ldap认证，填写正确的IP地址、端口号和基准DN；
# pscli --command=add_auth_provider_ldap --name=ldap_test
# --ip_addresses=x.x.x.x --base_dn=dc=xxx,dc=com --port=389
# --bind_dn=cn=root, dc=abc,dc=com --bind_password=xxx
...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)              # /mnt/volume/nas/nas_16_0_0_38
...
def executing_case1():
    ...
    msg2 = nas_common.add_auth_provider_ldap(name="nas_16_0_0_38_ldap", base_dn=LDAP_BASE_DN,
    ...
    msg2 = nas_common.get_auth_providers_ldap(ids=id_id)
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
    ...
    """ 3> 通过命令行pscli --command=check_auth_provider --ids=x查看认证配置信息是否能够正确的连接ldap认证"""
    log.info("\t[3. check_auth_provider ]")
