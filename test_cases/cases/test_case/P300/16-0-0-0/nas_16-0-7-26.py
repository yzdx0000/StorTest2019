...
#   2、add_smb_export_auth_clients指定name为存在的group name，type为GROUP，run_as_root为false，permission_level为rw；
#   pscli --command=add_smb_export_auth_clients --export_id=x --name=exist_group_name --type=GROUP --run_as_root=false --permission_level=rw
#   3、update_smb_global_config配置home_dir；
#   pscli --command=update_smb_global_config --id=x --home_dir=volume:/homedir
#   4、mstsc到某windows设备，使用用户名进行挂载，在弹出的登录框中输入exist_group_name组中任意存在的用户名和密码；
...
def executing_case():
    ...
    auth_provider_name = 'nas_16_0_7_26_ad_auth_name'
    ...
    unix_id_range = [7000, 7099]
    # for create_access_zone
    node_ids = nas_common.get_node_ids()
    access_zone_name = 'nas_16_0_7_26_access_zone_name'
    # for create_file
    dir_name = 'nas_16-0-7-26_dir'
    create_file_path = nas_common.ROOT_DIR + dir_name
    create_home_dir_file_path = nas_common.ROOT_DIR + 'nas_16-0-7-26_homedir'
    # for create_smb_export，不配authorization_ip即允许所有ip
    export_name = 'nas_16_0_7_26_smb_export_name'
    ...
    disk_symbol = 'x:'
    mount_path = '\\\\10.2.40.49\\' + nas_common.AD_USER_2
    user_passwd = nas_common.AD_USER_PW
    user1 = 'adtest\\' + nas_common.AD_USER_1   # 预期：登录失败
    user2 = 'adtest\\' + nas_common.AD_USER_2   # 预期：登录成功
    # for 写数据
    filename = 'file_16-0-7-26'
    ...
    time.sleep(10)
    retcode, output = nas_common.smb_mount(disk_symbol, mount_path, user_passwd, user1)
    log.info(output)
    common.judge_rc(retcode, 86, 'smb_mount failed')
    ...
    time.sleep(10)
    retcode, output = nas_common.smb_mount(disk_symbol, mount_path, user_passwd, user2)
    log.info(output)
