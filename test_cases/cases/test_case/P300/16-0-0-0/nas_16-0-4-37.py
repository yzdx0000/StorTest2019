...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：16-0-x-x 启动NTFSACL后，不允许创建NTFSACL
#######################################################
...
def executing_case():
    """
        1、创建一个开启NTFS ACL且不允许创建NTFS ACL的smb共享；
        pscli --command=create_smb_export --access_zone_id=1 --export_name=smb_2 --export_path=parastor:/smb_2/ --enable_ntfs_acl=true --allow_create_ntfs_acl=false
        2、get_smb_exports查看步骤1中配置信息是否准确；
    ...
    check_result2 = nas_common.create_smb_export(access_zone_id=access_zone_id,
                                                 export_name="nas_16_0_4_37_smb_export_name",
