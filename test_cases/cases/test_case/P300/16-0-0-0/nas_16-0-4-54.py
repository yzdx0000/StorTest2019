...
def executing_case():
    """
        ...
        pscli --command=create_smb_export --access_zone_id=x --export_name=smb_exp_test --export_path=volume:/nas/smb_dir --authorization_ip=x.x.x.1
        4、查看smb共享路径配置成功，信息是否与配置信息匹配；
        pscli --command=get_smb_exports
        5、修改授权IP地址为x.x.x.2
        pscli --command=update_smb_export --id=x --authorization_ip=x.x.x.2
        6、查看smb信息，授权ip地址已经修改完成；
        pscli --command=get_smb_exports
    :return:
    """
    ...
    old_authorization_ip = "1.1.1.1"
    new_authorization_ip = "1.1.1.2"
    ...
    check_result3 = nas_common.create_smb_export(access_zone_id=access_zone_id,
                                                 export_name="nas_16_0_4_54_smb_export_name",
                                                 export_path=create_file_path2,
                                                 authorization_ip=old_authorization_ip)
    ...
    check_result5 = nas_common.update_smb_export(export_id=result,
                                                 authorization_ip=new_authorization_ip)
    if check_result5["detail_err_msg"] != "":
        log.error(("%s Failed") % FILE_NAME)
        raise Exception(("%s Failed") % FILE_NAME)

    check_result6 = nas_common.get_smb_exports(ids=result)
...
class Nas_Class_16_0_4_54():
    def nas_method_16_0_4_54(self):
        nas_main()
