...
def executing_case():
    """
        1、使用脚本创建达到规格上限的FTP共享；[Support only one export directory in each accesszone]
    :return:
    """
    ...
    for i in range(num):
        ...
        dir_name = "nas_16-0-4-142_dir" + "_%s" % count
        create_file_path = nas_common.ROOT_DIR + dir_name
        export_name = "nas_16_0_4_142_ftp_export_name" + "_%s" % count
        ...
        if count < num:
            ...
            check_result3 = nas_common.get_ftp_exports(ids=result)
            ...
            for export in exports:
                if cmp(export, {
                    ...
                    log.error(("%s Failed") % FILE_NAME)
                    raise Exception(("%s Failed") % FILE_NAME)
        elif count == num:  # 第2次循环，创建失败
            check_result4 = nas_common.create_ftp_export(access_zone_id=access_zone_id,
                                                         user_name=nas_common.AD_USER_1,
                                                         export_path=create_file_path)
            exe_info = nas_common.get_ftp_exports()
            if exe_info['result']['total'] != 1:
                common.except_exit("ftp export info is incorrect !!!")
            else:
                log.info('a user can only create one ftp export !')
