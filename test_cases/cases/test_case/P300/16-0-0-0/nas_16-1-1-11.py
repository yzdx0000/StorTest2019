...
#    7、NFS客户端成功挂载到某机头节点并使用vdbench进行读写业务，同时触发oPara换主
#    8、删除读写数据
#    9、将域名解析文件恢复原状
...
def case():
    ...
    rc, typical_eth = ob_client.get_client_eth_by_ip(clientip=nas_common.CLIENT_IP_3, ip_for_eth=tmp_client_ip)
    ...
    p2 = Process(target=make_fault.kill_process, args=(SYSTEM_IP_0, 'oPara'))
