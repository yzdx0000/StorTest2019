...
def executing_case():
    """
        1、创建一个共享路径长度为4095的FTP共享；
        2、创建一个共享路径长度为4096的FTP共享；
    :return:
    """
    ...
    # create_file
    dir_name = ""
    base_dir = "nas_16-0-4-138_dir"
    get_file_list_path = ""
    dir_name_list = [base_dir]
    get_file_list_path_list = [nas_common.ROOT_DIR]
    create_file_path = nas_common.ROOT_DIR + "nas_16-0-4-138_dir"
    create_file_path_list = [create_file_path]
    # create_ftp_export
    export_name = "nas_16_0_4_138_ftp_export_name"
    ...
    for i in range(17): # 构造create_file_path_list 16次：前15次是256*15，最后一次是bbb和ccc；循环创建、查询17次
        ...
        if i < 15:      # 循环前15次
            ...
            file_path_base = os.path.join(NAS_PATH, base_dir)
            ...
            dir_name = "c" * (1018 - len(file_path_base) - 60*16 - len("/"))
            dir_name_list.append(dir_name)
    ...
    print len(create_file_path_list[i])
