...
def case():
    ...
    # 修改目录权限
    cmd = 'chmod 755 %s' % dir_child_1
    common.run_command(snap_common.CLIENT_IP_1, cmd)
    cmd = 'chmod 555 %s' % file_test_1
    common.run_command(snap_common.CLIENT_IP_1, cmd)
    ...
    # 4> 修改目录权限
    cmd = 'chmod 522 %s' % dir_child_1
    common.run_command(snap_common.CLIENT_IP_1, cmd)
    cmd = 'chmod 333 %s' % file_test_1
    common.run_command(snap_common.CLIENT_IP_1, cmd)
    ...
    # 获取文件权限
    rc, stdout = snap_common.get_file_permission(snap_common.CLIENT_IP_2, dir_child_1)
    if stdout != '755':
        log.error("%s permission is not right!!!" % dir_child_1)
        raise Exception("%s permission is not right!!!" % dir_child_1)
    rc, stdout = snap_common.get_file_permission(snap_common.CLIENT_IP_2, file_test_1)
    if stdout != '555':
        log.error("%s permission is not right!!!" % dir_child_1)
        raise Exception("%s permission is not right!!!" % dir_child_1)
