...
# date 2018-01-19
# @summary：
#    修改文件时，kill主lmos节点的oPara，检查快照内容
# @steps:
#    1、部署3个客户端；
#    2、客户端1在目录/mnt/parastor/snap/下执行vdbench 00脚本（创建文件）；
#    3、对目录/mnt/parastor/snap/创建快照a1；
#    4、客户端2在目录/mnt/parastor/snap/下执行vdbench（修改文件数据），在修改的过程中kill主lmos节点的oPara；
#    5、客户端2在目录/mnt/parastor/.snapshot/下执行vdbench 01脚本（校验数据正确性）；
#    6、删除快照；
#    7、3个客户端检查是否有快照路径入口；
#
# @changelog：
