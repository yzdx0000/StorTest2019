...
#    5、对快照a1进行revert；
#    6、revert完成后，客户端2在/mnt/parastor/snap_parent/使用vdbench运行01脚本（vdbench验证数据）；
#    7、删除快照，删除快照的过程中kill主lmos节点的oPara；
#    8、3个客户端检查是否有快照路径入口；
#
# @changelog：
####################################################################################
...
def case():
    ...
    # 7> 并行删除快照，kill住lmos节点的oPara
    p1 = Process(target=_delete_snapshot)
