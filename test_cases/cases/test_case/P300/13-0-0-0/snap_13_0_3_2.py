...
# @steps:
#    1、部署3个客户端；
#    2、/mnt/liyao/snap下创建大小为16k的非空文件test_file1；
#    3、对目录创建快照；
...
SNAP_TRUE_PATH = os.path.join(snap_common.SNAP_PATH, FILE_NAME)             # /mnt/volume1/snap/snap_13_0_3_2
CREATE_SNAP_PATH = os.path.join(snap_common.SNAP_PATH_BASENAME, FILE_NAME)  # /snap/snap_13_0_3_2


def case():
    '''2>创建文件'''
    file_path = os.path.join(SNAP_TRUE_PATH, 'test_file1')
    cmd = 'dd if=%s of=%s bs=1k count=16' % (snap_common.get_system_disk(snap_common.CLIENT_IP_1), file_path)
