...
# date 2018-05-28
# @summary：
#   对根目录创建配额（逻辑空间，硬阈值），配额限制小于初始文件占用总空间，创建快照并查看回滚结果
# @steps:
#    1、在/mnt/volume1/下创建总容量为1000M的文件；
#    2、对上述目录创建logical_hard_threshold，配额限制为500M；
#    3、对根目录创建快照；
#    4、删除目录下容量大小为200M的文件；
#    5、在根目录/mnt/volume1/下尝试写入总空间大小为300M的文件；
#    6、执行revert并查看回滚结果；
#    7、删除配额；
#
# @changelog：
...
SNAP_TRUE_PATH = os.path.join(snap_common.SNAP_PATH, FILE_NAME)             # /mnt/volume1/snap/snap_13_0_8_14
CREATE_SNAP_PATH = os.path.join(snap_common.SNAP_PATH_BASENAME, FILE_NAME)  # /snap/snap_13_0_8_14
...
def case():
    ...
    rc, check_result = quota_common.create_one_quota(path=quota_path,
                                                     ...
                                                     logical_hard_threshold=524288000)
    ...
    # 5> 目录下尝试创建总容量为300M的文件
    file_name = temp_name + '_again'
    file_path = os.path.join(BASE_SNAP_PATH, file_name)
    cmd = 'dd if=%s of=%s bs=1M count=300' % (snap_common.get_system_disk(snap_common.CLIENT_IP_1), file_path)
    ...
    for i in range(length):
        ...
        print statistic_sum
    if statistic_sum > 819200:                                   # 若目录下文件占用总空间超过配额限制，则报错
