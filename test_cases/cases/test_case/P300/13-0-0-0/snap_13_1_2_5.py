...
# date 2018-04-17
#@summary：
#    创建完文件快照，修改文件，拔掉数据盘，观察快照内容
#@steps:
#    1、部署3节点集群，配比4 2 1；
#    2、在/mnt/parastor/snap/下创建文件snap_file（大小1g，dd命令创建）；
#    3、对文件/mnt/parastor/snap/snap_file创建快照a1；
#    4、对文件/mnt/parastor/snap/snap_file进行修改，过程中拔出一个节点的两块数据盘；
#    5、检查目录/mnt/parastor/.snapshot/下快照
#    6、删除快照；
#    7、检查是否有快照路径入口；
#
#@changelog：
...
SNAP_TRUE_PATH = os.path.join(snap_common.SNAP_PATH, FILE_NAME)              #/mnt/liyao/snap/snap_13_1_2_5
CREATE_SNAP_PATH = os.path.join(snap_common.SNAP_PATH_BASENAME, FILE_NAME)   #/snap/snap_13_1_2_5
...
def data_change(file_name):
    time.sleep(10)
    cmd = "echo 'nice to meet you'>> %s" % file_name
...
def case():
    ...
    if p1.exitcode != 0:
        ...
        case_flag = False
        #raise Exception("modify file failed!!!!!!")
    ...
    # 5> 数据校验
    snap_file = os.path.join(snap_common.SNAPSHOT_PAHT, 'snap_13_1_2_5_snapshot1')
