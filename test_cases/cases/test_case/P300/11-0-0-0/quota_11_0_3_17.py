...
# Date: 2018-09-03
# @summary：
#    用户组：对父目录a创建硬阈值配额2000，创建子目录b硬阈值为1000，子目录写1000个文件，再写1个文件失败
#    在父目录下再写1000个文件，再写文件失败，删除子目录配额，在子目录中创建文件仍失败
# @steps:
#    1.创建用户用户组
#    2.创建目录FILENAME/nesting1_1
#    3.创建目录FILENAME配额,硬阈值2000 等待配额状态为work
#    4.创建子目录FILENAME/nesting1_1配额,硬阈值1000,等待配额状态为work
#    5.子目录创建1001个文件，预期total_inode=1000
#    6.父目录创建1001个文件，预期父目录下total_inode=2000
...
def case():
    ...
    log.info("5> 子目录创建1001文件，预期total_inode=1000")
    ...
    dir1_1_inode = quota_common.group_total_inodes(quota_common.CLIENT_IP_1, dir1_1, quota_common.QUOTA_GROUP)
    if dir1_1_inode != 1000:
        rc_lst[sys._getframe().f_lineno] = 'dir: %s total_inode: %s ' % (dir1_1, dir1_1_inode)

    log.info("6> 父目录创建1001个文件，预期父目录下total_inode=2000")
    quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_2, dir0, 1001, 1, 'b',  quota_common.QUOTA_USER, quota_id=quota_id_lst[0])
    dir0_inode = quota_common.group_total_inodes(quota_common.CLIENT_IP_2, dir0, quota_common.QUOTA_GROUP)
    if dir0_inode != 2000:
    ...
    dir1_1_inode = quota_common.group_total_inodes(quota_common.CLIENT_IP_3, dir1_1, quota_common.QUOTA_GROUP)
    if dir1_1_inode != 1000:
        rc_lst[sys._getframe().f_lineno] = 'dir: %s total_inode: %s ' % (dir1_1, dir1_1_inode)
