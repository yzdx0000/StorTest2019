...
# Date: 2018-10-24
# @summary：
#    用户:目录配额  父目录下写入属主为用户和root的文件，查看是否只统计用户；
#         改变属主为root文件的属主为用户，查看此时的配额统计是否正确；删除
#         底层所有文件，查看经过上述操作后，配额规则是否生效
# @steps:
#    1.创建用户，用户组
#    2.创建目录FILENAME
#    3.创建父目录配额,硬阈值1G 等待配额状态为work
#    4.父目录下创建512M属主为quota_user的文件
#    5.父目录下创建100M属主为root的文件
#    6.将父目录下属主为quota_user的文件改变为属主为root的文件,延时5s
#    7.查看父目录的已使用量应为0
#    8.删除父目录的所有文件
#    9.查看规则是否生效，父目录下创建1100M属主为quota_user的文件,预计1G
# @changelog：
#################################################################
...
def case():
    ...
    log.info('4> 父目录下创建512M属主为quota_user的文件')
    if LARGE_BS:
        quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_1, dir0, 1, 512, 'quota_user',
                                                                quota_common.QUOTA_USER)
    else:
        quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_1, dir0, 512, 1, 'quota_user',
                                                                quota_common.QUOTA_USER)
    ...
    log.info('5> 父目录下创建100M属主为root的文件')
    ...
    log.info('9> 查看规则是否生效，父目录下创建1100M属主为quota_user的文件,预计1G')
