...
# Date: 2018-09-03
# @summary：
#    用户 父目录软阈值1000，宽限时间60秒，子目录软阈值1000，宽限时间60秒，子目录创建1001文件开始计时，
#    过期后俩目录无法创建文件
# @steps:
#    1.用户用户组
...
#    5.子目录创建1001文件开始计时，过期后俩目录无法创建文件
#    6.父目录删除配额创建文件成功
#    7.子目录删除配额创建文件成功
# @changelog：
#################################################################
...
def case():
    ...
    log.info("5> 子目录创建1001文件开始计时，过期后俩目录无法创建文件")
    ...
    dir_inode = quota_common.user_total_inodes(quota_common.CLIENT_IP_1, dir0,quota_common.QUOTA_USER)
    if dir_inode != 1101:
        rc_lst[sys._getframe().f_lineno] = 'dir: %s total_inode: %s ' % (dir0, dir_inode)
