...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-0-1-70 目录用户组逻辑空间配额硬阈值之chgrp方式
#######################################################
...
def executing_case():
    ...
    quota_common.change_file_group(quota_common.CLIENT_IP_1, quota_common.QUOTA_PATH,
                                   user_file_list, quota_common.QUOTA_GROUP)
    ...
    quota_common.change_file_group(quota_common.CLIENT_IP_1, quota_common.QUOTA_PATH,
                                   user_file_list, quota_common.QUOTA_OTHER_GROUP)
    ...
    if total_file_size != quota_common.FILE_SIZE_2G:
        ...
        log.error("11-0-1-70 Failed")
        raise Exception("11-0-1-70 Failed")
    else:
        log.info("11-0-1-70 Succeed")
    return
...
class Quota_Class_11_0_1_70():
    def quota_method_11_0_1_70(self):
