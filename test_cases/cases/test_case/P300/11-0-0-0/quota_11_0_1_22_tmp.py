...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-0-1-22 目录物理空间配额混合阈值，硬阈值先生效
#######################################################
...
def executing_case():
    ...
    '''
    # 再尝试继续写入文件
    quota_common.creating_files(quota_common.CLIENT_IP_2, quota_common.QUOTA_PATH, 1, 100, "b")

    # 检查配额是否生效
    if quota_common.dir_total_file_size(quota_common.CLIENT_IP_1, quota_common.QUOTA_PATH) != "2008M":
        raise Exception("11-0-1-22 Failed")
    else:
        print "11-0-1-22 Succeed"
...
def preparing_environment():
    ...
    2、创建配额测试相关的目录和文件
    '''
    # 针对目录配置物理空间为2G硬阈值、3G软阈值（宽限天数为1天）、4G建议阈值的配额
...
class Quota_Class_11_0_1_22():
    def quota_method_11_0_1_22(self):
