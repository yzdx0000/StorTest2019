...
# Date: 2018-09-03
# @summary：
#    对父目录a创建硬阈值配额1000，创建子目录b硬阈值为1000，在父目录下创建文件，
#    第1000个失败，在子目录创建文件失败，删除父目录配额，在子目录创建文件成功
# @steps:
#    1.创建目录FILENAME/nesting1_1
#    2.创建目录FILENAME配额,硬阈值1000 等待配额状态为work
#    3.创建子目录FILENAME/nesting1_1配额,硬阈值1000,等待配额状态为work
#    4.父目录创建1001个文件，预期total_inode=1000
#    5.子目录创建文件，预期失败
#    6.父目录删除配额，子目录创建文件成功，创建1001个，预期1000个
# @changelog：
#################################################################
...
def case():
    ...
    log.info("4> 父目录创建1001个文件，预期total_inode=1000")
    quota_common.creating_files(quota_common.CLIENT_IP_1, dir0, 1001, 1, 'a', quota_id=quota_id_lst[0])
    ...
    log.info("6.父目录删除配额，子目录创建文件成功，创建1001个，预期1000个")
    ...
    quota_common.creating_files(quota_common.CLIENT_IP_3, dir1_1, 1001, 1, 'c')
...
def main():
    ...
    case()
    prepare_clean.quota_test_clean(FILE_NAME, fault=False)
