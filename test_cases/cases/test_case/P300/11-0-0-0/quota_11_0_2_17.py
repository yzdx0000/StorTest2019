...
def executing_case():
    ...
    # 针对该目录用户先写入10万个小文件
    quota_common.creating_1k_files_by_designated_user_or_group(quota_common.CLIENT_IP_1, quota_common.QUOTA_PATH,
                                                               100000, 1, "a", quota_common.QUOTA_USER)
    ...
    print "filenr_used_nr = %s" % (filenr_used_nr)

    # 结果检查
    if check_result1["detail_err_msg"] != "" \
            or filenr_used_nr != 100000:
        rc, stdout = quota_common.get_all_quota_info()
        common.judge_rc(rc, 0, "get quota failed")
        log.error("11-0-2-17 Failed")
        raise Exception("11-0-2-17 Failed")
    else:
        log.info("11-0-2-17 Succeed")
    return
...
class Quota_Class_11_0_2_17():
    def quota_method_11_0_2_17(self):
