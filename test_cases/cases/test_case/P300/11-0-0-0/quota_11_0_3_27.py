...
# Date: 2018-09-03
# @summary：
#    父目录软阈值1G，宽限时间60秒，三个子目录软阈值300M，宽限时间60秒，3个子目录分别创建400M文件，
#    过期后所有目录无法创建文件
# @steps:
#    1.创建目录FILENAME/nesting1_1  nesting1_2  nesting1_3
#    2.创建目录FILENAME配额,软阈值1G 宽限时间60秒 等待配额状态为work
#    3.创建子目录FILENAME/nesting1_x/配额,子目录软阈值300M，宽限时间60秒 等待配额状态为work
...
def case():
    ...
    log.info("2> 创建目录FILENAME配额,软阈值1G 宽限时间60秒 等待配额状态为work")
    ...
    log.info("3> 创建子目录FILENAME/nesting1_x配额,子目录软阈值300M，宽限时间60秒 等待配额状态为work")
    for i in range(3):
    ...
    log.info("4> 3个子目录分别创建400M文件开始计时，过期后3目录无法创建文件")
    for i in range(3):
        if LARGE_BS:
            quota_common.creating_files(quota_common.CLIENT_IP_1, dir1_[i], 1, 400, 'a', quota_id=quota_id_lst[i+1])
        else:
            quota_common.creating_files(quota_common.CLIENT_IP_1, dir1_[i], 400, 1, 'a', quota_id=quota_id_lst[i + 1])

    dir_size = quota_common.dir_total_file_size(quota_common.CLIENT_IP_1, dir1_[0])
    if dir_size != 400 * quota_common.FILE_SIZE_1M:
        rc_lst[sys._getframe().f_lineno] = 'dir: %s total_size: %s ' % (dir1_[0], dir_size)
    dir_size = quota_common.dir_total_file_size(quota_common.CLIENT_IP_2, dir1_[1])
    if dir_size != 400 * quota_common.FILE_SIZE_1M:
        rc_lst[sys._getframe().f_lineno] = 'dir: %s total_size: %s ' % (dir1_[1], dir_size)
    dir_size = quota_common.dir_total_file_size(quota_common.CLIENT_IP_3, dir1_[2])
    if dir_size != 400 * quota_common.FILE_SIZE_1M:
    ...
    for i in range(3):
        quota_common.creating_files(quota_common.CLIENT_IP_1, dir1_[i], 1, 1, 'b', quota_id=quota_id_lst[1+1])
