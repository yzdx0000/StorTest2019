...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-0-1-72 目录用户组逻辑空间配额建议阈值之chgrp方式
#######################################################
...
def executing_case():
    ...
    # 再尝试继续写入文件
    # （1）先写500M属组为root的文件，预期写入成功
    quota_common.creating_files(quota_common.CLIENT_IP_2, quota_common.QUOTA_PATH,
                                512, 1, "c")
    # （2）再写500M属组为quota_other_group的文件，预期写入成功
    quota_common.creating_files(quota_common.CLIENT_IP_1, quota_common.QUOTA_PATH,
                                512, 1, "d")
    ...
    # （3）再写1G属组为quota_group的文件，预期写入成功
    quota_common.creating_files(quota_common.CLIENT_IP_2, quota_common.QUOTA_PATH,
                                1024, 1, "e")
    ...
    if total_file_size != quota_common.FILE_SIZE_3G:
        rc, check_result3 = quota_common.get_all_quota_info()
        common.judge_rc(rc, 0, "get quota failed")
        log.error("11-0-1-72 Failed")
        raise Exception("11-0-1-72 Failed")
    else:
        log.info("11-0-1-72 Succeed")
    return
...
class Quota_Class_11_0_1_72():
    def quota_method_11_0_1_72(self):
