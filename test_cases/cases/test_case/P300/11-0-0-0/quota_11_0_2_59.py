...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-0-2-59 修改目录用户组配额规则之改变配额类型
#######################################################
...
def executing_case():
    ...
    if (check_result1["detail_err_msg"] != "" or
        ...
        rc, check_result = quota_common.get_all_quota_info()
        common.judge_rc(rc, 0, "get quota failed")
        log.error("11-0-2-59 Failed")
        raise Exception("11-0-2-59 Failed")
    else:
        log.info("11-0-2-59 Succeed")
    return
