...
#    time.sleep(86400)
    ...
    if total_file_size != quota_common.FILE_SIZE_1M * (2048 + soft_val):
        rc, stdout = quota_common.get_all_quota_info()
        common.judge_rc(rc, 0, "get quota failed")
        log.error("11-0-1-11 Failed")
        raise Exception("11-0-1-11 Failed")
    else:
        log.info("11-0-1-11 Succeed")
    return
...
def preparing_environment():
    ...
    # 针对目录配置逻辑空间为2G软阈值，宽限天数为1天的配额【临时：--filenr_grace_time=60s】
    ...
    rc, check_result = quota_common.create_one_quota(path=('%s:/%s' % (quota_common.VOLUME_NAME, quota_dir)),
                                                     ...
                                                     logical_soft_threshold=quota_common.FILE_SIZE_2G,
...
class Quota_Class_11_0_1_11():
    def quota_method_11_0_1_11(self):
