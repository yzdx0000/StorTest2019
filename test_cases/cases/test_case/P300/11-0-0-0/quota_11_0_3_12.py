...
# Date: 2018-09-03
# @summary：
#    用户 父目录软阈值2000，宽限时间60秒，子目录软阈值1000，宽限时间60秒，子目录创建1001文件开始计时，
...
def case():
    ...
    log.info('6> 父目录创建1001文件开始计时，过期后子目录无法创建文件')
    ...
    dir_inode = quota_common.user_total_inodes(quota_common.CLIENT_IP_2, dir0, quota_common.QUOTA_USER)
    if dir_inode != 2102:
        rc_lst[sys._getframe().f_lineno] = 'dir: %s total_inode: %s ' % (dir0, dir_inode)
    ...
    dir_inode = quota_common.user_total_inodes(quota_common.CLIENT_IP_3, dir1_1, quota_common.QUOTA_USER)
    if dir_inode != 1201:
        rc_lst[sys._getframe().f_lineno] = 'dir: %s total_inode: %s ' % (dir1_1, dir_inode)
