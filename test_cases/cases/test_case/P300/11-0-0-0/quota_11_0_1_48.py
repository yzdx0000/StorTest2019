...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-0-1-48 目录用户逻辑空间配额混合阈值，建议阈值先生
#         效，软阈值其次生效，限期内硬阈值不生效之chown方式
#######################################################
...
#    time.sleep(86400)
    ...
    if total_file_size != quota_common.FILE_SIZE_1M * (soft_val_2 + 1536*2):
        ...
        log.error("11-0-1-48 Failed")
        raise Exception("11-0-1-48 Failed")
    else:
        log.info("11-0-1-48 Succeed")
    return
...
def preparing_environment():
    ...
    # 针对目录用户配置逻辑空间为4G硬阈值、3G软阈值（宽限天数为1天）、2G建议阈值的配额【临时：--logical_grace_time=60s】
...
class Quota_Class_11_0_1_48():
    def quota_method_11_0_1_48(self):
