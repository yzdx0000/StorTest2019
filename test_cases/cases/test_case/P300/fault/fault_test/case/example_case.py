...
def case(case_log_path):
    ...
    file_name = FILE_NAME.replace('_', '-') + '.conf'
    fault_config_file = os.path.join(fault_config_path, file_name)
    fault_num = fault_param['fault_num']
    fault_num = 5
    print fault_config_file, fault_num
    # 判断故障配置文件是否存在
    if os.path.exists(fault_config_file) is False:
        main_log.error("fault config do not exist, please add it in config dir!")
        sys.exit(1)
    ...
    if if_run_cosbench is False:
        main_log.info("cosbench is not run, fault will start after 5 min... ")
        time.sleep(300)
        main_log.info("start fault...")
        obj_fault_test.start(case_log_path=case_log_path,
                             config_file=fault_config_file,
                             test_nums=fault_num)
    else:
        for i in range(20000):
            main_log.info("---------wait for fault times: %s------------" % i)
            # 判断是否启用故障
            main_log.info('obj_fault_test.is_running:%s' % obj_fault_test.is_running)
            main_log.info('cosbench_test.IF_MAKE_FAULT:%s' % cosbench_test.IF_MAKE_FAULT)
            if (obj_fault_test.is_running is False) and (cosbench_test.IF_MAKE_FAULT is True):
                main_log.info("start fault...")
                obj_fault_test.start(case_log_path=case_log_path,
                                     config_file=fault_config_file,
                                     test_nums=fault_num)
    ...
    for i in range(20000):
        ...
        main_log.info("obj_fault_test: %s" % obj_fault_test.is_running)

        # 故障结束
        if obj_fault_test.is_running is False:
            main_log.info("stop all stress work!!!")
            for opt in operation_list:
                opt.stop()

            # 判断故障线程是否正常结束
            if obj_fault_test.return_value is True:
                main_log.info("obj_fault_test thread is stop in normal")
                case_result = True
            else:
                main_log.error("obj_fault_test thread is stop in abnormal")
            break

        # 压力业务结束
        elif (check_env_test.is_running() and
            ...
            main_log.info("stop all work include fault!!!")
            obj_fault_test.stop()
            for opt in operation_list:
                opt.stop()
            break
    ...
    while True:
        ...
        if (check_env_test.is_running() or cosbench_test.is_running() or
                ...
                iozone_posix_test.is_running() or obj_fault_test.is_running) is True:
            continue
        else:
            break
...
def main():
    ...
    if case_result is False:
        main_log.info('>step 05：重新部署系统')
        prepare_all.auto_install_and_prepare(case_log_path)
        ...
        if check_result == 0:
            ...
            main_log.error('environment can not recover, please deploy parastor')
            main_log.info('>step 06：重新部署系统')
            prepare_all.auto_install_and_prepare(case_log_path)

    main_log.info('>step 07：将用例结果输出')
    if not case_result:
