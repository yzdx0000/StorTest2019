...
def case():
    ...
    file_path = os.path.join(test_path, 'file_5T')
    rc, stdout = s3_common.truncate_file(file_path, file_size)
    common.judge_rc(rc, 0, "create file %s failed!!!" % file_path)
    """分割文件为1G大小"""
    os.chdir(test_path)
    cmd = "split -b 5G %s -d -a 4" % file_path
    common.command(cmd)
