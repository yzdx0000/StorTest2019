...
# *******************************************
# ACL验证函数
# 用ak2对桶ACL修改为读权限，在使用ak2进行对象列表获取操作。
# 用ak2对acc1的对象读取ACL操作,下载操作。
# *******************************************
def confirm_acl(bucket_name, objectname, filepath, ak2, accid1, accid2):
    log.info("【桶完全权限验证1】,使用ak2对acc1的桶读取对象列表.")
    rc8, opt8 = s3_common.get_bucket_acl_info(bucket_name, ak2)
    judge_result(rc8, FILE_NAME)
    common.judge_rc(rc8, 0, "%s failed!" % FILE_NAME)
    ...
    rc2, opt2 = s3_common.get_obj_in_bucket(bucket_name, ak2)
    judge_result(rc2, FILE_NAME)
    common.judge_rc(rc2, 0, "%s failed!" % FILE_NAME)

    log.info("【桶完全权限验证4】,使用ak2对acc1的桶读ACP操作.")
    rc3, opt3 = s3_common.get_bucket_acl_info(bucket_name, ak2)

    log.info("【对象完全权限验证1】使用ak2对acc1的对象读取ACL操作。")
    rc4, opt4, opt5, opt6 = s3_common.get_object_acl(
        bucket_name, objectname, ak2)
    judge_result(rc2, FILE_NAME)
    common.judge_rc(rc2, 0, "%s failed!" % FILE_NAME)

    log.info("【对象完全权限验证2】使用ak2对acc1的对象进行下载操作，验证对象的ACL读权限")
    rc1, opt7 = s3_common.download_object(
    ...
    return opt1, opt2, opt3, opt6, opt8, md5_d
...
def case():
    ...
    log.info("5> ACL验证")
    opt3, opt4, opt5, opt6, opt7, md5_d = confirm_acl(
        bucket_name1, object_name1, FILE_NAME, ak2, accid1, accid2)

    if "200 OK" in opt1 and "200 OK" in opt2 and opt6[0] == "FULL_CONTROL" and "200 OK" in opt7\
            and "200 OK" in opt3 and "200 OK" in opt4 and "403" in opt5 and md5_o == md5_d:
        pass
    else:
