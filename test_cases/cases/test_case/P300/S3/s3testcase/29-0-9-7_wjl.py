...
# -*- coding:utf-8 -*-
# **********************************************#
# 标题：功能测试，用例编号29-0-9-7
# 使用方法：python 29-0-9-xx_wjl.py
# 将账户1的bucket赋予账户2写ACP权限。
...
def confirm_acl(bucket_name, objectname, filepath, ak2, accid1, accid2):
    log.info("【桶写ACP权限验证1】使用ak2对acc1的桶ACL修改为READ。")
    rc1, opt1 = s3_common.set_bucket_acl(
        bucket_name, ak2, accid1, email1, accid2, email2, "READ")
    judge_result(rc1, FILE_NAME)
    common.judge_rc(rc1, 0, "%s failed!" % FILE_NAME)

    log.info("【桶写ACP权限验证2】使用ak2对acc1的桶进行GET操作，读取对象列表。")
    rc2, opt2 = s3_common.get_obj_in_bucket(bucket_name, ak2)
    judge_result(rc2, FILE_NAME)
    common.judge_rc(rc2, 0, "%s failed!" % FILE_NAME)

    log.info("【桶写ACP权限验证3】,使用ak2对acc1的桶读ACP操作.")
    ...
    return opt1, opt2, opt3, opt6, md5_d
...
def case():
    ...
    log.info("5> ACL验证")
    opt3, opt4, opt5, opt6, md5_d = confirm_acl(
        bucket_name1, object_name1, FILE_NAME, ak2, accid1, accid2)

    if "200 OK" in opt1 and "200 OK" in opt2 and opt6[0] == "FULL_CONTROL" \
