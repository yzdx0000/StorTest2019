...
# -*- coding:utf-8 -*-
#**********************************************#
# 标题：功能测试，用例编号29-0-17-49_51_53
# 使用方法：python 29-0-17-49_51_53_wjl.py
...
def judge_result_judge(rc):
    judge_result(rc, "29-0-17-49_wjl")
    judge_result(rc, "29-0-17-51_wjl")
    judge_result(rc, "29-0-17-53_wjl")
...
def case():
    ...
    rc, opt = s3_common.set_object_acl(
        bucket_name1, object_name1, ak1, account_id1, email1, account_id2, email2, ACL_TYPE, sig)
    log.info(opt)
    log.info("******设置的读权限,end.******")
    judge_result_judge(rc)
...
# 验证写ACP权限
    ...
    string = "GET" + "\n" + "" + "\n" + "" + "\n" + "" + \
        "\n" + "" + "/" + bucket_name1 + "/" + object_name1
    sig = create_sig(sk2, string)
    log.info("******验证写ACP权限，使用ak2验证读权限,start.******")
    rc, opt = s3_common.download_object(
        bucket_name1, object_name1, FILE_NAME + "_down", ak2, sig)
    print rc
    log.info(opt)
    log.info("******验证写ACP权限，使用ak2验证读权限,end.******")
    rc, d_md5 = common.run_command_shot_time("md5sum %s" % FILE_NAME + "_down")
    d_md5 = d_md5[0:32]
    log.info("d_md5: %s" % d_md5)
    print type(d_md5)
    ...
    string = "GET" + "\n" + "" + "\n" + "" + "\n" + "" + \
        "\n" + "" + "/" + bucket_name1 + "/" + object_name1
    sig = create_sig(sk2, string)
    log.info("******验证完全权限，使用ak2验证读权限,start.******")
    rc, opt = s3_common.download_object(
        bucket_name1, object_name1, FILE_NAME + "_down", ak2, sig)
    print rc
    log.info(opt)
    log.info("******验证完全权限，使用ak2验证读权限,end.******")
    rc, d_md5 = common.run_command_shot_time("md5sum %s" % FILE_NAME + "_down")
    d_md5 = d_md5[0:32]
    log.info("d_md5: %s" % d_md5)
    print type(d_md5)
    ...
    StringToSign = "GET" + "\n" + "" + "\n" + "" + "\n" + "" + \
            "\n" + "" + "/" + bucket_name1 + "/" + object_name1 + "?acl"
    sig = create_sig(sk2, StringToSign)
    log.info("******验证完全权限，使用ak2读取buc1的ACL权限,start.******")
    rc, opt1, opt2, opt3 = s3_common.get_object_acl(
            bucket_name1, object_name1, ak2, sig)
    print opt3
    log.info("******验证完全权限，使用ak2读取buc1的ACL权限,end.******")
    if ACL_TYPE in opt3:
        pass
    else:
        judge_result_judge(-1)
...
def main():
    ...
    result.result("29_0_17_49_wjl", "0")
    result.result("29_0_17_51_wjl", "0")
    result.result("29_0_17_53_wjl", "0")
