...
# -*- coding:utf-8 -*-
#**********************************************#
# 标题：功能测试，用例编号29-0-17-40
# 使用方法：python 29-0-17-40_wjl.py
# 在http模式下，上传对象后错误sig获取对象列表。
# 作者：王建磊
# 创建时间：2018/10/09
#**********************************************#
import sys
...
def case():
    ...
    rc1, opt1 = s3_common.get_all_object_in_bucket(bucket_name, ak, "  ")
    rc2, opt2 = s3_common.get_all_object_in_bucket(
        bucket_name, ak, sig + "@#%$#FD")
    rc3, opt3 = s3_common.get_all_object_in_bucket(bucket_name, ak, "!@#$@#%=")
    rc4, opt4 = s3_common.get_all_object_in_bucket(
        bucket_name, ak, "DSG#@" + sig)
    if opt1 == opt2 == opt3 == opt4 is None:
        rc = 0
    else:
        rc = -1
    judge_result(rc, FILE_NAME)
    common.judge_rc(rc, 0, "%s failed!" % FILE_NAME)
    # if opt[0] == object_name:
    # 	pass
    # else:
