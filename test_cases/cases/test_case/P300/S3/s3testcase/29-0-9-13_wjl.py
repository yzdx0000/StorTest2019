...
# *******************************************
# 设置账户ACL函数
# 设置ACL，key=1;将acc1的bucket赋予acc2写权限。
...
def confirm_acl(bucket_name, objectname, ak2, accid1, accid2, key):
    ...
    opt = []
    if key == "1":
        log.info("【桶写权限验证1】,使用ak2对acc1的桶上传对象.")
...
def case():
    ...
    opt2 = acl_set_write(
        ...
        accid2,
        key="1")
    log.info("5.2> ACL写权限验证")
    rc_l2, opt_l2 = confirm_acl(
        bucket_name1, object_name1, ak2, accid1, accid2, key="1")

    if "200 OK" in opt1 and "200 OK" in opt2 and "200 OK" in opt_l2[0] and "403" in opt_l2[1] and "200 OK" in \
            opt_l1[0] and "200 OK" in opt_l1[1] and "200 OK" in opt_l1[2] and "403" in opt_l1[3]:
        pass
    else:
