...
#    5、向账户2中上传5个桶
#    6、获取账户1和账户2的桶列表
#    7、账户1赋予账户2其中一个桶的WRITE_ACP权限
#    8、验证账户2对账户1的桶具有WRITE_ACP权限
#    9、清理环境
# @changelog：
####################################################################################
...
def case():
    ...
    log.info("6> 账户%s赋予账户%s对桶%s的WRITE_ACP权限" % (account_name_1, account_name_2, bucket_name))
    rc, stdout = s3_common.set_bucket_acl(bucket_name, certificate_id_1, account_id_1, account_email_1,
                                          account_id_2, account_email_2, "WRITE_ACP", exe_node_ip=None)
    judge_result(rc, FILE_NAME)
    common.judge_rc(rc, 0, "账户%s赋予账户%s对桶%s的WRITE_ACP权限"
                           "时失败" % (account_name_1, account_name_2, bucket_name))

    log.info("7> 通过用账户%s对账户%s对桶%s设置READ权限，验证WRITE_ACP权限"
             "是否设置成功" % (account_name_2, account_name_3, bucket_name))
    ...
    judge_result(rc, FILE_NAME)
    common.judge_rc(rc, 0, "验证WRITE_ACP权限是否设置成功时,失败")
