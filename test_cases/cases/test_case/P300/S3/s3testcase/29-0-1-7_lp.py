...
def case():
    ...
    log.info("3> 账户%s赋予账户%s对桶%s的写权限" % (owner_account_name, dst_account_name, bucket_name))
    rc, stdout = s3_common.set_bucket_acl(bucket_name, owner_certificate_id, owner_account_id, owner_account_email,
                                          dst_account_id, dst_account_email, "WRITE", exe_node_ip=None)
    judge_result(rc, FILE_NAME)
    common.judge_rc(rc, 0, "账户%s赋予账户%s对桶%s的WRITE权限时"
                           "失败!!!" % (owner_account_name, dst_account_name, bucket_name))

    log.info("5> 验证账户%s对桶%s的WRITE权限是否设置成功" % (dst_account_name, bucket_name))
    ...
    judge_result(rc, FILE_NAME)
    common.judge_rc(rc, 0, "验证账户%s对桶%s的WRITE权限时，失败" % (dst_account_name, bucket_name))

    log.info("5> 验证账户%s对桶%s无读权限" % (dst_account_name, bucket_name))
    rc, stdout = s3_common.get_obj_in_bucket(bucket_name, dst_certificate_id, exe_node_ip=None)
    if '403 Forbidden' not in stdout:
        result.result(FILE_NAME, -1)
        common.judge_rc(-1, 0, "验证账户%s对桶%s无读权限时,返回码不是403" % (dst_account_name, bucket_name))
