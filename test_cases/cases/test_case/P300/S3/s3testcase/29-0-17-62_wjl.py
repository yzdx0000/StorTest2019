...
# -*- coding:utf-8 -*-
#**********************************************#
# 标题：功能测试，用例编号29-0-17-62
# 使用方法：python 29-0-17-62_wjl.py
# 在http模式下，并发下载对象过程中开启认证开关，并验证下载内容。
# 作者：王建磊
# 创建时间：2018/10/09
#**********************************************#
import sys
...
def download_obj(
    ...
    for i in range(obj_num):
        rc, opt = s3_common.download_object(
            bucketname, objectname, filepath + str(i), ak)
...
def case():
    ...
    o_md5 = o_md5[0:32]
    rc, opt = s3_common.add_object(
        bucketname, objectname, FILE_NAME, certificate_id)
    judge_result(rc, FILE_NAME)
