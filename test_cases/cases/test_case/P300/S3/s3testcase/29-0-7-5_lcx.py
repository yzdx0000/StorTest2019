...
def case():
    ...
    log.info("7> 多线程的方式实现下载对象过程中，删除bucket")
    file_down_path = os.path.join(test_path, object_name_1 + 'down')
    thread1 = MyThread(s3_common.download_object, args=(bucket_name_lst_base[0], object_name_1,
                                                  file_down_path, certificate_id,))
    thread2 = MyThread(s3_common.del_bucket, args=(bucket_name_lst_base[0], certificate_id,))
    ...
    log.info("down object and delete bucket success!")


    log.info("8> 检验MD5值")
    rc, base_file_md5 = s3_common.get_file_md5(file_path1)
    ...
    common.judge_rc_unequal(base_file_md5, down_file_md5, "md5 value is same!")
    log.info("the md5 value is not same, verify success!")
