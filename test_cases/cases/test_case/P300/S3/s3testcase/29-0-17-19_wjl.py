...
# -*- coding:utf-8 -*-
#**********************************************#
# 标题：功能测试，用例编号29-0-17-19
# 使用方法：python 29-0-17-19_wjl.py
# 在http模式下，错误sig获取桶的acl操作。
# 作者：王建磊
# 创建时间：2018/09/10
#**********************************************#
import sys
...
def case():
    ...
    rc1, opt1 = s3_common.get_bucket_acl_info(bucket_name, ak, " ")
    rc2, opt2 = s3_common.get_bucket_acl_info(
        bucket_name, ak, "@#%%$^" + sig + "%#$#$@")
    rc3, opt3 = s3_common.get_bucket_acl_info(bucket_name, ak, sig + "   &")
    rc4, opt4 = s3_common.get_bucket_acl_info(bucket_name, ak, sig + "=")
    log.info("******查询桶的ACL，end。******")

    if "403 Forbidden" in opt1 and "403 Forbidden" in opt2 and \
            "403 Forbidden" in opt3 and "403 Forbidden" in opt4:
        pass
    else:
