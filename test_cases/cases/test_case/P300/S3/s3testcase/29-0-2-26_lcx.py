...
def case():
    ...
    log.info("8> 修改ACL权限")
    """使用account2 修改account1 上传的对象的acl 权限 """
    ACL_TYPE = "READ"
    rc, stdout = s3_common.set_object_acl(bucket_name, object_name, certificate_id_2, account_id, ACCOUNT_EMAIL,
                                          account_id_2, ACCOUNT_EMAIL_2, ACL_TYPE)
    common.judge_rc(rc, 0, 'set object %s acl failed' % object_name)
    log.info("set object %s acl success!" % object_name)

    log.info("9> 验证修改后的ACL权限")
    """使用account2 获取account1 上传的对象的acl 权限——获取失败 """
    rc, stdout1, stdout2, stdout3 = s3_common.get_object_acl(
        bucket_name, object_name, certificate_id_2)
    common.judge_rc_unequal(rc, 0, 'get object %s acl failed' % object_name)
    log.info("get object %s acl failed!" % object_name)
