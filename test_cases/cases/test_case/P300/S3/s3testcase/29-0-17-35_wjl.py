...
# -*- coding:utf-8 -*-
#**********************************************#
# 标题：功能测试，用例编号29-0-17-35
# 使用方法：python 29-0-17-35_wjl.py
# 在http模式下，错误sig删除对象操作。
# 作者：王建磊
# 创建时间：2018/03/28
#**********************************************#
import sys
...
def case():
    ...
    log.info("5> 错误sig删除对象")
    string = "DELETE" + "\n" + "" + "\n" + "" + "\n" + "" + \
        "\n" + "" + "/" + bucket_name + "/" + object_name
    sig = create_sig(sk, string)
    rc1, opt1 = s3_common.del_object(bucket_name, object_name, ak, "  ")
    rc2, opt2 = s3_common.del_object(
        bucket_name, object_name, ak, "jkghsdlkh3242")
    rc3, opt3 = s3_common.del_object(
        bucket_name, object_name, ak, "#$%$#%&^&*")
    rc4, opt4 = s3_common.del_object(
        bucket_name, object_name, ak, sig + "artg")
