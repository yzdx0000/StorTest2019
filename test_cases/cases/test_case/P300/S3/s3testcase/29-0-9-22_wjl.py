...
# *******************************************
# 设置账户ACL函数
# 设置ACL，key=1;将acc1的object赋予acc2写ACP权限。
...
def confirm_acl(bucket_name, objectname, ak2, accid1, accid2, key):
    ...
    opt = []
    if key == "1":
        log.info("【对象写ACP权限验证1】,使用ak2修改acc1的对象为READ_ACP权限.")
        ...
        log.info("【对象完全权限验证3】,使用ak2修改acc1的对象为READ_ACP权限.")
        rc6, opt6 = s3_common.set_object_acl(
            bucket_name, objectname, ak2, accid1, email1, accid2, email2, "READ_ACP")
        rc.append(rc6)  # 200
        opt.append(opt6)
        judge_result(rc6, FILE_NAME)
        common.judge_rc(rc6, 0, "%s failed!" % FILE_NAME)

        log.info("【对象完全权限验证4】,使用ak2修改acc1的对象为WRITE_ACP权限.")
        rc7, opt7 = s3_common.set_object_acl(
            bucket_name, objectname, ak2, accid1, email1, accid2, email2, "WRITE_ACP")
        rc.append(rc7)  # 403
        opt.append(opt7)
    else:
        rc = []
...
def case():
    ...
    opt2 = acl_set_write(
        ...
        accid2,
        key="1")
    log.info("5.2> ACL写ACP权限验证")
    rc_l2, opt_l2 = confirm_acl(
        bucket_name1, object_name1, ak2, accid1, accid2, key="1")

    if "200 OK" in opt1 and "200 OK" in opt2 and "READ_ACP" == opt_l2[1][0] and "200" in opt_l2[0]\
        and "FULL_CONTROL" == opt_l1[0][0] and opt_l1[1] == md5_o \
