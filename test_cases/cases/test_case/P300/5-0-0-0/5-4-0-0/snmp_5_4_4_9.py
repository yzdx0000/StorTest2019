...
#   执行：
#           1>通过snmp协议方式获取nfs状态
#           2>通过p300系统发起请求获取到的nfs状态
#           3>对两种方式获取到的nfs状态进行比较
...
def case():
    ...
    for node_ip in allparastor_ips:
        snmp_nfs_status = snmp_common.get_one_node_service_nfs_status(node_ip)
        nfs_status = snmp_common.get_one_node_nfs_status(node_ip)
        log.info("nfs_status:%s" % nfs_status)

        if snmp_nfs_status != nfs_status:
