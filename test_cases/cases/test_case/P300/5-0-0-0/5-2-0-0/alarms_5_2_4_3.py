...
# @steps:
#   1、部署3节点集群环境
#   2、对目录/mnt/volume1/snap/alarms_5_2_4_3/创建文件数配额（硬阈值>软阈值），预期创建成功
#   3、在此目录下创建文件，数目超过软阈值
...
def case():
    ...
    hard_threshold_num = 10
    soft_threshold_num = 5
    rc, check_result = quota_common.create_one_quota(path=quota_path,
                                                     filenr_quota_cal_type='QUOTA_LIMIT',
                                                     filenr_hard_threshold=hard_threshold_num,
                                                     filenr_soft_threshold=soft_threshold_num,
                                                     filenr_grace_time=300)
    common.judge_rc(rc, 0, "create quota failed")

    '''3> 在此目录下创建文件，数目超过软阈值'''
    for i in range(hard_threshold_num):
