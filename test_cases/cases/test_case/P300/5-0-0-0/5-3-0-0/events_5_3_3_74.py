...
#   执行：
#           1>删除FTP共享
#           2>执行get_events查看删除FTP共享失败的结果显示
#           3>清理工作
# @changelog：
####################################################################################
...
ftp_path = nas_common.ROOT_DIR + "FTP_" + FILE_NAME  # parastor:/nas/FTP_events_5_3_3_74
...
def case():
    ...
    nas_common.delete_ftp_exports(fault_ftp_export_id)
    '''2> 执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)  # 删除FTP共享失败，延时
    code = '0x02030806'   # 删除FTP共享失败对应编号
    description = 'delete_ftp_exports_failed'
    '''删除FTP共享失败事件码是否在事件列表中'''
