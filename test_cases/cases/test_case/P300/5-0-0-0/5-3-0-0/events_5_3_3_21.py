...
#           2>创建AD认证服务
#   执行：
#           1>制作修改AD认证服务器成功事件（修改认证服务id）
#           2>执行get_events查看修改AD认证服务器成功的结果显示
# @changelog：
####################################################################################
...
def case():
    ...
    '''1> 修改AD认证服务器'''
    '''修改AD认证服务的名称，最大是63位'''
    update_username = "aaaaaaa1_aaaaaaa2_aaaaaaa3_aaaaaaa4_aaaaaaa5_aaaaaaa6__aaaaaaa7"
    check_result = nas_common.update_auth_provider_ad(provider_id=auth_provider_ad_id,
                                                      username=update_username)
    if check_result["detail_err_msg"] != "":
        common.except_exit("update ad provider failed!!")
    '''2> 执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)               # 修改AD认证服务器成功后，延时
    code = '0x02030403'                # 修改AD认证服务器失败对应编号
    description = 'update_ad_provider'
    '''修改AD认证服务器成功事件码是否在事件列表中'''
