...
#   执行：
#           1>制作添加AD认证服务器失败事件
#           2>执行get_events查看添加AD认证服务器失败的结果显示
# @changelog：
####################################################################################
...
def case():
    ...
    '''1> 制作添加AD认证服务器失败事件'''
    invalid_name = "1a1a2a3a4a5a6_a1a2a3a4a5a6"  # 名称以数字开头，不合法
    nas_common.add_auth_provider_ad(name=invalid_name,
                                    domain_name=nas_common.AD_DOMAIN_NAME,
                                    dns_addresses=nas_common.AD_DNS_ADDRESSES,    # 域名为none，预计添加AD失败
    ...
    code = '0x02030402'                # 添加AD认证服务器失败对应编号
    description = 'add_ad_provider_failed'
