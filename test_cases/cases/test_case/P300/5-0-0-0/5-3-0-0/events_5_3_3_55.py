...
#   执行：
#           1>创建SMB共享
#           2>执行get_events查看创建SMB共享成功的结果显示
#           3>清理工作
# @changelog：
####################################################################################
...
smb_path = nas_common.ROOT_DIR + "SMB_" + FILE_NAME                     # parastor:/nas/SMB_events_5_3_3_55
def case():
    """
    ...
    time.sleep(delay_time)               # 创建SMB共享成功，延时
    code = '0x02030701'                # 创建SMB共享成功对应编号
    description = 'create_smb_export'
    '''创建SMB共享成功事件码是否在事件列表中'''
    event_common.check_events_result(start_time, code, description)
    
    '''3> 清理工作'''
    '''3.1> 清理导出'''
    check_result = nas_common.delete_smb_exports(ids=smb_export_id)
    if check_result["detail_err_msg"] != "":
