...
#   执行：
#           1>创建NFS共享
#           2>执行get_events查看创建NFS共享成功的结果显示
#           3>清理工作
# @changelog：
####################################################################################
...
nfs_path = nas_common.ROOT_DIR + "NFS_" + FILE_NAME                     # parastor:/NFS_events_5_3_3_43.py
...
def case():
    ...
    if check_result["detail_err_msg"] != "":
        common.except_exit("create %s Failed" % nfs_path)    # /mnt/parastor/NFS_events_5_3_3_43.py
    ...
    time.sleep(delay_time)               # 创建NFS共享成功，延时
    code = '0x02030601'                # 创建NFS共享成功对应编号
    description = 'create_nfs_export'
    '''创建NFS共享成功事件码是否在事件列表中'''
