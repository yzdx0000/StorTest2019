...
#   3、删除上述影响策略，输入参数错误，预期删除失败
#   4、执行get_events，查看告警信息显示是否正确
#   5、删除上述影响策略，参数正群，预期成功
#
# @changelog：
####################################################################################
...
def case():
    ...
    '''3> 删除上述影响策略,参数错误预期失败'''
    rc, stdout = common.delete_jobengine_impact(0)
    if 0 != rc:
        log.error('deleting jobengine impact failed !!!')
    ...
    code = '0x02050014'
    description = 'deleting jobengine impact'
    ...
    common.judge_rc(rc, 0, 'delete jobengine impact')
