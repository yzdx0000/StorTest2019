...
def case():
    ...
    '''1> 删除认证服务器'''
    '''制造不合法id号，预期删除失败'''
    fault_auth_provider_ldap_id = auth_provider_ldap_id + 1000
    nas_common.delete_auth_providers(fault_auth_provider_ldap_id)

    '''2> 执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)               # 删除认证服务失败后，延时

    code = '0x02030414'                # 删除认证服务失败对应编号
    description = 'delete_provider_failed'

    '''删除认证服务器失败事件码是否在事件列表中'''
