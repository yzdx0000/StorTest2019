...
#   执行：
#           1>制作添加ldap认证服务器成功事件
#           2>执行get_events查看添加ldap认证服务器成功的结果显示
# @changelog：
####################################################################################
...
def case():
    ...
    time.sleep(delay_time)               # 添加ldap认证服务器后，延时

    code = '0x02030409'                # 添加ldap认证服务器成功对应编号
    description = 'add_ldap_provider'

    '''添加ldap认证服务器成功事件码是否在事件列表中'''
