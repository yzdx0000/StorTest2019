...
def case():
    ...
    '''1>修改ip池'''
    check_result = nas_common.update_vip_address_pool(vip_address_pool_id=vip_id,
                                                      domain_name="www.update.com")
    if check_result["detail_err_msg"] != "":
        common.except_exit("update_vip_address_pool failed!!")
    '''2>执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)    # 修改ip池后，延时
    code = '0x02030305'     # 修改ip池成功对应的编号
    description = 'update_vip_address_pool'
    '''修改ip池成功事件码是否在事件列表中'''
