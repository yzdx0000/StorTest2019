...
#   执行：
#           1>修改目录
#           2>执行get_events查看修改目录成功的结果显示
#           3>清理目录
# @changelog：
####################################################################################
FILE_NAME = os.path.splitext(os.path.basename(__file__))[0]              # 本脚本名字
SYSTEM_IP_0 = get_config.get_parastor_ip(0)
file_path = nas_common.ROOT_DIR + FILE_NAME                     # parastor:/nas/events_5_3_3_77
...
def case():
    ...
    '''1> 修改目录权限'''
    check_result = nas_common.update_file(path=file_path,
                                          posix_permission="rwxrwxrwx")
    if check_result["detail_err_msg"] != "":
        common.except_exit("update_file failed!!")
    '''2> 执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)               # 修改目录成功，延时
    code = '0x02031203'                # 修改目录成功对应编号
