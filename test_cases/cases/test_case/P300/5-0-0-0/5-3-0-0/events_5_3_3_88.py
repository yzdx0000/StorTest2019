...
def case():
    ...
    nas_common.disable_nas(fault_access_zone_id)
    '''2>执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s'% delay_time)
    time.sleep(delay_time)  # 禁用nas失败后，延时一段时间
    code = '0x02031504'   # 禁用nas失败的事件码
    description = 'disable_nas_failed'
    '''查看禁用nas失败的事件码是否在事件列表中'''
