...
#   执行：
#       1> 制作删除访问区失败事件
#       2> 执行get_events查看删除访问区失败事件的结果显示
# @changelog：
####################################################################################
...
def case():
    ...
    '''1>制作删除访问区失败事件'''
    '''令访问区id错误，预计创建失败'''
    access_zone_fault_id = access_zone_id + 1000
    nas_common.delete_access_zone(access_zone_fault_id)
    '''2>执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s'% delay_time)
    time.sleep(delay_time)    # 删除访问区，延时一段时间
    code = '0x02030104'     # 删除访问区成功的事件码
    description = 'delete_access_failed'
    '''查看删除访问区失败事件码是否在事件列表中'''
