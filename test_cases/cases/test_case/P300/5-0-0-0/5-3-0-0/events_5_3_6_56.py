...
#   3、执行create_client_auth添加客户端授权，输入参数错误，预期添加失败
...
def case():
    ...
    rc, stdout = ob_storage.get_storagepool_info()
    ...
    obj_client_auth.create_client_auth(client_ip_str, 0)   # 输入错误volume_id，预期创建授权失败
    ...
    '''5> 删除上述存储卷'''
    obj_volume.delete_volume(new_volume_id)
