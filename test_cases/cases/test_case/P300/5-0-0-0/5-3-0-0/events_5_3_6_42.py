...
#   5、执行delete_revert_snapshot，输入参数错误（预期失败）
...
def case():
    ...
    for i in range(10):
        ...
        cmd = 'dd if=%s of=%s bs=1M count=10' % ("/dev/zero", test_file)
    ...
    ''' 5> 执行delete_revert_snapshot，输入参数错误（预期失败）'''
    rc, stdout = snap_common.delete_revert_snapshot_by_id(0)
    if rc != 0:
