...
#   执行：
#           1>删除NFS客户端授权
#           2>执行get_events查看删除NFS客户端授权成功的结果显示
#           3>清理工作
# @changelog：
####################################################################################
...
def case():
    ...
    if check_result["detail_err_msg"] != "":
        common.except_exit("remove_nfs_export_auth_clients is failed!!")

    '''2> 执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)               # 删除NFS客户端授权成功，延时
    code = '0x02030611'                # 删除NFS客户端授权成功对应编号
    description = 'remove_nfs_export_auth_clients'
    '''删除NFS客户端授权成功事件码是否在事件列表中'''
