...
def case():
    ...
    '''1>删除ip池'''
    check_result = nas_common.delete_vip_address_pool(vip_id)
    if check_result["detail_err_msg"] != "":
        common.except_exit("delete_vip_address_pool failed!!")
    '''2>执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)    # 删除ip池后延时
    code = '0x02030303'     # 删除ip池成功对应的编号
    description = 'delete_ip_pools'
    '''删除ip池成功事件码是否在事件列表中'''
