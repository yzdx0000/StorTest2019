...
# date 2018-07-26
# @summary：
#    删除快照失败（等待get_events接口完成）
# @steps:
#   1、部署3节点集群环境
#   2、在/mnt/volume1/snap/snap_create_success/目录下创建数据
#   3、对/mnt/volume1/snap/snap_create_success/创建快照（参数输入正确，预期创建成功）
#   4、执行delete_snapshot，输入错误参数（预期删除失败）
#   5、执行get_events查看事件显示是否正确
#   6、删除快照，清理环境
#
# @changelog：
####################################################################################
...
def case():
    ...
    ''' 3> 对/mnt/volume1/snap/创建快照（参数输入正确，预期成功）'''
    ...
    ''' 4> 执行delete_snapshot，输入错误参数（预期删除失败）'''
    rc, stdout = snap_common.delete_snapshot_by_ids(0)
    if rc != 0:
