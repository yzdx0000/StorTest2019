...
#   执行：
#           1>修改NFS客户端授权
#           2>执行get_events查看修改NFS客户端授权失败的结果显示
#           3>清理工作
# @changelog：
####################################################################################
...
nfs_path = nas_common.ROOT_DIR + "NFS_" + FILE_NAME  # parastor:/NFS_events_5_3_3_52.py
...
def case():
    ...
    '''1>修改NFS客户端授权'''
    '''令客户端id不合法，预计修改失败'''
    fault_auth_client_id = auth_client_id + 1000
    nas_common.update_nfs_export_auth_client(auth_client_id=fault_auth_client_id,
                                             permission_level="ro")
    '''2>执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)   # 修改NFS客户端授权失败，延时
    code = '0x02030610'    # 修改NFS客户端授权失败对应编号
    description = 'update_nfs_export_auth_client!!'
    '''修改NFS客户端授权失败事件码是否在事件列表中'''
