...
def case():
    ...
    '''使删除的用户id不合法，预计删除失败'''
    fault_user_ids = user_id + 1000
    nas_common.delete_auth_users(ids=fault_user_ids)

    '''2> 执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)               # 删除本地用户失败，延时
    code = '0x02030506'                # 删除本地用户失败对应编号
    description = 'delete_auth_user_failed!!'
    '''删除本地用户失败事件码是否在事件列表中'''
