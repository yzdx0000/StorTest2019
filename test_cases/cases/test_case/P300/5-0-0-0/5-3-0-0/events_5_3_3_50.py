...
#   执行：
#           1>创建NFS客户端授权
#           2>执行get_events查看创建NFS客户端授权失败的结果显示
#           3>清理工作
# @changelog：
####################################################################################
...
nfs_path = nas_common.ROOT_DIR + "NFS_" + FILE_NAME                     # parastor:/NFS_events_5_3_3_50.py
...
def case():
    ...
    if check_result["detail_err_msg"] != "":
        common.except_exit("create %s Failed" % nfs_path)    # /mnt/parastor/NFS_events_5_3_3_50.py
    ...
    '''1>创建NFS客户端授权'''
    '''使得NFS授权id非法，预计创建NFS客户端授权失败'''
    fault_nfs_export_id = nfs_export_id + 1000
    nfs_export_auth_clients_name = get_config.get_client_ip(0)
    nas_common.add_nfs_export_auth_clients(export_id=fault_nfs_export_id,
    ...
    time.sleep(delay_time)               # 创建NFS客户端授权失败，延时
    code = '0x02030608'                # 创建NFS客户端授权失败对应编号
    description = 'add_nfs_export_auth_clients_failed'
    '''创建NFS客户端授权失败事件码是否在事件列表中'''
