...
#   执行：
#           1>修改SMB共享
#           2>执行get_events查看修改SMB共享失败的结果显示
#           3>清理工作
# @changelog：
####################################################################################
...
smb_path = nas_common.ROOT_DIR + "SMB_" + FILE_NAME  # parastor:/nas/SMB_events_5_3_3_58
...
def case():
    ...
    '''1>修改SMB共享目录'''
    '''令smb导出目录id不合法，预计修改失败'''
    fault_smb_export_id = smb_export_id + 1000
    nas_common.update_smb_export(export_id=fault_smb_export_id,
                                 description="update_smb_export")
    '''2> 执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)  # 修改SMB共享失败，延时
    code = '0x02030704'  # 修改SMB共享失败对应编号
    description = 'update_smb_export_failed'
    '''修改SMB共享失败事件码是否在事件列表中'''
