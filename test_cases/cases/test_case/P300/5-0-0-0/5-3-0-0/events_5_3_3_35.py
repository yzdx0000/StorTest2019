...
def case():
    ...
    '''创建一个测试用户组'''
    check_result = nas_common.create_auth_group(auth_provider_id=auth_provider_local_id,
                                                name=FILE_NAME+"_group_test")
    if check_result["detail_err_msg"] != "":
        common.except_exit("create_auth_test_group failed!!")
    update_group_id = check_result["result"]
    ...
    '''将用户所在的用户组id修改为测试用户组id'''
    check_result = nas_common.update_auth_user(user_id=user_id,
                                               password=111111,
                                               primary_group_id=update_group_id)
    if check_result["detail_err_msg"] != "":
        common.except_exit("update_auth_user failed!!")

    '''2> 执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)               # 修改本地用户成功，延时
    code = '0x02030503'                # 修改本地用户成功对应编号
    description = 'update_auth_user'
