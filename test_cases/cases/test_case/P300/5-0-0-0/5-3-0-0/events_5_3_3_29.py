...
#   执行：
#           1>制作修改LDAP认证服务器成功事件
#           2>执行get_events查看修改LDAP认证服务器成功的结果显示
# @changelog：
####################################################################################
...
def case():
    ...
    time.sleep(delay_time)               # 修改LDAP认证服务器成功后，延时

    code = '0x02030411'                # 修改LDAP认证服务器成功对应编号
    description = 'update_ldap_provider'

    '''修改LDAP认证服务器成功事件码是否在事件列表中'''
