...
def case():
    ...
    '''1>制作删除业务子网成功事件'''
    check_result = nas_common.delete_subnet(subnet_id)
    if check_result["detail_err_msg"] != "":
        common.except_exit("delete_subnet failed!!")
    '''2>执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s'% delay_time)
    time.sleep(delay_time)    # 删除业务子网成功后延时
    code = '0x02030205'     # 修删除业务子网成功事件对应的编号
    description = 'delete_subnet'
    '''修改业务子网成功事件码是否在事件列表中'''
