...
auth_user1_name = quota_common.QUOTA_USER + '1'
auth_user2_name = quota_common.QUOTA_USER + '2'
auth_other_group_name = quota_common.QUOTA_OTHER_GROUP
auth_other_user_name = quota_common.QUOTA_OTHER_USER


def auth_group_quota_check(path, inode_quota):
    log.info('##########auth_group_quota_check begin##########')
    """
    【su方式】
    先写入2000个属组是QUOTA_GROUP的文件,分两个用户写入
    """
    quota_common.creating_files_by_designated_user_or_group(server_ip_lst[0], path, inode_quota / 2, 1, "a",
                                                            auth_user1_name)
    quota_common.creating_files_by_designated_user_or_group(server_ip_lst[0], path, inode_quota / 2, 1, "b",
                                                            auth_user2_name)
    """再尝试继续写入文件"""
    """(1)先写100个属组为root的文件，预期写入成功"""
    quota_common.creating_files_by_designated_user_or_group(server_ip_lst[1], path, 100, 1, "c", "root")

    """(2)之后写100个属组为QUOTA_OTHER_GROUP的文件，预期写入成功"""
    quota_common.creating_files_by_designated_user_or_group(server_ip_lst[0], path, 100, 1, "d", auth_other_user_name)

    """"(3)最后尝试写2个属主为QUOTA_GROUP的文件，预期写入失败"""
    quota_common.creating_files_by_designated_user_or_group(server_ip_lst[0], path, 1, 1, "e", auth_user1_name)
    quota_common.creating_files_by_designated_user_or_group(server_ip_lst[1], path, 1, 1, "f", auth_user2_name)

    """检查配额是否生效"""
    group_total_inodes = quota_common.group_total_inodes(server_ip_lst[0], path, auth_group_name)
    if group_total_inodes != inode_quota:
        raise Exception("auth_group_quota_check failed, %s's quota is %d, but the group_total_inodes is %d"
                        % (auth_group_name, inode_quota, group_total_inodes))
    else:
        log.info("auth_group_quota_check Succeed")
...
def case():
    ...
    result4 = nas_common.create_auth_group(auth_provide_id, auth_other_group_name)
    password = '111111'
    auth_group_id = result3['result']
    auth_other_group_id = result4['result']
    nas_common.create_auth_user(auth_provide_id, auth_user1_name, password, auth_group_id)
    nas_common.create_auth_user(auth_provide_id, auth_user2_name, password, auth_group_id)
    nas_common.create_auth_user(auth_provide_id, auth_other_user_name, password, auth_other_group_id)

    """修改测试脚本目录权限"""
    cmd = 'chmod 777 %s' % DEFECT_TRUE_PATH
    common.run_command(client_ip_lst[0], cmd)

    """针对用户组设置inode数为2000的硬阈值配额, 并检查其有效性"""
    inode_quota = 2000
    rc, pscli_info = quota_common.create_one_quota(path=QUOTA_PATH,
                                                   ...
                                                   user_or_group_name=auth_group_name)
    common.judge_rc(rc, 0, "create  quota failed")
    auth_group_quota_check(QUOTA_TRUE_PATH, inode_quota)
