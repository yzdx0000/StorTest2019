...
def update_rep_policy(id=None, name=None, rep_pair_id=None, period_type=None, months=False, week_days=None,
                      days=None, hours=None, minute=None, expire_time=None, print_flag=True, fault_node_ip=None,
                      run_cluster=rep_common.MASTER):
    rc, stdout = rep_common.update_rep_policy(id=id, name=name, rep_pair_id=rep_pair_id,
                                              period_type=period_type, months=months, week_days=week_days, days=days,
                                              hours=hours, minute=minute, expire_time=expire_time,
...
def case():
    ...
    for i in range(20):
        name = FILE_NAME + str(i)
        rc, policyid = rep_common.get_rep_policy_id(name=name)
        common.judge_rc(rc, 0, 'get_rep_policy_id failed')
        policies_id_lst.append(policyid)

    log.info('4、[主]串行修改策略')
    for i in range(10):
        name = FILE_NAME + str(i)
        rc, pscli_info = rep_common.update_rep_policy(policies_id_lst[i], period_type=rep_common.BY_MONTH, days=1,
                                                      hours=1, minute=1, expire_time=0)
        common.judge_rc(rc, 0, 'update_rep_policy failed')


    log.info('5>[主]子进程N，同时修改N个策略')
    pro_lst = []
    for i in range(10, 10+THREADS_NUMBER):
        name = FILE_NAME + str(i)
        pro_lst.append(Process(target=update_rep_policy, args=(policies_id_lst[i], pair_lst[i], 'BY_YEAR', 1, 1, 1, 1,
                                                               0)))
