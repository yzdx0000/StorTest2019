...
def case():
    ...
    log.info('3> [主]创建一个文件，起pair任务，truncate遍历几种容量进行复制')
    '''准备文件获取信息'''
    str = 'a' * 1024
    cmd = 'echo %s > %s' % (str, os.path.join(m_dir_lst[0], 'file_1'))
    ...
    log.info('4> [主]truncate遍历几种容量进行复制')
    for size in ['10m', '500m', '10g', '600g', '1t']:
        cmd = 'truncate -s %s %s' % (size, os.path.join(m_dir_lst[0], 'file_1'))
        rc, stdout = common.run_command(rep_common.MASTER_RANDOM_NODE, cmd)
        common.judge_rc(rc, 0, cmd + 'failed')
        rc, md5_1 = rep_common.md5sum(rep_common.MASTER_RANDOM_NODE, os.path.join(m_dir_lst[0], 'file_1'))
        common.judge_rc(rc, 0, 'md5sum failed')
        ...
        common.judge_rc(rc, 0, 'md5sum failed')
        common.judge_rc(md5_1, md5_2, 'md5sum check failed')
        rc, tmp_inode = rep_common.get_inode(rep_common.SLAVE_RANDOM_NODE, os.path.join(s_dir_lst[0], 'file_1'))
        common.judge_rc(rc, 0, 'get_inode failed')
