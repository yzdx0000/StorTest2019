...
# date 2018-11-17
# @summary：
#    posix客户端权限测试：u2属于多个组，检测目录权限；原P200测试用例23-0-3-3。
# @steps:
...
def case():
    ...
    log.info("5> 测试主体：%s属于多个组，测试目录权限" % user_name_list[1])
    ...
    log.info("5-2> 给用户组%s设置rwx权限" % group_list[0])
    cmd = "ssh %s \"cd %s;setfacl -m g:%s:rwx %s\"" % (
        client_ip, posix_file_location, group_list[0], testdir)
    rc = common.command(cmd)
    common.judge_rc(rc, 0, "setfacl %s:rwx failed" % group_list[0])

    log.info("5-3> 用户%s向%s中创建新目录" % (user_name_list[1], testdir))
    sub_file_name = "%s" % user_name_list[1]
    cmd = "ssh %s \"su %s -c 'cd %s;touch %s/%s'\"" % (client_ip, user_name_list[1], posix_file_location, testdir, sub_file_name)
    rc = common.command(cmd)
    common.judge_rc(rc, 0, "make subdir failed, the acl dosen't take effect")
