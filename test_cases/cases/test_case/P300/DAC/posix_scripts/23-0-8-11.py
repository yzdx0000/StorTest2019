...
# date 2018-11-22
# @summary：
#    posix客户端权限测试：对文件进行chgrp操作，检验文件权限；原P200测试用例23-0-8-11。
# @steps:
...
def case():
    ...
    log.info("6> 测试主体：对文件进行追加写操作，检验文件权限")
    log.info("6-1> 创建测试文件并设置权限664")
    testfile = 'file_' + FILE_NAME
    cmd = "ssh %s \"su %s -c 'cd %s;touch %s;chmod 664 %s'\" " % (
    ...
    log.info("6-2> 修改文件的属组")
    cmd = "ssh %s \"cd %s;chgrp %s %s\"" % (
        client_ip, posix_file_location, group_list[1], testfile)
    rc = common.command(cmd)
    ...
    for u_name in user_name_list[:-1]:
        cmd_u_tmp = "ssh %s \"su %s -c 'cd %s;echo %s >> %s'\"" % (
            client_ip, u_name, posix_file_location, echo_content, testfile)
        rc = common.command(cmd_u_tmp)
        common.judge_rc(
            rc, 0, "user %s, echo >> failed, the acl dosen't take effect" %
                   u_name)

    cmd_u3 = "ssh %s \"su %s -c 'cd %s;echo %s >> %s'\"" % (
        client_ip, user_name_list[2], posix_file_location, echo_content, testfile)
    ...
    common.judge_rc_unequal(
        ...
        user_name_list[2])
    log.info("6-4> testfile权限检测成功，删除测试目录%s" % posix_file_location)
