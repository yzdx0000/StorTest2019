...
# date 2018-11-23
# @summary：
#    posix客户端权限测试：chmod其他控制项测试，对文件进行S_ISUID掩码设置，检验用户uid；原P200测试用例23-0-9-4。
# @steps:
...
#    5、测试主体，chmod其他控制项测试，对文件进行S_ISUID掩码设置，检验用户uid；
...
def case():
    ...
    log.info("5-3> 用户%s对%s进行chmod 4755 操作" %
             (user_name_list[0], c_file_compile))
    cmd = "ssh %s \"su %s -c 'cd %s;chmod 4755 %s'\"" % (
    ...
    (rc, uid) = common.run_command(client_ip, cmd)
    if int(uid) == int(user_id_list[0]):
