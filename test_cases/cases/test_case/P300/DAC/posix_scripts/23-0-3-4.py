...
# date 2018-11-17
# @summary：
#    posix客户端权限测试：u2属于多个组，检测文件权限；原P200测试用例23-0-3-4。
# @steps:
...
def case():
    ...
    common.judge_rc(rc, 0, "add user1 in client %s failed" % client_ip)

    cmd_add_user_2 = "ssh %s \"useradd -g %s -G %s,%s -u %d %s\"" % (
        client_ip, group_list[1], group_list[0], group_list[1], user_id_list[1], user_name_list[1])
    rc = common.command(cmd_add_user_2)
    common.judge_rc(rc, 0, "add user2 in client %s failed" % client_ip)

    log.info("5> 测试主体：%s属于多个组，测试文件权限" % user_name_list[1])
    log.info("5-1> 创建测试文件并设置权限555")
    testfile = 'file_' + FILE_NAME
    cmd = "ssh %s \"su %s -c 'cd %s;touch %s;chmod 555 %s'\" " % (
        client_ip, user_name_list[0], posix_file_location, testfile, testfile)
    rc = common.command(cmd)
    common.judge_rc(rc, 0, "touch testfile failed")

    log.info("5-2> 给用户组%s设置rw权限" % group_list[0])
    cmd = "ssh %s \"cd %s;setfacl -m g:%s:rw %s\"" % (
        client_ip, posix_file_location, group_list[0], testfile)
    rc = common.command(cmd)
    common.judge_rc(rc, 0, "setfacl %s:w failed" % group_list[0])

    log.info("5-3> 用户%s向%s中写入数据" % (user_name_list[1], testfile))
    echo_content = "hello, world"
    cmd = "ssh %s \"su %s -c 'cd %s;echo %s >> %s'\"" % (client_ip, user_name_list[1], posix_file_location, echo_content, testfile)
    rc = common.command(cmd)
    common.judge_rc(rc, 0, "echo >> failed, the acl dosen't take effect")
