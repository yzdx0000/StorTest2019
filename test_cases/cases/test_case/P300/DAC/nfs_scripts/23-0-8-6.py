...
# date 2018-11-22
# @summary：
#    nfs客户端权限测试：对目录进行chgrp操作，检验目录权限；原P200测试用例23-0-8-6。
# @steps:
...
def case():
    ...
    cmd = "ssh %s \"cd %s;chgrp %s %s\"" % (
        nfs_ip, nfs_file_location, group_list[1], testdir)
    rc = common.command(cmd)
    common.judge_rc(rc, 0, "chgrp %s failed" % group_list[1])
    log.info("6-3> 分别使用用户%s和%s对目录进行创建子文件操作" %
             (user_name_list[0], user_name_list[1]))
    for u_name in user_name_list[:-1]:
        cmd_tmp = "ssh %s \"su %s -c 'cd %s;touch %s/%s'\"" % (
            nfs_ip, u_name, nfs_file_location, testdir, 'file_' + u_name)
        rc = common.command(cmd_tmp)
        common.judge_rc(
            rc, 0, "user %s, touch file failed, the acl dosen't take effect" %
                   u_name)
    cmd_u3 = "ssh %s \"su %s -c 'cd %s;touch %s/%s'\"" % (
        nfs_ip, user_name_list[2], nfs_file_location, testdir, 'file_' + user_name_list[2])
