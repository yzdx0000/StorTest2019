...
# date 2018-11-19
# @summary：
#    nfs客户端权限测试：u1属于多个组，检测文件权限；原P200测试用例23-0-4-2。
# @steps:
...
def case():
    ...
    cmd_add_user = "ssh %s \"useradd -g %s -G %s,%s -u %d %s\"" % (
        nfs_ip, group_list[0], group_list[0], group_list[1], user_id, user_name)
    rc = common.command(cmd_add_user)
    common.judge_rc(rc, 0, "add user in client %s failed" % nfs_ip)

    log.info("6> 测试主体：%s属于多个组，测试目录权限" % user_name)
    log.info("6-1> 创建测试目录并设置权限555")
    testfile = 'file_' + FILE_NAME
    cmd = "ssh %s \"su %s -c 'cd %s;touch %s;chmod 555 %s'\" " % (
        nfs_ip, user_name, nfs_file_location, testfile, testfile)
    rc = common.command(cmd)
    common.judge_rc(rc, 0, "touch testfile failed")
    log.info("6-2> 给用户组%s设置rw权限" % group_list[1])
    cmd = "ssh %s \"cd %s;setfacl -m g:%s:rw %s\"" % (
        nfs_ip, nfs_file_location, group_list[1], testfile)
    rc = common.command(cmd)
    common.judge_rc(rc, 0, "setfacl %s:rw failed" % group_list[1])
    log.info("6-3> 用户%s向%s中写入数据" % (user_name, testfile))
    echo_content = "hello, world"
    cmd = "ssh %s \"su %s -c 'cd %s;echo %s >> %s'\"" % (
        nfs_ip, user_name, nfs_file_location, echo_content, testfile)
    rc = common.command(cmd)
    common.judge_rc_unequal(
        rc, 0, "echo >> success, the acl dosen't take effect")
    ...
    cmd_del_user = "ssh %s \"userdel %s\"" % (nfs_ip, user_name)
