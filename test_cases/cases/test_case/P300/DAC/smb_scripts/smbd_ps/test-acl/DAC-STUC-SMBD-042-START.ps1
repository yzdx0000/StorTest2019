...
#以指定用户u1启动powershell,创建文件file.txt
...
Start-Process powershell.exe -Credential $cred -NoNewWindow -Argumentlist "$xmlpath\DAC-STUC-SMBD-042-START-U1.bat" -Wait

#以指定用户u2启动powershell,并进行权限测试
$username=$ad_set+$test_user_u2
$userpasswd=ConvertTo-SecureString $pwd_u2 -AsPlainText -force
$cred=New-Object System.Management.Automation.PSCredential($username,$userpasswd)
Start-Process powershell.exe -Credential $cred -NoNewWindow -Argumentlist "$xmlpath\DAC-STUC-SMBD-042-START-U2.bat" -Wait