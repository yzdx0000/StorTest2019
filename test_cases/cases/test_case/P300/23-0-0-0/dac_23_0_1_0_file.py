...
def case():
    ...
    log.info(">2 扩展属性设置(nde:%s;client:%s)" % (case_ip, client_ip))
    # security， trusted， user
    ns_list = ["security", "trusted", "user"]
    path_top = os.path.join(prepare_clean.DEFECT_PATH, FILE_NAME)
    complite_path = path_top + "/" + "f_attr"

    cmd_tocuh_f = "touch %s" % complite_path
    (rc_t, std_f) = common.run_command(client_ip, cmd_tocuh_f)
    common.judge_rc(rc_t, 0, "touch file:" + std_f)
