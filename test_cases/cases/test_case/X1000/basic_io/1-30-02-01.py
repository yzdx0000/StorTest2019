...
#offset = "8192"
align = "3072"
xfersize1 = "(256k,20,512k,10,1028k,30,4096k,40),unmappct=20"
xfersize2 = "(256k,20,512k,10,1028k,30,4096k,40),unmappct=20"
...
for i in range(0,len(lun)):
    j=i+1
    if i % 2 == 0:
        vdb_xml = osan.vdb_write(sd="sd"+str(j),lun=lun[i],wd="wd"+str(j),xfersize=xfersize1,seekpct=seekpct,rdpct=rdpct)
    else:
        vdb_xml = osan.vdb_write(sd="sd"+str(j),lun=lun[i],wd="wd"+str(j),align=align,xfersize=xfersize2,seekpct=seekpct,rdpct=rdpct)
#生成vdbench测试用的配置文件
