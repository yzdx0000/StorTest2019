...
for i in range(0,len(lun)):
    j=i+1
    if i % 2 == 0:
        vdb_xml = osan.vdb_write(sd="sd"+str(j),lun=lun[i],wd="wd"+str(j),xfersize=xfersize1,rdpct=rdpct)
    else:
        vdb_xml = osan.vdb_write(sd="sd"+str(j),lun=lun[i],wd="wd"+str(j),align=align,xfersize=xfersize2,rdpct=rdpct)
#生成vdbench测试用的配置文件
