...
def case():
    ...
    env_manage.create_luns(nums=20)
    log.info("step:2.修改逻辑卷过程中关闭网卡")
    threads = []
    t1 = threading.Thread(target=env_manage.update_luns)
    threads.append(t1)
    t2 = threading.Thread(target=down_network, args=(node_ip1, subnet_eth))
    threads.append(t2)
    for t in threads:
        t.start()
    for t in threads:
