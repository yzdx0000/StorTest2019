...
def node_fault(s_ip, os_type):
    log.info("step:8.%s节点%s 故障" % (os_type, s_ip))
    os_info = env_manage.down_node(s_ip, os_type)
...
def lun_map_case():
    ...
    lun_los_id = env_manage.get_lun_default_los_id(lun_id)  # get lun los id
    lun_los_ip = env_manage.com_bd_disk.get_nodeip_by_losid(lun_los_id)  # lun los id in node ip
    log.info("step:4.获取 lun 所在节点IP %s" % (lun_los_ip))
    env_manage.deploy_ips.remove(lun_los_ip)  # remove lun los node ip
    log.info("step:5.获取一个其他节点的target id 用来lun map")
    node_ip, target_id = get_new_node_tag_id(env_manage.deploy_ips)  # get a new target
    log.info("step:7.创建lun map 过程中关闭lun los 源节点")
    os_type = env_manage.get_os_type(lun_los_ip)
    threads = []
    threads.append(threading.Thread(target=env_manage.create_lun_map, args=(node_ip, lun_id, target_id)))
    threads.append(threading.Thread(target=node_fault, args=(lun_los_ip, os_type)))  # down node
    for i in threads:
        i.start()
    for i in threads:
        i.join()
    log.info("step:9.%s 节点开机 节点类型%s,节点ID or IPMI IP: %s" % (lun_los_ip, os_types[0], os_infos[0]))
    env_manage.up_node(os_infos[0], os_types[0])
    env_manage.com_lh.get_os_status(lun_los_ip)
...
def case():
    log.info("******************* case start **********************")
    log.info("step:1.运行vdbench过程中进行lun los迁移，且在迁移过程中将源节点掉电")
    threads = []
