...
7、在业务继续，停止该访问分区iSCSI过程中，oPmgr主所在节点数据网故障，1分钟后恢复
8、内部数据一致
检查项：
...
def vdb_test():
    ...
    log.info("vdbench test have finished")


def net_down_osan(ip, name, access_id):
    time.sleep(30)
    log.info("will network down and disable san ")
    new_ip_list = []
    ...
    threads = []
    threads.append(threading.Thread(target=env_manage.osan.disable_san, args=(new_ip_list[0], access_id)))
    ...
    for t in threads:
        t.join()
    log.info("network and san have finished")


def run_vdb_case(orole_master_ip, data_net, az_id1):
    threads = []
...
def case():
    ...
    log.info("step:3.vdbech过程中，停止scsi，节点断网")
    rc = run_vdb_case(orole_master_ip, data_net, az_id1)
    if rc == 0:
        log.error("The vdbench not stop")
        os._exit(1)
    log.info("检查san 协议是否关闭成功")
    san_status = env_manage.com_lh.get_san_state(node_ip1)
    if all(san_status) is True:
        log.error("check all san is active, all san status:%s" % (san_status))
    else:
