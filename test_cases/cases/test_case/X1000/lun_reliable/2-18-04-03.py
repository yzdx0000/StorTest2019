...
def run_kill_thread():
    for i in range(3):
        ...
        log.info("step:2.1 node %s will kill pmgr thread process " % (orole_master_ip))
        env_manage.com_lh.kill_thread(s_ip=orole_master_ip, p_name="oRole", t_name="pmgr")
        time.sleep(20)


def del_lun():
    log.info("step:2.2 删除前5个lun")
...
def case():
    ...
    log.info("step:2.删除部分逻辑卷过程中kill opmgr 进程")
    threads = []
