...
# Date  :2018-8-10
'''
测试内容：添加vip过程中将oJmgs故障
测试步骤：
1）配置节点池，配置存储池
2）配置访问区
3）添加VIP池，添加过程中将oJmgs故障，从进程接管业务，添加成功
检查项：
1）节点池，存储池创建成功
2）访问区创建成功
3）VIP池添加成功
...
def run_kill_process():
    ids, ojmgs_ip = env_manage.com_lh.oJmgs_master_id()
    rc = env_manage.com_lh.get_cmd_status(node_ip1, cmd_name="delete_vip_address_pool")
    if rc == 0:
...
def case():
    log.info("step:1.创建访问区，创建svip，添加vip")
    env_manage.create_access()
    env_manage.create_subnet()
    env_manage.create_vip()
    log.info("step:2.删除vip过程中kill oJmgs进程")
    threads = []
    t1 = threading.Thread(target=run_kill_process)
    threads.append(t1)
    t2 = threading.Thread(target=env_manage.clean_vip_address_pool)
    threads.append(t2)
    for t in threads:
        t.start()
    for t in threads:
