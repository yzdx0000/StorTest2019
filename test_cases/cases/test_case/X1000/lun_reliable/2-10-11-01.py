...
def case():
    ...
    log.info("step:2.update lun 过程中，节点下点")
    i = 0
    for id in lun_ids:
        rc = env_manage.update_luns(s_ip=node_ip2, id=id)
        if rc == id:
            i = i + 1
        if i == 5:
            ...
            infos.append(info)
    lun_map = env_manage.create_lun_map(node_ip2)
    ...
    if len(lun_map_ids) == len(lun_ids):
        ...
        log.info("step:5.%s 节点开机 节点类型%s,节点ID or IPMI IP: %s" % (node_ip1, os_type, infos[0]))
