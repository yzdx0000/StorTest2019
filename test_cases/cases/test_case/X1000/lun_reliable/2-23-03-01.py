...
def case():
    ...
    log.info("step:3.vdbench测试（目的提高网络利用率）过程中修改逻辑卷")
    env_manage.create_lun(node_ip1, "LUN222")  # 创建lun
    threads = []
    t1 = threading.Thread(target=vdb_test)  # vdbench自动测试
    threads.append(t1)
    t2 = threading.Thread(target=env_manage.update_luns, args=(node_ip1,))  # 同时进行拔盘
    threads.append(t2)
    for i in threads:
        i.start()
    for i in threads:
