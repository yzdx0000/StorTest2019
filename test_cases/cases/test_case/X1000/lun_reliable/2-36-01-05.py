...
def lun_map_case():
    ...
    data_net = env_manage.com_lh.get_eth_name(s_ip=node_ip)[1]
    threads = []
    threads.append(threading.Thread(target=env_manage.create_lun_map, args=(lun_los_ip, lun_id, target_id)))
    threads.append(threading.Thread(target=date_net_fault, args=(node_ip, data_net)))  # 目的节点网卡
    for i in threads:
        i.start()
    for i in threads:
        i.join()
    log.info("step:9.%s 节点网卡 %s 恢复" % (node_ip, data_net))
    env_manage.up_network(node_ip, data_net)
