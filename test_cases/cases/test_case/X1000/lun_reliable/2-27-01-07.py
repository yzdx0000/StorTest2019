...
def case():
    log.info("step:1.杀进程关节点时创建逻辑卷")
    threads = []
    t1 = threading.Thread(target=running_break)
    threads.append(t1)
    t2 = threading.Thread(
        target=env_manage.create_lun, args=(
            node_ip3, "LUN1"))
    threads.append(t2)
    for t in threads:
        t.start()
    for t in threads:
        t.join()
    log.info("step:2.%s 节点开机" % (node_ip2))
    env_manage.up_node(infos[0], os_types[0])
    log.info("step:3.等待节点和服务恢复")
    rc = env_manage.com_lh.get_os_status(node_ip2)
    if rc == 0:
        return
...
if __name__ == '__main__':
    env_manage.rel_check_before_run(
        file_name, jnl_rep=3, free_jnl_num=0, node_num=5)
