...
def run_case():
    time.sleep(30)
    env_manage.create_lun(name="LUN2")  # 创建lun
    log.info("step:2.节点关机")
    os_type = env_manage.get_os_type(node_ip1)
    info = env_manage.down_node(node_ip1, os_type, "init 0")  # 节点关机
    log.info("step:3.删除一个逻辑卷")
    lun_id = env_manage.com_lh.get_lun_id_by_name(node_ip2, "LUN2")
