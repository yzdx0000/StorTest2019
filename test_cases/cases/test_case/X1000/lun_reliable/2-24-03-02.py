...
2、创建访问分区
3、启动i该访问分区SCSI
4、停止该访问分区iSCSI过程中，oPmgr主所在节点数据网故障，1分钟后恢复
...
def case():
    ...
    log.info("step:2.关闭san过程中断开网卡")
    threads = []
    t1 = threading.Thread(target=env_manage.disable_san, args=(new_ip_list[0],))
    threads.append(t1)
