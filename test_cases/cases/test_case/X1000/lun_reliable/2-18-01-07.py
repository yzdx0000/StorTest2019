...
def get_master_ip():
    ...
    return orole_master_ip


def loop_kill():
    log.info("step:1.2 kill 主oRole进程")
    for i in range(5):
        master_ip = get_master_ip()
        log.info("will kill %s node oRole process" % (master_ip))
        ReliableTest.run_kill_process(node_ip=master_ip, process="oRole")
        time.sleep(20)


def create_luns():
    log.info("step：1.1创建逻辑卷")
    for i in range(10):
        lun_name = "LUN" + str(i)
...
def case():
    log.info("step:1.创建逻辑卷过程中kill oRole 进程")
    threads = []
    t1 = threading.Thread(target=loop_kill)
    threads.append(t1)
