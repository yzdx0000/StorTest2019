...
def lun_map(ids):
    host_group_id = env_manage.osan.get_host_groups(s_ip=node_ip2)
    env_manage.osan.map_lun(s_ip=node_ip2, lun_ids=ids, hg_id=host_group_id[0])


def run_case():
    subnet_eth = env_manage.com_lh.get_vip_eth_name(node_ip1)
    time.sleep(60)
    lun_id = env_manage.create_lun(node_ip2, "LUN2")
...
def case():
    log.info("step:1.创建逻辑卷，运行vdb，关闭网卡")
    threads = []
