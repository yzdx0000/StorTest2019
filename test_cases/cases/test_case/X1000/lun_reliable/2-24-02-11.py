...
def down_network(ipaddr, name):
    rc = env_manage.com_lh.get_cmd_status(node_ip2, "delete_lun_map")
    if rc == 0:
...
def case():
    ...
    threads = []
    t1 = threading.Thread(target=down_network, args=(node_ip1, data_net))
    threads.append(t1)
    t2 = threading.Thread(target=env_manage.clean_lun_map, args=(node_ip2,))
    threads.append(t2)
    for t in threads:
        t.start()
    for t in threads:
    ...
    env_manage.up_network(ipaddr=node_ip1, name=data_net)
