...
def lun_map_case():
    ...
    subnet_eth = env_manage.com_lh.get_vip_eth_name(lun_los_ip)
    threads = []
    threads.append(threading.Thread(target=env_manage.create_lun_map, args=(node_ip, lun_id, target_id)))
    threads.append(threading.Thread(target=date_net_fault, args=(lun_los_ip, subnet_eth)))  # 源节点断网
    for i in threads:
        i.start()
    for i in threads:
        i.join()
    log.info("step:9.%s 节点网卡 %s 恢复" % (lun_los_ip, subnet_eth))
    env_manage.up_network(lun_los_ip, subnet_eth)
