...
5、创建LUN
6、修改vip地址池中有LUN关联的VIP过程中，oPmgr主节点数据网故障，1分钟后恢复
7、创建LUN映射
...
def change_vip():
    ...
    log.info("get new vip:%s" % (new_vips))
    vip_pool_list = env_manage.com2_osan.analysis_vip(new_vips)
    log.info("修改（删除）vip 为：%s" % (vip_pool_list))
    ...
    log.info("update vip success,will check it")
...
def case():
    ...
    threads.append(threading.Thread(target=env_manage.down_network, args=(orole_master_ip, data_net)))
    for t in threads:
        t.start()
    for t in threads:
        t.join()
