...
def run_kill(orole_ip):
    ReliableTest.run_kill_process(node_ip=orole_ip, process="oRole")


def case():
    log.info("step:1.创建逻辑卷")
    env_manage.create_luns()
    log.info("step:2.删除部分逻辑卷过程中kill oRole 进程")
    orole_master_ip = env_manage.com_lh.get_master_oRole(s_ip=node_ip1)
    threads = []
    t1 = threading.Thread(target=run_kill, args=(orole_master_ip,))
    threads.append(t1)
