...
# Date  :2018-8-10
'''
测试内容：删除vip过程中将oPmgr故障
测试步骤：
1）配置节点池，配置存储池
2）配置访问区
3）删除VIP池，删除过程中将oPmgr故障，从进程接管业务，删除成功
检查项：
1）节点池，存储池创建成功
2）访问区创建成功
3）VIP池添加成功
...
def case():
    ...
    log.info("step:2.删除vip过程中kill pmgr进程")
    orole_ip = env_manage.com_lh.get_master_oRole(node_ip1)
    threads = []
    t1 = threading.Thread(target=run_kill_thread)
    threads.append(t1)
    t2 = threading.Thread(target=env_manage.clean_vip_address_pool, args=(orole_ip,))
    threads.append(t2)
    for t in threads:
        t.start()
    for t in threads:
