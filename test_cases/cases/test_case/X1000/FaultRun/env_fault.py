...
case_list_path = "/home/StorTest/test_cases/cases/test_case/X1000/FaultRun/"
...
def analysis_xml_file(lun_list):
    ...
    with open(case_list_file, 'r') as f:
        for (num, line) in enumerate(f, 1):
            ...
            # Write sd default
            if jn_jro == "yes":
                # cmd = ("ssh %s \"mkdir /root/vdbench/journal/%s\"" % (client_ips[0], str(deploy_ips[0])))
                # commands.getstatusoutput(cmd)
                ...
                sd_default = "sd=default,journal=/root/vdbench/journal/%s,openflags=o_direct,align=%s,offset=%s\n" % (
                    str(client_ips[0]), str(align), str(offset))
