...
3、步骤3，命令设置成功
4、步骤4，业务正常发起
5、步骤5，业务性能相对步骤2中记录的性能值显著下降
"""

# testlink case: 1000-34071
import os
import time
import json
import utils_path
...
def run_vdb1():
    ...
    vdb_file = env_cache.gen_vdb_xml(lun=lun_name[:1], xfersize="4k", rdpct="100", thread=8, rhpct=0, seekpct=0)
...
def run_vdb2():
    lun_name = env_cache.osan.ls_scsi_dev(client_ip=client_ip1)
    vdb_file = env_cache.gen_vdb_xml(lun=lun_name[1:2], xfersize="4k", rdpct="100", thread=8, rhpct=0, seekpct=0)
    res = env_cache.run_vdb(vdb_file=vdb_file, c_ip=client_ip1, x_size="4k")
    return res
...
def case():
    ...
    log.info("测试前更新缓存")
    env_cache.re_cache_run_vdb(1)
    log.info("step:5.设置lun预读为自动")
    env_cache.set_cache(id=lun_id1, mode=1)
    # env_cache.update_osan_params(1)
    log.info("step:6.主机端下发顺序读业务")
    result1 = run_vdb1()
    log.info("测试前更新缓存")
    env_cache.re_cache_run_vdb(2)
    log.info("ste:7.设置lun预读为关闭")
    env_cache.set_cache(id=lun_id2, mode=0)
    # env_cache.update_osan_params(0)
    result2 = run_vdb2()
    log.info("step:8.对比两次测试结果")
    if float(result1["bw"]) - float(result2["bw"]) > 0:
