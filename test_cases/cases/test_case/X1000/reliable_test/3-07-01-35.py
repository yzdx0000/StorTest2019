...
4、在步骤3中的业务运行过程中，将节点A数据网断开；等待30秒将节点B数据网断开
5、等待30秒后将节点A数据网恢复，再等待30秒将节点B；
6、主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性
7、节点A上数据修复完成后，再次在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
8、比较存储内部数据一致性。"

:Changerlog:
"""
# testlink case: 1000-32679
import os
import sys
import commands
import threading
...
def up_down():
    log.info("故障点:一个业务节点的数据网和一个日志节点的一个数据网")
    time.sleep(100)
    log.info("Step4:在步骤3中的业务运行过程中，将业务节点数据网断开,业务节点:%s" % fault_io_ip_A)
    for eth_name in fault_eth_list_A:
        reliable.network_test(fault_io_ip_A, eth_name, 'down')
    time.sleep(30)
    log.info("Step5:30s后将日志节点的数据网断开,日志节点:%s" % jnl_node_ip)
    for eth_name in fault_eth_list_B:
        reliable.network_test(jnl_node_ip, eth_name, 'down')
    time.sleep(30)
    log.info("Step6:30s后将业务节点的数据网恢复,节点:%s" % fault_io_ip_A)
    for eth_name in fault_eth_list_A:
        reliable.network_test(fault_io_ip_A, eth_name, 'up')
    time.sleep(30)
    log.info("Step7:30s后将日志节点数据网恢复,节点:%s" % jnl_node_ip)
    for eth_name in fault_eth_list_B:
        reliable.network_test(jnl_node_ip, eth_name, 'up')
