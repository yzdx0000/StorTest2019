...
def up_down2(pipe_write):
    for i in range(8):
        log.info("Begin down node %s." % (fault_ip_1,))
        node_info = error.down_node(fault_ip=fault_ip_1)
        time.sleep(120)
...
def main():
    ...
    test_threads.append(threading.Thread(target=up_down2, args=(pipe_write,)))
