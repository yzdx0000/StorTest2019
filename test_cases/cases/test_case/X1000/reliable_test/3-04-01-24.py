...
4、free日志节点D下电后，一日志节点oStor进程挂住；
5、业务执行完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
6、节点D上电，且oStor进程恢复后数据修复完成，比较存储内部数据一致性；
:Changerlog:
"""
# testlink case: 1000-32562
import os,sys
import commands
import threading
import random
...
mix_S = osan.gen_vdb_xml(lun=osan.ls_scsi_dev(client_ips[1]),
                         ...
                         rdpct=0)
# 获取free日志节点ID
free_jnl_nodes = error.get_free_jnl_id()
nodes = osan.get_nodes()
if len(free_jnl_nodes) == 0:
    free_jnl_nodes = nodes
jnl_nodes = error.get_jnl_node_id()
# 随机选择一个节点
b_id = random.choice(free_jnl_nodes)
b_ip = disk.get_node_ip_by_id(n_id=b_id)
log.info("Select node is %s." % (b_id,))
if b_id in jnl_nodes:
    jnl_nodes.remove(b_id)
c_id = random.choice(jnl_nodes)
c_ip = disk.get_node_ip_by_id(n_id=c_id)


def up_down(pipein):
    time.sleep(30)
    log.info("4、free日志节点D %s下电" % (b_ip,))
    log.info("Begin shutdown node %s." % (b_ip,))
    m_b_info = error.down_node(fault_ip=b_ip)
    log.info("4、free日志节点D下电后，一日志节点%s oStor进程挂住；" % (c_ip,))
    ReliableTest.run_kill_process(c_ip, 'oStor')
    while True:
