...
4、在步骤3中的业务运行过程中，将A节点关机，将节点B数据网交替故障；
...
log.info("Got machine A's IP: %s, and machine B's IP: %s, dev name is %s, extra ip is %s." % (a_ip, b_ip, b_eths, b_extra))


def up_down(pipein):
    time.sleep(10)
    log.info("44、在步骤3中的业务运行过程中，将A节点关机，将节点B数据网交替故障；")
    log.info("Begin shutdown node:%s." % (a_ip, ))
    m_info = error.down_node(fault_ip=a_ip)
    for i in range(0,30):
        for tmp_eth in b_eths:
            b_eth = []
            b_eth.append(tmp_eth)
            log.info("Begin down data net %s through %s." % (b_eth, b_extra[0]))
            ReliableTest.run_down_net(b_extra[0], b_eth)
            time.sleep(10)
            log.info("Begin up data net %s through %s." % (b_eth, b_extra[0]))
