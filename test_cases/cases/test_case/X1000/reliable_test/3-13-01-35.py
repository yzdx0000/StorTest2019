...
4、在步骤3中的业务运行过程中，将节点A节点关机；
5、日志数据修复到B节点，修复完成后，添加新节点D，日志迁移到D节点，迁移时断开节点D数据网；
6、系统修复，修复完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
7、比较存储内部数据一致性。"

:Changerlog:
"""
# testlink case: 1000-32862
import os
import sys
import commands
import threading
...
def up_down():
    log.info("故障点:先将日志节点A下电,日志节点B接管日志业务后再将free日志节点的数据网断开")
    time.sleep(100)
    log.info("Step4:业务过程中,将日志节点A的下电,日志节点A:%s" % los_node_ip_A)
    vm_id_A = error.down_node(los_node_ip_A)
    time.sleep(100)

    log.info("Step5:B节点接管A节点的日志业务后,将free日志节点的数据网断开,free日志节点:%s" % free_node_ip)
    thread_list = []
    ...
    time.sleep(60)
    log.info("Step6:将free日志节点的数据网恢复,日志节点:%s" % free_node_ip)
    thread_list = []
    for eth_name in fault_eth_list_free:
    ...
    time.sleep(60)
    log.info("Step7:将日志节点A上电,日志节点:%s" % los_node_ip_A)
