...
type_info3 = env_manage_repair_test.get_os_type(node_ip3)
vm_ids3 = None
if type_info != "phy":
    vm_ids3 = env_manage_repair_test.Reliable_osan.vm_id(esxi_ip=esxi_ip, u_name=cp("esxi", "esxi_user"),
                                                         pw=cp("esxi", "esxi_passwd"),
                                                         node_ip=node_ip3)
else:
    vm_ids3 = ReliableTest.get_ipmi_ip(node_ip3)
...
def os_error(arg=3):
    ...
    decorator_func.timer(300, 25, log_interval=0)
...
def os_error2(arg=3):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    decorator_func.timer(100, 25, log_interval=0)
    env_manage_repair_test.down_node(node_ip3, type_info3)  # 下电

    decorator_func.timer(300, 25, log_interval=0)

    env_manage_repair_test.up_node(node_info=vm_ids3, type_info=type_info3)  # 上电
...
def case():
    ...
    iscsi_login()
    decorator_func.multi_threads(run_vdb1, run_vdb2, os_error, os_error2)
