...
deploy_ls.remove(m_zk_ip)
f_zk_ip = random.choice(deploy_ls)
log.info("Get fault process:master zk & follow zk")

def up_down():
    time.sleep(100)
    log.info("Step4:将主zk进程杀死,节点: %s." %(m_zk_ip))
    ReliableTest.run_kill_process(m_zk_ip,'zk')
    log.info("Step4:将从zk进程杀死,节点: %s." % (f_zk_ip))
    ReliableTest.run_kill_process(f_zk_ip,'zk')
    time.sleep(100)

    log.info("Step5:检查故障主zk进程状态,节点: %s." % (m_zk_ip))
    state1 = ReliableTest.check_process(m_zk_ip,'zk')
    if state1 == True:
        log.info("该节点zk进程状态正常,节点: %s" %(m_zk_ip))
    log.info("Step5:检查故障从zk进程状态,节点: %s." % (f_zk_ip))
    state2 = ReliableTest.check_process(f_zk_ip,'zk')
    if state2 == True:
        log.info("该节点zk进程状态正常,节点: %s" %(f_zk_ip))
