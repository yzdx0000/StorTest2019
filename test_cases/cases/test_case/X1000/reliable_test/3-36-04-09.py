...
:Date:20190120
:Description:
2副本节点池一个节点双业务网反复故障，另一节点单业务网反复故障
"""
import os
import random
import utils_path
...
def up_down2():
    time.sleep(60)
    for i in range(10):
        log.info("Begin to down io net:%s through %s." % (fault_eths_2[:1], extra_ip_2[0]))
        ReliableTest.run_down_net(extra_ip_2[0], fault_eths_2[:1])
        time.sleep(40)
        log.info("Begin to up io net:%s through %s." % (fault_eths_2[:1], extra_ip_2[0]))
        ReliableTest.run_up_net(extra_ip_2[0], fault_eths_2[:1])
        time.sleep(120)
    tag2.set()
