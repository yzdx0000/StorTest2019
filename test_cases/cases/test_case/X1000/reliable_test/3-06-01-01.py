...
a_eths,a_extra = error.get_ioip_info(node_id=los_node_id)
a_eth = []
a_eth.append(a_eths[0])
def up_down():
#    fault_ip = error.get_fault_ip()
    time.sleep(300)
    log.info("Begin down node 'ssh %s down %s'." %(a_extra[0],a_eth))
...
def vdb_jn():
    ...
    log.info("Begin up node 'ssh %s up %s'." %(a_extra[0],a_eth))
