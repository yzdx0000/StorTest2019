...
def up_down(pipe_write):
    ...
    for fault_ip in fault_ips.keys():
        log.info("Begin shutdown net: %s on node: %s." % (fault_ips[fault_ip], fault_ip))
        ReliableTest.run_down_net(fault_ip, fault_ips[fault_ip])
    time.sleep(40)
    for fault_ip in fault_ips.keys():
        log.info("Begin up net: %s on node: %s." % (fault_ips[fault_ip], fault_ip))
