...
4、在步骤3中的业务运行过程中，将节点A的业务网和数据网都断开；
5、等待告警上报后再恢复网络。
6、重复执行步骤4/5，业务完成后，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32875
import os
import sys
import commands
import threading
...
def up_down():
    log.info("故障点:反复故障一个节点的所有业务网卡和所有数据网卡")
    time.sleep(100)
    for count in range(3):
        log.info("第 %d 次故障业务节点A的业务网和数据网" % (count + 1))
        ...
        for thread in thread_ls1:
            thread.join()
        time.sleep(30)
        log.info("Step6:同时恢复故障节点所有业务网和数据网,业务节点:%s" % fault_io_ip)
        thread_ls2 = []
        for data_eth_name in data_eth_list:
            thread_ls2.append(threading.Thread(target=reliable.network_test, args=(fault_io_ip, data_eth_name, 'up')))
        for io_eth_name in io_eth_list:
            thread_ls2.append(threading.Thread(target=reliable.network_test, args=(fault_io_ip, io_eth_name, 'up')))
