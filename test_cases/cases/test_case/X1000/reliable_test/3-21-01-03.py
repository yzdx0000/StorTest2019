...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将节点A的oSan进程异常，再将oPmgr进程异常；
            5、检查oSan和oPmgr主进程状态
            6、主机1上业务完成后，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-33005
# testlink case: 1000-33005
import os
import sys
import commands
...
log.info("Get fault process:oSan & oRole")

def up_down():
    time.sleep(100)
    log.info("Step4:将故障节点oSan进程杀死,节点: %s." %(fault_node_ip))
    ...
    time.sleep(100)
    # 重新获取主,确保主起来,检查进程状态
    log.info("Step5:检查故障节点oSan进程状态,节点: %s." % (fault_node_ip))
