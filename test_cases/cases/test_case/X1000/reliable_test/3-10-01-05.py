...
def up_down():
    ...
    for eth in a_eths:
        ...
        time.sleep(30)
    log.info("6、前端业务挂住后，将主oPmgr进程kill掉")
    master_orole_id = error.get_orole_master_id(s_ip=deploy_ips[0])
    log.info("Got master oRole ID : %s" % (str(master_orole_id)))
    master_orole_ip = node.get_node_ip_by_id(master_orole_id)
    log.info("Got master oRole IP : %s" % (master_orole_ip))
