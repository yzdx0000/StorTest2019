...
def up_down():
    ...
    time.sleep(100)
    fault_threads=[]
    fault_threads.append(threading.Thread(target=ReliableTest.run_kill_process(fault_node_ip_A,'oRole')))
    fault_threads.append(threading.Thread(target=ReliableTest.run_kill_process(fault_node_ip_B,'oRole')))
    log.info("Step4:将两个节点的oRole进程同时杀死,节点: %s,%s." %(fault_node_ip_A,fault_node_ip_B))
    ...
    reliable.check_process_stat(fault_node_ip_B, 'oRole')
