...
c_ip = node.get_node_ip_by_id(c_id)
...
def up_down(pipein):
    ...
    log.info("5、将接管步骤3中的节点A的VIP的业务节点B %s 掉电；" % (b_ip,))
    log.info("Begin shutdown node %s." % (b_ip,))
    m_b_info = error.down_node(fault_ip=b_ip)
    if str(c_id) != str(b_id):
        log.info("5、在步骤3中的业务运行过程中，将接管步骤3中的节点A的VIP的业务节点B掉电后，随即%s节点oPmgr主被kill；"
                 % (c_ip,))
    ...
    log.info("Begin up io net %s through %s." % (a_eths, a_extra[0]))
