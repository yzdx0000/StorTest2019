...
4、将日志节点B下电；
5、主机1上业务完成后，在主机1上执行vdbench OLTPn.conf -jro比较一致性；
6、节点B下电超时后上电，启动数据修复，非日志节点E掉电；
7、节点E上电，数据修复完成，在主机1上执行vdbench -f OLTP.conf -jro比较一致性；
8、比较存储内部数据一致性
:Changerlog:
"""
# testlink case: 1000-32517
import os,sys
import commands
import threading
import random
...
def vdb_jn(pipeout):
    log.info("4、:在主机1上运行vdbench -f mix-O.conf -jn.")
    ...
    log.info("6、节点B下电超后，系统启动数据修复，数据修复中，在主机1上执行vdbench -f mix-O.conf -jro比较一致性；")
    osan.run_vdb(client_ips[0], mix_S_align, output=deploy_ips[0], jn_jro="jro")
    # 等待节点启动完成
    ...
    log.info("6、等待系统修复。")
    disk.check_bad_obj()
    log.info("7、节点E上电，数据修复完成，在主机1上执行vdbench -f mix-O.conf -jro比较一致性；")
    osan.run_vdb(client_ips[0], mix_S_align, output=deploy_ips[0], jn_jro="jro")
