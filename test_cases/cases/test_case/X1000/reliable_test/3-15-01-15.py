...
4、在步骤3中的业务运行过程中，将节点A和节点D数据网同时断开；
...
def up_down():
    ...
    time.sleep(100)
    log.info("Step4:修改等待超时时间，触发非日志节点B超时重建,节点:%s" % free_jnl_ip)
    reliable.run_down_node_wait(non_free_ip, '300000')
    time.sleep(30)
    log.info("Step5:在步骤3中的业务运行过程中，将非业务日志A节点和非日志B节点数据网络故障,节点:%s,%s" % (free_jnl_ip, non_free_ip))
    thread_list = []
    for eth_name in fault_eth_list_A:
        thread_list.append(threading.Thread(target=reliable.network_test, args=(free_jnl_ip, eth_name, 'down')))
    ...
    time.sleep(30)
    log.info("Step7:检查节点状态是否正常,节点:%s" % non_free_ip)
    node_state = reliable.get_node_state(non_free_ip)
    if node_state != 'NODE_STATE_HEALTHY':
        log.error("The node's state is not HEALTHY:%s" % node_state)
    log.info("Step7:将节点超时时间恢复默认值")
    reliable.run_down_node_wait(non_free_ip, '86400000')
