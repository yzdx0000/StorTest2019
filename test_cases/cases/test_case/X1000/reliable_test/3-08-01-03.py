...
4、在步骤3中的业务运行过程中，将节点A一个业务网断开；
5、检查主机业务运行状态，检查故障上报情况
6、确认故障已上报，断开所有节点的数据网
7、主机1上业务完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
:Changerlog:
"""
# testlink case: 1000-32710
import os
import sys
import commands
import threading
...
def up_down():
    log.info("故障点:一个节点的一个业务网卡和所有节点的数据网卡先后故障")
    time.sleep(100)
    log.info("Step4:在步骤3中的业务运行过程中，将该节点的一根业务网断开,节点: %s." % fault_io_ip)
    thread_list = []
    data_eth_lists = []
    ...
    event1.wait()
    event2.wait()
    log.info("Step5:故障上报后,将所有节点的数据网卡故障...")
    ...
    time.sleep(100)
    log.info("Step6:将故障业务节点的单个业务网恢复,节点: %s." % (fault_io_ip))
    reliable.network_test(fault_io_ip, io_eth_name, 'up')
    eth_index = 0
    for fault_ip in deploy_ips:
        log.info("Step7:将所有节点的所有数据网卡恢复,节点:%s" % (fault_ip))
