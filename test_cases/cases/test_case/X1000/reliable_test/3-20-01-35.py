...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将守护进程异常，再将oPmgr异常；
            5、检查oPmgr进程状态
            6、主机1上业务完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32995
import os
import sys
import commands
import threading
...
log.info("Get fault process:oMgcd & oRole")

def up_down():
    time.sleep(150)
    log.info("Step4:将故障节点oMgcd守护进程杀死,节点: %s." % (fault_node_ip))
    ReliableTest.run_kill_process(fault_node_ip,'oMgcd')
    log.info("Step4:将故障节点oRole进程杀死,节点: %s." %(fault_node_ip))
    ReliableTest.run_kill_process(fault_node_ip,'oRole')
    time.sleep(100)
    log.info("Step5:检查故障节点守护进程oMgcd进程状态,节点: %s." % (fault_node_ip))
    reliable.check_process_stat(fault_node_ip,'oMgcd')
    log.info("Step5:检查故障节点oRole进程状态,节点: %s." % (fault_node_ip))
