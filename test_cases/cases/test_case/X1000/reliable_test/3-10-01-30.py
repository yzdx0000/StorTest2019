...
master_zk_ip = os_rea.get_master_zk(deploy_ips)
log.info("Got machine A's info: fault ip is : %s,extra ip is %s,data device name is :%s"
         ",master zookeeper ip is %s." % (a_ip, a_extra, a_eths, master_zk_ip))


def up_down(pipein):
    time.sleep(10)
    log.info("4、在步骤3中的业务运行过程中，将节点A业务网断开，将ZK主进程故障；")
    log.info("Begin down node:%s, eth %s through %s." % (a_ip, a_eths, a_extra))
    ReliableTest.run_down_net(a_extra[0], a_eths)
    log.info("On node %s kill zk" % (b_ip,))
    ReliableTest.run_kill_process(b_ip, 'zookeeper')
    # 监控到匿名管道中写入数据后，退出循环
    while True:
