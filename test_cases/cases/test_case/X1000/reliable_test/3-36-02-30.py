...
:Date:20190215
:Description:
一访问分区中JNL主进程故障和另一节点共享盘故障，另一访问区一共享盘故障
"""
import os
import random
import utils_path
...
lun_los = disk.get_lun_los_pair()
losid = random.choice(lun_los.keys())
jnlip = disk.get_nodeip_by_losid(losid=losid)
...
def up_down(pipe_write):
    ...
    log.info("Begin to kill oPara on %s." % (jnlip,))
    ReliableTest.run_kill_process(node_ip=jnlip, process='oPara')
    time.sleep(320)
    for dip in fault_disk.keys():
        for fdisk in fault_disk[dip]:
