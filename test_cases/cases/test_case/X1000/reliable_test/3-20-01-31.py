...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将守护进程异常，再将oJmgs异常；
            5、检查oJmgs进程状态
            6、主机1上业务完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32992
import os
import sys
import commands
import threading
...
# oMgcd进程异常后，oJmgs异常
log.info("Get fault process:oMgcd & oJmgs")

def up_down():
    time.sleep(100)
    log.info("Step4:将该节点oMgcd守护进程杀死,节点: %s." %(m_oJmgs_ip))
    ReliableTest.run_kill_process(m_oJmgs_ip,'oMgcd')
    time.sleep(30)
    log.info("Step4:将该节点oJmgs守护进程杀死,节点: %s." %(m_oJmgs_ip))
    ReliableTest.run_kill_process(m_oJmgs_ip,'oJmgs')
    time.sleep(300)
    log.info("Step5:检查该节点oMgcd守护进程状态,节点: %s." % (m_oJmgs_ip))
