...
def up_down(pipe_write):
    ...
    down_ip = disk.get_node_ip_by_id(n_id=fault_node)
    fault_node = random.choice(nodes)
    fault_eths, fault_ip = error.get_ioip_info(node_id=fault_node)
    log.info("Begin down node: %s." % (down_ip,))
    m_info = error.down_node(fault_ip=down_ip)
    log.info("Begin shutdown data net: %s on node: %s." % (fault_eths, fault_ip[0]))
    ReliableTest.run_down_net(fault_ip[0], fault_eths)
    time.sleep(60)
    log.info("Begin up node: %s." % (down_ip,))
    error.up_node(node_info=m_info)
    log.info("Begin up io net: %s on node: %s." % (fault_eths, fault_ip[0]))
    ReliableTest.run_up_net(fault_ip[0], fault_eths)
