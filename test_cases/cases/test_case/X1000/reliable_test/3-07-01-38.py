...
4、在步骤3中的业务运行过程中，将节点C拔掉一个业务网，在将节点B数据网断开；
...
io_eth_name = random.choice(fault_io_eth_list)
fault_data_eth_list = error.get_io_eth(jnl_node_id)

def up_down():
    log.info("故障点:先将故障节点的单个业务网断开,再将数据网断开")
    time.sleep(100)
    log.info("Step4:在步骤3中的业务运行过程中，先将故障的节点的单个业务网断开,节点:%s" % jnl_node_ip)
    reliable.network_test(jnl_node_ip, io_eth_name, 'down')
    time.sleep(30)
