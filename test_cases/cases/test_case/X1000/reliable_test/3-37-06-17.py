...
def os_error(arg=3):
    ...
    for a in range(3):
        env_manage_repair_test.down_node(target_ip, type_info)  # 下电
...
def case():
    iscsi_login()
    decorator_func.multi_threads(run_vdb1, run_vdb2, network_error, network_error2,network_error3, process_thread)
    for i in io_eth_list1:
        env_manage_repair_test.Reliable_osan.network_test(s_ip=node_ip1,
                                                          net_name=i,
                                                          net_stat="up")
