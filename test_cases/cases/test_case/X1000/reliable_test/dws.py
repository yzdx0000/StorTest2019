...
def up_down():
    ...
    for i in range(400):
        ...
        err_type = random.randint(0,1)
        if err_type == 0:
            #log.info("Step3:每隔40秒，随机选择一个节点，随机选择一个进程")
            ...
            #log.info("每隔40秒，随机选择一个节点，随机选择一个网络")
            eth_type = random.choice(err_dict[1])
            if eth_type == 'data':
                err_eth, extra_ip = error.get_dataip_info(fault_id)
                #log.info("Step3:选择数据网，网络：%s，额外IP：%s" %(err_eth,extra_ip))
            else:
                err_eth, extra_ip = error.get_ioip_info(fault_id)
                #log.info("Step3:选择业务网，网络：%s，额外IP：%s" %(err_eth,extra_ip))
