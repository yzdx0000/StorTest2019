...
4、在步骤3中的业务运行过程中，将日志节点A节点数据网出现20%数据包乱序；
...
def up_down():
    ...
    log.info("Step4:业务读写过程中,将日志节点数据网数据包乱序,节点:%s" % fault_node_ip)
    thread_list = []
    for data_eth in data_eth_list:
        cmd = ("ssh %s 'tc qdisc add dev %s root netem delay 10ms reorder 20%% 50%%'" % (fault_node_ip, data_eth))
        log.info(cmd)
    ...
    for data_eth in data_eth_list:
        cmd = ("ssh %s 'tc qdisc del dev %s root netem delay 10ms reorder 20%% 50%%'" % (fault_node_ip, data_eth))
        log.info(cmd)
        rc = os.system(cmd)
        if rc == 0:
