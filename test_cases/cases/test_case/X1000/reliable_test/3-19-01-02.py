...
4、在步骤3中的业务运行过程中，将所有业务网络反复故障
6、检查VIP漂移情况"

:Changerlog:
"""
# testlink case: 1000-32957
import os
import sys
import commands
import threading
...
def up_down():
    log.info("故障点:业务完成后,将所有节点的业务网故障后恢复,后再进行数据校验.")
    time.sleep(100)
    event1.wait()
    event2.wait()
    time.sleep(15)
    ...
    log.info("将所有节点的业务网络反复故障")
    for count in range(3):
        log.info("第 %s 次故障所有节点的业务网络" % (count + 1))
        log.info("Step4:将所有节点的业务网故障.")
        for thread in thread_list_down:
            thread.start()
        for thread in thread_list_down:
            thread.join()
        time.sleep(60)
        log.info("Step5:将所有节点的业务网恢复.")
