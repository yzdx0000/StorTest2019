...
:Description:
            ...
            3、在主机1上运行vdbench -f mix_R_align.conf -jn；在主机2上运行vdbench -f mix-R.conf;
            4、在步骤3中的业务运行过程中，将节点A和原日志组节点B间隔5分钟先后掉电；
            5、在主机1上执行vdbench -f mix_R_align.conf -jro比较一致性；
            6、节点A和节点B上电恢复后数据修复完成，比较存储内部数据一致性；
:Changerlog:
"""
# testlink case: 1000-32500
import os,sys
import commands
import threading
import random
