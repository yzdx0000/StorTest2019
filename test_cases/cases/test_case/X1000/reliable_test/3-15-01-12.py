...
def up_down():
    ...
    time.sleep(100)
    log.info("Step4:在步骤3中的业务运行过程中，将A节点和B非日志节点数据网络同时故障,节点:%s,%s" % (fault_io_ip_A, non_free_ip))
    thread_list = []
    ...
    for io_eth_name in fault_eth_list_B:
        thread_list.append(threading.Thread(target=reliable.network_test, args=(non_free_ip, io_eth_name, 'down')))
    ...
    time.sleep(10)
    log.info("Step5:10s后将故障节点数据网恢复,节点:%s" % (fault_io_ip_A))
    for data_eth_name in fault_eth_list_A:
        reliable.network_test(fault_io_ip_A, data_eth_name, 'up')
    time.sleep(600)
    log.info("Step6:10min后将故障非日志节点数据网恢复,节点:%s" % (non_free_ip))
