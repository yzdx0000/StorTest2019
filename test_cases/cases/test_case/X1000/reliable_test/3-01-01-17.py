...
:Description:
            ...
            3、在主机1上运行vdbench -f mix-R-S-align.conf -jn；在主机2上运行vdbench -f mix-R.conf;
            4、在步骤3中的业务运行过程中，将节点A掉电和日志组节点B同时掉电；
            5、在主机1上执行vdbench -f mix-R-align.conf -jro比较一致性；
            6、10分钟后，节点A上电恢复；
            7、10分钟后，节点B上电恢复；
            8、在数据修复过程中在主机1上执行vdbench -f mix-R-align.conf -jro比较一致性；
            9、数据修复完成后在主机1上执行vdbench -f mix-R-align.conf -jro比较一致性；
            10、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32301
import os,sys
import commands
import threading
import random
...
def vdb_jn(pipein):
    ...
    for ip in deploy_ips:
        common.check_ip(ip)
        error.check_host(ip)
    log.info("Step8:在数据修复过程中在主机1上执行vdbench -f S-align.conf -jro比较一致性.")
    osan.run_vdb(client_ips[0], mix_O, output=deploy_ips[0], jn_jro="jro")
    for ip in deploy_ips:
        common.check_ip(ip)
        error.check_host(ip)
    for id in nodes:
        log.info("Step9:等待数据修复")
        while 'HEALTHY' not in node.get_node_state(id):
            time.sleep(20)
            log.info("Step9:等待数据修复ing")
    log.info("Step9:数据修复完成后在主机1上执行vdbench -f S-align.conf -jro比较一致性.")
    osan.run_vdb(client_ips[0], mix_O, output=deploy_ips[0], jn_jro="jro")
