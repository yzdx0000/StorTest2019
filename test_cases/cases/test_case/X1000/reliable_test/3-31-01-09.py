...
4、在步骤3中的业务运行过程中，将A节点关机，将节点B数据网断开不恢复；
...
log.info("Got machine A's IP: %s, and machine B's IP: %s, dev name is %s, extra ip is %s."
         % (a_ip, b_ip, b_eths, b_extra))
...
def up_down(pipein):
    ...
    if 0 != rc:
        ...
        exit(1)
    time.sleep(10)
    log.info("4、在步骤3中的业务运行过程中，将A节点关机，将节点B数据网断开不恢复；")
    log.info("Begin shutdown node:%s." % (a_ip, ))
    m_info = error.down_node(fault_ip=a_ip)
    log.info("Begin down data net %s through %s." % (b_eths, b_extra[0]))
    ReliableTest.run_down_net(b_extra[0], b_eths)
    time.sleep(120)
    log.info("Begin up data net %s through %s." % (b_eths, b_extra[0]))
...
def vdb_jn(pipeout):
    ...
    log.info("Run vdbench with jro.")
    osan.run_vdb(client_ips[0], mix_R_align, output=deploy_ips[0], jn_jro="jro")
    log.info("8、数据修复完成后，比较存储内部数据一致性")
    log.info("Run vdbench with jro.")
    osan.run_vdb(client_ips[0], mix_R_align, output=deploy_ips[0], jn_jro="jro")
