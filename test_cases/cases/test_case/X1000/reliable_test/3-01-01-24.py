...
:Description:
            ...
            5、节点A重启完成，数据修复中，在主机1上执行vdbench -f mix-R-align.conf -jro比较一致性；
            6、节点A数据修复完成，在主机1上执行vdbench -f mix-R-align.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32308
import os,sys
import commands
import threading
import random
...
def up_down():
    time.sleep(100)
    log.info("Step4:将节点A掉重启 : %s." %(fault_node_ip))
...
def vdb_jn():
    ...
    log.info("Step6:修复完成后在主机1上执行vdbench -f mix_R_align.conf -jro比较一致性。.")
    osan.run_vdb(client_ips[0], mix_R_align, output=deploy_ips[0], jn_jro="jro")
