...
4、在步骤3中的业务运行过程中，业务节点的业务网和数据网都断开，10分钟后恢复
5、数据修复完成后，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
6、10分钟后先恢复数据网，再回复业务网
7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32713
import os
import sys
import commands
import threading
...
def up_down():
    ...
    eth_index = 0
    for fault_ip in fault_ip_list:
        log.info("Step6:先将所有节点的所有数据网卡恢复,节点:%s" % fault_ip)
    ...
    eth_index = 0
    for fault_ip in fault_ip_list:
        log.info("Step7:再将所有节点的所有业务网卡恢复,节点:%s" % fault_ip)
