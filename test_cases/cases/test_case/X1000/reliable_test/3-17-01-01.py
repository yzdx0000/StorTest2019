...
def up_down():
    log.info("故障点:一个非日志节点的数据网故障")
    time.sleep(100)
    log.info("Step4:在步骤3中的业务运行过程中，将该节点的数据网断开,节点: %s." %(fault_node_ip))
    ...
    time.sleep(100)
    for fault_eth_name in fault_eth_list:
        log.info("Step5:将故障节点数据网的网卡恢复,网卡:%s" % fault_eth_name)
