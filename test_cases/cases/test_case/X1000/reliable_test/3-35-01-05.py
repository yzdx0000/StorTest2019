...
4、在步骤3中的业务运行过程中，将日志节点A节点数据网延迟增大到100ms；
...
mix_R_Align = osan.gen_vdb_xml(lun=osan.ls_scsi_dev(client_ips[0]),
                               xfersize="(4k,40,16k,20,64k,20,256k,10,512k,5,1024k,5)",
                               seekpct=0,
                               rdpct=0)
mix_R = osan.gen_vdb_xml(lun=osan.ls_scsi_dev(client_ips[1]),
                         xfersize="(4k,40,15k,20,127k,20,512k,10,2045k,10)",
                         offset=2048,
                         seekpct=0,
                         rdpct=0)
...
def up_down():
    ...
    log.info("Step4:业务读写过程中,将日志节点数据网延迟增大,节点:%s" % fault_node_ip)
    thread_list = []
    for data_eth in data_eth_list:
        cmd = ("ssh %s 'tc qdisc add dev %s root netem delay 100ms'" % (fault_node_ip, data_eth))
        log.info(cmd)
    ...
    time.sleep(10)
    log.info("数据网络注入故障后延迟情况,节点:%s" % fault_node_ip)
    ...
    for data_eth in data_eth_list:
        cmd = ("ssh %s 'tc qdisc del dev %s root netem delay 100ms'" % (fault_node_ip, data_eth))
        log.info(cmd)
        rc = os.system(cmd)
        if rc == 0:
