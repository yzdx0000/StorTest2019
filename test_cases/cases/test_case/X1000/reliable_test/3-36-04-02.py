...
error.rel_check_before_run(file_name, node_num=5, data_rep=3)
...
def up_down():
    ...
    m_info_1 = error.down_node(fault_ip=fault_ip_1)
    tag1.wait()
    log.info("Begin to startup node:%s." % (fault_ip_1,))
    error.up_node(node_info=m_info_1)
    tag3.set()
...
def up_down2():
    ...
    m_info_2 = error.down_node(fault_ip=fault_ip_2)
    tag2.wait()
    log.info("Begin to startup node:%s." % (fault_ip_2,))
    error.up_node(node_info=m_info_2)
    tag4.set()
