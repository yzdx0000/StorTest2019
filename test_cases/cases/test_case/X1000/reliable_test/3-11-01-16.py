...
def up_down():
    ...
    time.sleep(100)
    log.info("Step4:业务节点A的迁移后,将接管业务节点B %s 掉电." % test_node_ip[1])
    vm_info2 = error.down_node(test_node_ip[1])

    time.sleep(100)
    log.info("Step5:将业务节点A与B %s,%s 重新上电." % (test_node_ip[0], test_node_ip[1]))
    error.up_node(vm_info1)
    error.up_node(vm_info2)
