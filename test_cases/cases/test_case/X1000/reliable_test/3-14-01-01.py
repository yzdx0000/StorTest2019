...
4、在步骤3中的业务运行过程中，将节点A的数据网和业务网同时断一个网线；
5、业务完成后，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
6、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32868
import os
import sys
import commands
import threading
...
def up_down():
    ...
    time.sleep(100)
    log.info("Step4:在步骤3中的业务运行过程中，将节点一个数据网和一个业务网同时断开,业务节点:%s" % fault_io_ip)
    thread_ls = []
    thread_ls.append(threading.Thread(target=reliable.network_test, args=(fault_io_ip, data_eth_name, 'down')))
