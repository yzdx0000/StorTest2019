...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将节点A/B/C的ostor进程反复异常；
            5、检查各个节点ostor进程
            6、主机1上业务完成后，在主机1上执行vdbench -f OLTP.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-33009
# testlink case: 1000-33010
import os
import sys
import commands
import threading
...
def up_down():
    ...
    for count in range(3):
        log.info("第 %d 次杀死故障节点oStor进程" % (count + 1))
        fault_threads=[]
        ...
        log.info("Step4:将两个节点的oStor进程同时杀死,节点: %s,%s." %(fault_node_ip_A, fault_node_ip_B))
        ...
        while True:
            ...
            state_A = reliable.check_process_stat(fault_node_ip_A, 'oStor')
