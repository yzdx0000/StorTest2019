...
7、业务节点和硬盘都超时后恢复，系统进行数据校验；
8、数据修复完成后，比较存储内部数据一致性
'''
# testlink case: 1000-33158
import os
import sys
import utils_path
import common2
...
def up_down(pipein):
    ...
    if 0 != rc:
        ...
        exit(1)
    time.sleep(40)
    cmd = "ssh %s 'pscli --command=update_param --section=MGR --name=node_isolate_timeout --current=30000'" % (
        a_ip)
    rc, stdout = commands.getstatusoutput(cmd)
    if 0 != rc:
        log.error('update param failed!!!')
    ...
    ReliableTest.remove_disk(b_ip, disk_uuid, "data")
    # 监控到匿名管道中写入数据后，退出循环
    while True:
