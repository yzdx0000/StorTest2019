...
def network_error2(arg=1):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    decorator_func.timer(350, 25, log_interval=0)
    env_manage_repair_test.Reliable_osan.network_test(s_ip=node_ip2,
                                                      net_name=fault_eth_list2[0],
                                                      net_stat="down")
    env_manage_repair_test.Reliable_osan.network_test(s_ip=node_ip2,
                                                      net_name=fault_eth_list2[1],
                                                      net_stat="down")
    time.sleep(180)
    env_manage_repair_test.Reliable_osan.network_test(s_ip=node_ip2,
                                                      net_name=fault_eth_list2[0],
...
def case():
    iscsi_login()
    decorator_func.multi_threads(run_vdb1, run_vdb2, network_error, network_error2)
    for i in io_eth_list1:
        env_manage_repair_test.Reliable_osan.network_test(s_ip=node_ip1,
                                                          net_name=i,
                                                          net_stat="up")
