...
:Description:
            ...
            5、日志free节点D在接管节点A日志后重建过程中，将节点D重启；
            6、数据修复完成后，在主机1上执行vdbench -f mix-R-align.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32325
import os,sys
import commands
import threading
import random
...
if disk_rep == 1:
    ...
    if len(free_jnl_ids) == 0:
        print "Sorry,I can not run for I detect your cluster have no jnl node."
    else:
...
fault_node_b_ip = random.choice(free_jnl_ips)
node_info = []
def up_down():
    time.sleep(100)
    ...
    time.sleep(5)
    #将日志节点B下电
    log.info("Step5:将free日志节点D重启 : %s." % (fault_node_b_ip))
