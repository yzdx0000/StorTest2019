...
:Description:
            ...
            5、节点A重启完成，日志迁移到节点A过程中时，节点A掉电；
            6、节点A上电恢复，数据修复过程中在主机1上执行vdbench -f mix-R-align.conf -jro比较一致性；
            7、数据修复完成后，比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32324
import os,sys
import commands
import threading
import random
...
def up_down(pipeout):
    ...
    time.sleep(5)
    #将日志节点B下电
    log.info("Step5:将日志节点A下电 : %s." % (fault_node_b_ip))
    ...
    while True:
        ...
        if len(line) != 0:
            for vid in line.strip().split(','):
                vid = re.sub('\\r','',vid)
                log.info("Step6:节点A上电 : %s." %(vid.strip()))
