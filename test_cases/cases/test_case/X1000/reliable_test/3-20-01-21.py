...
deploy_ips.remove(m_oJmgs_ip)
f_oJmgs_ip = random.choice(deploy_ips)
log.info("Get fault process:master oJmgs & follow oJmgs")
...
def up_down():
    ...
    log.info("Step4:将节点从oJmgs进程挂住,节点: %s." % (f_oJmgs_ip))
    #reliable.run_pause_process('oJmgs', f_oJmgs_ip)
    ReliableTest.run_kill_process(f_oJmgs_ip, 'oJmgs')
    ...
    log.info("Step5:检查故障节点主oJmgs进程状态,节点: %s." % (m_oJmgs_ip))
    reliable.check_process_stat(m_oJmgs_ip,'oJmgs')
    log.info("Step5:检查故障从oJmgs进程状态,节点: %s." % (f_oJmgs_ip))
    reliable.check_process_stat(f_oJmgs_ip,'oJmgs')
    time.sleep(100)
    # log.info("Step5:恢复挂起节点主oJmgs进程,节点: %s." %(m_oJmgs_ip))
    # reliable.run_process('oJmgs',m_oJmgs_ip)
    # log.info("Step5:恢复挂起节点从oJmgs进程,节点: %s." % (f_oJmgs_ip))
    # reliable.run_process('oJmgs',f_oJmgs_ip)
    # time.sleep(100)
    log.info("Step6:检查恢复后节点主oJmgs进程状态,节点: %s." % (m_oJmgs_ip))
    reliable.check_process_stat(m_oJmgs_ip, 'oJmgs')
    log.info("Step6:检查恢复后从oJmgs进程状态,节点: %s." % (f_oJmgs_ip))
    reliable.check_process_stat(f_oJmgs_ip, 'oJmgs')
