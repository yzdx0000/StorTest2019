...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将节点A oMgcd进程反复挂住；
            5、检查oMgcd进程状态
            6、主机1上业务完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32990
import os
import sys
import commands
import threading
...
# 选择oMgcd进程进行异常操作
log.info("Get fault process:oMgcd")

def up_down():
    time.sleep(100)
    for count in range(0,3):
        log.info("Step4:将该节点oMgcd进程挂住,节点: %s." %(fault_node_ip))
        ReliableTest.run_kill_process(fault_node_ip,'oMgcd')
        time.sleep(100)
        log.info("Step5:检查该节点oMgcd进程状态,节点: %s." % (fault_node_ip))
        reliable.check_process_stat(fault_node_ip,'oMgcd')
        # log.info("Step5:恢复挂起节点oMgcd进程,节点: %s." % (fault_node_ip))
        # reliable.run_process('oMgcd', fault_node_ip)
        # time.sleep(100)
        log.info("Step5:检查该节点oMgcd进程状态,节点: %s." % (fault_node_ip))
        state = reliable.check_process_stat(fault_node_ip,'oMgcd')
        while True:
            if state == 'OK':
                break
            time.sleep(20)
