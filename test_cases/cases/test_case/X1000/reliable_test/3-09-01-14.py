...
# 获取非接口节点ID
node_no_inter_ids = list(set(nodes).difference(set(node_inter_ids)))
if len(node_no_inter_ids) == 0:
    node_no_inter_ids = nodes
a_id = random.choice(node_no_inter_ids)
...
def up_down():
    ...
    osan.run_pscli_cmd(cmd)
    time.sleep(10)
    log.info("4、在步骤3中的业务运行过程中，将节点A和节点D数据网同时断开；")
