...
def up_down():
    time.sleep(100)
    log.info("Step4:将故障节点从oJmgs进程挂住,节点: %s." %(fault_node_ip))
    #reliable.run_pause_process('oJmgs', fault_node_ip)
    ReliableTest.run_kill_process(m_oJmgs_ip, 'oJmgs')
    time.sleep(100)
    log.info("Step5:检查从oJmgs进程状态,节点: %s." % (fault_node_ip))
    reliable.check_process_stat(fault_node_ip, 'oJmgs')
    time.sleep(100)
    #log.info("Step4:恢复挂起节点oJmgs进程,节点: %s." %(fault_node_ip))
    #reliable.run_process('oJmgs', fault_node_ip)
    #time.sleep(100)
    log.info("Step5:检查恢复后oJmgs进程状态,节点: %s." % (fault_node_ip))
    reliable.check_process_stat(fault_node_ip, 'oJmgs')
