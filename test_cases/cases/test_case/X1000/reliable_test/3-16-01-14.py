...
def up_down(pipein):
    ...
    b_eths, bextras = error.get_ioip_info(node_id=b_id)
    log.info("5、数据网断开后业务切换到B节点，将B节点%s业务网%s闪断5次；" % (b_ip, b_eths))
    for i in range(5):
        log.info("Begin down node %s data net %s through %s %d times." % (b_ip, b_eths, bextras, i))
