...
def up_down(pipein):
    time.sleep(10)
    log.info("4、在步骤3中的业务运行过程中，将日志节点A断开一个业务网；")
    log.info("5、业务切到另一个业务网口上，再断开另一个业务网口。")
    for eth in a_eths:
        a_eth = []
        a_eth.append(eth)
...
def vdb_jn(pipeout):
    ...
    log.info("7、主机1上业务完成后，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；")
    ...
    for ip in deploy_ips:
        error.check_host(ip)
    disk.check_bad_obj()
    log.info("8、节点A和节点B恢复后数据修复完成，比较存储内部数据一致性；")
    log.info("Run vdbench with jro.")
    osan.run_vdb(client_ips[0], mix_R_align, output=deploy_ips[0], jn_jro="jro")
