...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将节点A的oPmgr进程异常后再将oStor进程异常；
            5、检查节点A oPmgr，oStor进程
            6、主机1上业务完成后，在主机1上执行vdbench -f mix-S-OLTP.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-33014
# testlink case: 1000-33015
import os
import sys
import commands
import threading
...
def up_down():
    # 进程先后故障
    time.sleep(100)
    log.info("Step4:将该节点的oRole进程杀死,节点: %s" % (fault_node_ip))
    ReliableTest.run_kill_process(fault_node_ip,'oRole')
    time.sleep(10)
    log.info("Step4:将该节点的oStor进程杀死,节点: %s" % (fault_node_ip))
