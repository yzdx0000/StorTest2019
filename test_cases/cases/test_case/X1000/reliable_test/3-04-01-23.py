...
4、日志节点B下电后，另一节点oStor进程挂住；
5、业务执行完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
6、节点B上电，且oStor进程恢复后数据修复完成，比较存储内部数据一致性；
:Changerlog:
"""
# testlink case: 1000-32561
import os,sys
import commands
import threading
import random
...
def up_down(pipein):
    ...
    los_nid_state_c = disk.get_jnl_state()
    while losid not in los_nid_state_c.keys():
        los_nid_state_c = disk.get_jnl_state()
        time.sleep(1)
    c_id = los_nid_state_c[losid][0]
    c_ip = disk.get_node_ip_by_id(n_id=c_id)
    log.info("4、日志节点B下电后，另一节点%s oStor进程kill；" % (c_ip,))
