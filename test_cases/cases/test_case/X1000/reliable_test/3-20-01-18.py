...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将节点A oMgcd进程反复故障；
            5、检查oMgcd进程状态
            6、主机1上业务完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32979
import os
import sys
import commands
import threading
...
def up_down():
    ...
    for count in range(5):
        log.info('第 %d 次杀死oMgcd进程' % (count + 1))
        log.info("Step4:将故障节点oMgcd进程杀死,节点: %s." %(fault_node_ip))
        ReliableTest.run_kill_process(fault_node_ip,'oMgcd')
        time.sleep(100)
        while True:
            log.info("Step5:检查故障节点oMgcd进程状态,节点: %s." % (fault_node_ip))
