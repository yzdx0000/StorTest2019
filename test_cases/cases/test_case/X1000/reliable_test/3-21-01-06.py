...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将节点A/B/C的ostor进程同时异常；
            5、检查各个节点ostor进程
            6、主机1上业务完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-33007
# testlink case: 1000-33008
import os
import sys
import commands
import threading
...
fault_node_ip_C = random.choice(deploy_ls)
log.info("Get fault process:oStor")

def up_down():
    time.sleep(200)
    fault_threads=[]
    fault_threads.append(threading.Thread(target=ReliableTest.run_kill_process(fault_node_ip_A, 'oStor')))
    fault_threads.append(threading.Thread(target=ReliableTest.run_kill_process(fault_node_ip_B, 'oStor')))
    fault_threads.append(threading.Thread(target=ReliableTest.run_kill_process(fault_node_ip_C, 'oStor')))
    log.info("Step4:将三个节点的oStor进程同时杀死,节点: %s,%s,%s." %(fault_node_ip_A,fault_node_ip_B,fault_node_ip_C))
    ...
    time.sleep(300)
    # 检查进程状态
    log.info("Step5:获取故障节点A的oStor进程状态,节点: %s." % (fault_node_ip_A))
    reliable.check_process_stat(fault_node_ip_A, 'oStor')
    log.info("Step5:获取故障节点B的oStor进程状态,节点: %s." % (fault_node_ip_B))
    reliable.check_process_stat(fault_node_ip_B, 'oStor')
    log.info("Step5:获取故障节点C的oStor进程状态,节点: %s." % (fault_node_ip_C))
    reliable.check_process_stat(fault_node_ip_C, 'oStor')
