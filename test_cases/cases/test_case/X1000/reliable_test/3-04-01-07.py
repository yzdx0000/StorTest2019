...
log.info("Got vip is %s" % (fault_vip,))
common.check_ip(fault_vip)
a_ip = osan.get_node_by_vip(fault_vip)
print a_ip
log.info("Got node ip is %s " % (a_ip,))
dips = deploy_ips
dips.remove(a_ip)
alive_ip = random.choice(dips)


def up_down(pipein):
    time.sleep(30)
    log.info("4、在步骤3中的业务运行过程中，将日志节点A %s 掉电；" % (a_ip,))
    log.info("Begin shutdown node %s." % (a_ip,))
    m_a_info = error.down_node(fault_ip=a_ip)
    b_id = error.get_orole_master_id(s_ip=alive_ip)
    b_ip = disk.get_node_ip_by_id(removed_ip=a_ip, n_id=b_id)
    log.info("5、oPmgr报单后，VIP漂移中，%s节点主oRole进程被kill；" % (b_ip,))
    time.sleep(1)
    ReliableTest.run_kill_process(b_ip, 'oRole')
    while True:
