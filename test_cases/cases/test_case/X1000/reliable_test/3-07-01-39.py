...
# 获取日志节点的数据网卡
fault_data_eth_list = error.get_data_eth(jnl_node_id)

def up_down():
    log.info("故障点:将故障节点的所有数据网断开,超时触发数据迁移,后恢复")
    time.sleep(100)
    log.info("Step4:在步骤3中的业务运行过程中,将故障节点的所有数据网断开,节点:%s" % jnl_node_ip)
    log.info("Step5:修改数据网超时时间,触发日志和数据迁移,节点:%s" % jnl_node_ip)
    reliable.run_down_node_wait(jnl_node_ip, '300000')
    time.sleep(315)
    thread_list = []
    ...
    for thread in thread_list:
        thread.join()
    time.sleep(200)
    log.info("Step6:恢复数据网,取消日志和数据迁移,恢复环境,节点:%s" % jnl_node_ip)
    for eth_name in fault_data_eth_list:
        reliable.network_test(jnl_node_ip, eth_name, 'up')
    time.sleep(30)
    log.info("将超时时间修改恢复默认时间,节点:%s" % jnl_node_ip)
    reliable.run_down_node_wait(jnl_node_ip, '86400000')
