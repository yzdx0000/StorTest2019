...
def up_down(pipein):
    ...
    log.info("4、将目的节点oSAN故障；")
    log.info("On node %s kill oSan" % (b_ip,))
    ReliableTest.run_kill_process(b_ip, 'oSan')
    # 监控到匿名管道中写入数据后，退出循环
    while True:
