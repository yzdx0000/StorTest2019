...
def network_error2(arg=1):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    decorator_func.timer(255, 25, log_interval=0)

    target_ip2 = q2.get()
    q2.put(item=target_ip2)
    log.info("获取故障节点的所有业务网")
    fault_node_id1 = env_manage_repair_test.break_down.get_node_id_by_ip(n_ip=target_ip2)
    ...
    for i in fault_eth_list1:
        env_manage_repair_test.Reliable_osan.network_test(s_ip=target_ip2,
                                                          net_name=i,
                                                          net_stat="down")
...
def network_error3(arg=1):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    decorator_func.timer(260, 25, log_interval=0)

    c = []
    while not q2.empty():
        c.append(q2.get_nowait())
    log.info(c)
    for s_ip in env_manage_repair_test.deploy_ips:
        if s_ip not in c:
            global target_ip2
            target_ip2 = s_ip
            break
...
def process_thread(arg=1):
    ...
    target_ip = env_manage_lun_manage.Reliable_osan.get_master_oRole(node_ip2)  # 获取oRale主所在节点IP
    q2.put(item=target_ip)
...
def case():
    iscsi_login()
    decorator_func.multi_threads(run_vdb1, run_vdb2, network_error, network_error2, os_error, process_thread)
    for i in io_eth_list1:
        env_manage_repair_test.Reliable_osan.network_test(s_ip=node_ip1,
                                                          net_name=i,
                                                          net_stat="up")
