...
if len(node_pool_ids) > 1:
    ...
    fault_eth_3, extra_3 = error.get_ioip_info(node_id=node_3)
tag1 = threading.Event()
tag2 = threading.Event()
tag3 = threading.Event()
fault_ip_1 = disk.get_node_ip_by_id(n_id=node_1)
fault_ip_2 = disk.get_node_ip_by_id(n_id=node_2)


def up_down():
    time.sleep(20)
    log.info("Begin to shutdown node: %s." % (fault_ip_1,))
    m_info = error.down_node(fault_ip=fault_ip_1)
    time.sleep(240)
    log.info("Begin to startup node: %s." % (fault_ip_1,))
...
def up_down2():
    time.sleep(60)
    log.info("Begin to shutdown node: %s." % (fault_ip_2,))
    m_info = error.down_node(fault_ip=fault_ip_2)
    time.sleep(240)
    log.info("Begin to startup node: %s." % (fault_ip_2,))
    error.up_node(node_info=m_info)
    time.sleep(300)
    tag2.set()
...
def up_down3():
    ...
    if len(node_pool_ids) > 2:
        log.info("Begin to down io net: %s through: %s." % (fault_eth_3, extra_3[0]))
        ReliableTest.run_down_net(extra_3[0], fault_eth_3)
        time.sleep(240)
        log.info("Begin to up io net: %s through: %s." % (fault_eth_3, extra_3[0]))
        ReliableTest.run_up_net(extra_3[0], fault_eth_3)
    tag3.set()
