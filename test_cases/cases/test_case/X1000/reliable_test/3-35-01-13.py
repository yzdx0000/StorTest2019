...
4、在步骤3中的业务运行过程中，将日志节点A节点业务网出现10%数据包损坏；
...
def up_down():
    ...
    log.info("Step4:业务读写过程中,将日志节点业务网增加10%%的数据包损坏,节点:%s" % fault_node_ip)
    thread_list = []
    for io_eth in io_eth_list:
        cmd = ("ssh %s 'tc qdisc add dev %s root netem corrupt 10%%'" % (fault_node_ip, io_eth))
        log.info(cmd)
    ...
    for io_eth in io_eth_list:
        cmd = ("ssh %s 'tc qdisc del dev %s root netem corrupt 10%%'" % (fault_node_ip, io_eth))
        log.info(cmd)
        rc = os.system(cmd)
        if rc == 0:
