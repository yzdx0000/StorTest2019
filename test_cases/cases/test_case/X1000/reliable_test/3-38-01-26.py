...
def network_error2(arg=1):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    decorator_func.timer(202, 25, log_interval=0)
...
def process_thread(arg=1):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    decorator_func.timer(200, 25, log_interval=0)
    ReliableTest.run_kill_process(node_ip=node_ip3, process="oSan")
    for b in range(5):
        if any([env_manage_lun_manage.Reliable_osan.check_process_stat(node_ip3, "oSan") in ("ok", "OK"),
                env_manage_lun_manage.Reliable_osan.check_process_stat(node_ip3, "oSan") in ("ready", "READY")]):
            break
        time.sleep(20)
    else:
...
def case():
    ...
    iscsi_login()
    decorator_func.multi_threads(run_vdb1, run_vdb2, network_error2, os_error, process_thread)
