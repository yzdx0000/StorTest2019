...
def network_error(arg=1):
    ...
    master_orole_ip = env_manage_lun_manage.Reliable_osan.get_master_oRole(node_ip2)  # 获取oRale主所在节点IP
    for sip in env_manage_repair_test.deploy_ips:
        if sip != master_orole_ip:
            global fault_ip
            fault_ip = sip
            break
    vip_pool_id = env_manage_lun_manage.osan.get_vip_pool_ids(fault_ip)[0]
    fault_vips1 = env_manage_lun_manage.osan.get_vips_by_node_dict(s_ip=fault_ip, vip_pool_id=vip_pool_id)
    log.info(fault_vips1)

    fault_node_id1 = env_manage_repair_test.break_down.get_node_id_by_ip(n_ip=fault_ip)

    io_eth_list1, a_extra1 = error.get_ioip_info(node_id=fault_node_id1)
    log.info(io_eth_list1)
    for i in io_eth_list1:
        env_manage_repair_test.Reliable_osan.network_test(s_ip=fault_ip,
                                                          net_name=i,
                                                          net_stat="down")
    ...
    fault_node_id2 = env_manage_repair_test.break_down.get_node_id_by_ip(n_ip=fault_ip)

    fault_eth_list2 = error.get_data_eth(fault_node_id2)
    log.info(fault_eth_list2)
    env_manage_repair_test.Reliable_osan.net_flash_test(fault_ip, fault_eth_list2)
...
def network_error2(arg=1):
    ...
    io_eth_list2, a_extra1 = error.get_ioip_info(node_id=fault_node_id2)
    log.info(io_eth_list2)

    for i in io_eth_list2:
        env_manage_repair_test.Reliable_osan.network_test(s_ip=target_ip,
                                                          net_name=i,
                                                          net_stat="down")
...
def os_error(arg=3):
    ...
    fault_ip = env_manage_lun_manage.Reliable_osan.get_master_oRole(node_ip2)  # 获取oRale主所在节点IP
    decorator_func.timer(200, 25)

    ReliableTest.run_kill_process(node_ip=fault_ip, process="oRole")
    for b in range(5):
        if any([env_manage_lun_manage.Reliable_osan.check_process_stat(fault_ip, "oRole") in ("ok", "OK"),
                env_manage_lun_manage.Reliable_osan.check_process_stat(fault_ip, "oRole") in ("ready", "READY")]):
            break
        time.sleep(20)
    else:
...
def process_thread(arg=1):
    ...
    target_ip = q.get_nowait()
    for a in range(3):
        ReliableTest.run_kill_process(node_ip=target_ip, process="oSan")
        for b in range(5):
            if any([env_manage_lun_manage.Reliable_osan.check_process_stat(target_ip, "oSan") in ("ok", "OK"),
                    env_manage_lun_manage.Reliable_osan.check_process_stat(target_ip, "oSan") in ("ready", "READY")]):
                break
            time.sleep(20)
        else:
            log.info("service not recover")
