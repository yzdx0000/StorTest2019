...
4、在步骤3中的业务运行过程中，将节点A oPara进程反复故障；
5、检查oPara进程状态
6、主机1上业务完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32971
import os
import sys
import commands
import threading
...
# 选择oSan进程进行异常操作
log.info("Get fault process:oPara")

def up_down():
    time.sleep(100)
    for count in range(5):
        log.info('第 %d 次杀死oPara进程' % (count + 1))
        log.info("Step4:将故障节点oPara进程杀死 : %s." %(fault_node_ip))
        ReliableTest.run_kill_process(fault_node_ip,'oPara')
        time.sleep(30)
        while True:
            log.info("Step5:检查故障节点oPara进程状态 : %s." % (fault_node_ip))
            state = reliable.check_process_stat(fault_node_ip, 'oPara')
            if state == 'OK':
                break
            time.sleep(20)
