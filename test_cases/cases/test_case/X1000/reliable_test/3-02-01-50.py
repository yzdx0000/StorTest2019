...
6、节点D下电超时后上电，启动数据修复，数据修复中，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
7、节点D数据修复完成，日志节点B下电；
8、节点B超时内上电，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
9、数据修复完成后比较存储内部数据一致性.
:Changerlog:
"""
# testlink case: 1000-32519
import os,sys
import commands
import threading
import random
...
def vdb_jn(pipein):
    ...
    log.info("6、节点D下电超时后上电，启动数据修复，数据修复中，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；")
    osan.run_vdb(client_ips[0], mix_S_align, output=deploy_ips[0], jn_jro="jro")
    # 等待节点启动完成
    ...
    while True:
        ...
        if 'up' in line:
            break
    log.info("8、节点B超时内上电，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；")
    osan.run_vdb(client_ips[0], mix_S_align, output=deploy_ips[0], jn_jro="jro")
