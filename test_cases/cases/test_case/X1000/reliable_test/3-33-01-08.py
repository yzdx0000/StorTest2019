...
def up_down(pipein):
    ...
    for diskid in disk_ids:
        Reliable_osan.remove_disk(node_ip=b_ip, disk_id=diskid, disk_usage="SHARED")
    time.sleep(30)

    for diskid in disk_ids:
        Reliable_osan.insert_disk(b_ip, disk_id=diskid, disk_usage="SHARED")
    # 监控到匿名管道中写入数据后，退出循环
    while True:
