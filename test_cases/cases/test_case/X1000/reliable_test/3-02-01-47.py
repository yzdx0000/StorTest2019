...
7、节点E上电，数据修复完成，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
8、比较存储内部数据一致性
:Changerlog:
"""
# testlink case: 1000-32516
import os,sys
import commands
import threading
import random
...
def vdb_jn(pipeout):
    log.info("4、:在主机1上运行vdbench -f mix-R-align.conf -jn.")
    ...
    log.info("6、节点B下电超后，系统启动数据修复，数据修复中，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；")
    osan.run_vdb(client_ips[0], mix_S_align, output=deploy_ips[0], jn_jro="jro")
    # 等待节点启动完成
    ...
    log.info("6、等待系统修复。")
    disk.check_bad_obj()
    log.info("7、节点E上电，数据修复完成，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；")
    osan.run_vdb(client_ips[0], mix_S_align, output=deploy_ips[0], jn_jro="jro")
