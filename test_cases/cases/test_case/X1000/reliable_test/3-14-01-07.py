...
def up_down():
    ...
    time.sleep(100)
    log.info("Step4:在步骤3中的业务运行过程中,将节点A的业务网所有网线断开10min,业务节点:%s" % fault_io_ip)
    thread_ls = []
    ...
    for thread in thread_ls:
        thread.join()
    time.sleep(30)
    log.info("Step5:业务网故障上报后,将节点A的数据网所有网线断开10min,业务节点:%s" % fault_io_ip)
    thread_ls1 = []
    ...
    for thread in thread_ls1:
        thread.join()
    time.sleep(600)
    log.info("Step6:先恢复故障节点业务网,业务节点:%s" % fault_io_ip)
    thread_ls3 = []
    for io_eth_name in io_eth_list:
    ...
    for thread in thread_ls3:
        thread.join()
    time.sleep(30)
    log.info("Step6:再恢复故障节点数据网,业务节点:%s" % fault_io_ip)
    thread_ls2 = []
    for data_eth_name in data_eth_list:
