...
def up_down(pipein):
    zk_master_ip = reliable.get_master_zk(deploy_ips=deploy_ips)
    zk_master_id = disk.get_node_id_by_ip(n_ip=zk_master_ip)
    time.sleep(30)
    log.info("4、在步骤3中的业务运行过程中，将业务节点A %s掉电，" % (a_ip,))
    log.info("Begin shutdown node %s." % (a_ip,))
    m_a_info = error.down_node(fault_ip=a_ip)
    time.sleep(1)
    if int(a_id) == int(zk_master_id):
        pass
    log.info("5、节点A的VIP接管节点在VIP漂移时，%s节点oMgcd进程被kill；" % (zk_master_ip,))
    ReliableTest.run_kill_process(zk_master_ip, 'oMgcd')
    while True:
