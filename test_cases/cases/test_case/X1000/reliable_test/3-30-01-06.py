...
4、在步骤3中的业务运行过程中，将A节点关机，将B节点oJnl线程长时间故障；
...
def up_down(pipein):
    time.sleep(10)
    log.info("4、在步骤3中的业务运行过程中，将A节点关机，将B节点oJnl线程长时间故障；")
    log.info("Begin shutdown node:%s." % (a_ip, ))
    m_info = error.down_node(fault_ip=a_ip)
    for i in range(1,30):
        log.info("On node %s kill oJnl/oPara %d times." % (b_ip, i))
