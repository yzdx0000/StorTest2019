...
:Description:
            ...
            5、在主机1上执行vdbench -f mix-OLTP.conf -jro比较一致性；
            6、节点A和节点B上电恢复后数据修复完成，比较存储内部数据一致性；
:Changerlog:
"""
# testlink case: 1000-32299
import os,sys
import commands
import threading
import random
...
vip = login.login()
#修改vdbench配置文件的参数值
mix_O = osan.gen_vdb_xml(lun=osan.ls_scsi_dev(client_ips[0]),
                               xfersize="(1k,100)",
                               seekpct=45,
                               rdpct=0)
...
def vdb_jn(pipein):
    log.info("Step3:在主机1上运行vdbench -f mix_O.conf -jn.")
    osan.run_vdb(client_ips[0], mix_O, output=deploy_ips[0],jn_jro="jn")
    log.info("Step5:主机1上业务完成后，在主机1上执行vdbench -f mix_O.conf -jro比较一致性.")
