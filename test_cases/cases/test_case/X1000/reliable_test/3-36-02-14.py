...
def up_down(pipe_write):
    ...
    fault_eths, fault_ip = error.get_dataip_info(node_id=fault_node)
    fault_ips[fault_ip[0]] = fault_eths
    fault_node = random.choice(nodes)
    fault_eths, fault_ip = error.get_dataip_info(node_id=fault_node)
    fault_ips[fault_ip[0]] = fault_eths[0].split()
    for fault_ip in fault_ips.keys():
        log.info("Begin shutdown data net: %s on node: %s." % (fault_ips[fault_ip], fault_ip))
        ReliableTest.run_down_net(fault_ip, fault_ips[fault_ip])
    time.sleep(40)
    for fault_ip in fault_ips.keys():
        log.info("Begin up data net: %s on node: %s." % (fault_ips[fault_ip], fault_ip))
