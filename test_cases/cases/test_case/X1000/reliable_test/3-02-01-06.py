...
:Description:
            ...
            6、节点C上电，数据修复中，在主机1上执行vdbench -f OLTP.conf -jro比较一致性；
            7、节点C数据修复完成，在主机1上执行vdbench -f OLTP.conf -jro比较一致性；
            8、比较存储内部数据一致性
:Changerlog:
"""
# testlink case: 1000-32475
import os,sys
import commands
import threading
import random
...
def up_down(pipeout):
    time.sleep(10)
    log.info("Step4:将日志节点C下电 : %s." %(fault_node_ip))
...
def vdb_jn(pipein):
    ...
    osan.run_vdb(client_ips[0], mix_O, output=deploy_ips[0], jn_jro="jro")
    log.info("Step7:等待节点C数据修复")
    while 'HEALTHY' not in node.get_node_state(fault_node_id):
        time.sleep(20)
        log.info("Step7:等待节点C数据修复ing")
    log.info("Step7:节点C数据修复完成，在主机1上执行vdbench -f OLTP.conf -jro比较一致性；")
    osan.run_vdb(client_ips[0], mix_O, output=deploy_ips[0], jn_jro="jro")
