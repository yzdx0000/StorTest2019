...
:Description:
            ...
            6、节点D上电恢复后，日志迁移到节点D过程中时，节点D重启；
            7、节点D上电恢复，数据修复过程中在主机1上执行vdbench -f mix-S-align.conf -jro比较一致性；
            8、节点A上电恢复，数据修复完成后，比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32328
import os, sys
import commands
import threading
import random
...
def up_down(pipeout):
    ...
    log.info("Step6:将节点D重启 : %s." % (fault_node_d_ip))
    tmp_d = error.down_node(fault_ip=fault_node_d_ip)
    tmp_d = re.sub('\r', '', tmp_d)
    # 将日志节点D上电
    time.sleep(40)
