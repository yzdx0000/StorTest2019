...
4、在步骤3中的业务运行过程中，将业务节点网配置成bond，节点A断开两个网线交替故障恢复5次；
...
def up_down():
    ...
    time.sleep(100)
    log.info("Step3:在bond配置完成后,将业务节点 %s 的两个业务网卡交替故障恢复5次." % test_node_ip)
    for count in range(5):
        log.info("第 %d 次交替故障节点 %s 的业务网卡." % ((count + 1), test_node_ip))
        for io_eth in io_eth_list:
            reliable.network_test(test_node_ip, io_eth, 'down')
            time.sleep(30)
            reliable.network_test(test_node_ip, io_eth, 'up')
            time.sleep(30)
    event1.wait()
    event2.wait()
    log.info("Step5:确定业务完成后,检查数据修复完成.")
    disk.check_bad_obj()
    log.info("Step6:将bond配置取消,恢复业务节点 %s 至初始状态." % test_node_ip)
