...
4、在步骤3中的业务运行过程中，将节点A的业务网单网线 断开；
5、等待故障上报后将节点A单个数据网断开；
6、业务完成后，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32871
import os
import sys
import commands
import threading
...
log.info("故障点:一个节点的一个业务网卡和一个数据网卡")

def up_down():
    time.sleep(100)
    log.info("Step4:在步骤3中的业务运行过程中，将节点一个业务网断开,业务节点:%s" % fault_io_ip)
    reliable.network_test(fault_io_ip, io_eth_name, 'down')
    time.sleep(30)
    log.info("Step4:业务网故障上报后,将节点一个数据网断开,业务节点:%s" % fault_io_ip)
    reliable.network_test(fault_io_ip, data_eth_name, 'down')
    time.sleep(100)
