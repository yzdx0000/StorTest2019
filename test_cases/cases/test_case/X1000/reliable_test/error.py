...
def get_ioip_info(node_id=None):
    ...
    for ctl_eth in ctl_eths:
        if ctl_eth in io_eths:
            io_eths.remove(ctl_eth)
    extra_ips = ReliableTest.get_extra_ips(node_ip=ctl_ips[0],node_id=node_id,vip=vip,svip=svip)
...
def get_free_jnl_id():
    ...
    """
    free_jnl_ids = []
    cmd = ('ssh %s "/home/parastor/tools/nWatch -t oRole -i 1 -c oRole#rolemgr_view_dump | grep -v fnc_version"' % (deploy_ips[0]))
    res, output = commands.getstatusoutput(cmd)
    if res != 0:
...
def rel_check_before_run(file_name, jnl_rep=None, data_rep=None, free_jnl_num=None, **kwargs):
    ...
    if jnl_rep != None and cur_jnl_rep < jnl_rep:
        log.error("I found you need jnl replica: %d [cur: %d]isn't euqal with the test request." % (jnl_rep, cur_jnl_rep))
        os._exit(-1)
    #检查数据副本数是否满足测试要求
    elif data_rep != None and cur_data_rep < data_rep:
        log.error("I found you need  data replica: %d [cur: %d]isn't euqal with the test request." % (data_rep, cur_data_rep))
        os._exit(-1)
    elif free_jnl_num != None and cur_jnl_num < free_jnl_num:
        log.error("I found you need free jnl number: %d [cur: %d]isn't euqal with the test request." % (free_jnl_num, cur_jnl_num))
        os._exit(-1)
...
def get_disk_in_vset(disk_type='data'):
    """
    :Author:Diws
    :Date:20190108
    :Description:获取一个vset内数据盘id和节点id
    :param disk_type:meta or data
    :return: dict, disk_info: disk_id:node_id
    """
    disk_info = {}
    cmd = ("/home/parastor/tools/nWatch -i 1 -t oPara -a 'vmgrid=1 fout=0' -c oPara#vmgr_vset_dump | grep 'type:%s'"
           % (disk_type,))
    res, output = osan.run_pscli_cmd(pscli_cmd=cmd)
    vsets = re.findall(r"used_objnr:[1-9]+.*", output)
    vsets = vsets[0]
    if len(vsets) == 0:
        vsets = re.findall('used_objnr:.*', output)[0]
    vsets = re.sub('\[', '', vsets)
    vset_info = vsets.split(']')[1]
    vset_info = re.sub('\)\(|\(|\)', '/', vset_info)
    vset_info = vset_info.split('/')
    for vset in vset_info:
        if vset == ' ' or vset == '':
            continue
        else:
            tmp = vset.split(' ')
            disk_info[tmp[1]] = tmp[0]
    return disk_info