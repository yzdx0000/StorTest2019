...
4、在步骤3中的业务运行过程中，进程数据迁移，迁移过程中oJmgs主进程异常；
5、检查oJmgs主进程状态，检查数据迁移情况
6、主机1上业务完成后，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-33003
# testlink case: 1000-33003
import os
import sys
import commands
...
def up_down():
    ...
    time.sleep(100)
    log.info("Step4:将节点主oJmgs进程杀死,节点: %s." %(m_oJmgs_ip))
    ReliableTest.run_kill_process(m_oJmgs_ip, 'oJmgs')
    log.info("Step4:将节点从oJmgs进程杀死,节点: %s." %(fault_node_ip))
