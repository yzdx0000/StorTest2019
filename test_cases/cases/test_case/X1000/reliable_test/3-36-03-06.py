...
:Date:20190219
:Description:
同一日志组一个节点双业务网反复故障，另一节点单业务网反复故障
"""
import os
import random
import utils_path
...
down_eths_1, extra_ip_1 = error.get_ioip_info(node_id=fault_node_1)
nodes.remove(fault_node_1)
fault_node_2 = random.choice(nodes)
down_eths_2, extra_ip_2 = error.get_ioip_info(node_id=fault_node_2)
...
def up_down2():
    time.sleep(20)
    if fault_node_2 is not None:
        log.info("Begin to down node: %s through:%s." % (down_eths_2[:1], extra_ip_2[0]))
        ReliableTest.run_down_net(extra_ip_2[0], down_eths_2[:1])
        time.sleep(60)
        log.info("Begin to up node: %s through: %s." % (down_eths_2[:1], extra_ip_2[0]))
        ReliableTest.run_up_net(extra_ip_2[0], down_eths_2[:1])
        time.sleep(120)
    tag2.set()
