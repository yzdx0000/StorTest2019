...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将节点A重启，节点B接管节点A的日志；
            5、节点A重启完成，日志迁移到节点A过程中时，节点B重启；
            6、节点B上电恢复，数据修复过程中在主机1上执行vdbench -f mix-R-align.conf -jro比较一致性；
            7、数据修复完成后，比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32321
import os,sys
import commands
import threading
import random
...
def up_down():
    time.sleep(100)
    log.info("Step4:在步骤3中的业务运行过程中，将节点A重启，节点B接管节点A的日志 : %s." %(fault_node_a_ip))
    #将节点A下电
    ...
    time.sleep(60)
    #将日志节点B下电
    log.info("Step5:节点A重启完成，日志迁移到节点A过程中时，节点B重启 : %s." % (fault_node_b_ip))
