...
def up_down():
    ...
    share_disks = disk.get_rw_disk_by_node_id(s_ip=deploy_ips[0], node_id=fault_node_1, disk_type='share')[:1]
    data_disks = disk.get_rw_disk_by_node_id(s_ip=deploy_ips[0], node_id=fault_node_1, disk_type='data')[:2]
    share_disk_phy_ids = disk.get_disk_phyid_by_name(s_ip=fault_ip_1, disk_name=share_disks)
    data_disk_phy_ids = disk.get_disk_phyid_by_name(s_ip=fault_ip_1, disk_name=data_disks)
    log.info('Begin to remove share disk: %s and data disk: %s on %s.' % (share_disks, data_disks, fault_ip_1))
    ...
    time.sleep(500)
    log.info('Begin to add share disk: %s and data disk: %s on %s.' % (share_disks, data_disks, fault_ip_1))
    for did in share_disk_phy_ids:
...
def up_down2():
    time.sleep(20)
    if len(jnl_grps) > 1:
        fault_node_1 = random.choice(nodes_2)
        fault_ip_1 = disk.get_node_ip_by_id(n_id=fault_node_1)
        share_disks = disk.get_rw_disk_by_node_id(s_ip=deploy_ips[0], node_id=fault_node_1, disk_type='share')[:1]
        data_disks = disk.get_rw_disk_by_node_id(s_ip=deploy_ips[0], node_id=fault_node_1, disk_type='data')[:2]
        share_disk_phy_ids = disk.get_disk_phyid_by_name(s_ip=fault_ip_1, disk_name=share_disks)
