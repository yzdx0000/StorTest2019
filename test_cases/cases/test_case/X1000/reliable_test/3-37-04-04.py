...
def process_thread(arg=1):
    ...
    for a in range(3):
        ...
        ReliableTest.run_kill_process(node_ip=target_ip2, process="oSan")
        for b in range(5):
            if any([env_manage_lun_manage.Reliable_osan.check_process_stat(target_ip2, "oSan") in ("ok", "OK"),
                    env_manage_lun_manage.Reliable_osan.check_process_stat(target_ip2, "oSan") in ("ready", "READY")]):
                break
            time.sleep(20)
        else:
            log.info("service not recover")
