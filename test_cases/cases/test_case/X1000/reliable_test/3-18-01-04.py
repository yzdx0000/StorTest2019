...
4、在步骤3中的业务运行过程中，将节点D所有网络断开；
5、主机1上业务完成后，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
6、10分钟后恢复节点D网络，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
7、比较存储内部数据一致性。"
:Changerlog:
"""
# testlink case: 1000-32953
import os
import sys
import commands
import threading
...
def up_down():
    log.info("故障点:将一个非日志节点的所有数据网故障,10min后恢复数据网络,节点:%s" % non_jnl_ip)
    time.sleep(100)
    log.info("Step4:业务过程中,将非日志节点的所有数据网络同时断开,节点:%s" % non_jnl_ip)
    thread_list = []
    ...
    for thread in thread_list:
        thread.join()
    time.sleep(600)
    log.info("Step5:10min后,将故障节点的所有数据网恢复,节点:%s" % non_jnl_ip)

    for eth_name in fault_eth_list:
        thread_list.append(threading.Thread(target=reliable.network_test, args=(non_jnl_ip, eth_name, 'up')))
    for thread in thread_list:
        thread.start()
    for thread in thread_list:
        thread.join()
