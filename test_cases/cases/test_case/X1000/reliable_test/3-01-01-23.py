...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将节点A掉电和原日志组节点B同时重启；
            5、在主机1上执行vdbench -f OLTP.conf -jro比较一致性；
            6、节点B重启完成后，检查日志重建和日志迁移情况；
            7、日志重建和迁移完成后在主机1上执行vdbench -f OLTP.conf -jro比较一致性。
:Changerlog:
"""
# testlink case: 1000-32307
import os,sys
import commands
import threading
import random
...
def up_down():
    ...
    for ip in fault_node_ip:
        log.info("Step4:将节点A掉电和原日志组节点B同时重启 : %s." %(ip))
...
def vdb_jn():
    ...
    log.info("Step7:日志重建和迁移完成后在主机1上执行vdbench -f OLTP.conf -jro比较一致性。.")
    osan.run_vdb(client_ips[0], mix_O, output=deploy_ips[0], jn_jro="jro")
