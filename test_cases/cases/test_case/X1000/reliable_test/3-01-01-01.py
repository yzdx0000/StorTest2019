...
:Description:
        ...
        6、节点A超时内上电，数据修复中，在主机1上执行vdbench -f mix-s-align.conf -jro比较一致性；
        7、比较存储内部数据一致性。
:Changelog:
"""
# testlink case: 1000-32285
import os
import utils_path
import common2
import log
...
mix_S_align = osan.gen_vdb_xml(lun=lun1, xfersize=xfersize1, seekpct=seekpct, rdpct=rdpct)
...
def up_down(pipeout):
    ...
    res = error.down_node(fault_ip=fault_ip)
    os.write(pipeout, res)
def vdb_jn(pipein):
