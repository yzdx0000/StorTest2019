...
def up_down(pipein):
    time.sleep(30)
    log.info("4、在步骤3中的业务运行过程中，将节点A %s业务网%s断开；" % (a_ip, a_eths))
    log.info("Begin down node %s io net %s through %s." % (a_ip, a_eths, aextras))
    ReliableTest.run_down_net(aextras[0], a_eths)
    log.info("4、在VIP漂移时将%s主oPmgr重启；" % (master_orole_ip,))
    log.info("Begin shutdown node %s." % (master_orole_ip,))
    m_b_info = error.down_node(fault_ip=master_orole_ip)
    while True:
        line = os.read(pipein, 32)
        if 'up' in line:
            break
    log.info("Begin startup node %s." % (master_orole_ip,))
    error.up_node(node_info=m_b_info)
    if int(a_id) != int(master_orole_id):
