...
def up_down():
    ...
    log.info("Step5:在故障异常上报后，将B日志节点数据网络故障,节点:%s" % (jnl_node_ip))
    for data_eth_name in fault_eth_list_B:
        reliable.network_test(jnl_node_ip, data_eth_name, 'down')

    time.sleep(100)
    log.info("Step6:将故障节点数据网恢复,节点:%s,%s" % (fault_io_ip_A, jnl_node_ip))
    for data_eth_name in fault_eth_list_A:
