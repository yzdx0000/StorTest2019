...
5、日志数据修复到B节点，修复完成后，添加新节点D，日志迁移到D节点，迁移时断开节点B数据网；
6、系统修复，修复完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
7、比较存储内部数据一致性。"

:Changerlog:
"""
# testlink case: 1000-32861
import os
import sys
import commands
import threading
...
def up_down():
    ...
    log.info("Step5:B节点接管A节点的日志业务后,将B节点的数据网故障,日志节点B:%s" % los_node_ip_B)
    ...
    time.sleep(60)
    error.check_host(los_node_ip_A)
