...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将节点A同组的日志节点B业务网故障（节点B无业务）；
            5、节点A重启，
            6、在主机1上执行vdbench -f mix-R-align.conf -jro比较一致性；
            7、节点B上电后进行数据修复中，在主机1上执行vdbench -f mix-R-align.conf -jro比较一致性；
            8、节点B数据修复完成，在主机1上执行vdbench -f mix-R-align.conf -jro比较一致性；
            9、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32302
import os
import sys
import commands
import threading
