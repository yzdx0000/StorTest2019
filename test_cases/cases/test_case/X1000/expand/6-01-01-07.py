...
4、在步骤3中的业务运行过程中，向节点池中加入3个新节点，日志盘配置成共享盘，数据盘配置成数据盘，将数据盘加入到存储池中，将节点加入到访问区，存储节点启动均衡；
5、业务执行完成后，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
6、数据均衡完成后，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
7、比较存储内部数据一致性；
8、检查均衡后硬盘和节点容量，检查新加入的节点成功加入到访问区
"""
import os
import time
import threading
...
def add_node(pipe_write):
    ...
    for key in node_pool_info.keys():
        ...
        for s_ip in deploy_ips:
            if ReliableTest.check_ping(s_ip):
                lun_osan.update_node_pool(s_ip, node_ids, node_pool_id, node_pool_name)
                break
        break
    ...
    for s_ip in deploy_ips:
        if ReliableTest.check_ping(s_ip):
            ...
            break
    c_cap = error.get_cluster_cap()
    n_cap = error.get_node_cap(n_id=node_id)
    log.info("The capacity of the node you insert is %d." % (n_cap,))
...
def vdb_jn(pipe_read):
    ...
    osan.run_vdb(client_ips[0], mix_1, output=deploy_ips[0], jn_jro="jro")
    log.info("7、比较存储内部数据一致性.")
    disk.multi_check_part_lun_uniform_by_ip()
    log.info("8、检查均衡后硬盘和节点容量，比较存储内部数据一致性.")
