...
4、将新节点加入到集群中，将磁盘加入到存储池中，触发数据均衡；
5、数据均衡过程中原节点oPara进程异常；
...
def down_net(nodeid):
    ...
    :return: None
    """
    fault_ip = disk.get_node_ip_by_id(n_id=nodeid)
    log.info("5、数据均衡过程中扩容节点oPara进程异常.")
    log.info("Begin kill pro : oPara on %s." % (fault_ip,))
    ReliableTest.run_kill_process(fault_ip, 'oPara')
