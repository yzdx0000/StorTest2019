...
4、向集群中加入一新节点，将新节点加入到集群过程中，将新节点上的数据盘拔出
5、在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
6、数据修复完成后，比较存储内部数据一致性；
"""
import os
import time
import threading
...
def remove_disk():
    ...
    while True:
        ...
        if 'err' in line:
            break
    time.sleep(1.5)
    ssd_disks, data_disks = error.get_node_disks(dst_ip=dst_ip)
    fault_disk = random.choice(data_disks)
    fault_disk_name = []
    fault_disk_name.append(fault_disk)
    fault_disk_disk_phy_id = disk.get_disk_phyid_by_name(s_ip=dst_ip, disk_name=fault_disk_name)
    log.info("Begin remove disk: %s %s on %s." % (fault_disk, fault_disk_disk_phy_id, dst_ip))
    ReliableTest.remove_disk(dst_ip, fault_disk_disk_phy_id, 'data')
    time.sleep(60)
    log.info("Begin add disk: %s %s on %s." % (fault_disk, fault_disk_disk_phy_id, dst_ip))
    ReliableTest.insert_disk(dst_ip, fault_disk_disk_phy_id, 'data')
