...
4、向集群中加入新节点，并将硬盘加入存储池，启动数据均衡，扩容节点A过程中，将均衡源节点oSan进程杀掉
...
def down_net():
    ...
    :return: None
    """
    fault_ips = list(set(deploy_ips).difference(set(expand_ips)))
    read_pipe = os.open('/tmp/err_fifo', os.O_RDONLY | os.O_TRUNC)
    while True:
        line = os.read(read_pipe, 20)
        if len(line) != 0:
            sip = line
            break
    fault_ip = random.choice(fault_ips.remove(sip))
    log.info("4、扩容节点A过程中，将均衡源节点oSan进程杀掉.")
    log.info("Begin kill pro : oSan on %s." % (fault_ip,))
