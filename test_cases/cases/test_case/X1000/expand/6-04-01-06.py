...
def remove_disk(nodeid):
    ...
    s_node_ip = disk.get_node_ip_by_id(n_id=s_node_id)
    ssd_disks, data_disks = ReliableTest.get_share_monopoly_disk_ids(nodeid)
    s_ssd_disk, s_data_disks = ReliableTest.get_share_monopoly_disk_ids(s_node_id)
    fault_disk = random.choice(data_disks)
    s_fault_disk = random.choice(s_data_disks)
    fault_disk_name = []
    s_fault_disk_name = []
    fault_disk_name.append(fault_disk)
    s_fault_disk_name.append(s_fault_disk)
    ...
    s_fault_disk_disk_phy_id = disk.get_disk_phyid_by_name(s_ip=s_node_ip, disk_name=s_fault_disk_name)
    log.info("Begin remove disk: %s %s on %s." % (s_fault_disk, s_fault_disk_disk_phy_id, s_node_ip))
    ReliableTest.remove_disk(s_node_ip, s_fault_disk_disk_phy_id, 'data')
    time.sleep(60)
    ...
    log.info("Begin add disk: %s %s on %s." % (s_fault_disk, s_fault_disk_disk_phy_id, s_node_ip))
    ReliableTest.insert_disk(s_node_ip, s_fault_disk_disk_phy_id, 'data')
