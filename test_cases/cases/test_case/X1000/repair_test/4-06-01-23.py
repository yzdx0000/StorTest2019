...
4、业务过程中将存储池磁盘拔出一块；
5、将拔出的盘在超时内恢复后，数据修复过程中再次将该盘拔出
6、主机端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性
7、将故障的硬盘恢复，数据修复完成系统恢复正常后，比较内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-33300
import os
import sys
import commands
import threading
...
lun_ids = osan.get_lun(fault_node_ip)
lun_ids_half = lun_ids[len(lun_ids)/2:]
disk_ids = []
...
def up_down():
    time.sleep(100)
    log.info("Step4:业务过程中将存储中的一块磁盘拔出,节点:%s,磁盘:%s" % (fault_node_ip,disk_name))
    env_manage_repair_test.Reliable_osan.remove_disk(fault_node_ip, disk_phy_id, "DATA")
    time.sleep(100)
    log.info("Step5:将拔出的盘在超时内恢复,节点:%s,磁盘:%s" % (fault_node_ip,disk_name))
    env_manage_repair_test.Reliable_osan.insert_disk(fault_node_ip, disk_phy_id, "DATA")
    time.sleep(100)
    if 0 != env_manage_repair_test.Reliable_osan.check_rcvredobj(fault_node_ip):
        log.info("Step5:数据修复过程中,将映射到主机的Lun全部删掉")
        for lun_id in lun_ids_half:
            osan.delete_lun(fault_node_ip,lun_id)
...
def vdb_jn():
    ...
    osan.run_vdb(client_ips[0], mix_R_Align, output=deploy_ips[0],jn_jro="jn",time=1600)
    log.info("Step6:主机1上业务完成后，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性.")
    decorator_func.timer(120)
