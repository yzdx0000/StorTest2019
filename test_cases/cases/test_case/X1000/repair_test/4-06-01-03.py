...
def os_reboot(arg=3):
    ...
    # env_manage_repair_test.Reliable_osan.run_pause_process("oStor", node_ip1)  # 将源节点oStor进程故障
    ReliableTest.run_kill_process(node_ip1, "oStor")
...
def case():
    ...
    iscsi_login()
    log.info(
        "step2:在主机1上运行vdbench -f mix-R-Align.conf -jn；在主机2上运行vdbench -f mix-R.conf,业务过程中，业务过程中将一节点重启 ...")
    decorator_func.multi_threads(run_vdb1, run_vdb2, os_reboot)
    log.info("step3:节点重启恢复后，数据修复过程中将修复源节点oStor进程故障,主机端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性 ...")
