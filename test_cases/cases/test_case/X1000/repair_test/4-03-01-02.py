...
3、在主机1上运行vdbench -f min-seq-w.conf -jn；
4、业务完成拔掉一块数据盘；
5、等待超时后，触发被动重建，手动删除故障盘后插上一块容量相同的新盘；
6、将新盘加入到存储池
7、数据重建完成后，比较内部数据一致性
检查项:
...
def disk_error(arg=0):
    ...
    log.info(disk_ids)
    env_manage_repair_test.Reliable_osan.remove_disk(node_ip=fault_ip, disk_id=disk_phy_id, disk_usage="DATA")
...
def case():
    ...
    log.info("手动删除故障盘后插上一块容量相同的新盘,将新盘加入到存储池 ...")
    ...
    env_manage_repair_test.Reliable_osan.insert_disk(node_ip=fault_ip, disk_id=disk_phy_id, disk_usage="DATA")
    decorator_func.timer(10)
    env_manage_repair_test.Reliable_osan.add_disks(s_ip=fault_ip, node_ids=node_id, uuid=disk_uuid, usage="DATA",
