...
def case():
    ...
    iscsi_login()
    log.info(
        "在主机1上运行vdbench -f mix-R-Align.conf -jn；在主机2上运行vdbench -f mix-R.conf ...")
    decorator_func.multi_threads(run_vdb1, run_vdb2, disk_error)
    log.info("在业务压力过程中手动删除一块数据盘触发报单时，同时将oSan进程故障,主机端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性 ...")
    env_manage_repair_test.co2_osan.run_vdb(client_ip=client_ip1, vdb_xml=mix_R_Align, jn_jro='jro',
                                            output=node_ip1)
    log.info("将故障的数据盘插回，oSan恢复正常，数据修复完成系统恢复正常后，比较内部数据一致性 ...")
    ...
    if all([0 == env_manage_repair_test.Reliable_osan.check_rcvredobj(
            node_ip1), env_manage_repair_test.Reliable_osan.check_process_stat(node_ip1, 'oSan') == "OK"]):
