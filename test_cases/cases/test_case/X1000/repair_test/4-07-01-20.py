...
def case():
    ...
    iscsi_login()
    log.info(
        "step2:在主机1上运行vdbench -f mix-R-Align.conf -jn；在主机2上运行vdbench -f mix-R.conf,业务过程中，业务过程中手动删除2块数据盘 ...")
    decorator_func.multi_threads( run_vdb1, run_vdb2, disk_error, os_error)
    log.info("step3:数据重建时，重建源节点掉电,主机端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性 ...")
