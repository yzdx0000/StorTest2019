...
def os_reboot(arg=3):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    time.sleep(int(cp("wait_time", "remove_disk"))+20)
...
def case():
    ...
    log.info("step3:数据重建时，重建源节点重启,主机端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性 ...")
