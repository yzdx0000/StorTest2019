...
4、业务过程中，将节点拔出两块元数据盘；
5、拔出数据盘，在系统没有更新状态信息是，将ZK主进程故障；
6、将拔出的磁盘插回，数据修复完成恢复正常后，主机端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性
:Changerlog:
"""
import os
import sys
...
disk_ids = []
disk_ids_data = []
for sh_disk in share_disk:
    disk_phy_id = env_manage_repair_test.Reliable_osan.get_physicalid_by_name(fault_node_ip, sh_disk)
    disk_ids.append(disk_phy_id)
for da_disk in data_disk:
    disk_phy_id = env_manage_repair_test.Reliable_osan.get_physicalid_by_name(fault_node_ip, da_disk)
    disk_ids_data.append(disk_phy_id)
disk_phy_id_data = random.choice(disk_ids_data)  #磁盘物理id
disk_phy_id = random.choice(disk_ids)            #磁盘物理id
disk_ids.remove(disk_phy_id)
disk_phy_id1 = random.choice(disk_ids)           #磁盘物理id
...
disk_name_data = env_manage_repair_test.Reliable_osan.get_name_by_physicalid(fault_node_ip, disk_phy_id_data)  # 磁盘名字

disk_uuid = env_manage_repair_test.com_disk.get_disk_uuid_by_name(fault_node_id, disk_name)  # 磁盘uuid
disk_uuid1 = env_manage_repair_test.com_disk.get_disk_uuid_by_name(fault_node_id, disk_name)  # 磁盘uuid
disk_uuid_data = env_manage_repair_test.com_disk.get_disk_uuid_by_name(fault_node_id, disk_name_data)  # 磁盘uuid
...
disk_id_data = env_manage_repair_test.Reliable_osan.get_diskid_by_name(fault_node_ip, fault_node_id, disk_name_data) #集群中的磁盘id

stor_id_block = env_manage_repair_test.Lun_osan.get_storage__type_id(fault_node_ip)

def up_down():
    time.sleep(100)
    log.info("Step4:业务过程中将存储中的两块共享磁盘拔出,节点:%s,磁盘:%s,%s" % (fault_node_ip, disk_name ,disk_name1))
    env_manage_repair_test.Reliable_osan.remove_disk(fault_node_ip, disk_phy_id, "SHARED")
    env_manage_repair_test.Reliable_osan.remove_disk(fault_node_ip, disk_phy_id1, "SHARED")
    time.sleep(100)
    log.info("Step5:数据修复过程中,拔出一块数据盘,节点:%s,磁盘:%s" % (fault_node_ip, disk_name_data))
    env_manage_repair_test.Reliable_osan.remove_disk(fault_node_ip, disk_phy_id_data, "DATA")
    m_zk_ip = reliable.get_master_zk(deploy_ls)
    log.info("Step5:拔出数据盘后迅速故障主zk进程,节点;%s" % (m_zk_ip))
    ReliableTest.run_kill_process(m_zk_ip, 'zk')
    time.sleep(100)
    while True:
    ...
    log.info("Step5:将拔出的盘在超时内恢复,节点:%s,磁盘:%s,%s" % (fault_node_ip, disk_name, disk_name1))
    env_manage_repair_test.Reliable_osan.insert_disk(fault_node_ip, disk_phy_id, "SHARED")
    env_manage_repair_test.Reliable_osan.insert_disk(fault_node_ip, disk_phy_id1, "SHARED")
    env_manage_repair_test.Reliable_osan.insert_disk(fault_node_ip, disk_phy_id_data, "DATA")
