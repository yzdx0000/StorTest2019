...
# Date  :2018-8-7
"""
测试内容:副本下，一块数据盘故障超时后，手动删除该盘

步骤:
1、在节点A创建lun1-lun6
2、将逻辑卷映射至主机1上lun1-lun6
3、在主机1上运行vdbench -f min-seq-write.conf -jn；
4、业务完成前，将一个节点一块数据盘拔出，等待超时；
5、节点超时后将该数据盘手动删除，在主机1上运行vdbench -f min-seq-write.conf -jro；
...
def case():
    ...
    iscsi_login()
    log.info("step2:在主机1上运行vdbench -f min-seq-w-1l.conf -jn,业务完成前，将一个节点一块数据盘拔出，等待超时 ...")
    decorator_func.multi_threads(run_vdb, disk_error)
