# -*- coding:utf-8 _*-
"""
测试内容:数据修改写时，单节点数据网故障，超时内恢复

步骤:
1、在节点A创建lun1-lun6
2、将逻辑卷映射至主机1上lun1-lun6
3、在主机1上运行vdbench -f min-seq-write.conf -jn；
4、在步骤3业务进行过程中，将一个数据节点数据网断开；
5、超时内故障节点网络恢复，立即触发数据修复；
...
node_id = env_manage_repair_test.break_down.get_node_id_by_ip(n_ip=node_ip1)
...
def case():
    ...
    time.sleep(int(cp('timeout', 'node_timeout'))/int(cp('timeout', 'unit_time')) + 30)  # 设置等待超时的时间
    log.info(
    ...
    while True:
        time.sleep(30)
        if env_manage_repair_test.Reliable_osan.get_node_state(node_ip2) == "NODE_STATE_ZOMBIE":
            break
    for i in fault_eth_list:
        env_manage_repair_test.Reliable_osan.network_test(s_ip=node_ip2,
                                                          net_name=i,
