...
fault_node_id = env_manage_repair_test.break_down.get_node_id_by_ip(n_ip=node_ip1)
...
def case():
    ...
    for i in fault_eth_list:
        env_manage_repair_test.Reliable_osan.network_test(s_ip=node_ip1,
                                                          net_name=i,
                                                          net_stat="up")
    log.info("step3:插回被拔出的磁盘触发修复,修复过程中将目标节点数据网断开,主机端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性 ...")
