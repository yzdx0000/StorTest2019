...
4、业务过程中手动删除一块数据盘后立即杀掉主oPara进程
5、数据重建，主机端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性
6、数据重建完成后，比较内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-33329
import os
import sys
import commands
import threading
...
def up_down():
    time.sleep(150)
    log.info("Step5:重建过程将oPara进程故障,节点:%s" % (fault_node_ip))
    ReliableTest.run_kill_process(fault_node_ip,'oPara')
    time.sleep(30)
    log.info("Step6:检查oPara进程是否被正常拉起,节点:%s" %(fault_node_ip))
