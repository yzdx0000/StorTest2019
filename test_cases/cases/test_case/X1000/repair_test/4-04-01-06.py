...
4、业务完成前，将一个存储节点删除；
5、删除后触发主动重建；
6、重建完成后，主机端执行vdbench -f min-seq-write.conf -jro校验数据一致性。

检查项:
1、步骤4、节点池中移除节点，系统立即开始启动重建；
2、步骤5、管理端可查看到重建状态
3、步骤6、比较数据一致
...
def node_error(arg=1):
    ...
    env_manage_repair_test.Lun_osan.remove_node(node_ip1, 3)
...
def case():
    ...
    decorator_func.multi_threads(node_error, run_vdb)

    log.info("step3:主机端执行vdbench -f min-seq-write.conf -jro校验数据一致性 ...")
...
def main():
    ...
    # env_manage_repair_test.Lun_osan.add_nodes(node_ip1,)  # 还原本用例的环境
    #
