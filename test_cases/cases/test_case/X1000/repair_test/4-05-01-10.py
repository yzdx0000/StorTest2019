...
# 随机选择一个节点的数据盘
fault_node_ip = reliable.get_master_zk(deploy_ips)
...
disk_phy_id1 = random.choice(disk_ids1)
...
def up_down():
    ...
    log.info("Step5:数据修复过程中,拔出一块数据盘,节点:%s,磁盘:%s" % (fault_node_ip, disk_name1))
    ...
    time.sleep(100)
    log.info("Step7:将拔出的盘在超时内恢复,节点:%s,磁盘:%s,%s" % (fault_node_ip, disk_name ,disk_name1))
