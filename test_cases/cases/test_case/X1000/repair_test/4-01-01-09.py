...
def disk_error(arg=0):
    ...
    for i in range(1, 4):
        ...
        env_manage_repair_test.Reliable_osan.insert_disk(node_ip=node_ip1, disk_id=disk_ids[1], disk_usage="DATA")
        time.sleep(int(cp("wait_time", "interval_disk")))
    env_manage_repair_test.Reliable_osan.remove_disk(node_ip=node_ip1, disk_id=disk_ids[0], disk_usage="DATA")
    env_manage_repair_test.Reliable_osan.remove_disk(node_ip=node_ip1, disk_id=disk_ids[1], disk_usage="DATA")
...
def case():
    ...
    log.info("插回故障盘开始修复，修复过程中执行主机端执行vdbench -f min-seq-w-1l.conf -jro校验数据一致性...")
    log.info(disk_ids)
    env_manage_repair_test.Reliable_osan.insert_disk(node_ip=node_ip1, disk_id=disk_ids[0], disk_usage="DATA")
    env_manage_repair_test.Reliable_osan.insert_disk(node_ip=node_ip1, disk_id=disk_ids[1], disk_usage="DATA")
