...
def os_error(arg=0):
    ...
    for i in range(1, 11):
        time.sleep(15)
        ReliableTest.run_kill_process(node_ip1, 'oPara')  # 将oPara进程故障
...
def case():
    ...
    iscsi_login()
    log.info(
        "step2:在主机1上运行vdbench -f mix-R-Align.conf -jn；在主机2上运行vdbench -f mix-R.conf,业务过程中将存一节点掉电 ...")
    decorator_func.multi_threads(run_vdb1, run_vdb2, os_error)
    log.info("step3:将掉电节点上电恢复恢复后，数据修复过程中将oPara进程故障,主机端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性 ...")
