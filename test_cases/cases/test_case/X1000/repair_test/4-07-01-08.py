...
def kill_process(arg=4):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    decorator_func.timer(int(cp("wait_time", "remove_disk")) + 2)
...
def case():
    ...
    iscsi_login()
    log.info(
        "step2:在主机1上运行vdbench -f mix-R-Align.conf -jn；在主机2上运行vdbench -f mix-R.conf,业务过程中手动删除一块数据盘 ...")
    decorator_func.multi_threads(run_vdb1, run_vdb2, disk_error, kill_process)
    log.info("step3:数据重建时，重建目标盘故障,主机端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性 ...")
    ...
    env_manage_repair_test.Reliable_osan.check_disk_state(node_ip=node_ip1,
                                                          disk_uuid=disk_uuid,
                                                          disk_state="DISK_STATE_HEALTHY")  # 检测磁盘状态，若为HEALTHY，认为主动重建完成
