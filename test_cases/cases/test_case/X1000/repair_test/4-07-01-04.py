...
def up_down():
    time.sleep(120)
    log.info("Step5:重建过程将该另一块数据盘拔出,节点:%s,磁盘:%s" % (fault_node_ip,disk_name1))
    env_manage_repair_test.Reliable_osan.remove_disk(fault_node_ip, disk_phy_id1, "DATA")
    time.sleep(200)
    log.info("Step7:将拔出的数据盘插回,节点:%s,磁盘:%s" % (fault_node_ip,disk_name1))
    env_manage_repair_test.Reliable_osan.insert_disk(fault_node_ip, disk_phy_id1, "DATA")
    log.info("Step7:将删除的数据盘恢复,节点:%s,磁盘:%s" % (fault_node_ip, disk_name))
    while True:
