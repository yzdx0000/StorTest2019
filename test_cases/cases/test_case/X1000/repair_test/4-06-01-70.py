...
# Date  :2018-8-8
"""
测试内容:元数据修复过程中，修复非源和目的节点oStor进程故障
步骤:
...
def disk_error(arg=0):
    ...
    ReliableTest.run_kill_process(node_ip2, 'oStor')  # 将oStor进程故障
...
def case():
    ...
    iscsi_login()
    log.info(
        "step2:在主机1上运行vdbench -f mix-R-Align.conf -jn；在主机2上运行vdbench -f mix-R.conf,业务过程中,业务过程中将存储池元数据磁盘拔出一块 ...")
    decorator_func.multi_threads(run_vdb1, run_vdb2, disk_error)
    log.info("step3:将拔出的盘在超时内恢复，数据修复过程中将非修复源和目标节点oStor进程故障,主机端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性 ...")
