...
4、在业务压力过程中手动删除一个节点时，同时将oSan进程故障；
6、主机端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性
7、将故障的节点恢复，oStan进程恢复正常，数据修复完成系统恢复正常后，比较内部数据一致性。
:Changerlog:
"""
import os
import sys
...
def disk_fault():
    time.sleep(100)
    log.info("Step4:业务过程中主动将一个节点,节点:%s" % fault_node_ip)
    reliable.del_node(fault_node_ip, fault_node_id)

def up_down():
    time.sleep(120)
    log.info("Step5:删除节点过程中同时将oSan进程故障,节点:%s" % (fault_node_ip))
    ReliableTest.run_kill_process(fault_node_ip, 'oSan')
    log.info("Step6:检查oSan进程是否被正常拉起,节点:%s" %(fault_node_ip))
