...
def up_down():
    ...
    time.sleep(100)
    log.info("Step5:数据修复过程中,将集群一个节点的oStor进程故障，节点:%s" % (fault_node_ip1))
    ReliableTest.run_kill_process(fault_node_ip1, 'oStor')
    time.sleep(100)
    log.info("Step6:检查目标节点的oStor进程状态,节点:%s" % (fault_node_ip1))
    reliable.check_process_stat(fault_node_ip1, 'oStor')
