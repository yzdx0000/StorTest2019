...
def network_error(arg=1):
    ...
    env_manage_repair_test.Reliable_osan.network_test(s_ip=failnode_ip,
                                                      net_name=cp("create_subnet", "network_interface"),
                                                      net_stat="down")
...
def os_error(arg=3):
    ...
    vm_id = env_manage_repair_test.down_node(node_ip1, type_info)
...
def case():
    ...
    env_manage_repair_test.co2_osan.run_vdb(client_ip=client_ip1, vdb_xml=min_seq_w, jn_jro='jn', output=node_ip1)
    log.info("在步骤3业务进行过程中，将lun写满后，再重新执行步骤3业务中将一个数据节点上,将一个数据节点掉电")
    decorator_func.multi_threads(run_vdb_w, os_error)
    log.info("立即触发数据修复，超时内恢复 ...")
