...
def os_error(arg=3):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    time.sleep(int(cp("wait_time", "remove_disk")) + 50)  # 执行vdbench期间，先等待时间再开始故障的操作
    env_manage_repair_test.down_node(node_ip1, type_info)
...
def case():
    ...
    log.info("step3:修复过程中将修复目标节点掉电,主机端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性 ...")
