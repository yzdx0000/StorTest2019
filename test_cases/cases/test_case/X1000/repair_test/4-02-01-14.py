...
4、将一个节点所有元数据盘反复插拔10次间隔5分钟
5、插回拔出的共享盘，数据开始修复
6、主机端执行vdbench -f min-seq-w-1l.conf -jro校验数据一致性
7、修复完成后，比较内部数据一致性
...
def disk_error(arg=1):
    ...
    for i in range(0, 10):
        for l in L:
            env_manage_repair_test.Reliable_osan.remove_disk(node_ip=node_ip1, disk_id=l.split(",")[0],
                                                             disk_usage="SHARED")
        time.sleep(30)
        for l in L:
            env_manage_repair_test.Reliable_osan.insert_disk(node_ip=node_ip1, disk_id=l.split(",")[0],
                                                             disk_usage="SHARED")
        time.sleep(30)
...
def case():
    ...
    iscsi_login()
    log.info("step2:在主机1上运行vdbench -f min-seq-w-1l.conf -jn,将一个节点所有元数据盘反复插拔10次间隔5分钟 ...")
