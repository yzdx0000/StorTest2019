...
4、手动删除二块数据盘；
5、数据重建过程中将oJob节点掉电
6、在主机1上运行vdbench -f mix-R-Align.conf -jro;
7、节点上电 恢复，数据修复完成系统恢复正常后，比较内部数据一致性。
...
def process_error(arg=1):
    ...
    ReliableTest.run_kill_process(node_ip1, 'oJob')  # 将oJob进程故障
...
def os_error(arg=3):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    time.sleep(int(cp("wait_time", "remove_disk")) + 5)
...
def case():
    ...
    iscsi_login()
    log.info(
        "step2:在主机1上运行vdbench -f mix-R-Align.conf -jn；在主机2上运行vdbench -f mix-R.conf,业务过程中，手动删除二块数据盘...")
    decorator_func.multi_threads(run_vdb1, run_vdb2, disk_error, os_error)
    log.info("step3:数据重建过程中将oJob节点掉电,主机端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性 ...")

    env_manage_repair_test.co2_osan.run_vdb(client_ip=client_ip1, vdb_xml=mix_R_Align, jn_jro='jro',
                                            output=node_ip1)
    log.info("节点上电 恢复，数据修复完成系统恢复正常后，比较内部数据一致性 ...")
