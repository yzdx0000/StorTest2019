# -*- coding:utf-8 _*-
"""
测试内容:元数据盘分组中（元数据副本数-1）个元数据盘依次故障，部分超时内恢复

步骤:
1、在节点A创建LUN1
2、将逻辑卷映射至主机1上LUN1
3、在主机1上运行vdbench -f min-seq-w-1l.conf -jn；
4、业务完成或将一个共享池内的共享盘依次拔出元数据副本数-1个（间隔1分钟）；
5、2块故障盘1小时内恢复，等待1个小时后启动主机端执行vdbench -f min-seq-w-1l.conf -jro校验数据一致性
6、修复完成后，比较内部数据一致性
...
def case():
    ...
    env_manage_repair_test.co2_osan.run_vdb(client_ip=client_ip1, vdb_xml=min_seq_w, jn_jro='jn', output=node_ip1)
    log.info("业务完成或将一个共享池内的共享盘依次拔出元数据副本数-1个（间隔1分钟）")
    disk_error()

    log.info(
        "2块故障盘1小时内恢复，等待1个小时后启动主机端执行vdbench -f min-seq-w-1l.conf -jro校验数据一致性")
