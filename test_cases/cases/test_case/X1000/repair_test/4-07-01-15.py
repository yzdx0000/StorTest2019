...
7、故障节点数据网恢复后，数据重建和修复完成后，比较内部数据一致性。
...
def disk_error(arg=1):
    ...
    time.sleep(10)
    env_manage_repair_test.Reliable_osan.remove_disk(node_ip=node_ip1, disk_id=disk_phy_id1 ,disk_usage="DATA")
...
def case():
    ...
    decorator_func.multi_threads(run_vdb1, run_vdb2, disk_error, network_error)
    for i in fault_eth_list:
        env_manage_repair_test.Reliable_osan.network_test(s_ip=node_ip2,
                                                          net_name=i,
                                                          net_stat="up")
    ...
    env_manage_repair_test.Reliable_osan.insert_disk(node_ip=node_ip1, disk_id=disk_phy_id1, disk_usage="DATA")
