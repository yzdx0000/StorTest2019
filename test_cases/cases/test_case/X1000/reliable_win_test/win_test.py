...
4、在步骤3中的业务运行过程中，将主oJmgs进程异常；
5、从oJmgs接管主ojmgs服务，检查主ojmgs进程状态
...
# 获取主oJmgs的IP
fault_ip = deploy_ips[0]
log.info("Get fault process: oSan process")

def up_down():
    time.sleep(100)
    log.info("Step4:将节点oSan进程杀死,节点: %s." %(fault_ip))
    #ReliableTest.run_kill_process(fault_ip, 'oSan')
    time.sleep(100)
    # 重新获取主，确保主起来
    log.info("Step5:检查故障节点oSan进程状态,节点: %s." % (fault_ip))
    reliable.check_process_stat(fault_ip, 'oSan')
...
def win_vdb_run():
    ...
    osan.run_win_vdb(file_name, mix_R_win,jn_jro='no', time=900)
