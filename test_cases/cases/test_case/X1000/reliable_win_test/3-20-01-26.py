...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将主zk进程异常；
            5、从zk接管主zk服务，检查主zk进程状态
...
mix_R_win = osan.gen_win_vdb_conf(xfersize="(4k,80,16k,20)",
                                  ...
                                  rdpct=0)
# 获取主zk的IP
m_zk_ip=reliable.get_mster_zk(deploy_ips)
log.info("Get fault process:master zk")

def up_down():
    time.sleep(100)
    log.info("Step4:将主zk进程挂起,节点: %s." %(m_zk_ip))
    ReliableTest.run_pause_process(m_zk_ip,'zk')
    time.sleep(100)
    log.info("Step5:恢复挂起节点zk进程,节点: %s." % (m_zk_ip))
    reliable.run_process('zk', m_zk_ip)
    time.sleep(100)
    zk_ip = reliable.get_mster_zk(deploy_ips)
    log.info("Step5:检查新主zk进程状态,节点: %s." % (zk_ip))
    ReliableTest.check_process(zk_ip,'zk')
    log.info("Step5:检查挂起主zk进程状态,节点: %s." % (m_zk_ip))
    state = ReliableTest.check_process(m_zk_ip,'zk')
    while True:
        if state == True:
            log.info("该故障节点zk进程状态正常,节点: %s" % (m_zk_ip))
            break
        time.sleep(20)
...
def vdb_run():
    ...
    osan.run_vdb(client_ips[1], mix_R, output=deploy_ips[1],time=1200)
