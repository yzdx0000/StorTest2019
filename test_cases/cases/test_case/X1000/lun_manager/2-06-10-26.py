...
4、故障带业务的节点掉电，1分钟后恢复
5、自动均衡的过程中，将VIP迁移的源节点又故障，1分钟后恢复
...
def up_down():
    ...
    while osan.check_vip_state() != 1:
        ...
        count += 1
        time.sleep(1)
    log.info("Step5:自动均衡过程中,将源节点故障")
    log.info("将VIP均衡源节点掉电,节点:%s" % fault_ip1)
    vm_id = error.down_node(fault_ip)
    log.info("1min后将故障源节点恢复,节点:%s" % fault_ip1)
    time.sleep(60)
    error.up_node(vm_id)
    reliable.get_os_status_1(fault_ip1)
