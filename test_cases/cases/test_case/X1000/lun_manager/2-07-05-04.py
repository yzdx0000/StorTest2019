# -*- coding:utf-8 _*-
"""
测试内容:系统剩余空间小于节点容量，节点故障和恢复
1、部署3节点集群环境，配置日志3副本，配置访问分区为3个节点；
5、对齐 大小块混合顺序读写业务为脚本mix-S-Align.conf
...
4、将5条逻辑卷写满
5、一个节点下电后创建10条精简卷
6、步骤5中节点上电后再创建10条精简卷

检查项:
1、步骤2，10条逻辑卷创建成功
2、步骤3，10条精简卷创建成功
3、步骤4，5条逻辑卷写满，系统剩余可用容量为40G，可预留容量为27.5G左右
4、步骤5，节点下电后，系统剩余可用容量不变，可预留容量不变，所有卷的可用容量减少，精简卷创建失败，系统提示无可用空间
5、步骤6，节点恢复后，存储池可用容量恢复，卷可用容量恢复，精简卷创建成功
"""
...
def iscsi_login():
    ...
    min_seq_w1 = env_manage_lun_manage.co2_osan.gen_vdb_xml(
...
def create_lun_thin(arg=1):
    ...
    #     # log.info("存储池容量校验失败")
    #     # os._exit(1)
    # if 0.95 * (storage_pool_size - 10 * thick_lun_size - 10 * (
    #         int(cp("reserve", "thin_lun_reserve")) * 1024 * (1024 + 20))) < storage_pool_avail_bytes < 1.05 * (
    #         storage_pool_size - 10 * thick_lun_size - (int(cp("reserve", "thin_lun_reserve")) * 1024 * (1024 + 20))):
    #     pass
...
def map_lun():
    ...
    lun_ids = random.sample(env_manage_lun_manage.Reliable_osan.get_thick_lun(node_ip1), 5)
    for i in lun_ids:
...
def case():
    ...
    log.info("创建10条逻辑卷，单卷大小为2G，创建10条精简卷，单卷大小为10G,将5条逻辑卷写满")
    create_lun_thick()
    create_lun_thin()
    map_lun()
    iscsi_login()
