...
4、故障带业务的节点掉电，1分钟后恢复
5、自动均衡的过程中，将VPI迁移的源节点A业务网故障，1分钟后恢复
...
def up_down():
    ...
    while osan.check_vip_state() != 1:
        ...
        count += 1
        time.sleep(1)
    log.info("Step5:自动均衡过程中,将一个源节点的业务网故障")
    fault_id1 = breakdown.disk().get_node_id_by_ip(n_ip=fault_ip1)
    io_eth_list = error.get_io_eth(fault_id1)
    log.info("将VIP均衡的源节点业务网故障,节点:%s" % fault_ip1)
    threads = []
    ...
    for thread in threads:
        thread.join()
    log.info("1min后将故障节点业务网恢复,节点:%s" % fault_ip1)
    time.sleep(60)
    for eth in io_eth_list:
