# -*- coding:utf-8 _*-
"""
测试内容:并发写入thin lun数据触发空间预留确认时，oPmgr进程数据网闪断
...
4、在步骤3中的业务运行过程中，oPmgr主进程数据网以间隔5秒，断开时间30秒闪断10次；
...
def network_error(arg=1):
    ...
    for i in fault_eth_list:
        env_manage_lun_manage.Reliable_osan.net_flash_test(fault_ip, i)
...
def case():
    ...
    map_lun()
    iscsi_login()
    log.info("在步骤3中的业务运行过程中，在步骤3中的业务运行过程中，oPmgr主进程数据网以间隔5秒，断开时间30秒闪断10次")
    decorator_func.multi_threads(run_vdb1, network_error)
