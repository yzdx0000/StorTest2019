...
def case():
    ...
    [env_manage_lun_manage.osan.create_host_group(node_ip1, "hg_name_{}".format(i)) for i in range(5)]
    [env_manage_lun_manage.osan.add_host(node_ip1, "h_name{}".format(hg_id), hg_id) for hg_id in
     env_manage_lun_manage.osan.get_host_groups(node_ip1)]
    ...
    [osan.create_lun(s_ip=node_ip1, lun_type="THIN", lun_name='LUN_{}'.format(i), stor_pool_id=storage_pool_id,
                     acc_zone_id=az_id) for i in range(10)]
    ...
    lun_ids = env_manage_lun_manage.osan.get_lun(node_ip1)

    for i in env_manage_lun_manage.osan.get_host_groups(node_ip1):
        for j in random.sample(lun_ids, 2):
            env_manage_lun_manage.osan.map_lun(node_ip1, j, i)
            lun_ids.remove(j)

    msg = env_manage_lun_manage.osan.remove_hosts(node_ip1, env_manage_lun_manage.osan.get_hosts(node_ip1)[-1], need_judge=1)
    env_manage_lun_manage.assert_ins.assertNotequal(msg, '', 'judge failed')

    env_manage_lun_manage.clean_lun_map()
    env_manage_lun_manage.clean(target="clean_host")
