...
def case():
    ...
    osan.add_host(s_ip=deploy_ips[0], h_name='host1', hg_id=hg_ids[-1])
    h_ids = osan.get_hosts(s_ip=deploy_ips[0])
    ini_id = osan.add_initiator(s_ip=deploy_ips[0], h_id=h_ids[-1], iqn=cp("add_initiator", "iqn"),
    ...
    for lun_id in lun_ids:
        ...
        decorator_func.judge_target(
            ...
            'LUNMAP_READY')
    msg = osan.remove_initiator(s_ip=deploy_ips[0], ini_id=ini_id)
