...
def up_down():
    time.sleep(100)
    log.info("Step4:故障一个带业务节点的一个业务网卡,然后1min后恢复")
    ...
    fault_eth = random.choice(io_eth_list)
    reliable.network_test(fault_ip, fault_eth, 'down')
    time.sleep(60)
    reliable.network_test(fault_ip, fault_eth, 'up')
    time.sleep(100)
    log.info("Step5:等待自动均衡完成后,查看均衡情况")
