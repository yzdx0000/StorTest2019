# -*- coding:utf-8 _*-
"""
测试内容:并发创建thinl lun和thick lun，oPmgr主进程故障

步骤:
1、在访问区配置中，每个节点取1块磁盘创建副本模式存储池，创建存储池；并创建和配置对于的访问分区和vip地址池；
2、并发创建20条逻辑卷和20条精简卷时，oPmgr主以间隔1分钟反复被杀10次
...
def create_lun_thin(arg2):
    env_manage_lun_manage.osan.create_lun(s_ip=node_ip1, total_bytes=thin_lun_total, lun_type="THIN",
                                          lun_name='LUN{}'.format(arg2),
                                          stor_pool_id=storage_pool_id, acc_zone_id=az_id)


def process_error(arg=1):
    decorator_func.timer(5)
    ...
    for i in range(10):
        ...
        for j in range(5):
            if env_manage_lun_manage.Reliable_osan.check_process_stat(fault_ip, "oRole") in (
            "ok", "OK", "ready", "READY"):
                break
            time.sleep(20)
        else:
