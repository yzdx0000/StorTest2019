# -*- coding:utf-8 _*-
"""
测试内容:并发写入thin lun数据触发空间预留确认时，oPmgr节点掉电后恢复
...
4、在步骤3中的业务运行过程中，oPmgr主进程节点掉电恢复；
...
def iscsi_login():
    ...
    mix_R_Align = env_manage_lun_manage.co2_osan.gen_vdb_xml(
        max_range=reserve,maxdata="3G",
        thread=cp("vdbench", "threads"), lun=lun1,
        xfersize=xfersize1, seekpct=seekpct,
        rdpct=rdpct2)
    mix_R = env_manage_lun_manage.co2_osan.gen_vdb_xml(max_range=reserve,
                                                       maxdata="3G",
...
def case():
    ...
    map_lun()
    iscsi_login()
    log.info("在步骤3中的业务运行过程中，在步骤3中的业务运行过程中oPmgr主进程节点掉电恢复")
