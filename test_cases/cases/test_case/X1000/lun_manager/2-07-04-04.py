# -*- coding:utf-8 _*-
"""
测试内容:并发写入thin lun数据触发空间预留时，oSan进程数据网故障5分钟恢复
...
4、在步骤3中的业务运行过程中，oSan进程数据网故障5分钟恢复；
...
def iscsi_login():
    ...
    mix_R_Align = env_manage_repair_test.co2_osan.gen_vdb_xml(max_range=reserve,maxdata="3G",
...
def case():
    ...
    map_lun()
    iscsi_login()
    log.info("在步骤3中的业务运行过程中，在步骤3中的业务运行过程中，oSan进程数据网故障5分钟恢复")
