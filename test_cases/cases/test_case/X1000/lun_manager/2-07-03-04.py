# -*- coding:utf-8 _*-
"""
测试内容:并发创建thinl lun和thick lun，oSan节点闪断

步骤:
1、在访问区配置中，每个节点取1块磁盘创建副本模式存储池，创建存储池；并创建和配置对于的访问分区和vip地址池；
2、并发创建20条逻辑卷和20条精简卷时，oSan进程数据网以间隔5秒，断开时间30秒闪断10次

检查项:
1、步骤2，所有卷创建完成，无报错，创建性能无明显下降
"""
...
def network_error(arg=1):
    ...
    for i in fault_eth_list:
        env_manage_lun_manage.Reliable_osan.net_flash_test(fault_ip, i, 10, 30)
