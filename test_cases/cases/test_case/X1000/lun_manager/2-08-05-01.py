...
def case():
    ...
    # osan.update_param(s_ip=deploy_ips[0], section='MGR', name='mgcd_auto_mount_flag', current='1')  # 解除自动挂载
    for i in range(1, 3):
        osan.create_host_group(s_ip=deploy_ips[0], hg_name='myhostgroup{}'.format(i))
    hg_ids = osan.get_host_groups(s_ip=deploy_ips[0])

    h_id = osan.add_host(s_ip=deploy_ips[0], h_name='host1', hg_id=hg_ids[-1])
    osan.add_initiator(s_ip=deploy_ips[0], h_id=h_id, iqn=cp("add_initiator", "iqn1"),
    ...
    for lun_id in lun_ids:
        lun_map_id = osan.map_lun(s_ip=deploy_ips[0], lun_ids=lun_id, hg_id=hg_ids[-1])
        decorator_func.judge_target(
    ...
    osan.enable_iscisid(cli_ip=client_ips[1])
    osan.os_reboot(cli_ip=client_ips[1])
    env_manage_lun_manage.Reliable_osan.get_os_status_1(client_ips[0])
    decorator_func.timer(60)
