...
def get_avg_bandwidth(arg=1):
    ...
    osan.update_lun(s_ip=node_ip1, lun_id=lun_id, max_throughput=int(avg_bandwidth * 0.5))
    ...
    log.info("取消LUN的Qos设置")
    osan.update_lun(s_ip=node_ip1, lun_id=lun_id,qos_op="DISABLE")
    avg_bandwidth3 = env_manage_lun_manage.osan.Qos_count(c_ip=client_ip1, s_ip=node_ip1)
    env_manage_lun_manage.assert_ins.assert_less_than(avg_bandwidth, avg_bandwidth3, avg_bandwidth, percentage=1,
                                                      suc_msg="qos取消设置生效")
