...
def case():
    lun_id = osan.create_lun(s_ip=deploy_ips[0], total_bytes='102400000', lun_type="THICK",
                             lun_name='LUN1',
                             stor_pool_id=2, acc_zone_id=1,node_parity_num="2")
    log.info(lun_id)
    result = (osan.get_option_single(s_ip=deploy_ips[0], command="get_luns", indexname="luns"
                                     , argv2="layout", ids="ids", argv1=lun_id))['node_parity_num']
    log.info(result)
