...
def case():
    ...
    iqn = env_manage_lun_manage.osan.write_iqn(cli_ip=client_ip, iqn='iqn.20207015.com.sugon')
    ...
    for lun_id in lun_ids:
        ...
        decorator_func.judge_target(
            ...
            'LUNMAP_READY')
    login.login()
    disk = (env_manage_lun_manage.osan.ls_scsi_dev(client_ip=client_ip))[0]
    cmd=("ssh %s fdisk -l |grep '%s'|awk -F , '{print $2}'|awk '{print $1}' " % (client_ip, disk))
    final = commands.getstatusoutput(cmd)
    final=final[-1].split("\n")[-1]
    env_manage_lun_manage.assert_ins.assertEqual(final, '1073741824')
