...
4、故障带业务的节点的业务网口，1分钟后恢复
5、自动均衡的过程中，oRole主进程故障
...
def up_down():
    time.sleep(100)
    log.info("Step4:故障一个带业务节点的业务网卡,1min后恢复")
    ...
    log.info("将故障节点的所有业务网故障,节点:%s" % fault_ip)
    threads = []
    ...
    for thread in threads:
        thread.join()
    log.info("1min后恢复该节点所有业务网,节点:%s" % fault_ip)
    time.sleep(60)
    ...
    log.info("Step5:自动均衡过程中,oRole主进程故障")
