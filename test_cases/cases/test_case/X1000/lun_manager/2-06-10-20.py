...
def up_down():
    ...
    time.sleep(10)
    # 查看目前均衡业务的状态
    log.info("Step5:自动均衡过程中,增加两倍节点个数的VIP")
    count = 0
    while osan.check_vip_state() != 1:
        ...
        time.sleep(1)
    subnet_id = osan.get_subnet_id(deploy_ips[0])
    vips = cp('add_vip_address_pool', 'vips_add')
    osan.add_vip_address_pool(s_ip=deploy_ips[0], subnet_id=subnet_id[0], domain_name='sugon1.com', vip=vips,
                              ip_failover_policy='IF_ROUND_ROBIN', rebalance_policy='RB_AUTOMATIC')
