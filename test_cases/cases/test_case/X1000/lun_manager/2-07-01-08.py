# -*- coding:utf-8 _*-
"""
测试内容:thin lun和thick lun预留容量大于存储池容量
...
检查项:
1、步骤2，10条逻辑卷创建成功
2、步骤3，40条精简卷创建成功，2条精简卷创建失败，提示空间不足
"""
...
thick_lun_size = int((storage_pool_size - thin_lun_total) / 20 / float(cp("reserve", "multiple")))
...
def create_lun_error():
    msg = env_manage_lun_manage.osan.create_lun(s_ip=node_ip1, total_bytes=storage_pool_size, lun_type="THIN",
                                                lun_name='lun_82',
...
def case():
    ...
    log.info("创建20条逻辑卷，单卷大小为2G")
    create_thick_lun()
    log.info("创建42条精简卷，单卷大小为50G")
    create_thin_lun()
    create_lun_error()
