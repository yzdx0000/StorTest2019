# -*- coding:utf-8 _*-
"""
测试内容:并发写入thin lun数据触发空间预留时，oRole主切换
...
4、在步骤3中的业务运行过程中，将oRole主杀掉，然后立即替换掉oRole二进制文件；
5、业务执行完成后，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
6、比较存储内部数据一致性；

检查项:
1、步骤4，故障过程中业务正常，oRole杀死后被拉起失败切主；
2、步骤5，数据比较一致；
3、步骤6，内部数据比较一致。
"""
...
def process_error(arg=1):
    ...
    cmd = ("ssh {} \"mv /home/parastor/bin/oRole  /home/parastor/bin/oRole_bak\"".format(fault_ip))  # 杀死主oRole，并替换进程文件
...
def case():
    ...
    map_lun()
    iscsi_login()
    log.info("在步骤3中的业务运行过程中，将oRole主杀掉，然后立即替换掉oRole二进制文件")
    ...
    env_manage_lun_manage.co2_osan.run_vdb(client_ip=client_ip1, vdb_xml=mix_R_Align, jn_jro='jro', output=node_ip1,
                                           whether_change_xml="N")
    cmd = ("ssh {} \"mv /home/parastor/bin/oRole_bak  /home/parastor/bin/oRole\"".format(fault_ip))  # 还原进程文件
    commands.getstatusoutput(cmd)
    for i in range(5):
