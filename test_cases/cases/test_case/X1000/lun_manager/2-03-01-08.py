...
thick_lun_size1 = int(1073741824 / float(cp("reserve", "multiple")))

thick_lun_size2 = int(2147483648 / float(cp("reserve", "multiple")))
...
def iscsi_login():
    ...
    mix_R_Align = env_manage_repair_test.co2_osan.gen_vdb_xml(
        max_range=None,
        maxdata="{}M".format(int(1073741824 * 1.2 / 1024 / 1024)),
    ...
    mix_R_Align2 = env_manage_repair_test.co2_osan.gen_vdb_xml(
        ...
        maxdata="{}M".format(int(4294967296 * 1.2 / 1024 / 1024)),
        xfersize=xfersize1, seekpct=seekpct,
        rdpct=rdpct1)
...
def case():
    ...
    lun_id2 = env_manage_lun_manage.osan.create_lun(s_ip=node_ip1, total_bytes=thick_lun_size1, lun_type="THIN",
                                                   lun_name='LUN2', stor_pool_id=storage_pool_id, acc_zone_id=az_id)
    env_manage_lun_manage.osan.map_lun(s_ip=node_ip1, lun_ids=lun_id1, hg_id=hg_id)
    env_manage_lun_manage.osan.map_lun(s_ip=node_ip1, lun_ids=lun_id2, hg_id=hg_id)
...
def run_dd(arg=0):
    global luns
    login.login()
    luns = env_manage_repair_test.Lun_osan.ls_scsi_dev(client_ip1)
    env_manage_lun_manage.Reliable_osan.dd_command(client_ip1,luns[0],'4M','250')
    env_manage_lun_manage.Reliable_osan.dd_command(client_ip1, luns[1], '4M', '250')

def run_vdb_read_thread(arg=0):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    iscsi_login()
    env_manage_repair_test.co2_osan.run_vdb(client_ip=client_ip1, vdb_xml=mix_R, output=node_ip1,
                                            whether_change_xml="N")
...
def update_lun_thread(arg=1):
    ...
    env_manage_lun_manage.osan.update_lun(s_ip=node_ip1, lun_id=lun_id1, total_bytes=thick_lun_size2)
    env_manage_lun_manage.osan.update_lun(s_ip=node_ip1, lun_id=lun_id2, total_bytes=thick_lun_size2)
...
def re_scan():
    ...
    env_manage_lun_manage.assert_ins.assert_less_than(thick_lun_size2, lun_size2, thick_lun_size2, percentage=1,
                                                      suc_msg="修改生效")
...
def main():
    ...
    update_lun_thread()
    re_scan()
    env_manage_lun_manage.Reliable_osan.dd_command(client_ip1, luns[0], '4M', '500')
    env_manage_lun_manage.Reliable_osan.dd_command(client_ip1, luns[1], '4M', '500')
