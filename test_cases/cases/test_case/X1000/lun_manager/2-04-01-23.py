...
def iscsi_login():
    ...
    [env_manage_lun_manage.osan.create_lun(s_ip=node_ip1, lun_type="THIN",
                                           ...
                                           stor_pool_id=storage_pool_id, acc_zone_id=az_id) for i in range(2)]
    [env_manage_lun_manage.osan.create_lun(s_ip=node_ip1, lun_type="THICK",
                                           lun_name='LUN_{}'.format(i),
                                           stor_pool_id=storage_pool_id, acc_zone_id=az_id,total_bytes=536870912) for i in range(2,5)]
...
def get_avg_bandwidth(arg=1):
    ...
    osan.update_lun(s_ip=node_ip1, lun_id=lun_ids[0], max_throughput=int(avg_bandwidth * 0.2))
    osan.update_lun(s_ip=node_ip1, lun_id=lun_ids[1], max_throughput=int(avg_bandwidth * 0.2))
    osan.update_lun(s_ip=node_ip1, lun_id=lun_ids[2], max_throughput=int(avg_bandwidth * 0.1))

    avg_bandwidth2 = env_manage_lun_manage.osan.Qos_count(c_ip=client_ip1, s_ip=node_ip1)
