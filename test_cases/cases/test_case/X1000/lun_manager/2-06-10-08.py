...
6、修改VIP地址池的自动均衡策略为轮询
7、等待自动均衡完成后，查看3个节点的VIP均衡情况
8、下电节点A，查看VIP均衡情况
9、等节点A恢复后启动自动修复，修复完成或查看VIP均衡情况"
...
def case():
    ...
    log.info("Step6:将自动均衡策略从连接数修改为轮询")
    Lun_managerTest.update_vip_policy(s_ip=deploy_ips[0], ip_failover_policy='IF_ROUND_ROBIN')

    log.info("Step7:检查是否按照轮询策略均衡成功")
    while osan.check_vip_state() != 0:
        log.info("等待自动均衡完成中.")
        time.sleep(1)
    eth1_vips = osan.get_vips_by_eth(deploy_ips[0], 1, eth1)
