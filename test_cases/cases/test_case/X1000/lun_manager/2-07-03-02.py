# -*- coding:utf-8 _*-
"""
测试内容:并发创建thinl lun和thick lun，oPmgr主进程切换

步骤:
1、在访问区配置中，每个节点取1块磁盘创建副本模式存储池，创建存储池；并创建和配置对于的访问分区和vip地址池；
2、并发创建20条逻辑卷和20条精简卷时，oPmgr主被杀死后oPmgr换主（通过替换oRole二进程文件方式进行）

检查项:
1、步骤2，oPmgr主被拉起前，创建挂住，恢复后创建正常，直到所有卷创建完成。
"""
...
def case():
    ...
    log.info("并发创建20条逻辑卷和20条精简卷时，oPmgr主以间隔1分钟反复被杀10次")
    ...
    commands.getstatusoutput(cmd)
    for i in range(5):
        if env_manage_lun_manage.Reliable_osan.check_process_stat(fault_ip, "oRole") in ("ok", "OK", "ready", "READY"):
            break
        time.sleep(20)
    else:
    ...
    env_manage_lun_manage.Reliable_osan.compare_data()
