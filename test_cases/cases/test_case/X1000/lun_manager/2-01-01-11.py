...
need_size=int(4 * 70368744177664)
if storage_pool_size<=need_size:
    exit(-1)


def case():
    lun_id = osan.create_lun(s_ip=deploy_ips[0], total_bytes='536870912000', lun_type="THICK",
                             lun_name='LUN1',
    ...
    env_manage_lun_manage.osan.delete_lun(node_ip1,lun_id)

    lun_id = osan.create_lun(s_ip=deploy_ips[0], total_bytes='4398046511104', lun_type="THICK",
                             lun_name='LUN2',
                             stor_pool_id=storage_pool_id, acc_zone_id=az_id)
    log.info(lun_id)
    decorator_func.judge_target(
    ...
    lun_id = osan.create_lun(s_ip=deploy_ips[0], total_bytes='17592186044416', lun_type="THICK",
                             lun_name='LUN3',
                             stor_pool_id=storage_pool_id, acc_zone_id=az_id)
    log.info(lun_id)
    decorator_func.judge_target(
    ...
    lun_id = osan.create_lun(s_ip=deploy_ips[0], total_bytes='70368744177664', lun_type="THICK",
                             lun_name='LUN4',
                             stor_pool_id=storage_pool_id, acc_zone_id=az_id)
    log.info(lun_id)
    decorator_func.judge_target(
