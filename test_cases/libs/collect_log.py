...
def check_core_size():
    ...
    :Date:2019-02-28
    :Author:Diws
    :Description:检查最新出的core的大小，在30s内是否发生变化；如果不变，则认为core已经生成完毕；如果检查过程中有新core
    产生，则也不再检查新出的core
    :return:
    """
    for sip in deploy_ips:
        cmd = ("ssh root@%s 'ls -lt /home/parastor/log/core* 2> /dev/null | sed -n 1p'" % (sip,))
        res, output = commands.getstatusoutput(cmd)
        core_name = output.split(' ')[-1]
        if len(core_name) <= 1:
            continue
        cmd = ("ssh root@%s 'ls -lt %s 2> /dev/null | sed -n 1p'" % (sip, core_name))
        while True:
            res, output = commands.getstatusoutput(cmd)
            s1 = output.split(' ')[4]
            time.sleep(30)
...
def collect_parastor(s_ip=None, dst=None):
    ...
    print cmd
    commands.getstatusoutput(cmd)
    log_list = ["dbginfo_stub", "errinfo.sh", "gdb.dump", "gdb.dump.opmgr", "msginfo.sh", "potrc", "gdb.*"]
    for i in log_list:
    ...
    for num in range(node_nums):
        cmd = (
                    "ssh root@%s '/home/parastor/tools/nWatch -t oPara -i %s -a \"vmgrid=1 fout=0 fpath=/a/b/c\" -c oPara#vmgr_vset_dump >> /home/xlog/oPara.vmgr_vset_dump'"
                    % (s_ip, str(num + 1)))
        os.system(cmd)
        cmd = (
                    "ssh root@%s '/home/parastor/tools/nWatch -t oPara -i %s -a \"vmgrid=1\" -c oPara#vmgr_flattenrecord_dump >> /home/xlog/oPara.vmgr_flattenrecord_dump'"
                    % (s_ip, str(num + 1)))
        os.system(cmd)
        cmd = (
                    "ssh root@%s '/home/parastor/tools/nWatch -t oPara -i %s -a \"vmgrid=1\" -c oPara#vmgr_flattennr_dump >> /home/xlog/oPara.vmgr_flattennr_dump'"
                    % (s_ip, str(num + 1)))
        os.system(cmd)
        cmd = (
                    "ssh root@%s '/home/parastor/tools/nWatch -t oPara -i %s -a \"vmgrid=1\" -c oPara#vmgr_topo_dump >> /home/xlog/oPara.vmgr_topo_dump'"
                    % (s_ip, str(num + 1)))
        os.system(cmd)
        cmd = (
                    "ssh root@%s '/home/parastor/tools/nWatch -t oStor -i %s -c OBS#jobperf -a wbcache >> /home/xlog/oStor.get_obs_jobperf_wbcache'"
                    % (s_ip, str(num + 1)))
        os.system(cmd)
