...
class Remote(object):
    ...
    def run_keyword(self, name, args=None, kwargs=None):
        ...
        if result.status != 'PASS':
            log.error("message='%s', details='%s', fatal='%s', continuable='%s'" %
                      (result.error, result.traceback, result.fatal, result.continuable))
            raise Exception("message='%s', details='%s', fatal='%s', continuable='%s'" %
                            (result.error, result.traceback, result.fatal, result.continuable))
...
class XmlRpcRemoteClient(object):
    ...
    def run_keyword(self, name, args, kwargs):
        # run_keyword_args = [name]
        # if args:
        #    run_keyword_args.append(args)
        # if kwargs:
        #    run_keyword_args.append(kwargs)
...
def is_dict_like(item):
    return (isinstance(item, (dict, UserDict)))
