...
def arg_analysis():
    ...
    parser.add_option("-r", "--reinstall",
                      action="store_true",
                      dest="re_ins",
                      help="If you want to install the system to replace the current system, configure this parameter")
...
def run_tests():
    ...
    first_case = test_files[0][2]
    if re.findall(r'/2-0[1-8]', first_case):
        install_cmd = install_cmd2
    elif re.findall(r'/4-', first_case):
                ...
                if RE_INSTALL is True:
                    ...
                    if res != 0:
                        ...
                        if res != 0:
                            ...
                            exit(1)
                    os_rel.asyn_ntp()
                if get_config.get_tl() and "VMware" not in mac_stdout:
    ...
    cmd_cp = ("cp %s %s_%s" % (change_file, change_file, now_time))
    os.system(cmd_cp)
