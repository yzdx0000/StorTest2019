# -*- coding:utf-8 _*-
"""
测试内容:一个节点数据网故障后写数据，超时内恢复

步骤:
1、在节点A创建lun1-lun6
2、将逻辑卷映射至主机1上lun1-lun6
3、在步骤3业务进行过程中，将lun写满后，再重新执行步骤3业务中将一个数据节点上的一个数据盘拔掉；（新卷参考/home/parastor/tools/nWatch -t oSan -i 1 -c oSan#get_lmeta估算卷容量）
4、业务过程中拔掉一个节点的数据网；
5、在超时时间(24小时)内恢复，
...
检查项:
1、步骤4、断开一个节点数据网，业务不受影响，系统上报网络异常信息；
2、步骤5、持续修改写数据，数据网恢复后立即触发数据修复；
3、步骤6、修复过程中，内外部数据校验一致；
4,、步骤7、修复完成后，内部数据校验一致。

"""

# testlink case: 1000-33203
import os, sys
import utils_path
import common2
import common
...
node_id = env_manage_repair_test.break_down.get_node_id_by_ip(n_ip=failnode_ip)
...
def network_error(arg=1):
    ...
    for i in fault_eth_list:
        env_manage_repair_test.Reliable_osan.network_test(s_ip=failnode_ip,
                                                          net_name=i,
                                                          net_stat="down")
...
def case():
    ...
    env_manage_repair_test.co2_osan.run_vdb(client_ip=client_ip1, vdb_xml=min_seq_w, jn_jro='jn', output=node_ip1)
    log.info("在步骤3业务进行过程中，将lun写满后，再重新执行步骤3业务中将一个数据节点上,将一个数据节点数据网断开")
    decorator_func.multi_threads(run_vdb_w, network_error)
    log.info("超时内故障节点网络恢复，立即触发数据修复...")
    for i in fault_eth_list:
        env_manage_repair_test.Reliable_osan.network_test(s_ip=failnode_ip,
                                                          net_name=i,
                                                          net_stat="up")
