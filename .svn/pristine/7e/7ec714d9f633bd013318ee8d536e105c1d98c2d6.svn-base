...
# Date  :2018-9-5
"""
测试内容:所有元数据盘依次故障恢复，每次需等上次坏对象修复完成后操作

步骤:
1、在节点A创建LUN1
2、将逻辑卷映射至主机1上LUN1
3、在主机1上运行vdbench -f min-seq-w-1l.conf -jn；
4、业务完成后将日志节点的一个共享盘拔出；
5、在主机1上运行vdbench -f min-seq-r-1l.conf ，超时内插回拔出的共享盘，开始修复。
6、主机端执行vdbench -f min-seq-w-1l.conf -jro校验数据一致性
7、数据修复完成后，比较内部数据一致性；
8、重复执行步骤6/7将所有共享盘测试一遍
...
def case():
    ...
    log.info(
        "step4:在主机1上运行vdbench -f min-seq-r-1l.conf ，超时内插回拔出的共享盘，开始修复，主机端执行vdbench -f min-seq-write.conf -jro校验数据一致性 ...")
