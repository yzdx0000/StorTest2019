...
# date 2018-11-22
# @summary：
#    nfs客户端权限测试：chmod其他控制项测试，对文件进行S_ISUID掩码设置，检验用户uid；原P200测试用例23-0-9-1。
# @steps:
...
#    6、测试主体，chmod其他控制项测试，对文件进行S_ISUID掩码设置，检验用户uid；
...
def case():
    ...
    log.info("6-3> 用户%s对%s进行chmod 4755 操作" %
    ...
    (rc, uid) = common.run_command(nfs_ip, cmd)
