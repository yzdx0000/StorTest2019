...
#   执行：
#           1>修改NFS共享
#           2>执行get_events查看修改NFS共享失败的结果显示
#           3>清理工作
# @changelog：
####################################################################################
...
nfs_path = nas_common.ROOT_DIR + "NFS_" + FILE_NAME                     # parastor:/NFS_events_5_3_3_46.py
...
def case():
    ...
    '''3>创建共享目录'''
    check_result = nas_common.create_file(nfs_path)    # /mnt/parastor/events_5_3_3_46.py
    if check_result["detail_err_msg"] != "":
    ...
    '''1> 修改导出NFS共享目录'''
    '''令NFS导出id不合法，预计修改操作失败'''
    fault_export_id = nfs_export_id + 1000
    nas_common.update_nfs_export(export_id=fault_export_id,
                                 description="update_nfs_export_finished!")

    '''2> 执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)               # 修改NFS共享失败，延时
    code = '0x02030604'                # 修改NFS共享失败对应编号
    description = 'update_nfs_export_failed'
    '''修改NFS共享失败事件码是否在事件列表中'''
