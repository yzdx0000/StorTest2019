...
# Date: 2018-09-03
# @summary：
#    用户组： 父目录软阈值2G，宽限时间60秒，子目录软阈值1G，宽限时间60秒，子目录创建1100M文件开始计时，
#    过期后子目录无法创建文件，父目录再创建1100M文件，创建成功，过期后创建失败
# @steps:
#    1.创建用户用户组
#    2.创建目录FILENAME/nesting1_1
#    3.创建目录FILENAME配额,软阈值2G 宽限时间60秒 等待配额状态为work
#    4.创建子目录FILENAME/nesting1_1/配额,子目录软阈值1G，宽限时间60秒 等待配额状态为work
#    5.子目录创建1100M文件开始计时，过期后子目录无法创建文件
#    6.父目录再创建1100M文件，创建成功，过期后创建失败
#    7.父目录删除配额创建文件成功
#    8.子目录删除配额创建文件成功
# @changelog：
#################################################################
...
def case():
    ...
    log.info("3> 创建父目录配额,软阈值2G 宽限时间60秒 等待配额状态为work")
    ...
    log.info("4> 创建子目录FILENAME/nesting1_1配额,子目录软阈值1G，宽限时间60秒 等待配额状态为work")
    ...
    log.info("5> 子目录创建1100M文件开始计时，过期后子目录无法创建文件")
    if LARGE_BS:
        ...
        quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_1, dir1_1, 1100, 1, 'a',
                                                                quota_common.QUOTA_USER, quota_id=quota_id_lst[1])
    dir_size = quota_common.user_or_group_total_file_size(quota_common.CLIENT_IP_1, dir1_1, quota_common.QUOTA_USER)
    ...
    dir_size = quota_common.user_or_group_total_file_size(quota_common.CLIENT_IP_2, dir1_1, quota_common.QUOTA_USER)
    if dir_size != quota_common.FILE_SIZE_1M * 1100:
        rc_lst[sys._getframe().f_lineno] = 'dir: %s total_size: %s ' % (dir1_1, dir_size)

    log.info('6> 父目录再创建1100M文件，创建成功，过期后创建失败')
    if LARGE_BS:
    ...
    if dir_size != quota_common.FILE_SIZE_1M * 2200:
        rc_lst[sys._getframe().f_lineno] = 'dir: %s total_size: %s ' % (dir0, dir_size)
    log.info('wait 75s')
    time.sleep(75)
    quota_common.wait_soft_threshold_over_time(qmgr_ip, quota_id_lst[0], quota_common.SOFT_TIME_LOGICAL)
    ...
    dir_size = quota_common.user_or_group_total_file_size(quota_common.CLIENT_IP_1, dir0, quota_common.QUOTA_USER)
    if dir_size != quota_common.FILE_SIZE_1M * 2200:
        rc_lst[sys._getframe().f_lineno] = 'dir: %s total_size: %s ' % (dir0, dir_size)
    ...
    if LARGE_BS:
        quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_2, dir0, 1, 100, 'e',
                                                                quota_common.QUOTA_USER)
    else:
        quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_2, dir0, 100, 1, 'e',
                                                                quota_common.QUOTA_USER)
    dir_size = quota_common.user_or_group_total_file_size(quota_common.CLIENT_IP_2, dir0, quota_common.QUOTA_USER)
    if dir_size != quota_common.FILE_SIZE_1M * 2300:
        rc_lst[sys._getframe().f_lineno] = 'dir: %s total_size: %s ' % (dir0, dir_size)
    ...
    time.sleep(10)
    if LARGE_BS:
        quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_3, dir1_1, 1, 200, 'f',
                                                                quota_common.QUOTA_USER)
    else:
        quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_3, dir1_1, 200, 1, 'f',
                                                                quota_common.QUOTA_USER)
    dir_size = quota_common.user_or_group_total_file_size(quota_common.CLIENT_IP_3, dir1_1, quota_common.QUOTA_USER)
    if dir_size != quota_common.FILE_SIZE_1M * 1300:
        rc_lst[sys._getframe().f_lineno] = 'dir: %s total_size: %s ' % (dir1_1, dir_size)
