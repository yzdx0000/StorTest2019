...
node_ip4 = env_manage_repair_test.deploy_ips[3]
...
fault_node_id4 = env_manage_repair_test.break_down.get_node_id_by_ip(n_ip=node_ip4)

io_eth_list4, a_extra4 = error.get_ioip_info(node_id=fault_node_id4)
log.info(io_eth_list4)

fault_eth_list4 = error.get_data_eth(fault_node_id4)
log.info(fault_eth_list4)
...
def network_error2(arg=1):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    decorator_func.timer(200, 25, log_interval=0)
    env_manage_repair_test.Reliable_osan.net_flash_test(node_ip3, fault_eth_list3)
...
def os_error(arg=3):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    decorator_func.timer(200, 25, log_interval=0)
    env_manage_repair_test.down_node(node_ip2, type_info)  # 下电

    decorator_func.timer(60, 20)
...
def process_thread(arg=1):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    decorator_func.timer(200, 25, log_interval=0)
    ReliableTest.run_kill_process(node_ip=node_ip2, process="oSan")
    for b in range(5):
        if any([env_manage_lun_manage.Reliable_osan.check_process_stat(node_ip2, "oSan") in ("ok", "OK"),
                env_manage_lun_manage.Reliable_osan.check_process_stat(node_ip2, "oSan") in ("ready", "READY")]):
            break
        time.sleep(20)
    else:
...
def case():
    ...
    iscsi_login()
    decorator_func.multi_threads(run_vdb1, run_vdb2, network_error, network_error2, process_thread)
    for i in io_eth_list1:
        env_manage_repair_test.Reliable_osan.network_test(s_ip=node_ip1,
                                                          net_name=i,
                                                          net_stat="up")
