...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-0-1-66 目录用户组inode数配额混合阈值，硬阈值最小
#######################################################
...
def executing_case():
    ...
    # 针对目录用户组配置inode数为3000硬阈值、2000软阈值（宽限天数为1天）、4000建议阈值的配额
    ...
    if check_result["err_no"] == 0:
        ...
        log.error("11-0-1-66 Failed")
        raise Exception("11-0-1-66 Failed")
    else:
        log.info("11-0-1-66 Succeed")
    return
...
class Quota_Class_11_0_1_66():
    def quota_method_11_0_1_66(self):
