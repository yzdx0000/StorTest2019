...
log.info("Got machine A's info: fault ip is : %s,extra ip is %s,data device name is :%s" % (a_ip, a_extra, a_eths))


def up_down(pipein):
    time.sleep(10)
    log.info("、在步骤3中的业务运行过程中，将节点A业务网断开，")
    log.info("Begin down node:%s, eth %s through %s." % (a_ip, a_eths, a_extra))
    ReliableTest.run_down_net(a_extra[0], a_eths)
    time.sleep(10)
    b_ip = osan.get_node_by_vip(v_ip)
    log.info("4、重新获取vip %s 所在节点 %s" % (v_ip, b_ip))
    log.info("4、将目的节点oMgcd故障；")
    log.info("On node %s kill oMgcd" % (b_ip,))
    ReliableTest.run_kill_process(b_ip, 'oMgcd')
    # 监控到匿名管道中写入数据后，退出循环
    while True:
