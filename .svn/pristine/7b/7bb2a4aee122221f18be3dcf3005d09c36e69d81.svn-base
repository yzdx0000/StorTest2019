...
def case():
    ...
    '''1>制作删除业务子网失败事件'''
    subnet_fault_id = subnet_id + 1000  # 制作不合法id从而触发删除业务子网失败事件
    nas_common.delete_subnet(subnet_fault_id)
    '''2>执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s'% delay_time)
    time.sleep(delay_time)    # 删除业务子网失败后延时
    code = '0x02030206'     # 删除业务子网失败事件对应的编号
    description = 'delete_subnet_failed'
    '''删除业务子网失败事件码是否在事件列表中'''
