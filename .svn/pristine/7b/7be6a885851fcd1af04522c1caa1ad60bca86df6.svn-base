...
def executing_case1():
    ...
    access_zone_name = "access_zone_16_0_2_23"
    node = common.Node()
    ids = node.get_nodes_id()
    print ids
    access_zone_node_id_16_0_2_23 = ','.join(str(p) for p in ids)
    m = len(ids)
    node_name_list = nas_common.get_node_name_list(ids)
    cmd = "create_access_zone"
    msg1 = nas_common.create_access_zone(node_ids=access_zone_node_id_16_0_2_23, name=access_zone_name)
    ...
    cmd = "enable_nas --access_zone_id=%s" % access_zone_id
    ...
    for j in range(0, m):
        ...
        exe_info = node.get_node_state(k)
        if exe_info != 'NODE_STATE_HEALTHY':
            common.except_exit('node state is not healthy !!!')
    ...
    access_zone_node_id_16_0_2_23 = ','.join(str(p) for p in ids)
    node_name_list = nas_common.get_node_name_list(ids)
    cmd = "ssh %s update_access_zone --id=%s --node_ids=%s" \
          % (node_ip, access_zone_id, access_zone_node_id_16_0_2_23)
    msg1 = nas_common.update_access_zone(access_zone_id=access_zone_id,
                                         node_ids=access_zone_node_id_16_0_2_23)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, msg1))
