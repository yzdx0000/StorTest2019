...
def case():
    ...
    storage_pool_size_M = int(storage_pool_size / 1024 / 1024)

    thick_lun_size = int(
        (storage_pool_size - int(cp("reserve", "thin_lun_reserve"))) / float(cp("reserve", "multiple")))
    global vdb_maxdata
    vdb_maxdata = int((storage_pool_size - thick_lun_size) * 2 / 1024 / 1024)
    ...
    env_manage_lun_manage.osan.create_lun(s_ip=node_ip1, total_bytes=thick_lun_size, lun_type="THICK",
                                          lun_name='LUN_thick', stor_pool_id=storage_pool_id, acc_zone_id=az_id)
    lun_id = env_manage_lun_manage.osan.create_lun(s_ip=node_ip1, total_bytes=storage_pool_size * 5, lun_type="THIN",
...
def run_vdb1_thread(arg=0):
    ...
    env_manage_repair_test.co2_osan.run_vdb(client_ip=client_ip1, vdb_xml=mix_R_Align, output=node_ip1,
                                            whether_change_xml="N", need_judge=1, unmap="N")
...
def main():
    ...
    env_manage_lun_manage.clean(target="lun")#针对存储池写满；删除lun会卡主的情况作下校验
