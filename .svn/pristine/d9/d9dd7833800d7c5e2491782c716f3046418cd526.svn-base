...
#    7、NFS客户端成功挂载到某机头节点并使用vdbench进行读写业务，同时触发oRole换主
#    8、删除读写数据
#    9、将域名解析文件恢复原状
...
def case():
    ...
    master_node_id = stdout.strip().split(':')[-1]
    ...
    p2 = Process(target=make_fault.kill_process, args=(master_node_ip, 'oRole'))
    ...
    if p2.exitcode != 0:
        common.except_exit('kill oRole failed !!!')
