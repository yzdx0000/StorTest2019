...
def running_break():
    ...
    type = env_manage.get_os_type(power_ip[0])
    info = env_manage.down_node(power_ip[0], type, "init 0")
    os_types.append(type)
    infos.append(info)


def create_lun():
    env_manage.deploy_ips.remove(oJmgs_ips[0], power_ip[0])
