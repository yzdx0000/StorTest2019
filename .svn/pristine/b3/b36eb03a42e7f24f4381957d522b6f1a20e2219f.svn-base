...
2）创建逻辑卷属性默认，选择pool2存储池，创建逻辑卷LUN1，容量5GB，创建逻辑卷LUN2，容量10GB，创建逻辑卷LUN3，容量10GB；
3）提交后创建成功。
...
def case():
    ...
    decorator_func.judge_target(
        ...
        'LUN1')
    lun_id = osan.create_lun(s_ip=deploy_ips[0], total_bytes='10737418240', lun_type="THICK",
                             lun_name='LUN2',
                             stor_pool_id=storage_pool_id, acc_zone_id=az_id)
    decorator_func.judge_target(
        osan.get_option_single(s_ip=deploy_ips[0], command="get_luns", indexname="luns"
