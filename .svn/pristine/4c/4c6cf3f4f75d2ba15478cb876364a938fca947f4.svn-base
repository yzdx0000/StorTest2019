...
4、业务过程中将存储池磁盘拔出一块；
5、插回被拔出的磁盘触发修复；
6、修复过程中将修复源节点反复掉电又恢复
7、主机1端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性
8、掉电节点上电恢复，数据修复完成系统恢复正常后，比较内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-33296
import os
import sys
import commands
import threading
...
def up_down():
    ...
    time.sleep(100)
    log.info("Step6:数据修复过程中,将一个源节点反复掉电,节点:%s" % fault_node_ip1)
    for count in range(3):
        log.info("源节点第 %d 次掉电..." % (count + 1))
