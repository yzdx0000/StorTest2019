...
def pro_fault(s_ip):
    log.info("step:8.%s节点进程故障" % (s_ip))
    ReliableTest.run_kill_process(node_ip=s_ip, process="oJmgs")
    time.sleep(10)
    log.info("节点关闭完成")
    return
...
def lun_map_case():
    ...
    log.info("step:7.创建lun map 过程中lun los oJmgs进程故障")
    ...
    threads.append(threading.Thread(target=pro_fault, args=(master_ojmgs_ip,)))  # down node
    for i in threads:
        i.start()
    for i in threads:
        i.join()
    log.info("step:9.检查节点服务状态")
    env_manage.com_lh.get_os_status(node_ip)
