...
# date 2018-05-25
# @summary：
#    对根目录创建配额（文件数，硬阈值），配额限制小于初始文件数，创建快照并查看回滚结果
# @steps:
#    1、在/mnt/volume1/下创建100个非空文件；
#    2、对上述目录创建filenr为50的硬阈值；
#    3、对目录创建快照；
#    4、删除根目录下的30个文件；
#    5、执行revert并查看回滚结果；
#    6、删除配额、快照及创建的文件；
...
SNAP_TRUE_PATH = os.path.join(snap_common.SNAP_PATH, FILE_NAME)             # /mnt/volume1/snap/snap_13_0_8_2
CREATE_SNAP_PATH = os.path.join(snap_common.SNAP_PATH_BASENAME, FILE_NAME)  # /snap/snap_13_0_8_2
...
def case():
    ...
    if stdout.strip() != '100':
        log.error('file num is not 100, is %s' % stdout)
        raise Exception('file num is not 100, is %s' % stdout)
