...
#    4、通过nfs客户端先写入2000M的文件，宽限时间内再写入1000M文件
#    5、过宽限时间后尝试继续写入文件，检查配额是否生效
#    6、删除配额及所创建的文件
...
def case():
    ...
    '''4> 通过nfs客户端先写入2000M的文件，宽限时间内再写入1000M的文件'''
    for i in range(20):
