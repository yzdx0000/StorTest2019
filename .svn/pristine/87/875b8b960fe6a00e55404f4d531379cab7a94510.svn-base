...
def up_down():
    time.sleep(100)
    log.info("Step4:业务中故障一个带业务的节点,然后恢复.")
    ...
    time.sleep(30)
    log.info("Step5:等待自动均衡完成后,查看均衡情况是否符合预期.")
    ...
    if len(set(vips_ls)) == 1 and sum(vips_ls) == len(deploy_ips) * 4:
        log.info("VIP自动均衡成功,且每个节点VIP数量为4.")
    else:
        log.error("异常!VIP在2min内自动均衡失败,数量分布情况:%s" % ','.join(map(str, vips_ls)))
        os._exit(1)
