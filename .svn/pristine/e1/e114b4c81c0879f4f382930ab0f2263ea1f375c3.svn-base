...
def vdb_test(run_ip):
    ...
    env_manage.com2_osan.run_vdb(client_ip=run_ip, vdb_xml=vdb_file, output=node_ip1, jn_jro="jn", time=600)
...
def case():
    ...
    log.info("step:3.ISCSI login")
    env_manage.create_iscsi_login()
    log.info("step:4.The node to power off.")
    os_type = env_manage.get_os_type(node_ip1)
    info = env_manage.down_node(node_ip1, os_type, "init 0")
    log.info("step:5.vdbench test")
    vdb_test(client_ip1)
    log.info("step:6.%s 节点开机 节点类型%s,节点ID or IPMI IP: %s" % (node_ip1, os_type, info))
