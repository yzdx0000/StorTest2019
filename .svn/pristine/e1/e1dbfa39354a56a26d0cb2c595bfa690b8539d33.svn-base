...
Description :
            ...
            4、在步骤3中的业务运行过程中，一块元数据盘依次故障后恢复；
            5、元数据修复完成后，在故障下一块元数据盘；
            6、重复步骤4和步骤5，直到遍历所有的元数据盘；
            7、主机1上业务完成后，在主机1上执行vdbench -f mix-R-align.conf-jro比较一致性；
            8、元数据修复完成后，比较存储内部元数据一致性。
'''
...
def up_down(pipeout):
    for ip in fault_disk.keys():
        ...
        #     disk.delete_disk(s_ip=ip, disk_id=disk_id)
        # time.sleep(20)
        # for disk_uuid in disk_uuids.keys():
        #     if int(disk_uuids[disk_uuid]) == int(node.get_node_id_by_ip(ip)):
        #         disk.add_disk(s_ip=ip, uuid=disk_uuid, usage="SHARED",node_id=disk_uuids[disk_uuid])
        # time.sleep(30)
