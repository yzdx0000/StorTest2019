...
# date 2018-11-29
# @summary：
# x-x-x-x     用户/用户组，删除认证服务器，被enable的访问分区引用
# @steps:
# 1.创建ad，ldap，ldap-pdc，nis认证：每创建一种认证，创建一个访问分区引用认证，enable_nas，验证FTP功能
# 删除认证（预期失败），disable_nas, 删除访问分区
# @changelog：
#
#######################################################
import os
...
def executing_case1():
    ...
    """创建访问分区"""
    access_zone_id, az_node_id = create_access_zone(auth_provider_id_list_ad)
    """启动NAS"""
    ...
    """创建访问分区"""
    access_zone_id, az_node_id = create_access_zone(auth_provider_id_list_ldap)
    """启动NAS"""
    enable_nas(access_zone_id=access_zone_id)
    """验证FTP的功能"""
    ftp_id = random.choice(az_node_id)
    ...
    """创建访问分区"""
    access_zone_id, az_node_id = create_access_zone(auth_provider_id_list_ldap_pdc)
    """启动NAS"""
    enable_nas(access_zone_id=access_zone_id)
    """验证FTP的功能"""
    ftp_id = random.choice(az_node_id)
    ...
    """创建访问分区"""
    access_zone_id, az_node_id = create_access_zone(auth_provider_id_list_nis)
    """启动NAS"""
    enable_nas(access_zone_id=access_zone_id)
    """验证FTP的功能"""
    ftp_id = random.choice(az_node_id)
