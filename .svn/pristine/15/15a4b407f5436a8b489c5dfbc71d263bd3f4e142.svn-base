...
# date 2018-06-12
# @summary：
# 16_0_1_29          特殊字符用户、用户组测试
# @steps:
# case1、连上含有带特殊字符用户名和用户组名的AD认证服务器；2、查看用户名和用户组名是否显示正确
# @changelog：
#
#######################################################
import os
...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)  # /mnt/volume/nas/nas_16_0_1_29
...
def executing_case1():
    ...
    wait_time1 = random.randint(150, 155)
    time.sleep(wait_time1)

    log.info("\t[case1 add_auth_provider_ad ]")
    global ad_name
    ad_name = "nas_16_0_1_29_ad"
    ...
    global ad_id_16_0_1_29
    ad_id_16_0_1_29 = msg2["result"]

    '''2> get_auth_providers_ad'''
    log.info("\t[case1-2 get_auth_provider_ad ]")
    cmd = "get_auth_providers_ad"
    check_result2 = nas_common.get_auth_providers_ad(ids=ad_id_16_0_1_29)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    msg2 = check_result2
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    ...
    if id != ad_id_16_0_1_29:
        raise Exception('%s id error!!!' % node_ip)
    if key != ad_id_16_0_1_29:
    ...
    check_result2 = nas_common.check_auth_provider(provider_id=ad_id_16_0_1_29)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    msg2 = check_result2
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    ...
    access_zone_name = "access_zone_16_0_1_29"
    node = common.Node()
    ids_list = node.get_nodes_id()
    global ids_list
    print ids_list
    global access_zone_node_id_16_0_1_29
    access_zone_node_id_16_0_1_29 = ','.join(str(p) for p in ids_list)
    global m
    m = len(ids_list)
    cmd = "create_access_zone "
    check_result2 = nas_common.create_access_zone(node_ids=access_zone_node_id_16_0_1_29,
                                                  name=access_zone_name,
                                                  auth_provider_id=ad_id_16_0_1_29)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    msg2 = check_result2
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
        raise Exception('%s create_access_zone failed!!!' % (node_ip))
    global access_zone_id_16_0_1_29
    access_zone_id_16_0_1_29 = msg2["result"]

    '''5> get_access_zones'''
    log.info("\t[case1-5 get_access_zones ]")
    cmd = "get_access_zones "
    check_result2 = nas_common.get_access_zones(ids=access_zone_id_16_0_1_29)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    msg2 = check_result2
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    ...
    if auth_provider_id != ad_id_16_0_1_29:
        raise Exception('%s auth_provider_id %s != %s error!!!' % (node_ip, auth_provider_id, ad_id_16_0_1_29))
    ...
    if access_zones_id != access_zone_id_16_0_1_29:
        raise Exception('%s access_zones_id %s != %s error!!!' % (node_ip, access_zones_id, access_zone_id_16_0_1_29))
    ...
    cmd = "enable_nas "
    check_result2 = nas_common.enable_nas(access_zone_id=access_zone_id_16_0_1_29)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    msg2 = check_result2
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    ...
    time.sleep(wait_time1)
    cmd = "get_auth_users "
    rc, check_result2 = nas_common.get_auth_users(auth_provider_id=ad_id_16_0_1_29)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    msg2 = check_result2
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    ...
    cmd = "get_auth_groups "
    check_result2 = nas_common.get_auth_groups(auth_provider_id=ad_id_16_0_1_29)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    msg2 = check_result2
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
