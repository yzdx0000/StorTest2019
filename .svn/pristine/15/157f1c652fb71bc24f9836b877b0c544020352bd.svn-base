...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-0-2-10 不支持目录配额规则嵌套之目录用户组配额规则
#######################################################
...
def executing_case():
    ...
    2、结果检查
    '''
    # 尝试在父目录a的子目录b上再加一个目录用户组配额规则，其中目录a上同时
    # 有以下三种规则：目录配额规则、目录用户配额规则和目录用户组配额规则
    ...
    if (check_result1["detail_err_msg"] != "" or
        ...
        log.error("11-0-2-10 Failed")
        raise Exception("11-0-2-10 Failed")
    else:
        log.info("11-0-2-10 Succeed")
    return
...
class Quota_Class_11_0_2_10():
    def quota_method_11_0_2_10(self):
