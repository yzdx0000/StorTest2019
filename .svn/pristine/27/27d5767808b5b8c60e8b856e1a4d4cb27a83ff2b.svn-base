...
a_eths, aextras = error.get_dataip_info(node_id=a_id)
nodes = osan.get_nodes(s_ip=deploy_ips[0])
none_jnl_ids = error.get_non_free_node_id()
if len(none_jnl_ids) != 0:
    b_id = random.choice(none_jnl_ids)
else:
    nodes.remove(a_id)
