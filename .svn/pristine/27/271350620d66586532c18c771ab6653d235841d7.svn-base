...
def disk_error(arg=1):
    ...
    for l in L:
        env_manage_repair_test.Reliable_osan.remove_disk(node_ip=node_ip1, disk_id=l.split(",")[0], disk_usage="SHARED")

    for l in L2:
        env_manage_repair_test.Reliable_osan.remove_disk(node_ip=node_ip2, disk_id=l.split(",")[0], disk_usage="SHARED")

    time.sleep(300)

    for l in L:
        env_manage_repair_test.Reliable_osan.insert_disk(node_ip=node_ip1, disk_id=l.split(",")[0], disk_usage="SHARED")

    for l in L2:
        env_manage_repair_test.Reliable_osan.insert_disk(node_ip=node_ip2, disk_id=l.split(",")[0], disk_usage="SHARED")
...
def case():
    ...
    iscsi_login()
    decorator_func.multi_threads(run_vdb1, run_vdb2, network_error, disk_error)
    for i in io_eth_list1:
        env_manage_repair_test.Reliable_osan.network_test(s_ip=node_ip1,
                                                          net_name=i,
                                                          net_stat="up")
