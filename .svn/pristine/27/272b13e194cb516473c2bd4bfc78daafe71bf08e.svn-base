...
4、向集群中加入新节点，将新节点加入到集群中过程中，将该节点掉电5分钟后恢复；
...
def down_node():
    ...
    while True:
        ...
        if 'err' in line:
            break
    time.sleep(1.5)
    log.info("4、扩容节点过程中，将该节点掉电5分钟后恢复")
    log.info("Begin down node : %s." % (dst_ip,))
    node_info = error.down_node(fault_ip=dst_ip)
    time.sleep(300)
    log.info("Begin up node : %s through %s." % (dst_ip, str(node_info)))
...
def main():
    ...
    test_threads.append(threading.Thread(target=down_node))
