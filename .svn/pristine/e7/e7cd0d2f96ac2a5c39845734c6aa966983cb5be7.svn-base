...
# date 2018-11-22
# @summary：
#    nfs客户端权限测试：对目录进行chown操作，检验目录权限；原P200测试用例23-0-8-2。
# @steps:
...
def case():
    ...
    cmd = "ssh %s \"cd %s;chown %s:%s %s\"" % (
        nfs_ip, nfs_file_location, user_name_list[1], group_list[1], testdir)
    rc = common.command(cmd)
    ...
    cmd_u1 = "ssh %s \"su %s -c 'cd %s;touch %s/%s'\"" % (
        nfs_ip, user_name_list[0], nfs_file_location, testdir, 'file_' + user_name_list[0])
    ...
    cmd_u2 = "ssh %s \"su %s -c 'cd %s;touch %s/%s'\"" % (
        nfs_ip, user_name_list[1], nfs_file_location, testdir, 'file_' + user_name_list[1])
