...
def case():
    ...
    log.info("id:%s  %s\n"
             ...
             "subnet_mask:%s  %s\n"
             "subnet_gateway:%s  %s\n"
             "network_interfaces:%s  %s\n" % (subnet['id'], subnet_id, subnet['name'], subnet_name, subnet['svip'],
                                              nas_common.SUBNET_SVIP, subnet['subnet_mask'], nas_common.SUBNET_MASK,
                                              subnet['subnet_gateway'], nas_common.SUBNET_GATEWAY,
                                              subnet['network_interfaces'][0],
                                              nas_common.SUBNET_NETWORK_INTERFACES))
    ...
    if exe_info["err_msg"] != "" or exe_info["detail_err_msg"] != "":
        log.error('delete vip address pool failed !!!')
        raise Exception('delete vip address pool failed !!!')
