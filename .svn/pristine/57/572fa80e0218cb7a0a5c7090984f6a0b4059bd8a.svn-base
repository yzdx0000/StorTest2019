...
def up_down(pipein):
    ...
    time.sleep(10)
    los_nid_state_d = disk.get_jnl_state(removed_ip=a_ip)
    log.info("After inject error,the loss' state are{los_id:[node_id, los_state]}:%s" % (los_nid_state_d,))
    # 获取新的los所在节点ID
    d_id = los_nid_state_d[losid][0]
    d_ip = disk.get_node_ip_by_id(removed_ip=a_ip, n_id=d_id)
    log.info("5、节点D %s 重启，节点B接管节点D并重建日志；" % (d_ip,))
    log.info("Begin shutdown node %s." % (d_ip,))
    m_d_info = error.down_node(fault_ip=d_ip)
    time.sleep(30)
    log.info("Begin startup node %s." % (d_ip,))
    error.up_node(node_info=m_d_info)
    common.check_ip(d_ip)
    c_id = error.get_orole_master_id()
    c_ip = disk.get_node_ip_by_id(n_id=c_id)
    log.info("The major oRole's IP is %s." % (c_ip,))
    log.info("6、节点D上电恢复后，日志迁移到节点D过程中时，%s oRole主故障；" % (c_ip,))
