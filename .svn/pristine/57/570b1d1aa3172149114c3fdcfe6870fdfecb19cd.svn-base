...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-0-2-51 修改目录用户配额规则之改大、改小建议阈值
#######################################################
...
def executing_case():
    ...
    # 验证配额是否生效，预期成功再写入1025M文件
    quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_1, quota_common.QUOTA_PATH,
                                                            1025, 1, "c", quota_common.QUOTA_USER)
    ...
    if (check_result1["detail_err_msg"] != "" or
            total_file_size1 != quota_common.FILE_SIZE_1G + 1024*1024 or
            check_result2["detail_err_msg"] != "" or
            total_file_size2 != quota_common.FILE_SIZE_2G + 1024*1024*2 or
            check_result3["detail_err_msg"] != "" or
            total_file_size3 != quota_common.FILE_SIZE_3G + 1024*1024*3):
        rc, check_result = quota_common.get_all_quota_info()
        common.judge_rc(rc, 0, "get quota failed")
        log.error("11-0-2-51 Failed")
        raise Exception("11-0-2-51 Failed")
    else:
        log.info("11-0-2-51 Succeed")
    return
...
class Quota_Class_11_0_2_51():
    def quota_method_11_0_2_51(self):
