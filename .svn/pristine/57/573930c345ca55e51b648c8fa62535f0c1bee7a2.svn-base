...
4、在步骤3中的业务运行过程中，将业务节点网配置成bond，节点A断开两个网线；
...
def up_down():
    ...
    time.sleep(100)
    log.info("Step3:在bond配置完成后,将业务节点 %s 的两个业务网卡断开." % test_node_ip)
    for io_eth in io_eth_list:
        reliable.network_test(test_node_ip, io_eth, 'down')

    event1.wait()
    event2.wait()
    log.info("Step5:将业务节点 %s 的故障业务网卡恢复." % test_node_ip)
    for io_eth in io_eth_list:
        reliable.network_test(test_node_ip, io_eth, 'up')
    time.sleep(60)
    log.info("Step6:确定业务完成后,检查数据修复完成.")
    disk.check_bad_obj()
