...
def case():
    ...
    threads.append(t1)
    t2 = threading.Thread(target=env_manage.create_access, args=(node_ip3, '3,4', "accesszone1"))
    threads.append(t2)
    ...
    for t in threads:
        t.join()
    log.info("%s 节点开机 节点类型%s,节点ID or IPMI IP: %s" % (node_ip1, os_types[0], infos[0]))
