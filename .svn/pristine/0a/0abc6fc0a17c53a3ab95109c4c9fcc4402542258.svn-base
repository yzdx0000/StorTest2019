...
:Description:
            ...
            6、10分钟后恢复数据网，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32942
import os
import sys
import commands
import threading
...
def up_down():
    log.info("故障点:一个非日志节点的数据网两根网线同时故障10min")
    time.sleep(100)
...
def vdb_jn():
    ...
    while 'HEALTHY' not in node.get_node_state(fault_node_id):
        log.info("Step7:等待节点数据修复ing")
        time.sleep(20)
    log.info("Step8:节点数据修复完成，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；")
    osan.run_vdb(client_ips[0], mix_R_Align, output=deploy_ips[0], jn_jro="jro")
