...
def iscsi_login():
    ...
    mix_R_Align = env_manage_repair_test.co2_osan.gen_vdb_xml(max_range="100M",
                                                              thread=cp("vdbench", "threads"), lun=lun1,
                                                              xfersize=xfersize1, seekpct=seekpct,
                                                              rdpct=rdpct2)
    mix_R = env_manage_repair_test.co2_osan.gen_vdb_xml(thread=cp("vdbench", "threads"), lun=lun1, xfersize=xfersize1,
                                                        seekpct=seekpct,
                                                        rdpct=rdpct2, offset=int(cp("vdbench", "offset")))
    mix_R_Align2 = env_manage_repair_test.co2_osan.gen_vdb_xml(thread=cp("vdbench", "threads"), lun=lun1,
                                                               max_range="200M",
                                                               xfersize=xfersize1, seekpct=seekpct,
                                                               rdpct=rdpct2)
...
def case():
    ...
    lun_id = env_manage_lun_manage.osan.create_lun(s_ip=node_ip1, total_bytes='157286400', lun_type="THIN",
...
def update_lun_thread(arg=1):
    ...
    env_manage_lun_manage.osan.update_lun(s_ip=node_ip1, lun_id=lun_id, total_bytes="271581184")
...
def re_scan():
    ...
    env_manage_lun_manage.assert_ins.assertEqual(271581184, lun_size2)
...
def main():
    ...
    case()  # 用例步骤
    decorator_func.multi_threads(run_vdb1_thread, update_lun_thread)
    iscsi_login()
