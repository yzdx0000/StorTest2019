...
#           2>创建NIS认证服务
#   执行：
#           1>制作修改NIS认证服务器失败事件（修改认证服务id）
#           2>执行get_events查看修改NIS认证服务器失败的结果显示
# @changelog：
####################################################################################
...
def case():
    ...
    code = '0x02030408'                # 修改nis认证服务器失败对应编号
    description = 'update_nis_provider_failed'
