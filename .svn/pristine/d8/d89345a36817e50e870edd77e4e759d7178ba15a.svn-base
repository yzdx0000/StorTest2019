...
4、在步骤3中的业务运行过程中，将节点A的业务网所有网线 断开；
5、等待故障上报后将节点A数据网断开；
6、业务完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32870
import os
import sys
import commands
import threading
...
def up_down():
    ...
    for thread in thread_ls:
        thread.join()
    time.sleep(30)
    log.info("Step4:业务网故障上报后,将节点A的数据网所有网线断开,业务节点:%s" % fault_io_ip)
    thread_ls1 = []
    ...
    for thread in thread_ls1:
        thread.join()
    time.sleep(100)
    log.info("Step5:将故障节点数据网的网卡恢复,节点:%s" % fault_io_ip)
    for data_eth_name in data_eth_list:
        reliable.network_test(fault_io_ip, data_eth_name, 'up')
    log.info("Step5:将故障节点业务网的网卡恢复,节点:%s" % fault_io_ip)
    for io_eth_name in io_eth_list:
