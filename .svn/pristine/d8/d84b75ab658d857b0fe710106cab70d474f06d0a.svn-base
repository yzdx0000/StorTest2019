...
#   执行：
#       1> 删除NAS协议参数
#       2> 执行get_events查看删除NAS协议参数失败的结果显示
# @changelog：
####################################################################################
...
def case():
    ...
    '''3>强制添加NAS协议参数'''
    check_result = nas_common.update_nas_protocol_param(protocol_type='NFS',
                                                        access_zone_id=access_zone_id,
                                                        param_key="test",
                                                        param_value="1",
                                                        force="true")
    if check_result["detail_err_msg"] != "":
        common.except_exit("update_nas_protocol_param failed!!")
    ...
    nas_common.delete_nas_protocol_param(protocol_type='NFS',
                                         access_zone_id=fault_access_zone_id,
                                         param_key="test")
    '''2>执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)  # 删除NAS协议参数失败，延时一段时间

    code = '0x02031304'   # 删除NAS协议参数失败事件码
    description = 'update_nas_protocol_param'
    '''删除NAS协议参数失败事件码是否在事件列表中'''
