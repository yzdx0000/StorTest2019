...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-0-1-6 目录inode数配额混合阈值，硬阈值最小
#######################################################
...
def executing_case():
    ...
    2、结果检查
    '''
    # 针对目录配置inode数为2000硬阈值、4000软阈值（宽限天数为1天）、3000建议阈值的配额，检查配额是否生效
    ...
    if check_result["err_no"] == 0:
        ...
        log.error("11-0-1-6 Failed")
        raise Exception("11-0-1-6 Failed")
    else:
        log.info("11-0-1-6 Succeed")
    return
...
class Quota_Class_11_0_1_6():
    def quota_method_11_0_1_6(self):
