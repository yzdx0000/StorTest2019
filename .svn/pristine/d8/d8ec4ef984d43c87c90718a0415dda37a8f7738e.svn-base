...
def case():
    ...
    ''' 4> 执行delete_snapshot_strategy,输入错误参数，删除快照策略（预期失败）'''
    # 删除快照策略
    rc, stdout = snap_common.delete_snapshotstrategy_by_ids(0)
    if rc != 0:
        log.info('snapshot strategy delete failed !!!')
    ...
    code = '0x02060042'
    description = 'deleting snapshot_strategy'
