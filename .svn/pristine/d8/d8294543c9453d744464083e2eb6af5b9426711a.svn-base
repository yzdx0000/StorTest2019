...
"""获取指定节点数据盘和共享盘"""
share_disk, data_disk = Reliable_osan.get_share_monopoly_disk_ids(s_ip=node_ip1, node_id=node_id)
disk_ids = []
L = []
for share_disk in share_disk:
    disk_phy_id = Reliable_osan.get_physicalid_by_name(node_ip1, share_disk)
    disk_ids.append(disk_phy_id)
log.info(disk_ids)
for i in range(0, len(disk_ids)):
    disk_phy_id = disk_ids[i]  # 磁盘物理id
    disk_name = Reliable_osan.get_name_by_physicalid(node_ip1, disk_phy_id)  # 磁盘名字
    disk_uuid = com_disk.get_disk_uuid_by_name(node_id=node_id, disk_name=disk_name)  # 磁盘uuid
    disk_id = Reliable_osan.get_diskid_by_name(s_ip=node_ip1, node_id=node_id,
                                               disk_name=disk_name)  # 集群中的磁盘id
    L.append("{},{},{},{}".format(disk_phy_id, disk_name, disk_uuid, disk_id))
log.info(L)
stor_id_block = Lun_osan.get_storage__type_id(s_ip=node_ip1, type="SHARED")


def up_down(pipein):
    time.sleep(10)
    log.info("4、在步骤3中的业务运行过程中，将A节点业故障，将节点B所有日志盘拔掉再恢复；")
    log.info("Begin shutdown node:%s." % (a_ip,))
    m_info = error.down_node(fault_ip=a_ip)
    log.info("On node %s remove disk." % b_ip)
    for l in L:
        Reliable_osan.remove_disk(node_ip=b_ip, disk_id=l.split(",")[0], disk_usage="SHARED")
    Reliable_osan.run_down_disk_wait(s_ip=b_ip, timeout=int(cp('timeout', 'disk_timeout')))
    time.sleep(int(cp('timeout', 'disk_timeout')) + 10)
    for l in L:
        Reliable_osan.check_disk_state(node_ip=node_ip1,
                                       disk_uuid=l.split(",")[-2],
                                       disk_state="DISK_STATE_ZOMBIE")  # 检测磁盘状态，若为ZOMBIE，认为重建完成
        com_disk.remove_disks_asyn(disk_ids=l.split(",")[-1])
        Reliable_osan.insert_disk(node_ip=node_ip1, disk_id=l.split(",")[0], disk_usage="SHARED")
        Reliable_osan.add_disks(s_ip=node_ip1, node_ids=node_id, uuid=l.split(",")[-2],
                                usage="SHARED",
