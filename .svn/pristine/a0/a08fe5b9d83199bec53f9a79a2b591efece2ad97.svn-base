...
#   执行：
#           1>修改SMB全局配置信息
#           2>执行get_events查看修改SMB全局配置成功的结果显示#
#           3>清理工作
# @changelog：
####################################################################################
...
smb_path = nas_common.ROOT_DIR + "SMB_" + FILE_NAME                     # parastor:/nas/SMB_events_5_3_3_67
...
def case():
    ...
    '''1>修改SMB全局配置信息'''
    check_result = nas_common.update_smb_global_config(smb_global_config_id=id,
                                                       enable_change_notify="false")
    if check_result["detail_err_msg"] != "":
        common.except_exit("update_smb_global_config failed!!")
    '''2> 执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)               # 修改SMB全局配置成功，延时
    code = '0x02030713'                # 修改SMB全局配置成功对应编号
    description = 'update_smb_global_config'
    '''修改SMB全局配置成功事件码是否在事件列表中'''
