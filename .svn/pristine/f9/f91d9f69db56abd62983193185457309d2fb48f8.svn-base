...
# date 2018-09-08
# @summary：
#    上传对象过程中，随机选择一个杀一个节点的oOss进程
# @steps:
#    1、创建账户；
#    2、检查账户创建成功
#    3、创建证书；
#    4、上传桶
#    5、上传对象过程中，随机选择一个杀一个节点的oOss进程
#    6、清理环境
# @changelog：
####################################################################################
...
def for_kill_process(node_ip, service_type, othername):
    ...
    if service_state == 'OK':
        ...
        make_fault.kill_process(node_ip, proc_name)
        while True:
            rc, service_state = s3_common.get_node_service_state(node_id, service_type)
...
def put_many_obj(count, bucket_name, filepath, certificate_id):
    for j in range(count):
        ...
        rc, stdout = s3_common.add_object(bucket_name, object_name, filepath,
                                          certificate_id, exe_node_ip=None, retry=True)
        judge_result(rc, FILE_NAME)
...
def case():
    ...
    log.info("5> 随机选择一个节点，杀oOss进程,不恢复")
    obj_node = common.Node()
    ...
    log.info('此次选择杀节点%s的oOss进程' % killoossip)

    log.info("6> 杀节点%s的oOss进程过程中，上传对象" % killoossip)
    p1 = Process(target=put_many_obj, args=(object_num, bucket_name, filepath, certificate_id,))
    p2 = Process(target=for_kill_process, args=(killoossip, service_type, othername))
    p1.daemon = True
    p2.daemon = True
    p1.start()
    p2.start()
