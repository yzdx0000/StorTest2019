...
# 8、命令行通过IP addr观察VIP是否绑定到3个节点到对应的网卡上（相同网段的网卡）；
...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)  # /mnt/volume1/snap/snap_16_0_3_58
...
def executing_case1():
    ...
    node_ip = SYSTEM_IP
    access_zone_name = "access_zone_16_0_3_58"
    node = common.Node()
    ids = node.get_nodes_id()
    print ids
    access_zone_node_id_16_0_3_58 = ','.join(str(p) for p in ids)
    msg1 = nas_common.create_access_zone(node_ids=access_zone_node_id_16_0_3_58, name=access_zone_name)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        common.except_exit('%s create_access_zone failed!!!' % node_ip)
    access_zone_id_16_0_3_58 = msg1["result"]

    """获取访问分区信息"""
    log.info("\t[1-2 get_access_zones ]")
    msg1 = nas_common.get_access_zones(ids=access_zone_id_16_0_3_58)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
    ...
    msg2 = nas_common.enable_nas(access_zone_id=access_zone_id_16_0_3_58)
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        common.except_exit('%s enable_nas failed!!!' % node_ip)

    """2、创建业务子网"""
    log.info("\t[2 create_subnet ]")
    sub_name = "subnet_16_0_3_58"
    ...
    check_result1 = nas_common.create_subnet(access_zone_id=access_zone_id_16_0_3_58,
                                             name=sub_name,
    ...
    subnet_id_16_0_3_58 = check_result1["result"]

    """获取子网信息"""
    log.info("\t[2 get_subnets ]")
    msg1 = nas_common.get_subnets(ids=subnet_id_16_0_3_58)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        common.except_exit('%s get_subnets failed!!!' % node_ip)
    access_zone_id = msg1["result"]["subnets"][0]["access_zone_id"]
    if access_zone_id != access_zone_id_16_0_3_58:
        raise Exception('%s access_zone_id failed %s != %s !!!' % (node_ip, access_zone_id, access_zone_id_16_0_3_58))
    idd = msg1["result"]["subnets"][0]["id"]
    if idd != subnet_id_16_0_3_58:
        raise Exception('%s id failed %s != %s !!!' % (node_ip, idd, subnet_id_16_0_3_58))
    ...
    msg1 = nas_common.add_vip_address_pool(subnet_id=subnet_id_16_0_3_58,
    ...
    vip_address_pool_id_16_0_3_58 = msg1["result"]

    """4、通过pscli --command=get_vip_address_pools查看与配置的信息是否匹配"""
    log.info("\t[4 get_vip_address_pools ]")
    msg1 = nas_common.get_vip_address_pools(ids=vip_address_pool_id_16_0_3_58)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
    ...
    if idd != vip_address_pool_id_16_0_3_58:
        raise Exception('%s id failed %s != %s !!!' % (node_ip, idd, vip_address_pool_id_16_0_3_58))
    ...
    if subnet_id != subnet_id_16_0_3_58:
        raise Exception('%s subnet_id failed %s != %s !!!' % (node_ip, subnet_id, subnet_id_16_0_3_58))
    ...
    vip_2_vip_addresses = nas_common.VIP_2_ADDRESSES
    msg1 = nas_common.update_vip_address_pool(vip_address_pool_id=vip_address_pool_id_16_0_3_58,
    ...
    if supported_protocol != vip_supported_protocol:
        raise Exception(
            '%s supported_protocol failed %s != %s !!!' % (node_ip, supported_protocol, vip_supported_protocol))
    vip_addresses = msg1["result"]["ip_address_pools"][0]["vip_addresses"][0]
    if vip_addresses != vip_2_vip_addresses:
        raise Exception('%s vip_addresses failed %s != %s !!!' % (node_ip, vip_addresses, vip_2_vip_addresses))
