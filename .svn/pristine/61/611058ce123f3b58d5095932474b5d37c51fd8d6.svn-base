...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-0-1-61 目录用户组inode数配额硬阈值之chgrp方式
#######################################################
...
def executing_case():
    ...
    # （3）最后尝试写100个属主为quota_group的文件，预期写入失败
    quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_2, quota_common.QUOTA_PATH,
    ...
    if total_inodes != 2000:
        ...
        log.error("11-0-1-61 Failed")
        raise Exception("11-0-1-61 Failed")
    else:
        log.info("11-0-1-61 Succeed")
    return
...
class Quota_Class_11_0_1_61():
    def quota_method_11_0_1_61(self):
