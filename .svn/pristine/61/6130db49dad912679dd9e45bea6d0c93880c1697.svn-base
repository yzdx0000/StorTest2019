...
def executing_case1():
    ...
    access_zone_name = "access_zone_16_0_2_22"
    ids = [1]
    m = len(ids)
    access_zone_node_id_16_0_2_22 = ','.join(str(p) for p in ids)
    node_name_list = nas_common.get_node_name_list(ids)
    cmd = "create_access_zone "
    msg1 = nas_common.create_access_zone(node_ids=access_zone_node_id_16_0_2_22, name=access_zone_name)
    ...
    msg1 = nas_common.update_access_zone(access_zone_id=access_zone_id,
                                         node_ids=access_zone_node_id_16_0_2_22)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, msg1))
