...
# date 2018-04-25
# @summary：
# 16-0-4-85      共享名长度边界值测试
# @steps:
# case1、创建一个共享名以字母开头的含数字和下划线的长度不超过31位的NFS共享；
# case2、创建一个共享名以字母开头的含数字和下划线的长度超过31位的NFS共享；
# @changelog：
#
#######################################################
import os
...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)  # /mnt/volume1/snap/snap_16_0_4_85
...
#######################################################
# 1.executing_case1
# @function：创建一个共享名以字母开头的含数字和下划线的长度不超过31位的NFS共享；
# @parameter：
#       node_ip：执行cmd命令的节点ip
#       cmd：创建一个共享名以字母开头的含数字和下划线的长度不超过31位的NFS共享；
# @return：
...
def executing_case1():
    ...
    access_zone_name = "access_zone_16_0_4_85"
    node = common.Node()
    ids = node.get_nodes_id()
    print ids
    access_zone_node_id_16_0_4_85 = ','.join(str(p) for p in ids)
    msg1 = nas_common.create_access_zone(node_ids=access_zone_node_id_16_0_4_85, name=access_zone_name)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        common.except_exit('%s create_access_zone failed!!!' % node_ip)
    global access_zone_id_16_0_4_85
    access_zone_id_16_0_4_85 = msg1["result"]

    # 2> 启动NAS
    log.info("\t[ enable_nas ]")
    msg2 = nas_common.enable_nas(access_zone_id=access_zone_id_16_0_4_85, protocol_types="NFS")
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        common.except_exit('%s enable_nas failed!!!' % node_ip)

    # 3> 查看NAS是否按配置启动
    log.info("\t[ get_access_zones ]")
    check_result3 = nas_common.get_access_zones(ids=access_zone_id_16_0_4_85)
    name = check_result3["result"]["access_zones"][0]["name"]
    if name != access_zone_name:
        common.except_exit('%s enable_nas failed!!!' % node_ip)
    global auth_provider_id_16_0_4_85
    auth_provider_id_16_0_4_85 = check_result3["result"]["access_zones"][0]["auth_provider_id"]
    ...
    #     os.makedirs(prepath)
    global nfs_path
    nfs_path = nas_common.ROOT_DIR + "nfs_dir_16_0_4_85"
    global NAS_PATH
    NAS_PATH = get_config.get_one_nas_test_path() + "/nfs_dir_16_0_4_85"
    cmd = "create_file "
    ...
    nfs_export_name = "nfs_exp_test_16_0_4_85_aaaaaaaaa123456789012345678901234567890123456789012345678"
    cmd = "create_nfs_export "
    check_result1 = nas_common.create_nfs_export(access_zone_id=access_zone_id_16_0_4_85, export_name=nfs_export_name,
    ...
    global nfs_export_id_16_0_4_85
    nfs_export_id_16_0_4_85 = msg1["result"]

    """4>检查导出路径"""
    log.info("\t[ case1 get_nfs_exports ]")
    cmd = "get_nfs_exports"
    check_result = nas_common.get_nfs_exports(ids=nfs_export_id_16_0_4_85)
    ...
    if access_zone_id != access_zone_id_16_0_4_85:
...
#######################################################
# 2.executing_case2
# @function：创建一个共享名以字母开头的含数字和下划线的长度超过31位的NFS共享；
# @parameter：
#       node_ip：执行cmd命令的节点ip
#       cmd：创建一个共享名以字母开头的含数字和下划线的长度超过31位的NFS共享；
# @return：
#       check_result —— 执行create命令的字典格式返回值
# @steps:
# 1> 检查目录并creat file
...
def executing_case2():
    ...
    # if not isExists:
    #     os.makedirs(prepath)
    nfs_path = nas_common.ROOT_DIR + "nfs_dir_16_0_4_85_2"
    global NAS_PATH_2
    NAS_PATH_2 = get_config.get_one_nas_test_path() + "/nfs_dir_16_0_4_85_2"
    cmd = "create_file "
    check_result6 = nas_common.create_file(path=nfs_path, posix_permission="rwxr-xr-x")
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result6))
    msg6 = check_result6
    ...
    nfs_export_name_2 = "nfs_exp_test_16_0_4_85_aaaaaaaaa1234567890123456789012345678901234567890123456789"
    cmd = "create_nfs_export "
    check_result1 = nas_common.create_nfs_export(access_zone_id=access_zone_id_16_0_4_85, export_name=nfs_export_name_2,
                                                 export_path=nfs_path)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
    msg1 = check_result1
    if msg1["err_msg"] != "ILLEGAL_ARGUMENT" or msg1["detail_err_msg"] != "Export name 'nfs_exp_test_16_0_4_85_aaaaaaaaa1234567890123456789012345678901234567890123456789' length:81 exceed the max length:80":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
        raise Exception('%s create_nfs_export failed!!!' % node_ip)
    return
...
def clearing_environment():
    ...
    check_result1 = nas_common.delete_nfs_exports(ids=nfs_export_id_16_0_4_85)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
    msg1 = check_result1
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
    ...
    if rc != 0:
        ...
        log.info("WARNING: \n cmd = %s\n stdout = %s\n stderr = %s" % (cmd, stdout, stderr))
    log.info(stdout)
    # prepath1 = '/mnt/volume/nas/nfs_dir_16_0_4_85'
    # prepath2 = '/mnt/volume/nas/nfs_dir_16_0_4_85_2'
