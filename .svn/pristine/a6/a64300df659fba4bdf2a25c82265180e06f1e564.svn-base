...
def iscsi_login(ips=None, cli_ips=None, svip=None):
...
def run_vdb(client_ip, vdb_xml, jn_jro=None, output=None, time=1200, execute="Y", whether_change_xml="Y",
    ...
    if execute == "N":
        ...
        if time != None:
            ...
            cmd1 = (
                    "ssh %s '%s/vdbench -f %s -jn  -o /root/output/%s_jn'" % (
...
def os_down_osan(access_id):
    time.sleep(30)
    log.info("will down oRole master node when disable san protocol")
    threads = []
    ...
    for t in threads:
        t.join()
    log.info("os down and disable san finished")


def run_vdb_case(az_id):
    threads = []
    threads.append(threading.Thread(target=vdb_test))
    threads.append(threading.Thread(target=os_down_osan, args=(az_id,)))
    for t in threads:
        t.start()
    for t in threads:
        t.join()
...
def case():
    ...
    log.info("step:2.Start scsi protocol")
    ...
    log.info("step:3. ISCSI登录")
    iscsi_login(ips=node_ip1, cli_ips=client_ip1, svip=svip1)
    log.info("step:4.will shut down master oRole node when stop scsi protocol")
    rc = run_vdb_case(az_id1)
    if rc == 0:
        log.error("The vdbench not stop")
        os._exit(1)
    log.info("step:5.检查san 协议是否停止成功")
    san_status = env_manage.com_lh.get_san_state(node_ip1)
    if all(san_status) is True:
        log.error("check all san is active, all san status:%s, will exit" % (san_status))
        os._exit(1)
    else:
    ...
    env_manage.com_lh.get_os_status(down_node[0])
