...
# date 2018-07-27
# @summary：
#    快照策略更新成功（等待get_events接口完成）
# @steps:
#   1、部署3节点集群环境
#   2、在/mnt/volume1/snap/snap_create_success/目录下创建数据
#   3、对上述目录创建快照策略（参数输入正确，预期成功）
#   4、执行update_snapshot_strategy更新快照策略，输入参数正确（预期更新成功）
...
def case():
    ...
    snap_strategy_id = stdout['id']

    rc, stdout = snap_common.update_snapstrategy(id=snap_strategy_id,
                                                 period_type='BY_DAY',
                                                 hours=1,
                                                 minute=1,
                                                 expire_time=180)
    common.judge_rc(rc, 0, 'snapshot strategy %s update failed !!!' % snap_strategy_name)
