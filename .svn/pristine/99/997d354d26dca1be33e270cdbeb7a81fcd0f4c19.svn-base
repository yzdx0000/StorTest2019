...
# date 2018-05-04
# @summary：
# 16-0-4-106       端口限制secure
# @steps:
# case 	1、设置NFS授权客户端的port_constraint为secure；
# pscli --command=add_nfs_export_auth_clients --export_id=1 --name=10.20.104.43
# --permission_level=rw --port_constraint=secure
# case2、设置NFS授权客户端的port_constraint为insecure；
# pscli --command=add_nfs_export_auth_clients --export_id=1 --name=10.20.104.43
# --permission_level=rw --port_constraint=insecure
# case3、验证secure与insecure权限；
# @changelog：
#
#######################################################
import os
...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)  # /mnt/volume1/snap/snap_16_0_4_106
...
#######################################################
# 1.executing_case1
# @function：设置NFS授权客户端的port_constraint为secure；
# @parameter：
#       node_ip：执行cmd命令的节点ip
#       cmd：设置NFS授权客户端的port_constraint为secure；
# @return：
...
def executing_case1():
    ...
    access_zone_name = "access_zone_16_0_4_106_1"
    node = common.Node()
    ...
    global access_zone_id_16_0_4_106_1
    access_zone_id_16_0_4_106_1 = msg1["result"]

    # 2> 启动NAS
    log.info("\t[ enable_nas ]")
    msg2 = nas_common.enable_nas(access_zone_id=access_zone_id_16_0_4_106_1, protocol_types="NFS")
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        common.except_exit('%s enable_nas failed!!!' % node_ip)

    # 3> 查看NAS是否按配置启动
    log.info("\t[ get_access_zones ]")
    check_result3 = nas_common.get_access_zones(ids=access_zone_id_16_0_4_106_1)
    name = check_result3["result"]["access_zones"][0]["name"]
    if name != access_zone_name:
        common.except_exit('%s enable_nas failed!!!' % node_ip)
    global auth_provider_id_16_0_4_106_1
    auth_provider_id_16_0_4_106_1 = check_result3["result"]["access_zones"][0]["auth_provider_id"]
    ...
    #     os.makedirs(prepath)
    global nfs_path
    nfs_path = nas_common.ROOT_DIR + "nfs_dir_16_0_4_106_1"
    global NAS_PATH
    NAS_PATH = get_config.get_one_nas_test_path() + "/nfs_dir_16_0_4_106_1"
    ...
    nfs_export_name = "nfs_exp_test_16_0_4_106_1"
    ...
    cmd = "create_nfs_export --access_zone_id=%s " \
          ...
          % (access_zone_id_16_0_4_106_1,
             nfs_export_name,
             nfs_path, description_nfs)
    check_result1 = nas_common.create_nfs_export(access_zone_id=access_zone_id_16_0_4_106_1,
    ...
    global nfs_export_id_16_0_4_106_1
    nfs_export_id_16_0_4_106_1 = msg1["result"]

    """4> get_ftp_exports"""
    log.info("\t[ case1-4 get_nfs_exports ]")
    cmd = "get_nfs_exports --ids=%s" % nfs_export_id_16_0_4_106_1
    check_result = nas_common.get_nfs_exports(ids=nfs_export_id_16_0_4_106_1)
    ...
    if access_zone_id != access_zone_id_16_0_4_106_1:
    ...
    auth_clients_port_constraint = 'secure'
    cmd = "add_nfs_export_auth_clients --export_id=%s " \
          "--name=%s --permission_level=%s --write_mode=%s --port_constrain=%s" \
          % (nfs_export_id_16_0_4_106_1,
             auth_clients_name,
             auth_clients_permission_level,
             auth_clients_write_mode,
             auth_clients_port_constraint)
    check_result = nas_common.add_nfs_export_auth_clients(export_id=nfs_export_id_16_0_4_106_1,
    ...
    global nfs_export_clients_id_16_0_4_106_1
    nfs_export_clients_id_16_0_4_106_1 = msg["result"][0]

    """7> get_nfs_export_auth_clients"""
    log.info("\t[ case1-7 get_nfs_export_auth_clients ]")
    cmd = "get_nfs_export_auth_clients --export_ids=%s" % nfs_export_id_16_0_4_106_1
    check_result = nas_common.get_nfs_export_auth_clients(export_ids=nfs_export_id_16_0_4_106_1)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    msg = check_result
    if msg["err_msg"] != "" or msg["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    ...
    if id != nfs_export_clients_id_16_0_4_106_1:
        raise Exception('%s id failed!!!' % node_ip)
    if export_id != nfs_export_id_16_0_4_106_1:
    ...
    auth_clients_mount_dir = "/mnt/nfs_dir_16_0_4_106_1"
    cmd = "ssh %s ls /mnt/nfs_dir_16_0_4_106_1" % auth_clients_name
    rc, stdout, stderr = shell.ssh(node_ip, cmd)
    if stdout == "":
        cmd = "ssh %s umount -l /mnt/nfs_dir_16_0_4_106_1" % auth_clients_name
        rc, stdout, stderr = shell.ssh(node_ip, cmd)
        cmd = "ssh %s rm -rf /mnt/nfs_dir_16_0_4_106_1" % auth_clients_name
        rc, stdout, stderr = shell.ssh(node_ip, cmd)
        if rc != 0:
            log.error("WARNING: \ncmd = %s\nstdout = %s\nstderr = %s" % (cmd, stdout, stderr))
...
#######################################################
# 2.executing_case2
# @function：设置NFS授权客户端的port_constraint为insecure；
# @parameter：
#       node_ip：执行cmd命令的节点ip
#       cmd：设置NFS授权客户端的port_constraint为insecure；
# @return：
#       check_result —— 执行create命令的字典格式返回值
# @steps:
# 1> 检查目录并creat file
...
#######################################################
def executing_case2():
    global access_zone_id_16_0_4_106_2
    access_zone_id_16_0_4_106_2 = access_zone_id_16_0_4_106_1
    ...
    #     os.makedirs(prepath)
    global nfs_path
    nfs_path = nas_common.ROOT_DIR + "nfs_dir_16_0_4_106_2"
    global NAS_PATH_2
    NAS_PATH_2 = get_config.get_one_nas_test_path() + "/nfs_dir_16_0_4_106_2"
    cmd = "create_file --path=%s --posix_permission=rwxr-xr-x" % nfs_path
    check_result6 = nas_common.create_file(path=nfs_path, posix_permission="rwxr-xr-x")
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result6))
    msg6 = check_result6
    ...
    global nfs_export_name
    nfs_export_name = "nfs_exp_test_16_0_4_106_2"
    global description_nfs
    description_nfs = 'old_export_description'
    cmd = "create_nfs_export --access_zone_id=%s " \
          "--export_name=%s --export_path=%s --description=%s" \
          % (access_zone_id_16_0_4_106_2,
             nfs_export_name,
             nfs_path, description_nfs)
    check_result1 = nas_common.create_nfs_export(access_zone_id=access_zone_id_16_0_4_106_2,
                                                 export_name=nfs_export_name,
                                                 export_path=nfs_path,
                                                 description=description_nfs)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
    ...
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
        raise Exception('%s create_nfs_export failed!!!' % node_ip)
    global nfs_export_id_16_0_4_106_2
    nfs_export_id_16_0_4_106_2 = msg1["result"]

    """4> get_ftp_exports"""
    log.info("\t[ case2-4 get_nfs_exports ]")
    cmd = "get_nfs_exports --ids=%s" % nfs_export_id_16_0_4_106_2
    check_result = nas_common.get_nfs_exports(ids=nfs_export_id_16_0_4_106_2)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    msg = check_result
    if msg["err_msg"] != "" or msg["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    ...
    export_path = check_result["result"]["exports"][0]["export_path"]
    description = check_result["result"]["exports"][0]["description"]
    if access_zone_id != access_zone_id_16_0_4_106_2:
        raise Exception('%s access_zone_id failed!!!' % node_ip)
    if export_name != nfs_export_name:
        raise Exception('%s export_name failed!!!' % node_ip)
    if export_path != nfs_path:
    ...
    cmd = "add_nfs_export_auth_clients --export_id=%s " \
          "--name=%s --permission_level=%s --write_mode=%s --port_constraint=%s" \
          % (nfs_export_id_16_0_4_106_2,
             auth_clients_name,
             auth_clients_permission_level,
             auth_clients_write_mode,
             auth_clients_port_constraint)
    check_result = nas_common.add_nfs_export_auth_clients(export_id=nfs_export_id_16_0_4_106_2,
                                                          name=auth_clients_name,
                                                          permission_level=auth_clients_permission_level,
                                                          write_mode=auth_clients_write_mode,
                                                          port_constraint=auth_clients_port_constraint)
    ...
    if msg["err_msg"] != "" or msg["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
        raise Exception('%s add_nfs_export_auth_clients failed!!!' % node_ip)
    global nfs_export_clients_id_16_0_4_106_2
    nfs_export_clients_id_16_0_4_106_2 = msg["result"][0]

    """7> get_nfs_export_auth_clients"""
    log.info("\t[ case2-7 get_nfs_export_auth_clients ]")
    cmd = "get_nfs_export_auth_clients --export_ids=%s" % nfs_export_id_16_0_4_106_2
    check_result = nas_common.get_nfs_export_auth_clients(export_ids=nfs_export_id_16_0_4_106_2)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    msg = check_result
    if msg["err_msg"] != "" or msg["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    ...
    if anonuid != 65534:
        raise Exception('%s anonuid failed!!!' % node_ip)
    if id != nfs_export_clients_id_16_0_4_106_2:
        raise Exception('%s id failed!!!' % node_ip)
    if export_id != nfs_export_id_16_0_4_106_2:
        raise Exception('%s export_id failed!!!' % node_ip)
    if name != auth_clients_name:
        raise Exception('%s name failed!!!' % node_ip)
    if permission_constraint != "root_squash":
    ...
    """10-1> 客户端创建mount路径"""
    global auth_clients_mount_dir
    auth_clients_mount_dir = "/mnt/nfs_dir_16_0_4_106_2"
    cmd = "ssh %s ls /mnt/nfs_dir_16_0_4_106_2" % auth_clients_name
    rc, stdout, stderr = shell.ssh(node_ip, cmd)
    if stdout == "":
        cmd = "ssh %s umount -l /mnt/nfs_dir_16_0_4_106_2" % auth_clients_name
        rc, stdout, stderr = shell.ssh(node_ip, cmd)
        cmd = "ssh %s rm -rf /mnt/nfs_dir_16_0_4_106_2" % auth_clients_name
        rc, stdout, stderr = shell.ssh(node_ip, cmd)
        if rc != 0:
            log.error("WARNING: \ncmd = %s\nstdout = %s\nstderr = %s" % (cmd, stdout, stderr))
            raise Exception('%s rm -rf client file failed!!!' % node_ip)
...
#######################################################
# 3.executing_case3
# @function：验证secure与insecure权限；
# @parameter：
#       node_ip：执行cmd命令的节点ip
#       cmd：验证secure与insecure权限；
# @return：
#       check_result —— 执行create命令的字典格式返回值
# @steps:
...
def executing_case3():
    """1> 测试secure权限"""
    auth_clients_name = nas_common.NFS_1_CLIENT_IP
    log.info("\t[ case3-1 测试secure权限 ]")
    cmd1 = "file1=`grep %s /etc/exports | head -n 1` " % auth_clients_name + \
           "&& file2=`echo ${file1#*async,}` && echo ${file2%,root_squash*}"
    rc, stdout, stderr = shell.ssh(node_ip, cmd1)
    print stdout
    if rc != 0 or stdout != 'secure\n':
        log.info("rc = %s" % (rc))
        log.info("WARNING: \ncmd = %s\nstdout = %s\nstderr = %s" % (cmd1, stdout, stderr))
        raise Exception('%s 测试secure权限 failed!!!' % node_ip)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd1, stdout))

    """2> 测试insecure权限"""
    log.info("\t[ case3-2 测试insecure权限 ]")
    cmd2 = "file1=`grep %s /etc/exports | tail -n 1` " % auth_clients_name + \
           "&& file2=`echo ${file1#*async,}` && echo ${file2%,root_squash*}"
    rc, stdout, stderr = shell.ssh(node_ip, cmd2)
    print stdout
    if rc != 0 or stdout != "insecure\n":
        log.info("rc = %s" % (rc))
        log.info("WARNING: \ncmd = %s\nstdout = %s\nstderr = %s" % (cmd2, stdout, stderr))
        raise Exception('%s 测试insecure权限 failed!!!' % node_ip)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd2, stdout))

    """3> 客户端umount共享路径"""
    log.info("\t[ case3-3-1 客户端umount共享路径 1]")
    cmd1 = "umount -fl /mnt/nfs_dir_16_0_4_106_1"
    ...
    log.info("\t[ case3-3-2 客户端umount共享路径 2]")
    cmd1 = "umount -fl /mnt/nfs_dir_16_0_4_106_2"
    rc, stdout, stderr = shell.ssh(auth_clients_name, cmd1)
    print stdout
    if rc != 0:
        log.info("rc = %s" % (rc))
    ...
    """4> 客户端删除mount路径"""
    log.info("\t[ case3-4-1 客户端删除mount路径 1]")
    cmd1 = "rm -rf /mnt/nfs_dir_16_0_4_106_1"
    rc, stdout, stderr = shell.ssh(auth_clients_name, cmd1)
    print stdout
    if rc != 0:
        log.info("rc = %s" % (rc))
    ...
    log.info("\t[ case3-4-2 客户端删除mount路径 2]")
    cmd1 = "rm -rf /mnt/nfs_dir_16_0_4_106_2"
    rc, stdout, stderr = shell.ssh(auth_clients_name, cmd1)
    print stdout
    if rc != 0:
        log.info("rc = %s" % (rc))
