...
def case():
    ...
    log.info("6> 删除桶，同时设置账户所有者账户桶下对象的ACL权限")
    ACL_TYPE = "FULL_CONTROL"
    threads = []
    thread1 = MyThread(s3_common.del_bucket, args=(bucket_name, certificate_id, ))
    thread2 = MyThread(s3_common.set_object_acl, args=(bucket_name, object_name, certificate_id, account_id,
