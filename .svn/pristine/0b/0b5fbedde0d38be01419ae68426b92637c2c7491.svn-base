...
def disk_error1(arg=1):
    ...
    time.sleep(30)
    env_manage_repair_test.Reliable_osan.remove_disk(node_ip=node_ip1, disk_id=disk_ids1[0], disk_usage="SHARED")


def disk_error2(arg=1):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    time.sleep(30)
    env_manage_repair_test.Reliable_osan.remove_disk(node_ip=node_ip1, disk_id=disk_ids2[0], disk_usage="DATA")


def node_error(arg=1):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    env_manage_repair_test.Lun_osan.remove_node(node_ip2, node_id)
...
def case():
    ...
    decorator_func.multi_threads(run_vdb1, run_vdb2, node_error,disk_error1,disk_error2)
