...
def case():
    ...
    '''将用户所在的用户组id修改为非法用户组id，预计修改失败'''
    fault_group_id = group_id + 1000
    nas_common.update_auth_user(user_id=user_id,
                                password=111111,
                                primary_group_id=fault_group_id)

    '''2> 执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)               # 修改本地用户失败，延时
    code = '0x02030504'                # 修改本地用户失败对应编号
    description = 'update_auth_user_failed'
