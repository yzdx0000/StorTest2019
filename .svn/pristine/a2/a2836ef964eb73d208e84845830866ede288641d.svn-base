...
# date 2018-05-24
# @summary：
#    对目录创建配额（逻辑空间，硬阈值），配额限制小于初始文件占用总空间，创建快照并查看回滚结果
# @steps:
#    1、在/mnt/volume1/snap/下创建总容量为1G的文件；
#    2、对上述目录创建logical_hard_threshold，配额限制为500M；
#    3、对目录创建快照；
#    4、删除目录下容量大小为200M的文件；
#    5、在目录/mnt/volume1/snap/下尝试写入300M的文件；
#    6、执行revert并查看回滚结果；
#    7、删除配额；
#
# @changelog：
...
SNAP_TRUE_PATH = os.path.join(snap_common.SNAP_PATH, FILE_NAME)             # /mnt/volume1/snap/snap_13_0_8_6
CREATE_SNAP_PATH = os.path.join(snap_common.SNAP_PATH_BASENAME, FILE_NAME)  # /snap/snap_13_0_8_6
...
def case():
    ...
    # 执行完创建文件的操作后，检查目录下文件总空间;预期600M文件无法写入，目录下文件占用的总空间为
    common.run_command(snap_common.CLIENT_IP_1, cmd)
