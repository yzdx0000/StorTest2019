...
def case():
    ...
    log.info("3> 为账户%s上传5个桶" % owner_account_name)
    ...
    log.info("4> 向账户%s的桶%s中上传对象" % (owner_account_name, bucket_name))
    ...
    log.info("5> 为账户%s上传5个桶" % dst_account_name)
    dst_bucket_name_lst = []
    for j in range(5):
    ...
    log.info("7> 账户%s赋予账户%s对桶%s的READ权限" % (owner_account_name, dst_account_name, bucket_name))
    rc, stdout = s3_common.set_bucket_acl(bucket_name, owner_certificate_id, owner_account_id, owner_account_email,
                                          dst_account_id, dst_account_email, "READ", exe_node_ip=None)
    judge_result(rc, FILE_NAME)
    common.judge_rc(rc, 0, "账户%s赋予账户%s对桶%s的READ权限时"
                           "失败!!!" % (owner_account_name, dst_account_name, bucket_name))

    log.info("8> 验证账户%s对桶%s的读权限是否设置成功" % (dst_account_name,bucket_name))
    rc, stdout = s3_common.get_all_object_in_bucket(bucket_name, dst_certificate_id, exe_node_ip=None)
    judge_result(rc, FILE_NAME)
    common.judge_rc(rc, 0, "验证账户%s对桶%s的读权限时失败" % (dst_account_name, bucket_name))

    log.info("9> 验证账户%s对桶%s无写权限" % (dst_account_name, bucket_name))
    object_name = bucket_name + 'yanzheng_obj1'
    rc, stdout = s3_common.add_object(bucket_name, object_name, filepath, dst_certificate_id, exe_node_ip=None)
    if '403 Forbidden' not in stdout:
        result.result(FILE_NAME, -1)
        common.judge_rc(-1, 0, "验证账户%s对桶%s无写权限时,返回码不是403" % (dst_account_name, bucket_name))
