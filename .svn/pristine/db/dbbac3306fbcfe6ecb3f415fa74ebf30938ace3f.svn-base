...
def case():
    ...
    '''1>启用nas服务'''
    '''令访问分区id不合法，预计启用nas失败'''
    fault_access_zone_id = access_zone_id + 1000
    nas_common.enable_nas(fault_access_zone_id)
    '''2>执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s'% delay_time)
    time.sleep(delay_time)  # 启用nas失败后，延时一段时间
    code = '0x02031502'     # 启用nas失败的事件码
    description = 'enable_nas_failed'
    '''查看启用nas失败的事件码是否在事件列表中'''
