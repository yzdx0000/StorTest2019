...
def case():
    ...
    os.mkdir(test_path)
    file_path = os.path.join(test_path, 'file_500m')
    rc, stdout = s3_common.create_file_m(file_path, 5*100)
    ...
    file_down_path = os.path.join("/tmp", object_name_1 + 'down1')
    rc, stdout = s3_common.download_object(bucket_name, object_name_1, file_down_path, certificate_id)
    common.judge_rc(rc, 0, "download object %s failed!!!" % object_name_1)
    """获取原始文件MD5值"""
    ...
    log.info("7> 创建5G文件并上传对象")
    """创建5G文件"""
    file_path = '/tmp/s3testfile2'
    rc, stdout = s3_common.create_file_m(file_path, 5 * 1024)
    common.judge_rc(rc, 0, "create file %s failed!!!" % file_path)
    ...
    log.info("8> 验证上传对象一致性")
    file_down_path = os.path.join("/tmp", object_name_2 + 'down2')
    rc, stdout = s3_common.download_object(bucket_name, object_name_2, file_down_path, certificate_id)
    common.judge_rc(rc, 0, "download object %s failed!!!" % object_name_2)
    """获取原始文件MD5值"""
    rc, base_file_md5 = s3_common.get_file_md5(file_path)
    common.judge_rc(rc, 0, "get file %s failed!!!" % (file_path))
    """获取上传文件的MD5值"""
