...
def case():
    ...
    log.info("step:2.删除 lun 过程中节点关机")
    threads = []
    ...
    for t in threads:
        t.join()
    log.info("step:3.%s 节点开机 节点类型%s,节点ID or IPMI IP: %s" % (node_ip1, os_types[0], infos[0]))
...
def main():
    ...
    setup()
    case()
    log.info("step:clean running environment")
    env_manage.clean_test_env()
