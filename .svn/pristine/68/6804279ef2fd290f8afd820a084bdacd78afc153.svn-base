...
# date 2018-11-17
# @summary：
#    posix客户端权限测试：u1属于多个组，检测文件权限；原P200测试用例23-0-3-2。
# @steps:
...
def case():
    ...
    log.info("5> 测试主体：%s属于多个组，测试文件权限" % user_name)
    log.info("5-1> 创建测试文件并设置权限555")
    testfile = 'file_' + FILE_NAME
    cmd = "ssh %s \"su %s -c 'cd %s;touch %s;chmod 555 %s'\" " % (
        client_ip, user_name, posix_file_location, testfile, testfile)
    ...
    log.info("5-3> 用户%s向%s中写入数据" % (user_name, testfile))
    echo_content = "hahaha"
    cmd = "ssh %s \"su %s -c 'cd %s;echo %s >> %s'\"" % (client_ip, user_name, posix_file_location, echo_content, testfile)
    rc = common.command(cmd)
    common.judge_rc_unequal(rc, 0, "echo >> testfile success, the acl dosen't take effect")
