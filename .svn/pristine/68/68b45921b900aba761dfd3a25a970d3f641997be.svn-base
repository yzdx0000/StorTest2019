...
def node_off(node_ip):
    os_type = env_manage.get_os_type(node_ip)
    info = env_manage.down_node(node_ip, os_type, "init 0")
...
def case():
    ...
    threads = []
    t1 = threading.Thread(target=node_off, args=(node_ip1,))
    threads.append(t1)
    t2 = threading.Thread(target=env_manage.clean_lun, args=(node_ip3,))
    threads.append(t2)
    t3 = threading.Thread(target=net_down, args=(node_ip2, subnet_eth))
    threads.append(t3)
    for t in threads:
        t.start()
    for t in threads:
    ...
    env_manage.com_lh.get_os_status(node_ip3)
