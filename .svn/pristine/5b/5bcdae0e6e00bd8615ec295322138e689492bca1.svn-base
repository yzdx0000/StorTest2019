...
# Date  :2018-9-15
"""
测试内容:副本下，数据重建过程中，删除被重建数据对应的LUN
步骤:
1、在节点A上创建12条LUN，其中LUN1-LUN6对应对齐大小块混合读写，LUN7-LUN12为非对齐大小块混合读写；
2、使用2个主机，主机1映射LUN1-LUN6，主机2映射LUN7-LUN12；
3、在主机1上运行vdbench -f mix-R-Align.conf -jn；在主机2上运行vdbench -f mix-R.conf;
4、业务过程中手动删除lun7-lun12对应存储池二块磁盘；
5、数据重建过程中将lun7-lun12删除
6、主机1端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性
7、数据修复完成系统恢复正常后，比较内部数据一致性。

检查项:
1、步骤4，手动删除成功，业务正常，数据重建启动
2、步骤5、lun7-lun12容量能完全释放，系统业务正常
3、步骤6、数据校验一致
4、步骤7、系统恢复后数据继续重建和数据修复直到完成，内部数据校验一致
"""
# testlink case: 1000-33348
import os
import sys
import utils_path
import common2
...
node_id = env_manage_repair_test.nodeids[0]
node_id2 = env_manage_repair_test.nodeids[1]
...
def lun_error(arg=3):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    time.sleep(int(cp("wait_time", "remove_disk")) + 10)
    lun_ids = []
    lun_map_ids = []
    for i in ["LUN7", "LUN8", "LUN9", "LUN10", "LUN11", "LUN12"]:
        lun_ids.append(
            env_manage_repair_test.Lun_osan.gen_dict(s_ip=node_ip1, command="get_luns", arg1="name", arg2="id",
                                                     arg3="luns", target=i))  # 根据lun名获取对应lun  id
    for j in ["LUN7", "LUN8", "LUN9", "LUN10", "LUN11", "LUN12"]:
        lun_map_ids.append(
            env_manage_repair_test.Lun_osan.gen_dict1(s_ip=node_ip1, command="get_luns", arg1="name",
                                                      arg2="lunMapIds",
                                                      arg3="luns", target=j))  # 根据lun名获取对应lun_map id

    for lun_map_id in lun_map_ids:
        env_manage_repair_test.Lun_osan.delete_lun_map(s_ip=node_ip1, map_id=lun_map_id)

    for lun_id in lun_ids:
        env_manage_repair_test.Lun_osan.delete_lun(s_ip=node_ip1, lun_id=lun_id)
...
def os_error(arg=3):
    ...
    ReliableTest.run_kill_process(node_ip1, "oSan")
...
def case():
    ...
    iscsi_login()
    log.info(
        "step2:在主机1上运行vdbench -f mix-R-Align.conf -jn；在主机2上运行vdbench -f mix-R.conf,业务过程中，业务过程中手动删除一块数据盘 ...")
    decorator_func.multi_threads(run_vdb1, run_vdb2, disk_error, lun_error)
    log.info("step3:数据重建时，重建目标节点数据网故障,主机端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性 ...")
    ...
    luns = []
    for i in range(7, 13):
        lun_id = env_manage_repair_test.Lun_osan.create_lun(s_ip=node_ip1, lun_type="THIN",
                                                            lun_name='LUN{}'.format(i),
                                                            stor_pool_id=2, acc_zone_id=1, stripe_width=3)
        luns.append(lun_id)
    for i in luns:
        if i % 2 == 0:
            env_manage_repair_test.Lun_osan.map_lun(s_ip=node_ip1, lun_ids=i, hg_id=1)
        else:
            env_manage_repair_test.Lun_osan.map_lun(s_ip=node_ip1, lun_ids=i, hg_id=2)
    env_manage_repair_test.co2_osan.run_vdb(client_ip=client_ip1, vdb_xml=mix_R_Align, jn_jro='jro',
                                            output=node_ip1)
