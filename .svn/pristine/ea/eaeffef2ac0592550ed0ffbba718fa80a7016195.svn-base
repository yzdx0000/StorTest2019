...
# date 2018-11-26
# @summary：
# x-x-x-x     用户/用户组，添加认证服务器，反复enable,disable NAS
# @steps:
...
def executing_case1():
    ...
    for i in range(1, times + 1):
        ...
        log.info("\t[2 enable_nas %s/%s time]" % (i, times))
        ...
        """获取用户/用户组"""
        log.info("\t[2 创建访问分区后获取用户/用户组 %s/%s time]" % (i, times))
        ...
        """初步验证用户/用户组"""
        log.info("\t[2 初步验证用户/用户组 %s/%s time]" % (i, times))
        ...
        """3. disable NAS"""
        log.info("\t[3 disable NAS  %s/%s time]" % (i, times))
        ...
        log.info("\t[3 disable NAS 后获取用户/用户组 ]")
