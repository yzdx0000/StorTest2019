...
6、数据修复到free节点，修复完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性
7、比较存储内部数据一致性。
'''
# testlink case: 1000-32720
import os
import sys
import utils_path
import common2
...
def up_down():
    time.sleep(10)
    log.info("4、在步骤3中的业务运行过程中，将节点A和节点B数据网同时断开；")
...
def vdb_jn():
    log.info("3、在主机1上运行vdbench -f mix-R-Align.conf -jn；在主机2上运行vdbench -f mix-R.conf;")
    ...
    log.info("5、主机1上业务完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；")
    log.info("Run vdbench with jro on %s." % (client_ips[0]))
    osan.run_vdb(client_ips[0], mix_R_align, output=deploy_ips[0], jn_jro="jro")
    log.info("6、数据修复到free节点，修复完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性")
    log.info("Run vdbench with jro on %s." % (client_ips[0]))
    osan.run_vdb(client_ips[0], mix_R_align, output=deploy_ips[0], jn_jro="jro")
