...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将从oPmgr进程异常；
            5、检查从oPmgr进程状态
            6、主机1上业务完成后，在主机1上执行vdbench -f OLTP.conf -jro比较一致性；
            8、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32966
import os
import sys
import commands
import threading
...
log.info("Get fault process: follow oRole")

def up_down():

    time.sleep(100)
    log.info("Step4:检查节点主oRole进程状态,节点: %s." %(m_oRole_ip))
    reliable.check_process_stat(m_oRole_ip,'oRole')
    log.info("Step4:将故障节点从oRole进程杀死,节点: %s." %(fault_node_ip))
    ReliableTest.run_kill_process(fault_node_ip,'oRole')

    time.sleep(200)
    log.info("Step5:检查节点主oRole进程状态,节点: %s." %(m_oRole_ip))
    reliable.check_process_stat(m_oRole_ip,'oRole')
    log.info("Step5:检查故障节点从oRole进程状态,节点: %s." % (fault_node_ip))
