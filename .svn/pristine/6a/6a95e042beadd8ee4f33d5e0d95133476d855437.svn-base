...
# date 2018-07-26
# @summary：
#    创建快照策略失败（等待get_events接口完成）
# @steps:
#   1、部署3节点集群环境
#   2、在/mnt/volume1/snap/snap_create_success/目录下创建数据
#   3、对上述目录重复创建名称相同的快照策略（预期第一个成功，后续失败）
...
def case():
    ...
    for i in range(3):
        rc, stdout = snap_common.create_snapstrategy(snap_strategy_name, path, 'BY_DAY', hours=1, minute=1,
                                                     expire_time=0)
        if rc != 0:
            log.info('snapshot %s create failed !!!' % snap_strategy_name)
        log.info('waiting for 10s')
        time.sleep(10)
