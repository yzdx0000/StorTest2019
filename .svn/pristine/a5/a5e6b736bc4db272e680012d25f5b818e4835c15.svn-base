...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：16-0-x-x 开启home目录，OS/2 Style Extended Attributes 打开
#######################################################
...
def executing_case():
    """
        1、设置home目录后，再设置enable_os2style_ex_attrs参数；
        pscli --command=update_smb_global_config --id=1 --home_dir="parastor:/homes" --enable_os2style_ex_attrs=true
        pscli --command=update_smb_global_config --id=1 --home_dir="parastor:/homes" --enable_os2style_ex_attrs=false
    :return:
    """
    ...
    create_file_path = nas_common.ROOT_DIR + "homes-nas_16-0-4-65_dir"
    ...
    if allow_create_ntfs_acl != True \
            ...
            or enable_ntfs_acl != True \
            or enable_oplocks != True \
            or enable_os2style_ex_attrs != True \
            or enable_send_ntlmv2 != False \
            or home_dir != create_file_path:
        log.error(("%s Failed") % FILE_NAME)
        raise Exception(("%s Failed") % FILE_NAME)

    # 环境恢复，enable_os2style_ex_attrs="false"
    ...
    check_result6 = nas_common.get_smb_global_configs()
    if check_result6["detail_err_msg"] != "":
        log.error(("%s Failed") % FILE_NAME)
        raise Exception(("%s Failed") % FILE_NAME)
    config = check_result6["result"]["configs"][0]  # 只update第一个
    allow_create_ntfs_acl = config["allow_create_ntfs_acl"]
    enable_alternative_datasource = config["enable_alternative_datasource"]
    enable_change_notify = config["enable_change_notify"]
    enable_dos_attributes = config["enable_dos_attributes"]
