...
#   执行：
#           1>删除本地用户组失败
#           2>执行get_events查看删除本地用户组失败的结果显示
# @changelog：
####################################################################################
...
def case():
    ...
    '''1> 删除本地用户组'''
    '''使删除的组id为无效的组id，预计删除失败'''
    fault_group_id = group_id + 1000
    nas_common.delete_auth_groups(fault_group_id)

    '''2> 执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)  # 删除本地用户组失败，延时

    code = '0x02030510'   # 删除本地用户组失败对应编号
    description = 'delete_auth_group_failed'

    '''删除本地用户组失败事件码是否在事件列表中'''
