...
4、在步骤3中的业务运行过程中，将oPara守护进程异常是oPara进程异常；
5、检查oPara进程状态
6、主机1上业务完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-33001
import os,sys
import commands
import threading
import random
...
# 选择oSan进程进行异常操作
log.info("Get fault process:oMgcd & oPara")

def up_down():
    time.sleep(100)
    log.info("Step4:将该节点oMgcd守护进程杀死,节点: %s." % (fault_node_ip))
    ReliableTest.run_kill_process(fault_node_ip, 'oMgcd',)
    log.info("Step4:将节点oPara进程杀死 : %s." %(fault_node_ip))
    ReliableTest.run_kill_process(fault_node_ip, 'oPara')
    time.sleep(100)
    log.info("Step5:检查故障节点oMgcd进程状态 : %s." % (fault_node_ip))
    reliable.check_process_stat(fault_node_ip, 'oMgcd')
    log.info("Step5:检查故障节点oPara进程状态 : %s." % (fault_node_ip))
