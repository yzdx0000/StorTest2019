...
#   执行：
#           1>修改FTP共享
#           2>执行get_events查看修改FTP共享失败的结果显示
#           3>清理工作
# @changelog：
####################################################################################
...
ftp_path = nas_common.ROOT_DIR + "FTP_" + FILE_NAME  # parastor:/nas/FTP_events_5_3_3_72
...
def case():
    ...
    '''1>修改FTP共享目录'''
    '''令ftp导出目录id不合法，预计修改失败'''
    fault_ftp_export_id = ftp_export_id + 1000
    nas_common.update_ftp_export(export_id=fault_ftp_export_id)
    '''2> 执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)  # 修改FTP共享失败，延时
    code = '0x02030804'   # 修改FTP共享失败对应编号
    description = 'update_ftp_export_failed'
    '''修改FTP共享失败事件码是否在事件列表中'''
