...
def case():
    ...
    log.info('5> 父目录再创建1100M文件，创建成功，过期后创建失败')
    if LARGE_BS:
        quota_common.creating_files(quota_common.CLIENT_IP_3, dir0, 1, 1100, 'c', quota_id=quota_id_lst[0])
    else:
        quota_common.creating_files(quota_common.CLIENT_IP_3, dir0, 1100, 1, 'c', quota_id=quota_id_lst[0])
    ...
    if LARGE_BS:
        quota_common.creating_files(quota_common.CLIENT_IP_2, dir0, 1, 100, 'e')
    else:
        quota_common.creating_files(quota_common.CLIENT_IP_2, dir0, 100, 1, 'e')
    dir_size = quota_common.dir_total_file_size(quota_common.CLIENT_IP_2, dir0)
    ...
    if LARGE_BS:
        quota_common.creating_files(quota_common.CLIENT_IP_3, dir1_1, 1, 200, 'f')
    else:
        quota_common.creating_files(quota_common.CLIENT_IP_3, dir1_1, 200, 1, 'f')
    dir_size = quota_common.dir_total_file_size(quota_common.CLIENT_IP_3, dir1_1)
