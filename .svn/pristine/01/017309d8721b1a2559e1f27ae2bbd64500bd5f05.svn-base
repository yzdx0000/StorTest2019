...
:Author: wuyq
:Description:
1、创建VIP地址池，配置2倍节点个数的VIP，IP切换均衡策略使用轮询，自动均衡策略设置为自动
2、查看自动均衡策略设置
3、创建LUN和对应的映射，并在主机端登录并下发读写业务
4、故障带业务的节点掉电，1分钟后恢复
5、自动均衡的过程中，将VPI迁移的源节点故障，1分钟后恢复
...
def up_down():
    ...
    max_vips = max(vips)
    node_index = vips.index(max_vips)
    fault_ip1 = deploy_ips[node_index]
    count = 0
    while osan.check_vip_state() != 1:
        if count == 10:
            break
        log.info("等待开始自动均衡.")
        count += 1
    ...
    log.info("Step5:自动均衡过程中,将源节点的数据网故障")
    fault_id1 = breakdown.disk().get_node_id_by_ip(n_ip=fault_ip1)
    data_eth_list = error.get_data_eth(fault_id1)

    log.info("将VIP均衡的源节点数据网故障,节点:%s" % fault_ip1)
    threads = []
    for eth in data_eth_list:
        threads.append(threading.Thread(target=reliable.network_test, args=(fault_ip1, eth, 'down')))
    for thread in threads:
        thread.start()
    for thread in threads:
        thread.join()
    log.info("1min后将故障节点数据网恢复,节点:%s" % fault_ip1)
    time.sleep(60)
    for eth in data_eth_list:
        reliable.network_test(fault_ip1, eth, 'up')
    ...
    vips = [len(osan.get_vips_by_node(x, 1)) for x in deploy_ips]
    if len(set(vips)) == 1 and vips[0] == 4:
...
def vdb_jn():
    mix_R_Align = com2.gen_vdb_xml(lun=osan.ls_scsi_dev(client_ips[0]),
                                   ...
                                   seekpct=0,
                                   rdpct=0)
    log.info("Step4:在主机上运行vdbench -f mix-R-Align.conf -jn.")
    com2.run_vdb(client_ips[0], mix_R_Align, output=deploy_ips[0], jn_jro="jn", time=900)
    log.info("Step5:主机上业务完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性.")
