...
def case():
    ...
    svip = osan.get_option_single(s_ip=node_ip1,
                                  ...
                                  argv2="svip",
                                  argv1=sub_id)
    new_ip1 = osan.check_ping_ip(svip, 1)
    osan.add_vip_address_pool(s_ip=node_ip1, subnet_id=sub_id,
                              domain_name="{}1".format(cp("add_vip_address_pool", "domain_name")),
                              vip=new_ip1)
    new_ip2 = osan.check_ping_ip(svip, 1)
    osan.add_vip_address_pool(s_ip=node_ip1, subnet_id=sub_id,
                              domain_name="{}2".format(cp("add_vip_address_pool", "domain_name")),
                              vip=new_ip2)
    new_ip3 = osan.check_ping_ip(svip, 1)
    osan.add_vip_address_pool(s_ip=node_ip1, subnet_id=sub_id,
                              domain_name="{}3".format(cp("add_vip_address_pool", "domain_name")),
                              vip=new_ip3)
    new_ip4 = osan.check_ping_ip(svip, 1)
    osan.add_vip_address_pool(s_ip=node_ip1, subnet_id=sub_id,
                              domain_name="{}4".format(cp("add_vip_address_pool", "domain_name")),
                              vip=new_ip4)
    new_ip5 = osan.check_ping_ip(svip, 1)

    msg = osan.add_vip_address_pool(s_ip=node_ip1, subnet_id=sub_id,
                                    domain_name="{}5".format(cp("add_vip_address_pool", "domain_name")),
                                    vip=new_ip5, need_judge=1)
