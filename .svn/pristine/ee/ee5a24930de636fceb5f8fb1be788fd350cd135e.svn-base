...
def up_down(pipein):
    time.sleep(10)
    log.info("4、在步骤3中的业务运行过程中，将A节点数据网故障，将节点B所有日志盘拔掉；")
    log.info("Begin down data net %s through %s ." % (fault_io_ip, io_eth_name))
    ReliableTest.run_down_net(fault_io_ip, io_eth_name)
    log.info("On node %s remove disk." % b_ip)
    for diskid in disk_ids:
        Reliable_osan.remove_disk(node_ip=b_ip, disk_id=diskid, disk_usage="DATA")
    time.sleep(30)

    for diskid in disk_ids:
        Reliable_osan.insert_disk(b_ip, disk_id=diskid, disk_usage="DATA")
    # 监控到匿名管道中写入数据后，退出循环
    while True:
