...
4、在步骤3中的业务运行过程中，将节点A/B/C同一个业务网口断开；
5、主机1上业务完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
6、比较存储内部数据一致性。
'''
# testlink case: 1000-32735
import os
import sys
import utils_path
import common2
...
for jid in use_jnl_ids:
    ...
    j_eths, j_extra = error.get_ioip_info(node_id=jid)
    fault_info[jip] = [j_eths, j_extra]
for fault_key in fault_info.keys():
    if len(fault_info[fault_key][0]) < 2:
        fault_nodes_num = 2
...
def up_down(pipein):
    time.sleep(10)
    fnum = 0
    log.info("4、在步骤3中的业务运行过程中，将节点A/B/C同一个数据网口断开；")
