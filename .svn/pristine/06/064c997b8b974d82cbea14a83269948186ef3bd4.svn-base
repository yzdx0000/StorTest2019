...
#   4、执行restart_jobengine，输入参数失败，预期下发失败
#   5、执行get_events进行结果显示检查
#   6、取消上述任务
#
# @changelog：
####################################################################################
...
def case():
    ...
    common.judge_rc(rc, 0, 'pause_jobengine')

    '''4> 执行restart_jobengine，输入参数正确，预期下发失败'''
    rc, stdout = common.restart_jobengine(select_type, 0)
    if 0 != rc:
        log.error('restart jobengine failed !!!')
