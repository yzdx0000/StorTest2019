...
def down_node(nodeid):
    ...
    log.info("5、将节点 %s 和 %s 掉电不恢复." % (nodeip, s_node_ip))
    ...
    node_info_1 = error.down_node(fault_ip=nodeip)
    log.info("Begin down node : %s." % (s_node_ip,))
    node_info_2 = error.down_node(fault_ip=s_node_ip)
    time.sleep(300)
    log.info("Begin up node : %s through %s." % (nodeip, str(node_info_1)))
    error.up_node(node_info=node_info_1)
    log.info("Begin up node : %s through %s." % (s_node_ip, str(node_info_2)))
    error.up_node(node_info=node_info_2)
