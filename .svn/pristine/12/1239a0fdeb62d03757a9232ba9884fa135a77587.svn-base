...
4、在步骤3中的业务运行过程中，将A节点数据网故障，将节点B oJob进程异常；
...
# oRole主进程节点
master_ojob = os_break.get_matid(deploy_ips[0])
master_ojob_ip = node.get_node_ip_by_id(master_ojob)
# 如果主oROle进程和A节点重复那么断数据网oRole也会切换，
# 同一节点就使用断网实现进程和网络故障。如果不在统一节点上就分开执行
if a_ip == master_ojob_ip:
    b_ip = a_ip
else:
    b_ip = master_ojob_ip
log.info(
...
def up_down(pipein):
    ...
    ReliableTest.run_kill_process(b_ip, 'oJob')  # 先杀进程防止在故障一个节点
