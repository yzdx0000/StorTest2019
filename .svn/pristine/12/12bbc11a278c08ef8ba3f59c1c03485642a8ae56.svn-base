...
def executing_case1():
    ...
    access_zone_name = "access_zone_16_0_2_25"
    node = common.Node()
    ids = node.get_nodes_id()
    access_zone_node_id_16_0_2_25 = ','.join(str(p) for p in ids)
    node_name_list = nas_common.get_node_name_list(ids)
    cmd = "create_access_zone"
    msg1 = nas_common.create_access_zone(node_ids=access_zone_node_id_16_0_2_25, name=access_zone_name)
    ...
    check_result3 = nas_common.get_access_zones(ids=access_zone_id)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result3))
    if check_result3["detail_err_msg"].find("cannot be found in resources") == -1 or \
       check_result3["err_msg"] != "ILLEGAL_ARGUMENT" or \
