...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将节点D和节点E网络反复故障；
            5、主机1上业务完成后，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
            6、数据修复过程中，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
            6、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32947
import os
import sys
import threading
import random
...
def up_down():
    ...
    for count in range(5):
        log.info("第 %d 次故障节点A与节点B的数据网" % (count + 1))
        log.info("Step4:将节点A的所有数据网断开,节点: %s." % (fault_node_ip1))
        for fault_eth_name1 in fault_eth_list1:
            reliable.network_test(fault_node_ip1, fault_eth_name1, 'down')
        log.info("Step4:将节点B的所有数据网断开,节点: %s." % (fault_node_ip2))
        for fault_eth_name2 in fault_eth_list2:
            reliable.network_test(fault_node_ip2, fault_eth_name2, 'down')
        time.sleep(30)
        log.info("Step5:将节点A的所有数据网恢复,节点: %s." % (fault_node_ip1))
        for fault_eth_name1 in fault_eth_list1:
            reliable.network_test(fault_node_ip1, fault_eth_name1, 'up')
        log.info("Step5:将节点B的所有数据网恢复,节点: %s." % (fault_node_ip2))
        for fault_eth_name2 in fault_eth_list2:
