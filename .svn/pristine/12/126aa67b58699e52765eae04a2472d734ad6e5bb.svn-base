...
#    6、对已存在的某个快照进行回滚；执行vdbench 02脚本对目录/mnt/volume1/snap/snap_repetitive_test/data_dir/进行校验；
...
#    10、反复执行修改数据、创建快照、快照回滚、校验数据的过程；
#    11、当快照数量n大于10，修改数据、创建快照（n+1），之后随机删除一个快照并随机选择一个现存快照回滚后进行校验；
#
# @changelog：
####################################################################################
...
def loop_data_snap(snap_name, snap_delete_flag=False):
    ...
    obj_vdbench.run_write_jn(DATA_DIR, jnl_path, snap_common.CLIENT_IP_1)
    ...
    for snapshot in snapshots:
        exist_snap_name = snapshot['name']
        exist_snap_id = snapshot['id']
        snap_name_set.append(exist_snap_name)
        snap_id_set.append(exist_snap_id)
        snap_name_id.setdefault(exist_snap_name, exist_snap_id)

        # 随机选择一个快照并进行校验
    check_snap_name = random.choice(snap_name_set)
    check_snap_id = snap_name_id[check_snap_name]
    check_jnl_name = 'jnl_' + check_snap_name.split('_')[-1]
    check_jnl_path = os.path.join(SNAP_TRUE_PATH, check_jnl_name)

    # 对随机选择的快照进行回滚
    snap_common.revert_snapshot_by_name(check_snap_name)
    snap_common.check_revert_finished(check_snap_id)

    # 在隐藏目录下校验快照内容
    obj_vdbench.run_check(DATA_DIR, check_jnl_path, snap_common.CLIENT_IP_1)
    return
