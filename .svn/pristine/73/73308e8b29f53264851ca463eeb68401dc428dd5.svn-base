...
def network_test(ipaddr):
    subnet_eth = env_manage.com_lh.get_vip_eth_name(ipaddr)
    env_manage.com_lh.net_flash_test(ipaddr, subnet_eth)
...
def case():
    ...
    log.info("step:3.创修改逻辑卷过程中网卡闪断")
    threads = []
    t1 = threading.Thread(target=env_manage.clean_lun_map, args=(node_ip2,))
    threads.append(t1)
    t2 = threading.Thread(target=network_test, args=(node_ip1,))
    threads.append(t2)
    for t in threads:
        t.start()
    for t in threads:
