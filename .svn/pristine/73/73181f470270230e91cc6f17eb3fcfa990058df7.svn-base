...
def net_pro_test(net_ip, net_name):
    log.info("kill 节点%s 的%s进程" % (pro_ips[0], "oRole"))
...
def case():
    ...
    env_manage.create_lun(ips=create_lun_ip[0])
    log.info("step:2.删除逻辑卷过程中进程和网卡异常")
    threads = []
    t1 = threading.Thread(target=net_pro_test, args=(test_net_ip[0], subnet_eth))
    threads.append(t1)
    t2 = threading.Thread(target=env_manage.clean_lun(ips=create_lun_ip[0], ))
    threads.append(t2)
    for t in threads:
        t.start()
    for t in threads:
