...
thick_lun_size = int(
    (storage_pool_size - int(cp("reserve", "thin_lun_reserve"))) / float(cp("reserve", "multiple")))

thin_lun_size = 2 * storage_pool_size
...
def case():
    ...
    lun_id1 = env_manage_lun_manage.osan.create_lun(s_ip=node_ip1, total_bytes=thick_lun_size, lun_type="THICK",
                                                    lun_name='LUN1', stor_pool_id=storage_pool_id, acc_zone_id=az_id)
    lun_id2 = env_manage_lun_manage.osan.create_lun(s_ip=node_ip1, total_bytes=thin_lun_size, lun_type="THIN",
...
def run_dd(arg=0):
    ...
    msg=env_manage_lun_manage.Reliable_osan.dd_command(client_ip1, luns[0], '4M', '250', need_judge=1)
    env_manage_lun_manage.assert_ins.assertIN("Input/output error", msg)
