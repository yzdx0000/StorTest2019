...
def iscsi_login1():
    ...
    lun1_name = lun1[0].split("/")[-1]
    lun2_name = lun2[0].split("/")[-1]
    cmd = ("ssh %s \"lsblk -b  |grep '%s' |awk '{print \$4}'\"" % (client_ip1, lun1_name))
    rc, ouput = commands.getstatusoutput(cmd)
    thick_lun_range = ouput
    cmd2 = ("ssh %s \"lsblk -b  |grep '%s' |awk '{print \$4}'\"" % (client_ip2, lun2_name))
    rc, ouput2 = commands.getstatusoutput(cmd2)
    thin_lun_range = ouput2
...
def iscsi_login2():
    ...
    rdpct2 = 100
    xfersize1 = cp("vdbench", "unmix_xfersize1")
    lun3 = env_manage_lun_manage.osan.ls_scsi_dev(client_ip2)
    lun3_name = lun3[0].split("/")[-1]
    cmd2 = ("ssh %s \"lsblk -b  |grep '%s' |awk '{print \$4}'\"" % (client_ip1, lun3_name))
    rc, ouput = commands.getstatusoutput(cmd2)
    thin_lun_range = ouput
    min_seq_w3 = env_manage_lun_manage.co2_osan.gen_vdb_xml(
        thread=cp("vdbench", "threads"), lun=lun3, xfersize=xfersize1,
        seekpct=seekpct, rdpct=rdpct1)
...
def run_vdb1(arg=1):
    log.info("write data to each of thick lun")
    env_manage_lun_manage.co2_osan.run_vdb(client_ip=client_ip1, vdb_xml=min_seq_w, output=node_ip1)


def run_vdb2(arg=0):
    log.info("write data to each of thin lun")
    env_manage_lun_manage.co2_osan.run_vdb(client_ip=client_ip2, vdb_xml=min_seq_w2, output=node_ip1)


def case():
    log.info("在访问区配置中，每个节点取1块磁盘创建副本模式存储池，创建存储池pool1，容量为50G；并创建和配置对于的访问分区和vip地址池;创建200条精简卷")
    log.info("创建10条逻辑卷，单卷大小为%s；创建10条精简卷，单卷大小为%s；为系统所有卷配置映射并映射到主机" % (thick_lun_size * 10, thin_lun_size))
    create_thick_lun()
    create_thin_lun()
    map_lun()
    iscsi_login1()
    ...
    log.info("iscsi logout and clean lun map")
    env_manage_lun_manage.clean_lun_map()
    for lun_id1 in env_manage_lun_manage.Reliable_osan.get_thin_lun(node_ip1):
        env_manage_lun_manage.osan.delete_lun(s_ip=node_ip1, lun_id=lun_id1)
    env_manage_lun_manage.osan.reserve_wait(time_wait=240)
    new_lun = []
    log.info("create new thin lun")
    for i in range(thick_num + thin_num, thick_num + thin_num + 2):
        new_lun.append(env_manage_lun_manage.osan.create_lun(s_ip=node_ip1, total_bytes=int(
            thin_lun_total / 2 / float(cp("reserve", "multiple"))),
                                                             lun_type="THICK", lun_name='lun_{}'.format(i),
                                                             stor_pool_id=storage_pool_id, acc_zone_id=az_id))
    log.info("create lun map to each of thick lun")
    for i in env_manage_lun_manage.Reliable_osan.get_thick_lun(node_ip1):
        env_manage_lun_manage.osan.map_lun(s_ip=node_ip1, lun_ids=i, hg_id=hg_id2)
    log.info("iscsi login")
    iscsi_login2()
    log.info("write data to each of new thick lun")
    env_manage_lun_manage.co2_osan.run_vdb(client_ip=client_ip2, vdb_xml=min_seq_w3, output=node_ip1)
