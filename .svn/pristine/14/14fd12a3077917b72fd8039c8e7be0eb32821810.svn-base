...
def case():
    ...
    log.info("3> 创建父目录配额,硬阈值2G 等待配额状态为work")
    ...
    log.info('5> 父目录下创建2200M个文件预期2G')
    if LARGE_BS:
        quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_1, dir0, 1, 2200, 'a',
                                                                quota_common.QUOTA_USER)
    else:
        quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_1, dir0, 2200, 1, 'a',
                                                                quota_common.QUOTA_USER)
    ...
    common.rm_exe(quota_common.CLIENT_IP_3, os.path.join(dir1_1, '*'))
