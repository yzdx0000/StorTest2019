...
# -*- coding:utf-8 -*-
#**********************************************#
# 标题：功能测试，用例编号29-0-17-15
# 使用方法：python 29-0-17-15_wjl.py
# 在http模式下，非法sig获取桶存量信息。
# 作者：王建磊
# 创建时间：2018/09/10
#**********************************************#
import sys
...
def case():
    ...
    log.info("******错误sig获取桶存量信息结果,start.******")
    rc1, storageinfo1 = bucket_get_storageinfo(
        bucket_name, ak, " " + sig + " ")
    rc2, storageinfo2 = bucket_get_storageinfo(
        bucket_name, ak, "QWERdsr1" + sig + "12QWEs")
    rc3, storageinfo3 = bucket_get_storageinfo(
        bucket_name, ak, "@#$%!" + sig + "%#$!")
    log.info("******错误sig获取桶存量信息结果,end.******")

    # 结果判断
    if storageinfo1 is None and storageinfo2 is None and storageinfo3 is None:
        pass
    else:
