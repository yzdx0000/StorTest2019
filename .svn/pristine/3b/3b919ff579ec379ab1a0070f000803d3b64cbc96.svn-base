...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将节点D两个数据网反复故障；
            5、主机1上业务完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
            6、恢复数据网，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32944
import os
import sys
import commands
import threading
...
def up_down():
    log.info("故障点:一个非日志节点的数据网反复故障")
    time.sleep(100)
    log.info("Step4:在步骤3中的业务运行过程中，将节点的两个数据网反复故障: %s." %(fault_node_ip))
    for count in range(5):
        log.info("第 %d 次故障节点的数据网" % (count + 1))
        time.sleep(30)
        log.info("将故障节点数据网A断开，网卡:%s" % fault_eth_name_A)
        reliable.network_test(fault_node_ip, fault_eth_name_A, 'down')
        log.info("将故障节点数据网B断开，网卡:%s" % fault_eth_name_B)
        reliable.network_test(fault_node_ip, fault_eth_name_B, 'down')
        time.sleep(30)
        log.info("将故障节点数据网A恢复，网卡:%s" % fault_eth_name_A)
        reliable.network_test(fault_node_ip, fault_eth_name_A, 'up')
        log.info("将故障节点数据网B恢复，网卡:%s" % fault_eth_name_B)
