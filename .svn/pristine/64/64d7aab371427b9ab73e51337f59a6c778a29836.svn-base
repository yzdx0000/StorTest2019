...
4、在步骤3中的业务运行过程中，将业务节点网配置成bond，将节点A掉电；
5、检查业务迁移到，迁移过程中将接管节点业务网断开;
...
def up_down():
    time.sleep(100)
    log.info("Step2:在业务过程中,将所有业务节点的业务网卡配置为bond.")
    for io_node_ip in node_and_eths.keys():
        io_eth_list = node_and_eths[io_node_ip]
        reliable.bond_conf(io_node_ip, io_eth_list, 'bond0')
    time.sleep(100)
    test_node_ip = random.sample(deploy_ips, 2)
    log.info("Step3:在bond配置完成后,将业务节点A %s 掉电." % test_node_ip[0])
    vm_info1 = error.down_node(test_node_ip[0])
    time.sleep(100)
    log.info("Step4:检查业务迁移,迁移过程中将接管业务节点B %s 业务网断开." % test_node_ip[1])
    io_eth_list = node_and_eths[test_node_ip[1]]
    for test_eth in io_eth_list:
        reliable.network_test(test_node_ip[1], test_eth, 'down')

    time.sleep(100)
    log.info("Step5:将业务节点A %s 重新上电." % test_node_ip[0])
    error.up_node(vm_info1)
    log.info("Step6:将业务节点B %s 业务网卡恢复." % test_node_ip[1])
    for test_eth in io_eth_list:
        reliable.network_test(test_node_ip[1], test_eth, 'up')
    ...
    disk.check_bad_obj()
    log.info("Step7:将bond配置取消,恢复所有业务节点至初始状态.")
    for io_node_ip in node_and_eths.keys():
        io_eth_list = node_and_eths[io_node_ip]
