...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)  # /mnt/volume1/snap/snap_16_0_3_41
...
def executing_case1():
    ...
    node_ip = SYSTEM_IP
    access_zone_name = "access_zone_16_0_3_41"
    node = common.Node()
    ids = node.get_nodes_id()
    print ids
    access_zone_node_id_16_0_3_41 = ','.join(str(p) for p in ids)
    msg1 = nas_common.create_access_zone(node_ids=access_zone_node_id_16_0_3_41, name=access_zone_name)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        common.except_exit('%s create_access_zone failed!!!' % node_ip)
    access_zone_id_16_0_3_41 = msg1["result"]

    """获取访问分区信息"""
    log.info("\t[1-2 get_access_zones ]")
    msg1 = nas_common.get_access_zones(ids=access_zone_id_16_0_3_41)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
    ...
    msg2 = nas_common.enable_nas(access_zone_id=access_zone_id_16_0_3_41)
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        common.except_exit('%s enable_nas failed!!!' % node_ip)

    """2、创建业务子网"""
    log.info("\t[2 create_subnet ]")
    sub_name = "subnet_16_0_3_41"
    ...
    check_result1 = nas_common.create_subnet(access_zone_id=access_zone_id_16_0_3_41,
                                             name=sub_name,
    ...
    subnet_id_16_0_3_41 = check_result1["result"]

    """获取子网信息"""
    log.info("\t[2 get_subnets ]")
    msg1 = nas_common.get_subnets(ids=subnet_id_16_0_3_41)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        common.except_exit('%s get_subnets failed!!!' % node_ip)
    access_zone_id = msg1["result"]["subnets"][0]["access_zone_id"]
    if access_zone_id != access_zone_id_16_0_3_41:
        raise Exception('%s access_zone_id failed %s != %s !!!' % (node_ip, access_zone_id, access_zone_id_16_0_3_41))
    idd = msg1["result"]["subnets"][0]["id"]
    if idd != subnet_id_16_0_3_41:
        raise Exception('%s id failed %s != %s !!!' % (node_ip, idd, subnet_id_16_0_3_41))
    ...
    msg1 = nas_common.add_vip_address_pool(subnet_id=subnet_id_16_0_3_41,
    ...
    vip_address_pool_id_16_0_3_41 = msg1["result"]

    """4、通过pscli --command=get_vip_address_pools查看与配置的信息是否匹配"""
    log.info("\t[4 get_vip_address_pools ]")
    msg1 = nas_common.get_vip_address_pools(ids=vip_address_pool_id_16_0_3_41)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
    ...
    if idd != vip_address_pool_id_16_0_3_41:
        raise Exception('%s id failed %s != %s !!!' % (node_ip, idd, vip_address_pool_id_16_0_3_41))
    ...
    if subnet_id != subnet_id_16_0_3_41:
        raise Exception('%s subnet_id failed %s != %s !!!' % (node_ip, subnet_id, subnet_id_16_0_3_41))
    ...
    vip_2_vip_addresses = nas_common.VIP_ADDRESSES
    ...
    if msg1["err_msg"] != "ILLEGAL_ARGUMENT" or \
       msg1["detail_err_msg"].find("already been used by IP address pool") == -1:
        common.except_exit('%s add_vip_address_pool_2 failed!!!' % node_ip)

    """6、通过pscli - -command = get_vip_address_pools查看与配置的信息是否匹配"""
    log.info("\t[6 get_vip_address_pools_2 ]")
    ...
    if vip_2_domain_name in domain_name_list:
        common.except_exit('添加vip池2，IP地址与第一个VIP池相同，成功，与预期不符')
        raise Exception('%s domain_name failed %s != %s !!!' % (node_ip, domain_name, vip_domain_name))
    ...
    cmd = "disable_nas "
    check_result2 = nas_common.disable_nas(access_zone_id=access_zone_id_16_0_3_41)
