...
nfs_path = os.path.join(nas_common.NAS_PATH, FILE_NAME)             # /mnt/wangguanglin/nas_test_dir/cus_6_1_0_29
create_file_path = os.path.join(nas_common.ROOT_DIR, FILE_NAME)         # wangguanglin:/nas_test_dir/cus_6_1_0_29
...
def case():
    """若访问分区已经创建好，该步骤需要注释掉"""
    """创建访问分区"""
    access_zone_name = "cus_6_1_0_29_access_zone_name1"
    ob_node = common.Node()
    node_ids = []
    node_ids = ob_node.get_nodes_id()
    print node_ids
    access_zone_node_id_6_1_0_29 = ','.join(str(p) for p in node_ids)
    check_result1 = nas_common.create_access_zone(node_ids=access_zone_node_id_6_1_0_29, name=access_zone_name)
    if check_result1["err_msg"] != "" or check_result1["detail_err_msg"] != "":
        log.error('%s Failed ' % FILE_NAME)
        raise Exception('%s create_access_zone failed!!!' % FILE_NAME)
    id_6_1_0_29 = check_result1["result"]

    """配置共享目录、授权协议、对外IP地址、授权客户端IP"""
    """enable nas"""
    check_result2 = nas_common.enable_nas(access_zone_id=id_6_1_0_29)
    ...
    """查看NAS是否按配置启动"""
    check_result3 = nas_common.get_access_zones(ids=id_6_1_0_29)
    name = check_result3["result"]["access_zones"][0]["name"]
    if name != access_zone_name:
        raise Exception('%s enable_nas failed!!!' % FILE_NAME)
    auth_provider_id_6_1_0_29 = check_result3["result"]["access_zones"][0]["auth_provider_id"]
    ...
    """添加本地用户组/用户"""
    check_result5 = nas_common.create_auth_group(auth_provider_id=auth_provider_id_6_1_0_29,
                                                 name="group_6_0_1_7")

    if check_result5["err_msg"] != "" or check_result4["detail_err_msg"] != "":
        log.error('%s Failed' % (FILE_NAME))
        raise Exception('%s create_auth_group Failed!!!' % FILE_NAME)
    primary_group_id_6_1_0_29 = check_result5["result"]
    """添加本地用户"""
    check_result6 = nas_common.create_auth_user(auth_provider_id=auth_provider_id_6_1_0_29,
                                                name='user_6_1_0_29', password='111111',
                                                primary_group_id=primary_group_id_6_1_0_29)
    ...
    check_result7 = nas_common.create_nfs_export(access_zone_id=id_6_1_0_29,
    ...
    nfs_export_id_16_1_0_29 = check_result7["result"]
    ...
    check_result8 = nas_common.add_nfs_export_auth_clients(export_id=nfs_export_id_16_1_0_29,
    ...
    """"3个nfs客户端创建大量目录与大量文件"""
    log.info('3个nfs客户端创建大量目录与大量文件')
    vdb = tool_use.Vdbenchrun(depth=10, width=100, files=500, size='4k', threads=40, xfersize='4k', elapsed=86400)
