...
def case():
    ...
    rc, output = s3_common.set_object_acl_multi(
        bucket_name, object_name, certificate_id, account_id, ACCOUNT_EMAIL, des_account_info_list, operation)
    judge_result(rc, FILE_NAME)
    common.judge_rc(rc, 0, "set object multi acls failed!!!")

    log.info("7> 重新设置对象的acl，设置101条")
    operation = "READ"
    des_account_info_list = []
    ...
    if rc != -1:
        ...
        log.info("success, object acls number upper limit take effect")
        pass
    common.judge_rc(
        rc, -1, "failed, object acls number upper limit don't work!!!")

    log.info("8> 删除目标账户")
    s3_common.cleaning_environment(des_account_email_list)

    log.info("9> 用例%s执行成功" % FILE_NAME)
