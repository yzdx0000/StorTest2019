...
def case():
    ...
    # 4> 删除子目录
    common.rm_exe(snap_common.CLIENT_IP_1,dir_child_1)
    ...
    cmd = 'ls %s' % dir_child_1
    rc, stdout = common.run_command(snap_common.CLIENT_IP_2, cmd)
    if rc == 0:
        log.error("%s is exist!!!" % dir_child_1)
        raise Exception("%s is exist!!!" % dir_child_1)
