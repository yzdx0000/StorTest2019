...
#    7、NFS客户端成功挂载到某机头节点并使用vdbench进行读写业务，同时触发oJmgs换主
#    8、删除读写数据
#    9、将域名解析文件恢复原状
...
def case():
    ...
    '''5> 客户端进行域名解析，并添加主机路由表，检查vip池的域名是否可以ping通'''
    """域名解析"""
    nameserver_conf_ip = nas_common.CLIENT_IP_1
    ...
    #typical_eth = ob_client.get_client_eth_by_ip('.'.join(nas_common.SUBNET_NETWORK_INTERFACES.split('.')[:-1]))
    tmp_client_ip = '.'.join(nas_common.SUBNET_SVIP.split('.')[:-1]) + '.' + nas_common.CLIENT_IP_1.split('.')[-1]
    rc, typical_eth = ob_client.get_client_eth_by_ip(clientip=nas_common.CLIENT_IP_1, ip_for_eth=tmp_client_ip)
    common.judge_rc(rc, 0, 'failed')
    for i in range(0, diff_num + 1):
        tmp_vip_ip = vip_ip_parts[0] + '.' + vip_ip_parts[1] + '.' + vip_ip_parts[2] + '.' + str(int(vip_ip_parts[3]) + i)
    ...
    '''7> NFS客户端成功挂载到某机头节点并使用vdbench进行读写业务，同时触发oJmgs换主'''
    master_node_id = obj_node.get_master_mgr_id()
    master_node_ip = obj_node.get_node_ip_by_id(master_node_id)

    p1 = Process(target=judge_vdb_result, args=(local_path, local_path, nameserver_conf_ip))
    p2 = Process(target=make_fault.kill_process, args=(master_node_ip, 'oJmgs'))
    ...
    if p2.exitcode != 0:
        common.except_exit('kill oJmgs failed !!!')
