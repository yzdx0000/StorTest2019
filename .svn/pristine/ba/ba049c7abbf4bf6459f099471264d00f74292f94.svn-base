...
#   3、获取上述命令下发后的backend_id，首次执行cancel_jobengine,参数正确，预期取消下发成功；重复执行取消操作，预期下发失败
...
def case():
    ...
    '''3> 获取上述命令下发后的backend_id，重复执行cancel_jobengine，预期取消下发失败'''
    rc, stdout = common.get_jobengines()
    ...
    rc, stdout = common.cancel_jobengine("RMFS", backend_id)
    if 0 != rc:
        log.error('cancel jobengine failed !!!')
