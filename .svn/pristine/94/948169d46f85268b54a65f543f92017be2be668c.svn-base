...
def fault_disk():
    time.sleep(100)
    log.info("Step4:业务过程中将节点的两块数据盘拔出,触发被动重建,节点:%s,磁盘:%s,%s" % (fault_node_ip, disk_name, disk_name1))
...
def up_down():
    time.sleep(450)
    log.info("Step6:数据重建过程中,将该重建源节点数据网故障,节点:%s" % (fault_node_ip1))
    for fault_eth_name in fault_eth_list:
        log.info("将故障节点数据网的网卡故障,网卡：%s" % fault_eth_name)
        reliable.network_test(fault_node_ip1, fault_eth_name, 'down')
    time.sleep(100)
    for fault_eth_name in fault_eth_list:
        log.info("将故障节点数据网的网卡恢复,网卡: %s" % fault_eth_name)
        reliable.network_test(fault_node_ip1, fault_eth_name, 'up')
