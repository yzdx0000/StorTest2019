...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将节点D数据网两个网线长时间交替故障；
            5、主机1上业务完成后，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
            6、等待业务完成后恢复网络，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32941
import os
import sys
import commands
import threading
...
def up_down():
    log.info("故障点:一个非日志节点的两根数据网交替故障")
    time.sleep(100)
    log.info("Step4:在步骤3中的业务运行过程中，将故障节点的两根数据网交替故障恢复,节点:%s." %(fault_node_ip))
    log.info("Step4:将故障节点的A网卡 %s 故障" % fault_eth_name_A)
    reliable.network_test(fault_node_ip,fault_eth_name_A,'down')
    time.sleep(100)
    log.info("Step5:将故障节点的A网卡 %s 恢复" % fault_eth_name_A)
    reliable.network_test(fault_node_ip,fault_eth_name_A,'up')
    time.sleep(30)
    log.info("Step4:将故障节点的B网卡 %s 故障" % fault_eth_name_B)
    reliable.network_test(fault_node_ip,fault_eth_name_B,'down')
    time.sleep(100)
    log.info("Step5:将故障节点的B网卡 %s 恢复" % fault_eth_name_B)
