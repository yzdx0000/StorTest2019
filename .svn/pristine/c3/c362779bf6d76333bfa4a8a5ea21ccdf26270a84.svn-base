...
def remove_disk():
    ...
    for uuid in uuids:
        disk_id = env_manage.com_bd_disk.get_disk_id_by_uuid(s_ip=node_ip1, node_id=1, disk_uuid=uuid)
        log.info("remove disk xstor1000 disk ID : %s" % disk_id)
        env_manage.com_bd_disk.delete_disk(node_ip1, disk_id)
    log.info("finished")


def case():
    log.info("step:1.创建逻辑卷过程中向存储中添加新硬盘")
    threads = []
