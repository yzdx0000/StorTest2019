...
#    目录快照的批量删除。
# @steps:
#    1、对10个目录创建100个快照(每个目录10个快照，使用命令pscli --command=create_snapshot)；
...
def case():
    ...
    snap_name_lst = []
    for i in range(10):
        snap_true_path = os.path.join(SNAP_TRUE_PATH, 'test_dir_%d' % i)
        snap_create_path = os.path.join(CREATE_SNAP_PATH, 'test_dir_%d' % i)
        cmd = 'mkdir %s' % snap_true_path
