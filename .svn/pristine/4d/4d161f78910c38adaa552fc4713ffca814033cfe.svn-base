...
def case():
    ...
    rc, stdout = obj_client_auth.create_client_auth(client_ip_str, volume_id)
    common.judge_rc(rc, 0, 'create client auth fialed!!!')

    log.info('waiting for 10s')
    time.sleep(10)

    '''4> 删除授权'''
    rc, client_auth_ids = obj_client_auth.get_all_client_auth_ids(volume_id)
    common.judge_rc(rc, 0, 'get client auth')
    rc = obj_client_auth.delete_client_auth(0)   # 参数错误，预期删除失败
    common.judge_rc_unequal(rc, 0, 'delete client auth')

    '''5> 执行get_events查看操作信息显示是否正确'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)

    code = '0x02060066'
    description = 'delete client auth'
