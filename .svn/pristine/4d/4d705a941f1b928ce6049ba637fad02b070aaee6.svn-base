...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-0-1-40_sync 目录用户逻辑空间配额硬阈值之chown方式
#######################################################
...
def executing_case():
    ...
    quota_common.creating_files_by_designated_user_or_group_with_oflag_sync(quota_common.CLIENT_IP_1,
                                                                            quota_common.QUOTA_PATH,
                                                                            1024, 1, "a", quota_common.QUOTA_USER)
    quota_common.creating_files_by_designated_user_or_group_with_oflag_sync(quota_common.CLIENT_IP_2,
                                                                            quota_common.QUOTA_PATH,
    ...
    if total_file_size != quota_common.FILE_SIZE_2G:
        ...
        log.error("11-0-1-40_sync Failed")
        raise Exception("11-0-1-40_sync Failed")
    else:
        log.info("11-0-1-40_sync Succeed")
    return
...
class Quota_Class_11_0_1_40_sync():
    def quota_method_11_0_1_40_sync(self):
