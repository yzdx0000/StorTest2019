...
2）删除逻辑卷
3）在删除过程中从该磁盘池中移除一个磁盘
检查项：
移除磁盘
成功创建逻辑卷
'''

# testlink case: 1000-33694
import os
import random
import threading
import utils_path
...
def case():
    log.info("step:1.添加新硬盘，创建逻辑卷")
    add_new_disk(node_ip1)
    env_manage.create_lun()
    log.info("step:2.删除逻辑卷过程中向存储中移除硬盘")
    threads = []
    t1 = threading.Thread(target=env_manage.clean_lun, args=(node_ip1,))
    threads.append(t1)
    t2 = threading.Thread(target=remove_disk, args=(uuids,))
    threads.append(t2)
    for t in threads:
        t.start()
    for t in threads:
