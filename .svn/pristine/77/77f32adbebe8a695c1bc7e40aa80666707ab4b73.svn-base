...
def up_down():
    log.info("故障点:一个非日志节点的数据网单根网线")
    time.sleep(100)
    log.info("Step4:在步骤3中的业务运行过程中，将节点的一个数据网断开: %s,%s." %(fault_node_ip, fault_eth_name))
    reliable.network_test(fault_node_ip, fault_eth_name, 'down')
    time.sleep(200)
    log.info("Step5:在步骤3中的业务运行过程中，将节点的故障数据网恢复: %s,%s" %(fault_node_ip, fault_eth_name))
