...
#   4、3个nfs客户端总480线程，64M文件，4k读写块大小，混合读写测试；
#
#
...
nfs_path = os.path.join(nas_common.NAS_PATH, FILE_NAME)             # /mnt/wangguanglin/nas_test_dir/cus_6_1_0_13
create_file_path = os.path.join(nas_common.ROOT_DIR, FILE_NAME)         # wangguanglin:/nas_test_dir/cus_6_1_0_13
...
def case():
    ...
    print node_ids
    access_zone_node_id_6_1_0_13 = ','.join(str(p) for p in node_ids)
    check_result1 = nas_common.create_access_zone(node_ids=access_zone_node_id_6_1_0_13, name=access_zone_name)
    if check_result1["err_msg"] != "" or check_result1["detail_err_msg"] != "":
        log.error('%s Failed ' % FILE_NAME)
        raise Exception('%s create_access_zone failed!!!' % FILE_NAME)
    id_6_1_0_13 = check_result1["result"]

    """配置共享目录、授权协议、对外IP地址、授权客户端IP"""
    """enable nas"""
    check_result2 = nas_common.enable_nas(access_zone_id=id_6_1_0_13)
    ...
    """查看NAS是否按配置启动"""
    check_result3 = nas_common.get_access_zones(ids=id_6_1_0_13)
    name = check_result3["result"]["access_zones"][0]["name"]
    if name != access_zone_name:
        raise Exception('%s enable_nas failed!!!' % FILE_NAME)
    auth_provider_id_6_1_0_13 = check_result3["result"]["access_zones"][0]["auth_provider_id"]
    ...
    """添加本地用户组/用户"""
    check_result5 = nas_common.create_auth_group(auth_provider_id=auth_provider_id_6_1_0_13,
                                                 name="group_6_1_0_13")

    if check_result5["err_msg"] != "" or check_result4["detail_err_msg"] != "":
        log.error('%s Failed' % FILE_NAME)
        raise Exception('%s create_auth_group Failed!!!' % FILE_NAME)
    primary_group_id_6_1_0_13 = check_result5["result"]
    """添加本地用户"""
    check_result6 = nas_common.create_auth_user(auth_provider_id=auth_provider_id_6_1_0_13,
                                                name='user_6_1_0_13', password='111111',
                                                primary_group_id=primary_group_id_6_1_0_13)
    ...
    check_result7 = nas_common.create_nfs_export(access_zone_id=id_6_1_0_13,
    ...
    nfs_export_id_6_1_0_13 = check_result7["result"]
    ...
    check_result8 = nas_common.add_nfs_export_auth_clients(export_id=nfs_export_id_6_1_0_13,
    ...
    """"3个nfs客户端总480线程，64M文件，4k读写块大小，混合读写测试"""
    log.info('3个nfs客户端总480线程，64M文件，4k读写块大小，混合读写测试')
    vdb = tool_use.Vdbenchrun(depth=1, width=1, files=1, size='64m', threads=480, xfersize='4k', elapsed=600)
