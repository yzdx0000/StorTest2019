...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将主oPmgr异常，再将守护进程异常；
            5、检查oPmgr进程状态
            6、主机1上业务完成后，在主机1上执行vdbench -f OLTP.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32994
import os
import sys
import commands
import threading
...
log.info("Get fault process:oRole & oMgcd")

def up_down():
    time.sleep(100)
    log.info("Step4:将故障节点oMgcd守护进程杀死,节点: %s." % (m_oRole_ip))
    ReliableTest.run_kill_process(m_oRole_ip,'oMgcd')
    time.sleep(15)
    log.info("Step4:将主oRole进程杀死,节点: %s." %(m_oRole_ip))
    ReliableTest.run_kill_process(m_oRole_ip,'oRole')
    time.sleep(200)
    log.info("Step5:检查守护进程oMgcd进程状态,节点: %s." % (m_oRole_ip))
    reliable.check_process_stat(m_oRole_ip,'oMgcd')
    log.info("Step5:检查主oRole进程状态,节点: %s." % (m_oRole_ip))
