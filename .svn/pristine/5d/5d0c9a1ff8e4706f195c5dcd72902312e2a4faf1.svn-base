...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将节点A/B的oSan进程反复异常；
            5、检查各个节点oSan进程
            6、主机1上业务完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-33018
...
def up_down():
    ...
    for count in range(3):
        log.info("第 %d 次同时故障A、B节点oSan进程" % (count + 1))
        fault_threads=[]
        ...
        log.info("Step4:将A/B个节点的oSan进程同时杀死,节点: %s,%s." %(fault_node_ip_A,fault_node_ip_B))
        ...
        state_A = reliable.check_process_stat(fault_node_ip_A, 'oSan')
        log.info("Step5:获取故障节点B的oSan进程状态,节点: %s." % (fault_node_ip_B))
        state_B = reliable.check_process_stat(fault_node_ip_B, 'oSan')
        while True:
