...
m_oRole_ip = reliable.get_master_oRole(deploy_ips[0])
deploy_ls.remove(m_oRole_ip)
fault_node_ip = random.choice(deploy_ls)

log.info("Get fault process:master oRole & follow oRole")

def up_down():
    time.sleep(100)
    for count in range(3):
        log.info("第 %d 次杀死主oRole与从oRole进程" % (count + 1))
        log.info("Step4:将节点主oRole进程杀死 : %s." %(m_oRole_ip))
        ReliableTest.run_kill_process(m_oRole_ip,'oRole')
        log.info("Step4:将节点从oRole进程杀死 : %s." %(fault_node_ip))
        ReliableTest.run_kill_process(fault_node_ip,'oRole')

        time.sleep(100)
        while True:
            log.info("Step5:检查故障主oRole进程状态 : %s." % (m_oRole_ip))
            state1=reliable.check_process_stat(m_oRole_ip, 'oRole')
            log.info("Step5:检查故障从oRole进程状态 : %s." % (fault_node_ip))
            state2=reliable.check_process_stat(fault_node_ip, 'oRole')
            if (state1 == 'OK' or 'READY') and (state2 == 'OK' or 'READY'):
                break
            time.sleep(20)
