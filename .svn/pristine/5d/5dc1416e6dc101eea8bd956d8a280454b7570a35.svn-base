...
2）删除逻辑卷过程中制造oJmgs+oSan进程故障
检查项：
1）访问区配置成功
2）逻辑卷成功删除
'''

# testlink case: 1000-33954
import os
import time
import threading
import utils_path
...
def run_kill_process():
    log.info("step:2.1 The oSan and oRole process will be killed")
...
def case():
    ...
    log.info("step:2.创建逻辑卷时查杀oJmgs和某个节点上的oSan进程")
    threads = []
