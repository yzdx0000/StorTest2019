...
def disk_fault():
    time.sleep(100)
    log.info("Step4:业务过程中主动将共享盘删除,触发重建,节点:%s,磁盘:%s" % (fault_node_ip, disk_name))
    env_manage_repair_test.com_disk.remove_disks(disk_id)

def up_down():
    time.sleep(150)
    log.info("Step5:重建过程将重建目标节点重启,节点:%s" % (fault_node_ip))
    reliable.poweroff_os(fault_node_ip, 'init 6')
    time.sleep(100)
    log.info("检查故障节点启动状态,节点:%s" % fault_node_ip)
    while True:
        ...
        if 0 == rc:
            ...
            log.info("等待节点重新启动,节点:%s" % fault_node_ip)
        time.sleep(15)
