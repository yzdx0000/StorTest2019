...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)  # /mnt/volume1/snap/snap_16_0_3_56
...
def executing_case1():
    ...
    node_ip = SYSTEM_IP
    access_zone_name = "access_zone_16_0_3_56"
    node = common.Node()
    ids = node.get_nodes_id()
    print ids
    access_zone_node_id_16_0_3_56 = ','.join(str(p) for p in ids)
    msg1 = nas_common.create_access_zone(node_ids=access_zone_node_id_16_0_3_56, name=access_zone_name)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        common.except_exit('%s create_access_zone failed!!!' % node_ip)
    access_zone_id_16_0_3_56 = msg1["result"]

    """获取访问分区信息"""
    log.info("\t[1-2 get_access_zones ]")
    msg1 = nas_common.get_access_zones(ids=access_zone_id_16_0_3_56)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
    ...
    msg2 = nas_common.enable_nas(access_zone_id=access_zone_id_16_0_3_56)
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        common.except_exit('%s enable_nas failed!!!' % node_ip)

    """2、创建业务子网"""
    log.info("\t[2 create_subnet ]")
    sub_name = "subnet_16_0_3_56"
    ...
    check_result1 = nas_common.create_subnet(access_zone_id=access_zone_id_16_0_3_56,
                                             name=sub_name,
    ...
    subnet_id_16_0_3_56 = check_result1["result"]

    """获取子网信息"""
    log.info("\t[2 get_subnets ]")
    msg1 = nas_common.get_subnets(ids=subnet_id_16_0_3_56)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        common.except_exit('%s get_subnets failed!!!' % node_ip)
    access_zone_id = msg1["result"]["subnets"][0]["access_zone_id"]
    if access_zone_id != access_zone_id_16_0_3_56:
        raise Exception('%s access_zone_id failed %s != %s !!!' % (node_ip, access_zone_id, access_zone_id_16_0_3_56))
    idd = msg1["result"]["subnets"][0]["id"]
    if idd != subnet_id_16_0_3_56:
        raise Exception('%s id failed %s != %s !!!' % (node_ip, idd, subnet_id_16_0_3_56))
    ...
    msg1 = nas_common.add_vip_address_pool(subnet_id=subnet_id_16_0_3_56,
    ...
    vip_address_pool_id_16_0_3_56 = msg1["result"]

    """4、通过pscli --command=get_vip_address_pools查看与配置的信息是否匹配"""
    log.info("\t[4 get_vip_address_pools ]")
    msg1 = nas_common.get_vip_address_pools(ids=vip_address_pool_id_16_0_3_56)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
    ...
    if idd != vip_address_pool_id_16_0_3_56:
        raise Exception('%s id failed %s != %s !!!' % (node_ip, idd, vip_address_pool_id_16_0_3_56))
    ...
    if subnet_id != subnet_id_16_0_3_56:
        raise Exception('%s subnet_id failed %s != %s !!!' % (node_ip, subnet_id, subnet_id_16_0_3_56))
    ...
    msg1 = nas_common.update_vip_address_pool(vip_address_pool_id=vip_address_pool_id_16_0_3_56,
                                              domain_name=vip_2_domain_name)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        common.except_exit('%s update_vip_address_pool failed!!!' % node_ip)

    """7、通过pscli - -command = get_vip_address_pools查看与配置的信息是否匹配"""
    log.info("\t[7 get_vip_address_pools_2 ]")
    time.sleep(10)
    msg1 = nas_common.get_vip_address_pools(ids=vip_address_pool_id_16_0_3_56)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        common.except_exit('%s get_vip_address_pools!!!' % node_ip)
    ...
    if allocation_method != vip_allocation_method:
        raise Exception(
            '%s allocation_method failed %s != %s !!!' % (node_ip, allocation_method, vip_allocation_method))
    domain_name = msg1["result"]["ip_address_pools"][0]["domain_name"]
    if domain_name != vip_2_domain_name:
        raise Exception('%s domain_name failed %s != %s !!!' % (node_ip, domain_name, vip_2_domain_name))
    idd = msg1["result"]["ip_address_pools"][0]["id"]
    if idd != vip_address_pool_id_16_0_3_56:
        raise Exception('%s id failed %s != %s !!!' % (node_ip, idd, vip_address_pool_id_16_0_3_56))
    ip_failover_policy = msg1["result"]["ip_address_pools"][0]["ip_failover_policy"]
