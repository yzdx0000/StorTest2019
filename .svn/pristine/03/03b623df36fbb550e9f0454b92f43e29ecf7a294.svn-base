...
error.rel_check_before_run(file_name, jnl_rep=2, free_jnl_num=0, node_num=2, data_rep=2)
...
def up_down(pipein):
    ...
    common.check_ip(fault_vip)
    b_id = error.get_orole_master_id(s_ip=fault_vip)
    b_ip = disk.get_node_ip_by_id(removed_ip=a_ip, n_id=b_id)
    log.info("5、oPmgr报单前%s节点主oRole/oPmgr进程被kill；" % (b_ip,))
