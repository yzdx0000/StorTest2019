...
def case():
    ...
    ip_family = 'IPv4'
    before_update_eth = nas_common.SUBNET_NETWORK_INTERFACES
    exe_info = nas_common.create_subnet(access_zone_id=access_zone_id, name=subnet_name, ip_family=ip_family,
                                        ...
                                        subnet_gateway=nas_common.SUBNET_GATEWAY, network_interfaces=before_update_eth)
    if exe_info["err_msg"] != "" or exe_info["detail_err_msg"] != "":
    ...
    eth_judge_str = before_update_eth
    ...
    '''5> 修改业务子网“网卡”参数，并通过ip addr查看svip是否落到修改后的网卡上'''
    """修改网卡信息"""
    after_update_eth = nas_common.SUBNET_2_NETWORK_INTERFACES
    exe_info = nas_common.update_subnet(subnet_id, network_interfaces=after_update_eth)
    if exe_info["err_msg"] != "" or exe_info["detail_err_msg"] != "":
        common.except_exit('update subnet failed !!!')

    """查看svip是否落到修改后的网卡上"""
    eth_judge_str = after_update_eth
    eth_judge_lst = eth_judge_str.split(',')
    for node_ip in node_ip_list:
        cmd = 'ip addr'
        rc, stdout = common.run_command(node_ip, cmd)
    ...
    exe_info = nas_common.get_subnets(subnet_id)
    subnet = exe_info['result']['subnets'][0]
    if subnet['network_interfaces'][0] != after_update_eth:
