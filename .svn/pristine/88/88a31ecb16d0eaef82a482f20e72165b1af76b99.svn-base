...
#    3、针对目录配置逻辑空间为2000M硬阈值,1000M软阈值（宽限时间为2分钟），500M建议阈值的配额。
#    4、通过ftp客户端先写入1000M文件，2分钟内再写入1000M文件；宽限时间内再次尝试写入，检查硬阈值配额是否生效
#    5、通过ftp客户端先写入1000M文件，2分钟内再写入500M文件；超过宽限时间后再次尝试写入，检查软阈值配额是否生效
#    6、删除配额及所创建的文件
#
# @changelog：
####################################################################################
...
def case():
    ...
    '''7> 针对目录配置逻辑空间为2000M的硬阈值，1000M软阈值（宽限时间为150s），500M建议阈值'''
    quota_path = create_export_path
    hard_logical = 2097152000
    soft_logical = 1048576000
    grace_time = 150
    suggest_logical = 524288000
    rc, check_result = quota_common.create_one_quota(path=quota_path,
                                                     logical_quota_cal_type='QUOTA_LIMIT',
                                                     logical_suggest_threshold=suggest_logical,
                                                     logical_soft_threshold=soft_logical,
                                                     logical_grace_time=grace_time,
                                                     logical_hard_threshold=hard_logical)
    ...
    excess_file_num = 21
    test_file_total = 2202009600
    for i in range(excess_file_num):
    ...
    '''9> 删除全部文件，再次创建1200M文件，超过宽限时间后，尝试继续写入文件（测试软阈值是否生效）'''
    """删除共享目录和本地目录下的所有文件"""
    cmd = 'cd %s; rm -rf test*' % NAS_TRUE_PATH
    rc, stdout = common.run_command(export_sys_ip, cmd)
    common.judge_rc(rc, 0, 'delete test* in %s failed !!!' % NAS_TRUE_PATH)

    cmd = 'cd %s; rm -rf test*' % local_path
    rc, stdout = common.run_command(nas_common.FTP_1_CLIENT_IP, cmd)
    common.judge_rc(rc, 0, 'delete test* in %s failed !!!' % local_path)

    """再次创建并上传1200M的文件，触发grace_time开始计时"""
    excess_file_num = 12
    for i in range(excess_file_num):
        file_name = 'test_again' + str(i)
        file_path = os.path.join(local_path, file_name)
        cmd = 'dd if=%s of=%s bs=1M count=100' % (snap_common.get_system_disk(SYSTEM_IP_0), file_path)
        common.run_command(nas_common.FTP_1_CLIENT_IP, cmd)
    total_file = 'test_again*'

    execution_file = os.path.join(local_path, 'ftp_02.sh')
    cmd = "echo "" > %s &&" \
          "echo '#!/bin/bash' >> %s &&" \
          "echo 'ftp -nv << EOF' >> %s &&" \
          "echo 'prompt' >> %s &&" \
    ...
    again_file_size = 0
    for line in file_info_list:
        again_file_size = again_file_size + int(line.strip().split()[0])

    """超过宽限时间后，继续尝试写入文件"""
    log.info('waiting for 180s')
    time.sleep(180)
    try_file_num = 30
    for i in range(try_file_num):
        file_name = 'test_try' + str(i)
        file_path = os.path.join(local_path, file_name)
        cmd = 'dd if=%s of=%s bs=1M count=10' % (snap_common.get_system_disk(SYSTEM_IP_2), file_path)
        common.run_command(nas_common.FTP_1_CLIENT_IP, cmd)
    total_file = 'test_try*'
    execution_file = os.path.join(local_path, 'ftp_03.sh')
    cmd = "echo "" > %s &&" \
          "echo '#!/bin/bash' >> %s &&" \
          "echo 'ftp -nv << EOF' >> %s &&" \
          "echo 'prompt' >> %s &&" \
    ...
    try_file_size = 0
    for line in file_info_list:
        try_file_size = try_file_size + int(line.strip().split()[0])

    if try_file_size > again_file_size:
        common.except_exit('quota did not work well !!!')
