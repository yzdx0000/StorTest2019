...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将节点A的ostor进程异常后再将oPara进程异常；
            5、检查节点A ostor，oJnl进程
            6、主机1上业务完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-33019
# testlink case: 1000-33020
import os
import sys
import commands
import threading
...
log.info("Get fault process:oStor & oRole & oStor")

def up_down():
    # 进程先后故障
    time.sleep(100)
    fault_threads = []
    fault_threads.append(threading.Thread(target=ReliableTest.run_kill_process(fault_node_ip,'oSan')))
    fault_threads.append(threading.Thread(target=ReliableTest.run_kill_process(fault_node_ip,'oRole')))
    fault_threads.append(threading.Thread(target=ReliableTest.run_kill_process(fault_node_ip,'oStor')))
    for fault_thread in fault_threads:
        fault_thread.start()
    for fault_thread in fault_threads:
        fault_thread.join()
    log.info("Step4:将该节点的oSan & oRole & oStor进程同时杀死,节点: %s" % (fault_node_ip))
    time.sleep(300)
    # 检查进程状态
    log.info("Step5:获取故障节点的oSan进程状态,节点: %s." % (fault_node_ip))
    reliable.check_process_stat(fault_node_ip,'oSan')
    log.info("Step5:获取故障节点的oRole进程状态,节点: %s." % (fault_node_ip))
    reliable.check_process_stat(fault_node_ip,'oRole')
    log.info("Step5:获取故障节点的oStor进程状态,节点: %s." % (fault_node_ip))
