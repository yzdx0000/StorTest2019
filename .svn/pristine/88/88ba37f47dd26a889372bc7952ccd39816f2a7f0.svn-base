...
2）制造oRole进程挂起（gbd），在故障期间创建LUN
检查项：
1）访问区配置成功
2）逻辑卷成功创建
'''

# testlink case: 1000-33718
import os
import time
import utils_path
import log
...
def case():
    ...
    orole_id, orole_ip = env_manage.com_lh.m_oRole_id()
    log.info("step:2.挂起 oRole 进程")
    process_pause("oRole", orole_ip)
    log.info("step:3.创建逻辑卷")
    env_manage.create_lun(name="LUN2")
    log.info("step:4.恢复 oRole 进程")
    process_start("oRole", orole_ip)
