...
2、使用2个主机通过节点B建立iscsi连接，主机1映射LUN1-LUN6，主机2映射LUN7-LUN12；
...
def up_down(pipein):
    time.sleep(10)
    log.info("2、将业务网故障然后恢复，触发vip和los分离")
    ...
    log.info("重新获取vip %s 所在节点 %s" % (v_ip, b_ip))
    ...
    while True:
        ...
        if 'up' in line:
            break
    log.info("6、恢复业务网")
    log.info("Begin up node %s through %s." % (b_ip, b_extra))
