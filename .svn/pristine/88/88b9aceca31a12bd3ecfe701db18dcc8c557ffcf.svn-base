...
5、5分钟后插回被拔出的磁盘；
6、数据修复过程中再次拔出该盘直到超时
7、数据修复过程中，主机端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性
...
def case():
    ...
    log.info("step3:5分钟后插回被拔出的磁盘，数据修复过程中再次拔出该盘直到超时,主机端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性 ...")
