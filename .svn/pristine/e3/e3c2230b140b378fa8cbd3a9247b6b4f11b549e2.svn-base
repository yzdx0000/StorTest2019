...
def up_down():
    ...
    osan.get_vips_layout(1)
    osan.get_vips_layout(2)
    ...
    if sum(vips1) == len(deploy_ips) * 2:
        log.info("VIP池A中的VIP迁移成功")
    else:
        log.error("异常!VIP池A的VIP迁移失败")
        os._exit(1)
    # B池有可能也会均衡
    if sum(vips2) == len(deploy_ips) * 2:
        log.info("VIP池B中的VIP迁移成功")
    else:
        log.error("VIP池B中的VIP迁移失败")
    ...
    vips1 = [len(osan.get_vips_by_node(x, 1)) for x in sever_ips]
    vips2 = [len(osan.get_vips_by_node(x, 2)) for x in sever_ips]
    if len(set(vips1)) == 1 and vips1[0] == 2:
        log.info("VIP池A中的VIP自动均衡成功!每个节点VIP数量为2")
    else:
        log.error("异常!VIP池A的自动均衡失败,数量分布情况:%s" % [osan.get_vips_by_node(x, 1) for x in sever_ips])
        error.up_node(vm_id)
        os._exit(1)
    if len(set(vips2)) != 1 and sum(vips2) == len(deploy_ips) * 2:
        log.info("VIP池B中的VIP未自动均衡,符合预期")
    else:
        log.error("VIP池B中的VIP异常均衡,不符合策略;或VIP丢失")
