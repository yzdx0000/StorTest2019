...
def run_kill_process():
    rc = env_manage.com_lh.get_cmd_status(
        node_ip1, "map_luns_to_host_group")
    if rc == 0:
        master_orole_ip = env_manage.com_lh.get_master_oRole(node_ip1)
        ReliableTest.run_kill_process(node_ip=node_ip1, process="oSan")
        ReliableTest.run_kill_process(
            node_ip=master_orole_ip, process="oRole")
        return
    elif rc == 1:
...
def case():
    ...
    log.info("step:2.创建lun map时查杀oJmgs和某个节点上的oSan进程")
    threads = []
