...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)  # /mnt/volume1/snap/snap_16_0_4_101
...
def executing_case1():
    ...
    access_zone_name = "access_zone_16_0_4_101"
    node = common.Node()
    ...
    global access_zone_id_16_0_4_101
    access_zone_id_16_0_4_101 = msg1["result"]
    ...
    #     os.makedirs(prepath)
    global nfs_path
    nfs_path = nas_common.ROOT_DIR + "nfs_dir_16_0_4_101_1"
    global NAS_PATH
    NAS_PATH = get_config.get_one_nas_test_path() + "/nfs_dir_16_0_4_101_1"
    ...
    nfs_export_name = "nfs_exp_test_16_0_4_101_1"
    global description_nfs
    description_nfs = 'old_export_description'
    cmd = "create_nfs_export --access_zone_id=%s " \
          "--export_name=%s --export_path=%s --description=%s" % (access_zone_id_16_0_4_101, nfs_export_name,
                                                                  nfs_path, description_nfs)
    check_result1 = nas_common.create_nfs_export(access_zone_id=access_zone_id_16_0_4_101, export_name=nfs_export_name,
    ...
    global nfs_export_id_16_0_4_101
    nfs_export_id_16_0_4_101 = msg1["result"]

    # 启动NAS
    log.info("\t[ enable_nas ]")
    msg2 = nas_common.enable_nas(access_zone_id=access_zone_id_16_0_4_101, protocol_types="NFS")
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        common.except_exit('%s enable_nas failed!!!' % node_ip)

    # 查看NAS是否按配置启动
    log.info("\t[ get_access_zones ]")
    check_result3 = nas_common.get_access_zones(ids=access_zone_id_16_0_4_101)
    name = check_result3["result"]["access_zones"][0]["name"]
    if name != access_zone_name:
        common.except_exit('%s enable_nas failed!!!' % node_ip)
    global auth_provider_id_16_0_4_101
    auth_provider_id_16_0_4_101 = check_result3["result"]["access_zones"][0]["auth_provider_id"]
...
def executing_case2():
    ...
    cmd = "get_nfs_exports --ids=%s" % nfs_export_id_16_0_4_101
    check_result = nas_common.get_nfs_exports(ids=nfs_export_id_16_0_4_101)
    ...
    if access_zone_id != access_zone_id_16_0_4_101:
...
def executing_case3():
    ...
    cmd = "add_nfs_export_auth_clients --export_id=%s " \
          ...
          % (nfs_export_id_16_0_4_101, auth_clients_name, auth_clients_permission_level)
    check_result = nas_common.add_nfs_export_auth_clients(export_id=nfs_export_id_16_0_4_101,
    ...
    global nfs_export_clients_id_16_0_4_101
    nfs_export_clients_id_16_0_4_101 = msg["result"][0]
...
def executing_case4():
    ...
    cmd = "get_nfs_export_auth_clients --export_ids=%s" % nfs_export_id_16_0_4_101
    check_result = nas_common.get_nfs_export_auth_clients(export_ids=nfs_export_id_16_0_4_101)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    msg = check_result
    if msg["err_msg"] != "" or msg["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    ...
    if id != nfs_export_clients_id_16_0_4_101:
        raise Exception('%s id failed!!!' % node_ip)
    if export_id != nfs_export_id_16_0_4_101:
