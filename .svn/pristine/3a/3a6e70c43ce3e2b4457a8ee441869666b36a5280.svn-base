...
def case():
    ...
    for i in range(1):
        ...
        decorator_func.judge_target(
            ...
            cp("add_initiator", "alias"))

    osan.update_initiator(s_ip=deploy_ips[0], ini_id=ini_id, alias='update_alias', auth_type="CHAP",
                          user=cp("CHAP", "CHAP_user"),
                          passwd='update2018')
    osan.update_iscsid_conf(cli_ip=client_ips[0], CHAPTYPE='CHAP_updated_passwd', s_ip=client_ips[0])
    decorator_func.judge_target(
        osan.get_option_single(s_ip=deploy_ips[0], command='get_initiators', indexname='initiators',
                               argv2="chap_password",
                               ids="ids", argv1=ini_id),
        'update2018')
