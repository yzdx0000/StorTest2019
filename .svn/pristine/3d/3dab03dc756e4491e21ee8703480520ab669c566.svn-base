...
def up_down():
    ...
    time.sleep(100)
    log.info("Step6:等待业务完成后,集群所有节点重新启动")
    event1.wait()
    event2.wait()
    log.info("Step7:数据修复过程中,将整集群掉电后恢复,节点:%s" % (fault_node_ip))
    log.info("将整集群掉电...")
    vm_id_list = []
    ...
    for node_ip in deploy_ips:
        while True:
            ...
            time.sleep(15)
    time.sleep(30)
    log.info("Step8:系统整集群掉电完成后,重新跑上业务验证系统正常")
    event3.set()
...
def vdb_jn():
    ...
    event1.set()
    event3.wait()
    log.info("Step10:等待系统整集群掉电后,重新执行vdbench业务.")
    osan.run_vdb(client_ips[0], mix_R, output=deploy_ips[0], time=300)
