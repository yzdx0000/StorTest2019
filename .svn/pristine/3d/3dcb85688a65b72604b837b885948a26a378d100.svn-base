...
# date 2018-11-15
# @summary：
#    nfs目录inode嵌套配额在mv操作后，配额是否生效
# @steps:
#    1、创建ad访问分区，创建访问分区，启动NAS
#    2、创建共享目录，导出并授权某个用户的导出权限
#    3、针对目录配置inode硬阈值父:100;子目录：100
#    4、通过nfs客户端在子目录下写入30个文件
#    5、移动子目录下文件到父目录中；
#    6、在客户端父目录下继续写文件；预计100个
# @changelog：
####################################################################################
...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)             # /mnt/volume/nas_test_dir/quota_nas_2
...
def case():
    ...
    log.info("3> 针对目录配置inode数子目录200；父目录为100的硬阈值配额")
    ...
    log.info("3.1> 创建父目录配额，inode硬阈值100，等待配额生效")
    ...
    log.info("3.2> 创建子目录配额，inode硬阈值100，等待配额生效")
    ...
    log.info("4> 通过nfs客户端先在子目录中写入30个文件")
    for i in range(30):
    ...
    log.info("5> 移动子目录下文件到父目录中；在父目录中继续写；规则生效；预计100个")
    cmd = 'mv %s %s'%(dir1_1 + '/*',NAS_TRUE_PATH + '/')
    rc, stdout = common.run_command(nas_common.CLIENT_IP_1,cmd)
    common.judge_rc(rc, 0, "mv failed!!")

    log.info("6> 继续在客户端父目录下写文件；预计100个")
    for i in range(70):
        file_name = 'test' + str(i) + '_again'
        file_path = os.path.join(NAS_TRUE_PATH, file_name)
        cmd = 'echo 111 > %s' % file_path
        common.run_command(auth_name_ip, cmd)

    dir_inode = quota_common.dir_total_inodes(quota_common.CLIENT_IP_1, NAS_TRUE_PATH)
    if dir_inode != 100:
        rc_lst[sys._getframe().f_lineno] = 'dir: %s total_inode: %s ' % (NAS_TRUE_PATH, dir_inode)
