...
# Date  :2018-8-6
"""
测试内容:一节点一块元数据盘反复故障，超时内恢复

步骤:
1、在节点A创建LUN1
2、将逻辑卷映射至主机1上LUN1
3、在主机1上运行vdbench -f min-seq-w-1l.conf -jn；
4、业务完成或将一个日志节点的一个共享盘反复故障；
5、在超时时间（1小时）内恢复；
6、等待共享盘恢复后主机端执行vdbench -f min-seq-w-1l.conf -jro校验数据一致性
6、插回故障盘开始修复，修复完成后执行主机端执行vdbench -f min-seq-w-1l.conf -jro校验数据一致性
7、修复完成后，比较内部数据一致性
...
def case():
    ...
    log.info("step3:业务完成或将一个日志节点的一个共享盘反复故障...")
    disk_error()

    log.info(
        "step4:在超时时间（1小时）内恢复,等待共享盘恢复后主机端,主机端执行vdbench -f min-seq-write.conf -jro校验数据一致性 ...")
    time.sleep(60)
