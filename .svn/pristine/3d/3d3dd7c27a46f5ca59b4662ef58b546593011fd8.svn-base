...
# Date: 2018-09-03
# @summary：
#    用户 父目录软阈值1000，宽限时间60秒，三个子目录软阈值300，宽限时间60秒，3个子目录分别创建400文件，
#    过期后所有目录无法创建文件
# @steps:
#    1.创建 用户用户组
#    2.创建目录FILENAME/nesting1_1  nesting1_2  nesting1_3
#    3.创建目录FILENAME配额,软阈值1000 宽限时间60秒 等待配额状态为work
#    4.创建子目录FILENAME/nesting1_x/配额,子目录软阈值300，宽限时间60秒 等待配额状态为work
#    5.3子目录创建400文件开始计时，过期后3目录无法创建文件
# @changelog：
#################################################################
...
def case():
    ...
    log.info("3> 创建父目录配额,软阈值1000 宽限时间60秒 等待配额状态为work")
    ...
    log.info("4> 创建子目录FILENAME/nesting1_x配额,子目录软阈值300，宽限时间60秒 等待配额状态为work")
    for i in range(3):
    ...
    log.info("5> 3子目录创建400文件开始计时，过期后3目录无法创建文件")
    for i in range(3):
        quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_1, dir1_[i], 400, 1, 'a', quota_common.QUOTA_USER, quota_id=quota_id_lst[i+1])

    dir_inode = quota_common.user_total_inodes(quota_common.CLIENT_IP_1, dir1_[0], quota_common.QUOTA_USER)
    if dir_inode != 400:
        rc_lst[sys._getframe().f_lineno] = 'dir: %s total_inode: %s ' % (dir1_[0], dir_inode)
    dir_inode = quota_common.user_total_inodes(quota_common.CLIENT_IP_2, dir1_[1], quota_common.QUOTA_USER)
    if dir_inode != 400:
        rc_lst[sys._getframe().f_lineno] = 'dir: %s total_inode: %s ' % (dir1_[1], dir_inode)
    dir_inode = quota_common.user_total_inodes(quota_common.CLIENT_IP_3, dir1_[2], quota_common.QUOTA_USER)
    if dir_inode != 400:
    ...
    if dir_inode != 400:
        rc_lst[sys._getframe().f_lineno] = 'dir: %s total_inode: %s ' % (dir1_[1], dir_inode)
    dir_inode = quota_common.user_total_inodes(quota_common.CLIENT_IP_2, dir1_[2], quota_common.QUOTA_USER)
    if dir_inode != 400:
        rc_lst[sys._getframe().f_lineno] = 'dir: %s total_inode: %s ' % (dir1_[2], dir_inode)
    dir_inode = quota_common.user_total_inodes(quota_common.CLIENT_IP_3, dir0, quota_common.QUOTA_USER)
    if dir_inode != 1200:
