...
4、在步骤3中的业务运行过程中，将节点A oPara进程异常；
5、检查oPara进程状态
6、主机1上业务完成后，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32970
import os
import sys
import commands
import threading
...
def up_down():
    time.sleep(100)
    log.info("Step4:将故障节点的oPara进程杀死,节点: %s." %(fault_node_ip))
    ReliableTest.run_kill_process(fault_node_ip, 'oPara')
    time.sleep(100)
    log.info("Step5:检查故障节点的oPara进程状态,节点: %s." % (fault_node_ip))
