...
def case():
    ...
    ip_family = 'IPv4'
    eth_before_update = nas_common.SUBNET_NETWORK_INTERFACES
    exe_info = nas_common.create_subnet(access_zone_id=access_zone_id, name=subnet_name, ip_family=ip_family,
                                        ...
                                        network_interfaces=eth_before_update)
    if exe_info["err_msg"] != "" or exe_info["detail_err_msg"] != "":
    ...
    '''5> 输入无效的id'''
    eth_after_update = nas_common.SUBNET_2_NETWORK_INTERFACES
    invalid_subnet_id = 0
    exe_info = nas_common.update_subnet(invalid_subnet_id, eth_after_update)
    if exe_info["err_msg"] == "" or exe_info["detail_err_msg"] == "":
    ...
    exe_info = nas_common.get_subnets(subnet_id)
    subnet = exe_info['result']['subnets'][0]
    if subnet['network_interfaces'][0] != eth_before_update:
