...
def executing_case1():
    ...
    access_zone_name = "access_zone_16_0_4_105_1"
    node = common.Node()
    ...
    access_zone_id_16_0_4_105_1 = msg1["result"]

    # 2> 启动NAS
    log.info("\t[ enable_nas ]")
    msg2 = nas_common.enable_nas(access_zone_id=access_zone_id_16_0_4_105_1, protocol_types="NFS")
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        common.except_exit('%s enable_nas failed!!!' % node_ip)

    # 3> 查看NAS是否按配置启动
    log.info("\t[ get_access_zones ]")
    check_result3 = nas_common.get_access_zones(ids=access_zone_id_16_0_4_105_1)
    name = check_result3["result"]["access_zones"][0]["name"]
    if name != access_zone_name:
        common.except_exit('%s enable_nas failed!!!' % node_ip)
    global auth_provider_id_16_0_4_105_1
    auth_provider_id_16_0_4_105_1 = check_result3["result"]["access_zones"][0]["auth_provider_id"]
    ...
    access_zone_id_16_0_4_105_1 = msg["result"]["access_zones"][0]["id"]
    global access_zone_id_16_0_4_105_1
    ...
    global NAS_PATH_1
    NAS_PATH_1 = get_config.get_one_nas_test_path() + "/nfs_dir_16_0_4_105_1"
    ...
    cmd = "get_nfs_exports --ids=%s" % nfs_export_id_16_0_4_105_1
    ...
    cmd = "get_nfs_export_auth_clients --export_ids=%s" % nfs_export_id_16_0_4_105_1
    ...
    cmd = "ssh %s ls /mnt/nfs_dir_16_0_4_105_1" % auth_clients_name
    rc, stdout, stderr = shell.ssh(node_ip, cmd)
    if stdout == "":
        cmd = "ssh %s umount -l /mnt/nfs_dir_16_0_4_105_1" % auth_clients_name
        shell.ssh(node_ip, cmd)
        cmd = "ssh %s rm -rf /mnt/nfs_dir_16_0_4_105_1" % auth_clients_name
        rc, stdout, stderr = shell.ssh(node_ip, cmd)
        if rc != 0:
            log.error("WARNING: \ncmd = %s\nstdout = %s\nstderr = %s" % (cmd, stdout, stderr))
...
def executing_case2():
    ...
    access_zone_id_16_0_4_105_2 = access_zone_id_16_0_4_105_1
    ...
    cmd = "get_nfs_exports --ids=%s" % nfs_export_id_16_0_4_105_2
    ...
    cmd = "get_nfs_export_auth_clients --export_ids=%s" % nfs_export_id_16_0_4_105_2
    ...
    cmd = "ssh %s ls /mnt/nfs_dir_16_0_4_105_2" % auth_clients_name
    rc, stdout, stderr = shell.ssh(node_ip, cmd)
    if stdout == "":
        cmd = "ssh %s umount -l /mnt/nfs_dir_16_0_4_105_2" % auth_clients_name
        shell.ssh(node_ip, cmd)
        cmd = "ssh %s rm -rf /mnt/nfs_dir_16_0_4_105_2" % auth_clients_name
        rc, stdout, stderr = shell.ssh(node_ip, cmd)
        if rc != 0:
            log.error("WARNING: \ncmd = %s\nstdout = %s\nstderr = %s" % (cmd, stdout, stderr))
            raise Exception('%s rm -rf client file failed!!!' % node_ip)
...
def executing_case3():
    ...
    cmd1 = "file1=`grep %s /etc/exports | head -n 1` " % auth_clients_name + \
           "&& file2=`echo ${file1#*rw,}` && echo ${file2%,secure*}"
    rc, stdout, stderr = shell.ssh(node_ip, cmd1)
    print stdout
