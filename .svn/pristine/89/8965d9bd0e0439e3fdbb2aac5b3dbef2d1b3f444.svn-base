# -*- coding:utf-8 _*-
"""
测试内容:并发写入thin lun数据触发空间预留oPmgr确认后，oSan节点掉电后恢复
...
fault_ip = random.choice(env_manage_lun_manage.deploy_ips)
...
def os_error(arg=3):
    ...
    for i in range(10):
        if env_manage_lun_manage.Reliable_osan.check_process_stat(fault_ip, "oSan") in "OK,READY,ok,ready":
            ReliableTest.run_kill_process(node_ip=fault_ip, process="oSan")
        time.sleep(20)
...
def case():
    ...
    map_lun()
    iscsi_login()
    log.info("在步骤3中的业务运行过程中，在步骤3中的业务运行过程中，oSan节点掉电后恢复")
    decorator_func.multi_threads(run_vdb1, os_error)
    env_manage_lun_manage.co2_osan.run_vdb(client_ip=client_ip1, vdb_xml=mix_R_Align, jn_jro='jro',
                                           output=node_ip1,
                                           whether_change_xml="N")
