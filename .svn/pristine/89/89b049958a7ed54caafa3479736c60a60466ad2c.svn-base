...
def up_down():
    log.info("故障点:先将日志节点A数据网断开,日志节点B接管日志业务后再将日志节点B的数据网闪断")
    time.sleep(100)
    ...
    log.info("Step5:B节点接管A节点的日志业务后,将B节点的数据网闪断,日志节点B:%s" % los_node_ip_B)
    reliable.net_flash_test(node_ip=los_node_ip_B, netname=fault_eth_list_B)
