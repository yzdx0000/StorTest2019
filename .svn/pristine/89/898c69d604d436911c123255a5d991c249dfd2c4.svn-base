...
#   执行：
#           1>创建SMB客户端授权
#           2>执行get_events查看创建SMB客户端授权失败的结果显示
#           3>清理工作
# @changelog：
####################################################################################
...
smb_path = nas_common.ROOT_DIR + "SMB_" + FILE_NAME                     # parastor:/nas/SMB_events_5_3_3_62
...
def case():
    ...
    '''1>创建SMB客户端授权'''
    '''导出目录id不合法，预计创建失败'''
    fault_smb_export_id = smb_export_id + 1000
    nas_common.add_smb_export_auth_clients(export_id=fault_smb_export_id,
    ...
    time.sleep(delay_time)               # 创建SMB客户端授权失败，延时
    code = '0x02030708'                # 创建SMB客户端授权失败对应编号
    description = 'add_smb_export_auth_clients'
    '''创建SMB客户端授权失败事件码是否在事件列表中'''
