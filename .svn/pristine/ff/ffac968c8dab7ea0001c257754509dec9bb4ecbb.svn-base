...
if type_info != "phy":
    vm_ids = env_manage_repair_test.Reliable_osan.vm_id(esxi_ip=esxi_ip, u_name=cp('esxi', 'esxi_user'),
                                                        pw=cp('esxi', 'esxi_passwd'), node_ip=node_ip3)
else:
...
def os_reboot(arg=3):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    time.sleep(int(cp("wait_time", "remove_disk")) + 50)
...
def case():
    ...
    decorator_func.multi_threads(run_vdb1, run_vdb2, disk_error, os_reboot)
    log.info("step3:插回被拔出的磁盘触发修复,修复过程中将修复源节点重启,主机端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性 ...")
