...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将主oSan异常，再将守护进程异常；
            5、检查oSan进程状态
            6、主机1上业务完成后，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32996
import os
import sys
import commands
import threading
...
# 选择oSan进程进行异常操作
log.info("Get fault process:oSan & oMgcd")

def up_down():
    time.sleep(100)
    log.info("Step4:将故障节点oSan进程杀死 : %s." %(fault_node_ip))
    ...
    time.sleep(100)
    log.info("Step5:检查故障节点oSan进程状态 : %s." % (fault_node_ip))
