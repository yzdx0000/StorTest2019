...
4、在步骤3中的业务运行过程中，将业务节点A网配置成bond，所有业务和数据网线依次故障；
...
def up_down():
    ...
    time.sleep(100)
    log.info("Step3:在bond配置完成后,将业务节点 %s 的所有网卡依次故障." % test_node_ip)
    for eth in data_eth_list:
        log.info("将网卡 %s 故障." % eth)
        reliable.network_test(test_node_ip, eth, 'down')
        time.sleep(15)

    event1.wait()
    event2.wait()
    log.info("Step5:将业务节点 %s 的所有网卡恢复." % test_node_ip)
    for eth in data_eth_list:
