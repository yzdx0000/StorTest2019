...
4、在步骤3中的业务运行过程中，将业务节点网配置成bond，将节点A反复掉电重启3次；
...
def up_down():
    ...
    time.sleep(100)
    test_node_ip = random.choice(deploy_ips)
    log.info("Step3:在bond配置完成后,将业务节点 %s 反复掉电3次." % test_node_ip)
    for count in range(3):
        log.info("第 %d 次将业务节点 %s 掉电." % test_node_ip)
    ...
    log.info("Step5:等待业务完成后,检查数据修复完成")
    disk.check_bad_obj()
    log.info("Step6:将bond配置取消,恢复所有业务节点至初始状态.")
    for io_node_ip in node_and_eths.keys():
        io_eth_list = node_and_eths[io_node_ip]
