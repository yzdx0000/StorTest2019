...
def case():
    ...
    '''修改授权为1.1.0.45-90'''
    common.update_client_auth(client_auth_id, '1.1.0.45-90', volume1_id)

    '''修改授权为1.1.0.13,1.1.0.45-90'''
    common.update_client_auth(client_auth_id, '1.1.0.13,1.1.0.45-90', volume1_id)

    '''修改授权为1.1.0.13'''
    common.update_client_auth(client_auth_id, '1.1.0.13', volume1_id)

    '''修改授权，不添加client ip'''
    cmd = ("pscli --command=update_client_auth --auth_info=\'{\"id\":%s,\"ip\":\"\",\"volume_ids\":[%s]}\'" %(client_auth_id, volume1_id))
    log.info(cmd)
    rc, stdout, stderr = shell.ssh(system_ip, cmd)
    if 0 == rc:
        log.error("Execute command: \"%s\" failed. \nstdout: %s \nstderr: %s" % (cmd, stdout, stderr))
        ...
        if msg['err_no'] != 7005 or msg['err_msg'] != 'CLIENT_AUTH_IP_EMPTY':
            log.error("Execute command: \"%s\" failed. \nstdout: %s \nstderr: %s" % (cmd, stdout, stderr))
            raise Exception(
                "Execute command: \"%s\" failed. \nstdout: %s \nstderr: %s" % (cmd, stdout, stderr))
