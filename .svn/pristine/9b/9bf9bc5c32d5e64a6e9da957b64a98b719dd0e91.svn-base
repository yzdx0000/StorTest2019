...
def chmod_dir(ip , dir):
    lst = range(8)
    cmd = 'chmod %d%d%d %s' % (random.choice(lst), random.choice(lst), random.choice(lst), dir)
    rc, stdout = common.run_command(ip , dir)
...
def case():
    ...
    log.info('3> [主]主目录权限随机更改')
    chmod_dir(rep_common.MASTER_RANDOM_NODE, m_dir_lst[0])
