...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-0-1-78_su 目录用户组逻辑空间配额混合阈值，建议阈值先生
...
def executing_case():
    ...
    ob_node = common.Node()
    rc, quota_id = quota_common.get_one_quota_id(quota_common.NOTE_IP_1, quota_dir_for_id, quota_common.TYPE_GROUP,
    ...
    if total_file_size != quota_common.FILE_SIZE_1M * (soft_val_2 + 1536*2):
        ...
        log.error("11-0-1-78_su Failed")
        raise Exception("11-0-1-78_su Failed")
    else:
        log.info("11-0-1-78_su Succeed")
    return
...
class Quota_Class_11_0_1_78_su():
    def quota_method_11_0_1_78_su(self):
