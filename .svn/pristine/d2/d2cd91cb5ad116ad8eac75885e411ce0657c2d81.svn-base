...
# date 2018-08-01
# @summary：
#   三节点，升级后查看升级前设置的目录inode数软阈值配额的有效性
# @steps:
#   step1: 设置目录inode数软阈值配额，查看配额有效性
#   step2：执行在线升级
#   step3：再次查看配额的有效性
#
# @changelog：
...
def quota_soft_inode_check(path, quota_inode):
    """
    :author:               liujx
    :date:                 2018.08.01
    :description:          对于软阈值inode数的配额有效性检查
    :param path:           配额的路径
    ...
    log.info("second time: create 20 files within grace time, expect succeed")
    """再尝试另一个客户端继续写入文件，60秒内可以写入"""
    quota_common.creating_files(client_ip_lst[1], path, 20, 1, "b")

    log.info("Please wait for 70s")
    time.sleep(70)

    log.info("third time : create one file after grace time, expect fail")
    """再尝试另一个客户端继续写入文件，预期写入失败"""
    quota_common.creating_files(client_ip_lst[1], path, 1, 1, "c")
    """检查配额是否生效"""
    total_inodes = quota_common.dir_total_inodes(client_ip_lst[0], path)
    if total_inodes != (quota_inode + 20):
        log.error("quota_hard_inode_check failed, quota_inode=%d, but total_inode=%d" % (quota_inode+20, total_inodes))
        raise Exception("quota_hard_inode_check failed, quota_inode=%d, but total_inode=%d" % (quota_inode+20,
                                                                                               total_inodes))
    else:
        log.info("quota_soft_inode_check succeed")
...
def case():
    ...
    rc, pscli_info = quota_common.create_one_quota(path=QUORTA_PATH,
                                                   ...
                                                   filenr_soft_threshold=quota_inode,
                                                   filenr_grace_time=60)
    common.judge_rc(rc, 0, "create  quota failed")
    quota_soft_inode_check(QUORTA_TRUE_PATH, quota_inode)
