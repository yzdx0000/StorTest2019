...
# Date: 2018-09-03
# @summary：
#    用户组：父目录软阈值1G，宽限时间60秒，子目录软阈值1G，宽限时间60秒，子目录创建1100M文件开始计时，
#    过期后俩目录无法创建文件
# @steps:
#    1.创建用户用户组
...
#    5.子目录创建1100M文件开始计时，过期后俩目录无法创建文件
#    6.父目录删除配额创建文件成功
#    7.子目录删除配额创建文件成功
# @changelog：
#################################################################
...
def case():
    ...
    log.info("3> 创建目录FILENAME配额,软阈值1G 宽限时间60秒 等待配额状态为work")
    ...
    log.info("5> 子目录创建1100M文件开始计时，过期后俩目录无法创建文件")
    if LARGE_BS:
    ...
    if dir_size != 1100 * quota_common.FILE_SIZE_1M:
    ...
    quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_3, dir0, 1, 1, 'c', quota_common.QUOTA_USER, quota_id=quota_id_lst[0])
    ...
    quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_1, dir0, 100, 1, 'e',quota_common.QUOTA_USER)
    ...
    quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_2, dir1_1, 100, 1, 'f', quota_common.QUOTA_USER)
    dir_size = quota_common.user_or_group_total_file_size(quota_common.CLIENT_IP_2, dir1_1,quota_common.QUOTA_USER)
    if dir_size != 1200 * quota_common.FILE_SIZE_1M:
        rc_lst[sys._getframe().f_lineno] = 'dir: %s total_size: %s ' % (dir1_1, dir_size)
