...
wd=default,xfersize=4k,seekpct=0
wd=wd1,sd=sd1,rhpct=0,rdpct=0
wd=wd2,sd=sd1,rhpct=0,rdpct=100
rd=run1,wd=wd*,iorate=300,elapsed=600,maxdata=1G,threads=8,interval=1
预期：
1、步骤1，命令设置成功
2、步骤2，业务正常完成，记录性能
3、步骤3，命令设置成功
...
def gen_vdb_xml(max_range='100M', maxdata='1G', thread=8, offset=None, align=None, lun=None, xfersize=None, rdpct=None,
...
def run_vdbench():
    ...
    vdb_file = gen_vdb_xml(thread=8, lun=lun_name[:1], xfersize="4k", rdpct=["0", "100"],
                           seekpct=0, interval=1, rhpct=0)
...
def case():
    ...
    # lun_id2 = env_cache.create_lun(name="LUN2")
    ...
    log.info("step:6.主机端下发顺序读业务过程中业务节点掉电")
    threads = []
