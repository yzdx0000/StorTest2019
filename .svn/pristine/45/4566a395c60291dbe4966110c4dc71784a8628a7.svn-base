...
def get_avg_bandwidth(arg=1):
    ...
    osan.update_lun(s_ip=node_ip1, lun_id=lun_id, max_iops=int(avg_iops * 0.5))
    ...
    log.info("再次修改读带IOPS为实际值的30%")
    osan.update_lun(s_ip=node_ip1, lun_id=lun_id, max_iops=int(avg_iops * 0.3))
    avg_iops3 = env_manage_lun_manage.osan.Qos_count(c_ip=client_ip1, s_ip=node_ip1, qos_arg="iops")
    env_manage_lun_manage.assert_ins.assert_less_than(avg_iops, avg_iops3, avg_iops, percentage=0.3, suc_msg="qos修改生效")
