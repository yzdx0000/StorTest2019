...
:Description:
            ...
            6、数据修复过程中，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32940
import os
import sys
import commands
import threading
...
log.info("Get ethernet fault ip: %s" % (fault_node_ip))
...
def up_down():
    ...
    time.sleep(100)
    log.info("Step5:将故障节点的B网卡 %s 故障" % fault_eth_name_B)
    reliable.network_test(fault_node_ip, fault_eth_name_A, 'down')
    time.sleep(100)
