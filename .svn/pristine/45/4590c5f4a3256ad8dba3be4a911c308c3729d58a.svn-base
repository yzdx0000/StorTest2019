...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将节点D和节点E数据网同时故障；
            5、业务完成后，在主机1上执行vdbench -f OLTP.conf -jro比较一致性；
            6、数据修复过程中，在主机1上执行vdbench -f OLTP.conf -jro比较数据一致性；
            7、数据修复完成后，比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32945
import os
import sys
import threading
import random
...
# 随机选择两个业务接入节点ID并获取其IP
non_free_node_ids = error.get_non_free_node_id()
if len(non_free_node_ids) < 2:
    log.error("no jnl data node nums need >2")
    os._exit(1)
...
def up_down():
    time.sleep(200)
    log.info("Step4:将节点A的数据网和节点B的数据网同时断开,节点: %s,%s." % (fault_node_ip1, fault_node_ip2))
    threads_list = []
    for fault_eth_name1 in fault_eth_list1:
        threads_list.append(threading.Thread(target=reliable.network_test, args=(fault_node_ip1, fault_eth_name1, 'down')))
    for fault_eth_name2 in fault_eth_list2:
        threads_list.append(threading.Thread(target=reliable.network_test, args=(fault_node_ip2, fault_eth_name2, 'down')))
    for thread in threads_list:
        thread.start()
    for thread in threads_list:
        thread.join()
    time.sleep(100)
    log.info("Step5:将故障节点A的数据网卡恢复,节点:%s" % fault_node_ip1)
    for fault_eth_name1 in fault_eth_list1:
        reliable.network_test(fault_node_ip1, fault_eth_name1, 'up')
    log.info("Step5:将故障节点B的数据网卡恢复,节点:%s" % fault_node_ip2)
    for fault_eth_name2 in fault_eth_list2:
        reliable.network_test(fault_node_ip2, fault_eth_name2, 'up')
