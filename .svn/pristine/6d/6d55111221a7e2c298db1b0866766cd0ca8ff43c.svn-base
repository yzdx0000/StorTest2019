...
#   执行：
#           1>删除SMB客户端授权
#           2>执行get_events查看删除SMB客户端授权成功的结果显示
#           3>清理工作
# @changelog：
####################################################################################
...
smb_path = nas_common.ROOT_DIR + "SMB_" + FILE_NAME                     # parastor:/nas/SMB_events_5_3_3_65
...
def case():
    ...
    '''1>删除SMB客户端授权'''
    check_result = nas_common.remove_smb_export_auth_clients(ids=auth_clients_id)
    if check_result["detail_err_msg"] != "":
        common.except_exit("remove_smb_export_auth_clients is failed!!")
    '''2> 执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)               # 删除SMB客户端授权成功，延时
    code = '0x02030711'                # 删除SMB客户端授权成功对应编号
    description = 'remove_smb_export_auth_clients'
    '''删除SMB客户端授权成功事件码是否在事件列表中'''
