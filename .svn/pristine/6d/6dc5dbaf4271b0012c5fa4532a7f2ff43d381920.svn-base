...
# Date: 2018-08-09
# @summary：
#   add_smb_export_auth_clients的name参数使用group name，type参数使用GROUP，且组名不存在
# @steps:
...
#   2、add_smb_export_auth_clients指定name为不存在的group name，type为GROUP，run_as_root为true；
#   pscli --command=add_smb_export_auth_clients --export_id=x --name=not_exist_group_name --type=GROUP --run_as_root=true
#   3、update_smb_global_config配置home_dir；
#   pscli --command=update_smb_global_config --id=x --home_dir=volume:/homedir
#   4、mstsc到某windows设备，使用某个已存在组中的用户名进行挂载，在弹出的登录框中输入该用户名和密码；
#   5、尝试删除导出的策略；
#   6、尝试删除导出的目录；
# @changelog：
#   None
...
def executing_case():
    ...
    auth_provider_name = 'nas_16_0_7_27_ad_auth_name'
    ...
    unix_id_range = [8000, 8099]
    # for create_access_zone
    node_ids = '1,2,3'
    access_zone_name = 'nas_16_0_7_27_access_zone_name'
    # for create_file
    dir_name = 'nas_16-0-7-27_dir'
    ...
    export_name = 'nas_16_0_7_27_smb_export_name'
