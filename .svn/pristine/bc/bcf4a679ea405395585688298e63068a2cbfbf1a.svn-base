...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将节点A/B的oPmgr进程交替异常；
            5、检查各个节点ostor进程
            6、主机1上业务完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-33012
# testlink case: 1000-33013
import os
import sys
import commands
import threading
    ...
    for count in range(3):
        ...
        while True:
            if state_A == 'OK' or state_A == 'READY':
                break
            time.sleep(20)
        log.info("Step4:将B节点的oRole进程杀死,节点: %s" % (fault_node_ip_B))
        ReliableTest.run_kill_process(fault_node_ip_B,'oRole')
        time.sleep(100)
        log.info("Step5:获取故障节点B的oRole进程状态,节点: %s." % (fault_node_ip_B))
        state_B = reliable.check_process_stat(fault_node_ip_B, 'oRole')
        while True:
            if state_B == 'OK' or state_B == 'READY':
                break
            time.sleep(20)
