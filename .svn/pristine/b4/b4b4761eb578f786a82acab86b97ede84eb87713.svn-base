...
4、在步骤3中的业务运行过程中，将业务节点网配置成bond，将节点A、B同时掉电；
...
def up_down():
    ...
    log.info("Step3:在bond配置完成后,将业务节点 %s,%s 掉电." % (test_node_ip[0], test_node_ip[1]))
    vm_info1 = error.down_node(test_node_ip[0])
    vm_info2 = error.down_node(test_node_ip[1])
    time.sleep(100)
    log.info("Step4:将业务节点 %s,%s 重新上电." % (test_node_ip[0], test_node_ip[1]))
