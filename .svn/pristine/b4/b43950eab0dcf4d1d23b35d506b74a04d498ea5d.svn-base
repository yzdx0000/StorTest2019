...
# date 2018-05-24
# @summary：
#    对目录创建配额（文件数，软阈值），宽限时间内创建非空文件，创建快照并查看回滚结果
# @steps:
#    1、对目录/mnt/volume1/snap/创建配额（文件数，软阈值），配额限制为100，宽限时间2分钟；
#    2、在目录下创建150个文件（宽限时间内）；
#    3、对目录创建快照；
#    4、超过宽限时间后，删除50个文件；
#    5、在目录/mnt/volume1/snap/下尝试创建20个文件；
#    6、执行revert并查看回滚结果；
#    7、删除配额；
#
# @changelog：
...
SNAP_TRUE_PATH = os.path.join(snap_common.SNAP_PATH, FILE_NAME)             # /mnt/volume1/snap/snap_13_0_8_3
CREATE_SNAP_PATH = os.path.join(snap_common.SNAP_PATH_BASENAME, FILE_NAME)  # /snap/snap_13_0_8_3
