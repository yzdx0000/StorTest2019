...
# Date: 2018-09-03
# @summary：
#    用户组：目录配额  父目录硬阈值配额2000，子目录配额硬阈值为1000，父目录下创建2000个文件，子目录下创建文件失败
# @steps:
#    1.用户用户组
...
def case():
    ...
    common.rm_exe(quota_common.CLIENT_IP_1, os.path.join(dir0, 'file*'))

    log.info('wait 10s')
    time.sleep(10)
    quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_3, dir1_1, 1001, 1, 'b',
                                                            quota_common.QUOTA_USER)
    ...
    if dir1_1_inode != 1000:
        rc_lst[sys._getframe().f_lineno] = 'dir: %s expect: %s user_total_inode: %s ' % (dir0, 1000, dir1_1_inode)
