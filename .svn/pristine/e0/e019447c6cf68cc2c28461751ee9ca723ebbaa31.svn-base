...
def case():
    ...
    log.info("4> 获取桶%s的acl权限" % (bucket_name))
    rc = s3_common.get_bucket_acl(bucket_name, owner_certificate_id, exe_node_ip=None)
    judge_result(rc[0], FILE_NAME)
    common.judge_rc(rc[0], 0, "获取桶%s的acl权限, 失败" % bucket_name)
