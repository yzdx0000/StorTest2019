...
def confirm_acl(bucket_name, objectname, ak2, accid1, accid2, key):
    ...
    opt = []
    if key == "1":
        log.info("【桶读ACP权限验证1】,使用ak2对acc1的桶读取ACL.")
        rc1, opt1, opt3, opt4 = s3_common.get_bucket_acl(bucket_name, ak2)
        rc.append(rc1)  # "READ_ACP"
        opt.append(opt4)
        judge_result(rc1, FILE_NAME)
        common.judge_rc(rc1, 0, "%s failed!" % FILE_NAME)
        log.info("【桶读ACP权限验证2】,使用ak2对acc1的桶上传对象.")
...
def case():
    ...
    if "200 OK" in opt1 and "200 OK" in opt2 and "READ_ACP" == opt_l2[0][0] and "403" in opt_l2[1] and "200 OK"\
            in opt_l1[0] and "200 OK" in opt_l1[1] and "200 OK" in opt_l1[2] and "403" in opt_l1[3]:
        pass
    else:
