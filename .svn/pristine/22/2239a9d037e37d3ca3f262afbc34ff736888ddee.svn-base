...
def up_down():
    log.info("故障点:业务日志节点A与日志节点B的所有业务网")
    time.sleep(100)
    log.info("Step4:在步骤3中的业务运行过程中，将日志节点B业务网络故障,节点:%s" % (fault_node_ip_B))
    thread_list_B= []
    for io_eth in fault_eth_list_B:
        thread_list_B.append(threading.Thread(target=reliable.network_test, args=(fault_node_ip_B, io_eth, 'down')))
    ...
    for io_eth in fault_eth_list_B:
        reliable.network_test(fault_node_ip_B, io_eth, 'up')
