...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将ostor进程异常，守护进程异常；
            5、检查oStor进程状态
            6、主机1上业务完成后，在主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32998
import os
import sys
import commands
import threading
...
# 选择oSan进程进行异常操作
log.info("Get fault process:oStor & oMgcd")
...
def up_down():
    ...
    time.sleep(200)
    log.info("Step5:检查故障节点oStor进程状态,节点: %s." % (fault_node_ip))
    reliable.check_process_stat(fault_node_ip,'oStor')
    log.info("Step5:检查故障节点oMgcd守护进程状态,节点: %s." % (fault_node_ip))
