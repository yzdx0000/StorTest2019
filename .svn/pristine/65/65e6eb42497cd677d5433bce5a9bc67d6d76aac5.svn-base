...
def case():
    ...
    h_ids = osan.get_hosts(s_ip=deploy_ips[0])
    osan.add_initiator(s_ip=deploy_ips[0], h_id=h_ids[-1], iqn=cp("add_initiator", "iqn"),
                       alias=cp("add_initiator", "alias"))
    global lun_id
    for i in range(1, 2):
        lun_id = osan.create_lun(s_ip=deploy_ips[0], total_bytes='1073741824', lun_type="THICK",
    ...
    for lun_id in lun_ids:
        ...
        decorator_func.judge_target(
            ...
            'LUNMAP_READY')
    #lun_map_ids = osan.get_lun_maps(s_ip=deploy_ips[0])
