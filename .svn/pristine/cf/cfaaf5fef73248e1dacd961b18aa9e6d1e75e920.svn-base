...
测试内容:节点异常情况下创建逻辑卷
测试步骤：
逻辑卷创建过程中将日志组一台非业务节点拔掉电源
检查项：
逻辑卷创建成功
'''

# testlink case: 1000-33641
import os
import time
import random
import commands
...
def case():
    ...
    threads.append(t1)
    t2 = threading.Thread(target=os_down, args=(node_ip2,))  # 节点关机
    threads.append(t2)
    for i in threads:
        i.start()
    for i in threads:
        i.join()
    log.info("step:2.%s 节点开机 节点类型%s,节点ID or IPMI IP: %s" % (node_ip2, os_types[0], infos[0]))
