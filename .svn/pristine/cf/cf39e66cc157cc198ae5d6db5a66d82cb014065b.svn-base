...
# date 2018-05-07
# @summary：
# 16-0-4-121      修改到root_squash模式
# @steps:
# case1、设置NFS授权客户端的permission_constraint为all_squash；
# case2、修改NFS授权客户端的permission_constraint为root_squash；
# pscli --command=update_nfs_export_auth_client --id=1 --permission_constraint=root_squash
# @changelog：
#
#######################################################
import os
...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)  # /mnt/volume1/snap/snap_16_0_4_121
...
def executing_case1():
    ...
    access_zone_name = "access_zone_16_0_4_121"
    node = common.Node()
    ids = node.get_nodes_id()
    print ids
    access_zone_node_id_16_0_4_121 = ','.join(str(p) for p in ids)
    msg1 = nas_common.create_access_zone(node_ids=access_zone_node_id_16_0_4_121, name=access_zone_name)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        common.except_exit('%s create_access_zone failed!!!' % node_ip)
    global access_zone_id_16_0_4_121
    access_zone_id_16_0_4_121 = msg1["result"]

    # 2> 启动NAS
    log.info("\t[ enable_nas ]")
    msg2 = nas_common.enable_nas(access_zone_id=access_zone_id_16_0_4_121, protocol_types="NFS")
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        common.except_exit('%s enable_nas failed!!!' % node_ip)

    # 3> 查看NAS是否按配置启动
    log.info("\t[ get_access_zones ]")
    check_result3 = nas_common.get_access_zones(ids=access_zone_id_16_0_4_121)
    name = check_result3["result"]["access_zones"][0]["name"]
    if name != access_zone_name:
        common.except_exit('%s enable_nas failed!!!' % node_ip)
    global auth_provider_id_16_0_4_121
    auth_provider_id_16_0_4_121 = check_result3["result"]["access_zones"][0]["auth_provider_id"]

    """1> 检查目录并creat file"""
    log.info("\t[ case1-1 create_file ]")
    global nfs_path
    nfs_path = nas_common.ROOT_DIR + "nfs_dir_16_0_4_121"
    global NAS_PATH
    NAS_PATH = get_config.get_one_nas_test_path() + "/nfs_dir_16_0_4_121"
    cmd = "create_file "
    ...
    nfs_export_name = "nfs_exp_test_16_0_4_121"
    global description_nfs
    description_nfs = 'old_export_description'
    cmd = "create_nfs_export"
    check_result1 = nas_common.create_nfs_export(access_zone_id=access_zone_id_16_0_4_121,
    ...
    global nfs_export_id_16_0_4_121
    nfs_export_id_16_0_4_121 = msg1["result"]

    """4> get_ftp_exports"""
    log.info("\t[ case1-4 get_nfs_exports ]")
    cmd = "get_nfs_exports"
    check_result = nas_common.get_nfs_exports(ids=nfs_export_id_16_0_4_121)
    ...
    if access_zone_id != access_zone_id_16_0_4_121:
    ...
    check_result = nas_common.add_nfs_export_auth_clients(export_id=nfs_export_id_16_0_4_121,
    ...
    global nfs_export_clients_id_16_0_4_121
    nfs_export_clients_id_16_0_4_121 = msg["result"][0]

    """7> get_nfs_export_auth_clients"""
    log.info("\t[ case1-7 get_nfs_export_auth_clients ]")
    cmd = "get_nfs_export_auth_clients "
    check_result = nas_common.get_nfs_export_auth_clients(export_ids=nfs_export_id_16_0_4_121)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    msg = check_result
    if msg["err_msg"] != "" or msg["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    ...
    if id != nfs_export_clients_id_16_0_4_121:
        raise Exception('%s id failed!!!' % node_ip)
    if export_id != nfs_export_id_16_0_4_121:
    ...
    auth_clients_mount_dir = "/mnt/nfs_dir_16_0_4_121"
    cmd = "ssh %s ls /mnt/nfs_dir_16_0_4_121" % auth_clients_name
    rc, stdout, stderr = shell.ssh(node_ip, cmd)
    if stdout == "":
        cmd = "ssh %s umount -l /mnt/nfs_dir_16_0_4_121" % auth_clients_name
        shell.ssh(node_ip, cmd)
        cmd = "ssh %s rm -rf /mnt/nfs_dir_16_0_4_121" % auth_clients_name
        rc, stdout, stderr = shell.ssh(node_ip, cmd)
        if rc != 0:
            log.error("WARNING: \ncmd = %s\nstdout = %s\nstderr = %s" % (cmd, stdout, stderr))
    ...
    """11> 测试all_squash权限"""
    auth_clients_name = nas_common.NFS_1_CLIENT_IP
    log.info("\t[ case1-11 测试all_squash权限 ]")
...
#######################################################
# 2.executing_case2
# @function：修改NFS授权客户端的permission_constraint为root_squash；
# @parameter：
#       node_ip：执行cmd命令的节点ip
#       cmd：修改NFS授权客户端的permission_constraint为root_squash；
# @return：
#       check_result —— 执行create命令的字典格式返回值
# @steps:
# 1> 更改nfs客户端
...
def executing_case2():
    ...
    check_result = nas_common.update_nfs_export_auth_client(auth_client_id=nfs_export_clients_id_16_0_4_121,
                                                            permission_constraint=auth_clients_permission_constraint)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    msg = check_result
    if msg["err_msg"] != "" or msg["detail_err_msg"] != "":
    ...
    """4> 测试root_squash权限"""
    log.info("\t[ case2-4 测试root_squash权限 ]")
    ...
    cmd1 = "umount -fl /mnt/nfs_dir_16_0_4_121"
    ...
    cmd1 = "rm -rf /mnt/nfs_dir_16_0_4_121"
    rc, stdout, stderr = shell.ssh(auth_clients_name, cmd1)
    print stdout
    if rc != 0:
        log.info("rc = %s" % rc)
...
def clearing_environment():
    ...
    check_result1 = nas_common.delete_nfs_exports(ids=nfs_export_id_16_0_4_121)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
    msg1 = check_result1
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
