...
def up_down():
    ...
    for count in range(3):
        log.info("第 %d 次重启业务节点 %s." % ((count + 1), test_node_ip))
        time.sleep(10)
        log.info("Step5:将该业务节点 %s 重启." % test_node_ip)
        reliable.poweroff_os(test_node_ip, 'reboot')
        time.sleep(60)
        log.info("Step6:检查故障业务节点 %s 已恢复正常." % test_node_ip)
        reliable.get_os_status_1(test_node_ip)

    event1.wait()
    event2.wait()
    log.info("Step8:等待业务完成后,检查数据修复完成")
    disk.check_bad_obj()
    log.info("Step9:将bond配置取消,恢复业务节点 %s 至初始状态." % test_node_ip)
    reliable.bond_conf(test_node_ip, io_eth_list, 'bond0', switch=0)
    log.info("Step10:检查集群内部数据一致性.")
