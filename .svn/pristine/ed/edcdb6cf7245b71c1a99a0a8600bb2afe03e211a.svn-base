...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将节点A的oStor异常，再将oPmgr进程异常；
            5、检查oStor和oPmgr主进程状态
            6、主机1上业务完成后，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-33006
# testlink case: 1000-33006
import os
import sys
import commands
...
m_oRole_ip = reliable.get_master_oRole(fault_node_ip)
log.info("Get fault process:oStor&oRole")

def up_down():
    # 进程故障
    time.sleep(100)
    log.info("Step4:将故障节点oStor进程杀死,节点: %s." %(fault_node_ip))
    ...
    time.sleep(100)
    # 重新获取主,确保主起来,检查进程状态
    log.info("Step5:获取故障节点oStor进程状态,节点: %s." % (fault_node_ip))
    reliable.check_process_stat(fault_node_ip, 'oStor')
    log.info("Step5:检查故障节点oRole进程状态,节点: %s." % (m_oRole_ip))
