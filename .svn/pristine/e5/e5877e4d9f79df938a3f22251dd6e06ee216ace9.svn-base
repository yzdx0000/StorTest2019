...
:Description:
            ...
            5、五分钟后节点A上电恢复等数据修复完成后比较数据一致性；
            6、重复步骤4，步骤5动作4次；
            7、数据修复完成后，比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32311
import os,sys
import commands
import threading
import random
...
def up_down(pipeout):
    time.sleep(100)
    log.info("Step4:将节点A重启 : %s." % (fault_node_ip))
...
def vdb_jn(pipein):
    ...
    for i in range(4):
        ...
        log.info("Step5:修复完成后在主机1上执行vdbench -f mix_R_align.conf -jro比较一致性。.")
