...
# Date: 2018-09-03
# @summary：
#    用户：父目录软阈值1G，宽限时间60秒，三个子目录软阈值300M，宽限时间60秒，3个子目录分别创建400M文件，
#    过期后所有目录无法创建文件
# @steps:
#    1.创建用户用户组
...
def case():
    ...
    for i in range(3):
        rc, pscli_info = quota_common.create_one_quota(create_quota_dir1_[i],
                                                       ...
                                                       user_or_group_name=quota_common.QUOTA_USER,
                                                   logical_soft_threshold=quota_common.FILE_SIZE_1M * 300, logical_grace_time=60,
                                                   logical_quota_cal_type=quota_common.CAL_TYPE_LIMIT)
        common.judge_rc(rc, 0, "create  quota failed")
    ...
    log.info("5> 3个子目录分别创建400M文件开始计时，过期后3目录无法创建文件")
    for i in range(3):
        if LARGE_BS:
