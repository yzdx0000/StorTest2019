...
nodes = osan.get_nodes(deploy_ips)
...
def up_down(pipein):
    ...
    if str(c_id) == str(b_id):
        ...
        c_id = error.get_orole_master_id()
    if c_id in nodes:
        log.info("Master orole id is %s." % (str(c_id)))
        nodes.remove(c_id)
    d_id = random.choice(nodes)
    d_ip = disk.get_node_ip_by_id(n_id=d_id)
    log.info("The slave oRole's IP is %s." % (d_ip,))
    log.info("4、日志节点B下电，日志被接管时%s oRole备进程kill；" % (d_ip,))
    ReliableTest.run_kill_process(d_ip, 'oRole')
    while True:
