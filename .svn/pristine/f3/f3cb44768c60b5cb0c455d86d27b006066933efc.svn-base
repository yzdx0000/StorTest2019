...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将主oJmgs异常，再将守护进程异常；
            5、检查oMgcd进程状态
            6、主机1上业务完成后，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32991
import os
import sys
import commands
import threading
...
# 选择oJmgs进程异常时，oMgcd进程异常
log.info("Get fault process:oJmgs & oMgcd")

def up_down():

    time.sleep(200)
    log.info("Step4:将该节点oJmgs进程杀死,节点: %s." %(m_oJmgs_ip))
    ReliableTest.run_kill_process(m_oJmgs_ip,'oJmgs')
    log.info("Step4:将该节点oMgcds守护进程杀死,节点: %s." %(m_oJmgs_ip))
    ReliableTest.run_kill_process(m_oJmgs_ip,'oMgcd')
    time.sleep(300)
    log.info("Step5:检查该节点oJmgs进程状态,节点: %s." % (m_oJmgs_ip))
    reliable.check_process_stat(m_oJmgs_ip,'oJmgs')
    log.info("Step5:检查该节点oMgcd进程状态,节点: %s." % (m_oJmgs_ip))
    reliable.check_process_stat(m_oJmgs_ip, 'oMgcd')
