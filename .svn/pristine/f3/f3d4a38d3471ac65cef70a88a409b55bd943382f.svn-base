...
# Date: 2018-10-10
# @summary：
#    对父目录a创建硬阈值配额1G，创建子目录b硬阈值为2G，在父目录下创建文件512M，
#    移动父目录所有文件到子目录，删除子目录下文件，查看移动和删除操作后父目录和
#    父目录和子目录配额规则是否生效。
# @steps:
#    1.创建目录FILENAME/nesting1_1
#    2.创建目录FILENAME配额,硬阈值1G 等待配额状态为work
#    3.创建子目录FILENAME/nesting1_1为逻辑配额,硬阈值2G,等待配额状态为work
...
#    9.移动和删除操作后，查看配额规则是否有效
#    10.查看移动删除操作后的父目录和子目录的已使用量是否准确
# @changelog：
#################################################################
...
def case():
    ...
    log.info("3> 创建子目录FILENAME/nesting1_1为逻辑配额,硬阈值2G,等待配额状态为work")
    ...
    log.info("4> 父目录创建512M文件，预期total_size=512M")
    if LARGE_BS:
    ...
    log.info("7>父目录(预期512M):%s 子目录(预期512M):%s" %(dir0_logical_capacity, dir1_1_logical_capacity))
    ...
    if dir1_1_logical_capacity != 0:
        rc_lst[sys._getframe().f_lineno] = 'dir:%s total_size:%s' % (dir1_1, dir1_1_logical_capacity)
    log.info("8.2> 删除操作后检查：父目录(预计0):%s 子目录(预计0):%s" % (dir0_logical_capacity, dir1_1_logical_capacity))

    log.info("9> 移动和删除操作后，查看配额规则是否有效")
    log.info("9.1> 子目录创建1100M文件，预期1G")
    quota_common.creating_files(quota_common.CLIENT_IP_2, dir1_1, 1100, 1, 'after_mv_delete_create_again', quota_id=quota_id_lst[1])
    ...
    log.info("10> 查看移动删除操作后的父目录和子目录的已使用量是否准确")
    rc, quota_id_dir0 = quota_common.get_one_quota_id(create_quota_dir0,
                                                      quota_common.TYPE_CATALOG)
    common.judge_rc(rc, 0, "get_one_quota_id failed")
    dir0_logical_capacity = quota_common.get_quota_logical_used_capacity(quota_id_dir0)
    ...
    if dir1_1_logical_capacity != quota_common.FILE_SIZE_1G:
        rc_lst[sys._getframe().f_lineno] = 'dir:%s total_size:%s' % (dir1_1, dir1_1_logical_capacity)
    log.info("10.1> 父目录（预期1G）:%s 子目录（预期1G）:%s"%(dir0_logical_capacity, dir1_1_logical_capacity))
