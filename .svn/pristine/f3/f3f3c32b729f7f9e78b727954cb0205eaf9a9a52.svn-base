# -*- coding:utf-8 _*-
"""
测试内容:三副本下一个副本的所有对象损坏后写该数据

步骤:
1、在节点A创建LUN1
2、将逻辑卷映射至主机1上LUN1
3、在主机1上运行vdbench -f min-seq-w-1l.conf -jn；
4、业务完成后将存储池中一个节点的硬盘拔出，造成所有对象损坏；
5、在主机1上运行vdbench -f min-seq-r-1l.conf -jn；
6、插回故障盘开始修复，修复过程中执行主机端执行vdbench -f min-seq-w-1l.conf -jro校验数据一致性
7、数据修复完成后，比较内部数据一致性

检查项:
1、步骤4、业务执行完成后，系统处于待机状态，系统上报磁盘异常告警；
2、步骤5、读数据，检测到坏对象；
3、步骤6、修复过程中，内外部数据校验一致；
4,步骤7、修复完成后，内部数据校验一致。
"""

# testlink case: 1000-33193
import os
import sys
import random
import utils_path
...
def case():
    ...
    log.info("step3:业务完成后将存储池中一个节点的硬盘拔出，造成所有对象损坏；...")
    disk_error(1)
    log.info(
