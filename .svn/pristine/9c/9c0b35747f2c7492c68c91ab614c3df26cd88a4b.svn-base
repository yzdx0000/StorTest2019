...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-0-2-9 不支持目录配额规则嵌套之目录用户配额规则
#######################################################
...
def executing_case():
    ...
    2、结果检查
    '''
    # 尝试在父目录a的子目录b上再加一个目录用户配额规则，其中目录a上同时
    ...
    if (check_result1["detail_err_msg"] != "" or
        ...
        log.error("11-0-2-9 Failed")
        raise Exception("11-0-2-9 Failed")
    else:
        log.info("11-0-2-9 Succeed")
    return
...
class Quota_Class_11_0_2_9():
    def quota_method_11_0_2_9(self):
