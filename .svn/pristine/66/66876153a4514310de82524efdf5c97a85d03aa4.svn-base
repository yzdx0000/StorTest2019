...
#   执行：
#           1>删除SMB客户端授权
#           2>执行get_events查看删除SMB客户端授权失败的结果显示
#           3>清理工作
# @changelog：
####################################################################################
...
smb_path = nas_common.ROOT_DIR + "SMB_" + FILE_NAME                     # parastor:/nas/SMB_events_5_3_3_66
...
def case():
    ...
    nas_common.remove_smb_export_auth_clients(fault_auth_clients_id)
    '''2> 执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)               # 删除SMB客户端授权失败，延时
    code = '0x02030712'                # 删除SMB客户端授权失败对应编号
    description = 'remove_smb_export_auth_clients_failed'
    '''删除SMB客户端授权失败事件码是否在事件列表中'''
