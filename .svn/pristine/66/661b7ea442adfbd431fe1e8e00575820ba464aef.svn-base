...
a_data_eths, aextras = error.get_dataip_info(node_id=a_id)


def up_down(pipein):
    time.sleep(30)
    log.info("4、在步骤3中的业务运行过程中，将业务节点%s所有网络%s %s断开；" % (a_ip, a_eths, a_data_eths))
    log.info("Begin down node %s io net %s through %s." % (a_ip, a_eths, aextras))
    ReliableTest.run_down_net(aextras[0], a_eths)
    log.info("Begin down node %s data net %s through %s." % (a_ip, a_data_eths, aextras))
    ReliableTest.run_down_net(aextras[0], a_data_eths)
    time.sleep(600)
    log.info("5、10分钟后恢复数据网%s" % (a_data_eths,))
    log.info("Begin up node %s data net %s." % (a_ip, a_data_eths))
    ReliableTest.run_up_net(aextras[0], a_data_eths)
    while True:
