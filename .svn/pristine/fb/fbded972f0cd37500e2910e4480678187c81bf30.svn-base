...
4、在步骤3中的业务运行过程中，将A节点关机，将B节点ostor进程长时间故障；
...
def up_down(pipein):
    ...
    if 0 != rc:
        ...
        exit(1)
    time.sleep(10)
    log.info("4、在步骤3中的业务运行过程中，将A节点 %s 关机，将B节点 %s ostor进程长时间故障；" % (a_ip, b_ip))
    log.info("Begin shutdown node:%s." % (a_ip,))
    m_info = error.down_node(fault_ip=a_ip)
    log.info("On node %s kill oStor." % (b_ip,))
    cmd = ("ssh root@%s 'mv /home/parastor/bin/oStor mv /home/parastor/bin/oStor.bak'" % (b_ip,))
    ...
    while True:
        ...
        if 'up' in line:
            break
    log.info("7、等待节点超时后，将节点A %s 恢复 " % (a_ip,))
    log.info("Begin startup node %s through %s." % (a_ip, m_info))
    error.up_node(node_info=m_info)
    cmd = ("ssh root@%s 'mv /home/parastor/bin/oStor.bak mv /home/parastor/bin/oStor'" % (b_ip,))
    commands.getstatusoutput(cmd)
