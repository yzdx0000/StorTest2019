...
# Date  :2018-8-8
"""
测试内容:副本下，数据修复过程中，修复过程中将源节点数据网断开
步骤:
...
def case():
    ...
    log.info("step3:插回被拔出的磁盘触发修复,修复过程中将源节点数据网断开,主机端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性 ...")
    ...
    for i in fault_eth_list:
        env_manage_repair_test.Reliable_osan.network_test(node_ip2, i, "down")
    time.sleep(100)
    for i in fault_eth_list:
        env_manage_repair_test.Reliable_osan.network_test(node_ip2, i, "up")
    env_manage_repair_test.co2_osan.run_vdb(client_ip=client_ip1, vdb_xml=mix_R_Align, jn_jro='jro',
                                            output=node_ip1)
    log.info("step4:节点数据网恢复，数据修复完成系统恢复正常后，比较内部数据一致性 ...")
