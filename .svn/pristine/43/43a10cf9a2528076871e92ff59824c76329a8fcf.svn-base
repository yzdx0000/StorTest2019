...
#   7、disable nas, 删除vip_pool、删除子网、访问分区，清理环境
#
# @changelog：
####################################################################################
...
def case():
    ...
    '''3> 添加vip池，配置连续与单一结合的IP网段'''
    allocation_method_list = ['STATIC', 'DYNAMIC']
    used_protocol = 'NAS'
    used_method = random.choice(allocation_method_list)
    # vip_addresses_combine = nas_common.VIP_ADDRESSES + ',' + nas_common.VIP_2_ADDRESSES.split(',')[0]
    if '-' in nas_common.VIP_ADDRESSES and ',' in nas_common.VIP_ADDRESSES:
        vip_addresses_combine = nas_common.VIP_ADDRESSES
    elif '-' not in nas_common.VIP_ADDRESSES:
        vip_addresses_combine = nas_common.VIP_2_ADDRESSES + nas_common.VIP_ADDRESSES.split(',')[0]
    else:
        vip_ip_parts = re.split('[.-]', nas_common.VIP_ADDRESSES)
        constant_ip_start = int(vip_ip_parts[-2])
        constant_ip_end = int(vip_ip_parts[-2]) + 1
        constant_ip = vip_ip_parts[0] + '.' + vip_ip_parts[1] + '.' + vip_ip_parts[2] + '.' + \
                      str(constant_ip_start) + '-' + str(constant_ip_end)
        single_ip = vip_ip_parts[0] + '.' + vip_ip_parts[1] + '.' + vip_ip_parts[2] + '.' + vip_ip_parts[-1]
        vip_addresses_combine = constant_ip + ',' + single_ip

    exe_info = nas_common.add_vip_address_pool(subnet_id, nas_common.VIP_DOMAIN_NAME, vip_addresses_combine,
    ...
    if vip_pool_info['domain_name'] != nas_common.VIP_DOMAIN_NAME or vip_pool_info['subnet_id'] != subnet_id:
        common.except_exit('vip address pool information is wrong !!!')

    '''5> 命令行通过IP addr观察VIP是否绑定到3个节点到对应的网卡上（相同网段的网卡）'''
    ip_dict, xml_ip_list, eth_ip_list = nas_common.get_vip_from_eth(vip_addresses=constant_ip)
    ...
    ip_dict, xml_ip_list, eth_ip_list = nas_common.get_vip_from_eth(vip_addresses=single_ip)
    rc = nas_common.judge_vip_layinfo(ip_dict, xml_ip_list, eth_ip_list)
    common.judge_rc(rc, 0, 'there is something wrong with vip layout !!!')
    ...
    '''7> disable nas, 删除vip_pool、子网、访问分区，清理环境'''
    """disable nas 服务"""
