...
4、在步骤3中的业务运行过程中，将日志节点A节点业务网出现10%丢包；
...
def up_down():
    ...
    log.info("Step4:业务读写过程中,将日志节点业务网增加10%%的丢包率,节点:%s" % fault_node_ip)
    thread_list = []
    for io_eth in io_eth_list:
        cmd = ("ssh %s 'tc qdisc add dev %s root netem loss 10%%'" % (fault_node_ip, io_eth))
        log.info(cmd)
    ...
    time.sleep(10)
    log.info("业务网络注入故障后丢包情况,节点:%s" % fault_node_ip)
    reliable.check_ping(fault_ip=fault_node_ip, fault_eth_ip=phy_eth_ip, limit_loss=10)
    time.sleep(100)
    ...
    for io_eth in io_eth_list:
        cmd = ("ssh %s 'tc qdisc del dev %s root netem loss 10%%'" % (fault_node_ip, io_eth))
        log.info(cmd)
        rc = os.system(cmd)
        if rc == 0:
