...
disk_phy_id = random.choice(disk_ids)  # 元数据盘ID
disk_phy_id1 = random.choice(disk_ids1) # 数据盘ID
...
def up_down():
    ...
    for count in range(5):
        log.info("第 %d 次将一块源数据盘故障" % (count + 1))
        log.info("Step6:数据修复过程中,将一块修复源数据盘故障,节点:%s,磁盘:%s" % (fault_node_ip, disk_name1))
        env_manage_repair_test.Reliable_osan.remove_disk(fault_node_ip, disk_phy_id1, "DATA")
        time.sleep(30)
        log.info("Step7:将故障的源数据盘恢复,节点:%s,磁盘:%s" % (fault_node_ip, disk_name1))
