...
def up_down(pipein):
    ...
    while True:
        ...
        if 'down' in line:
            break
    time.sleep(60)
    log.info("\033[1;31;40m在业务运行过程中，将一带lmos日志角色节点删除；\033[0m")
    ip_lst = []
    lmos_node_ip_lst = this.get_lmos_ip_lst()
    node_orole_ip_lst = this.get_orole_node_ip_lst()

    if len(lmos_node_ip_lst) > len(node_orole_ip_lst):
        for n_ip in node_orole_ip_lst:
            lmos_node_ip_lst.remove(n_ip)
...
def vdb_jn(pipeout):
    ...
    this.vdbench_run(VOLUME, run_write_jn=True, ip_list=client_ip_list, elapsed=360, is_fault_exit=False)
