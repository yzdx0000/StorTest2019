...
#   执行：
#           1>删除本地用户组成功
#           2>执行get_events查看删除本地用户组成功的结果显示
# @changelog：
####################################################################################
...
def case():
    ...
    '''1> 删除本地用户组'''
    check_result = nas_common.delete_auth_groups(group_id)
    if check_result["detail_err_msg"] != "":
        common.except_exit("delete_auth_group")
    '''2> 执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)               # 删除本地用户组成功，延时
    code = '0x02030509'                # 删除本地用户组成功对应编号
    description = 'delete_auth_group'
    '''删除本地用户组成功事件码是否在事件列表中'''
