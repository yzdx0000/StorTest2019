...
# case4、修改认证IP；
# pscli --command=update_auth_provider_ldap --ip_addresses=x.x.x.x
...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)              # /mnt/volume/nas/nas_16_0_0_56
...
def executing_case1():
    ...
    check_result2 = nas_common.add_auth_provider_ldap(name="nas_16_0_0_56_ldap", base_dn=LDAP_BASE_DN,
    ...
    check_result2 = nas_common.get_auth_providers_ldap(ids=id_16_0_0_56)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    msg2 = check_result2
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    ...
    check_result2 = nas_common.check_auth_provider(provider_id=id_16_0_0_56)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    msg2 = check_result2
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    ...
    check_result2 = nas_common.update_auth_provider_ldap(name="nas_16_0_0_56_ldap", provider_id=id_16_0_0_56,
                                                         ip_addresses="10.2.41.140")
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
    msg2 = check_result2
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result2))
