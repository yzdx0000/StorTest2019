...
fault_eths_1, extra_ip_1 = error.get_dataip_info(node_id=node_1)
fault_eths_2, extra_ip_2 = error.get_dataip_info(node_id=node_2)


def up_down():
    time.sleep(20)
    log.info("Begin to down data net:%s through %s." % (fault_eths_1, extra_ip_1[0]))
    ReliableTest.run_down_net(extra_ip_1[0], fault_eths_1)
    time.sleep(40)
    log.info("Begin to up data net:%s through %s." % (fault_eths_1, extra_ip_1[0]))
...
def up_down2():
    time.sleep(60)
    log.info("Begin to down data net:%s through %s." % (fault_eths_2, extra_ip_2[0]))
    ReliableTest.run_down_net(extra_ip_2[0], fault_eths_2)
    time.sleep(40)
    log.info("Begin to up data net:%s through %s." % (fault_eths_2, extra_ip_2[0]))
