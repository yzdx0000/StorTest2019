...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-0-1-40 目录用户逻辑空间配额硬阈值之chown方式
#######################################################
...
def executing_case():
    ...
    # 检查配额是否生效
    total_file_size = quota_common.user_or_group_total_file_size(quota_common.CLIENT_IP_1, quota_common.QUOTA_PATH, quota_common.QUOTA_USER)
    if total_file_size != quota_common.FILE_SIZE_2G:
        rc, stdout = quota_common.get_all_quota_info()
        common.judge_rc(rc, 0, "get quota failed")
        log.error("11-0-1-40 Failed")
        raise Exception("11-0-1-40 Failed")
    else:
        log.info("11-0-1-40 Succeed")
    return
...
class Quota_Class_11_0_1_40():
    def quota_method_11_0_1_40(self):
