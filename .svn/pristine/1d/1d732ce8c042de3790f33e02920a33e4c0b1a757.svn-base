...
def case():
    ...
    '''1>制作修改访问分区失败事件'''
    '''令id不合法，预计修改失败'''
    update_node_ids = max(node_id_list)+1000  # 制作集群中不存在的节点id
    check_result = nas_common.update_access_zone(access_zone_id,
    ...
    code = '0x02030106'     # 修改访问分区失败的事件码
    description = 'update_access_zone_failed'
    '''查看修改访问分区失败事件码是否在事件列表中'''
