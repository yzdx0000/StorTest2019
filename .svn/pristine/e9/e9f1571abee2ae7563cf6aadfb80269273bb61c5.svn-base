...
4、一个节点的一块共享盘变为慢盘，延迟在1000ms以上（通过）；使用FIO工具（FIO
...
def up_down():
    time.sleep(100)
    log.info("Step4:业务过程中将节点 %s 的共享磁盘 %s 加压变成慢盘" % (fault_node_ip, fault_data_disk))
    ...
    os.system(cmd)
    time.sleep(60)
    log.info("Step5:确认节点 %s 的共享磁盘 %s 时延已经增大" % (fault_node_ip, fault_data_disk))
