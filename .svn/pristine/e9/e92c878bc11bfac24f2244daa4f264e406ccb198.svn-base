...
def case():
    ...
    special_char = ['~', '#', '$', '@', '%', '^', '+' '*']
    abnormal_name_list = random.sample(special_char, input_count)
    abnormal_name_str = ''.join(abnormal_name_list) + '.com'
    cmd = 'pscli --command=add_vip_address_pool --subnet_id=%s --domain_name=%s --vip_addresses=%s ' \
          '--supported_protocol=%s --allocation_method=%s --rebalance_policy=%s' % (subnet_id, abnormal_name_str,
          nas_common.VIP_ADDRESSES, used_protocol, used_method, 'RB_AUTOMATIC')
    rc, stdout = common.run_command(SYSTEM_IP_0, cmd)
    common.judge_rc_unequal(rc, 0, 'domain name length limitation did not work well !!!')

    '''4> 过pscli --command=get_vip_address_pools查看信息'''
    exe_info = nas_common.get_vip_address_pools()
    if exe_info['result']['total'] != 0:
