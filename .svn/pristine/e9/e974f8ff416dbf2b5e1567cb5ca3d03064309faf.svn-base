...
def disk_error(arg):
    ...
    env_manage_repair_test.Reliable_osan.remove_disk(node_ip=fault_ip, disk_id=disk_id, disk_usage="DATA")


def disk_insert(arg=1):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    env_manage_repair_test.Reliable_osan.insert_disk(node_ip=fault_ip, disk_id=disk_id, disk_usage="DATA")
...
def case():
    ...
    log.info("step3:业务完成后将存储池中一块硬盘拔出...")
    disk_error(1)
