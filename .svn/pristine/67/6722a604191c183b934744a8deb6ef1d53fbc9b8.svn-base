...
#########################################################
# author: liyao
# 概述：ad认证，1个多节点访问分区（之后分区修改为单节点），对根目录创建nfs导出，对各种IP形式授权（权限不同）
# 过程：1、创建一个3节点访问分区
#      2、修改访问分区为单节点访问区
...
def create_access_zones():
    ...
    ad_provider_name = 'ad_server'
    services_for_unix = random.choice(['NONE', 'RFC2307'])
    exe_info = nas_common.add_auth_provider_ad(name=ad_provider_name, domain_name=nas_common.AD_DOMAIN_NAME,
    ...
    return ad_provider_id, access_zone_id


def update_access_zones(access_zone_id, nodes_id_lst):
    '''修改访问分区中的节点数'''
    node_id_str = str(random.choice(nodes_id_lst))
    exe_info = nas_common.update_access_zone(access_zone_id=access_zone_id, node_ids=node_id_str)
    if exe_info["err_msg"] != "" or exe_info["detail_err_msg"] != "":
...
def case():
    """1> 创建访问分区，启动服务; 随后修改访问分区"""
    ad_provider_id, access_zone_id = create_access_zones()
    ...
    obj_node = common.Node()
    nodes_id_list = obj_node.get_nodes_id()
    update_access_zones(access_zone_id, nodes_id_list)
    ...
    abs_export_path = '/mnt/' + VOLUME_NAME
