...
def update_access():
    log.info("will update access node 1,2,3,4")
    env_manage.update_access(node_ip1, id_list="1,2,3,4")


def os_down_opmgr():
    time.sleep(30)
    log.info("will update the number of access zone node and master opmgr node shutdown ")
    threads = []
...
def case():
    ...
    log.info("step:4.set host and lun")
    ...
    log.info("step:5. ISCSI登录")
    create_iscsi_login(ips=node_ip1, cli_ips=client_ip1, svip=svip1)
    log.info("step:6.主机下发业务时，添加节点到访问区并使oPmgr主节点掉电")
    threads = []
    threads.append(threading.Thread(target=vdb_test))
    threads.append(threading.Thread(target=os_down_opmgr))
    for t in threads:
        t.start()
    for t in threads:
        t.join()
    log.info("step:7.check access number of node")
    new_nodes = env_manage.com_lh.get_access_zone_node()
    if len(new_nodes) == 4:
        log.info("step:8.the number of node meets expectation")
    else:
