...
# date 2018-11-22
# @summary：
#    nfs客户端权限测试：对文件进行chgrp操作，检验文件权限；原P200测试用例23-0-8-5。
# @steps:
...
def case():
    ...
    for u_name in user_name_list[:-1]:
        cmd_tmp = "ssh %s \"su %s -c 'cd %s;echo %s >> %s'\"" % (
            nfs_ip, u_name, nfs_file_location, echo_content, testfile)
        rc = common.command(cmd_tmp)
        common.judge_rc(
            rc, 0, "user %s, echo >> failed, the acl dosen't take effect" %
                   u_name)
    cmd_u3 = "ssh %s \"su %s -c 'cd %s;echo %s >> %s'\"" % (
        nfs_ip, user_name_list[2], nfs_file_location, echo_content, testfile)
