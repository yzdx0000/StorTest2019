...
# date 2018-11-17
# @summary：
#    nfs客户端权限测试：u1属于多个组，检测目录权限；原P200测试用例23-0-4-1。
# @steps:
...
def case():
    ...
    cmd = "ssh %s \"su %s -c 'cd %s;mkdir %s;chmod 555 %s'\" " % (
        nfs_ip, user_name, nfs_file_location, testdir, testdir)
    rc = common.command(cmd)
    ...
    log.info("6-3> 用户%s向%s中创建新目录" % (user_name, testdir))
    sub_dir_name = "%s" % user_name
    cmd = "ssh %s \"su %s -c 'cd %s;mkdir %s/%s'\"" % (
        nfs_ip, user_name, nfs_file_location, testdir, sub_dir_name)
    rc = common.command(cmd)
    common.judge_rc_unequal(
        rc, 0, "make subdir success, the acl dosen't take effect")
