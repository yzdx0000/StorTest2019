...
def iscsi_login():
    ...
    [env_manage_lun_manage.osan.create_lun(s_ip=node_ip1, lun_type="THICK",
                                           lun_name='LUN_{}'.format(i),
                                           stor_pool_id=storage_pool_id, acc_zone_id=az_id, total_bytes=536870912) for i
     in range(5, 10)]
...
def get_avg_bandwidth(arg=1):
    ...
    osan.update_lun(s_ip=node_ip1, lun_id=lun_ids[0], max_iops=int(avg_iops  * 0.4))
    osan.update_lun(s_ip=node_ip1, lun_id=lun_ids[1], max_iops=int(avg_iops * 0.4))
    osan.update_lun(s_ip=node_ip1, lun_id=lun_ids[6], max_iops=int(avg_iops  * 0.4))
    osan.update_lun(s_ip=node_ip1, lun_id=lun_ids[8], max_iops=int(avg_iops * 0.5))

    # avg_iops2 = env_manage_lun_manage.osan.Qos_count(c_ip=client_ip1, s_ip=node_ip1, qos_arg="iops")
    #
    # env_manage_lun_manage.assert_ins.assert_less_than(avg_iops, avg_iops2, avg_iops, percentage=1,
    #                                                   suc_msg="qos修改生效")
