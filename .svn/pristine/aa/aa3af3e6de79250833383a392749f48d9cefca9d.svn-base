...
def case():
    ...
    dir_lst = rep_common.create_dir(rep_common.MASTER_RANDOM_NODE, m_dir_lst[0], 'dir', 1)
    rc = rep_common.create_file(rep_common.MASTER_RANDOM_NODE, dir_lst[0], 'file', 1, 1)
    common.judge_rc(rc, 0, 'create_file failed')
    rc, md5_1 = rep_common.md5sum(rep_common.MASTER_RANDOM_NODE, os.path.join(dir_lst[0], 'file_1'))
    ...
    cmd  = 'ls %s' % (os.path.join(dir_lst[0], 'file_1')).replace(m_dir_lst[0], s_dir_lst[0])
    rc, stdout = common.run_command(rep_common.SLAVE_RANDOM_NODE, cmd)
    common.judge_rc(rc, 0, 'cmd %s failed' % cmd)
    rc, md5_2 = rep_common.md5sum(rep_common.SLAVE_RANDOM_NODE, (os.path.join(dir_lst[0], 'file_1')).
                                  replace(m_dir_lst[0], s_dir_lst[0]))
    common.judge_rc(rc, 0, 'md5sum failed')
    if md5_1 != md5_2:
        common.except_exit('md5 check failed')

    log.info('4> [主]删除某目录，并创建同名目录，在其中创建文件，起pair任务')
