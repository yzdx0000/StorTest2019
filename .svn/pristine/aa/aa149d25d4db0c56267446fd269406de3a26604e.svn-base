...
def executing_case():
    """
        1、输入错误的smb global configuration id来修改smb公共参数；
        pscli --command=update_smb_global_config --id=250 --home_dir="parastor:/homes" --enable_ntfs_acl=true --allow_create_ntfs_acl=false
        注：smb global configuration id 250不存在；
    :return:
    """
    ...
    check_result3 = nas_common.update_smb_global_config(smb_global_config_id=not_exist_id,
                                                        enable_ntfs_acl="true",
                                                        allow_create_ntfs_acl="false")
    if check_result3["detail_err_msg"].find("Can not find the smb global config by id") == -1:
        log.error(("%s Failed") % FILE_NAME)
        raise Exception(("%s Failed") % FILE_NAME)
    else:
