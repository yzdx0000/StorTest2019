...
def up_down(pipein):
    time.sleep(30)
    log.info("4、在步骤3中的业务运行过程中，将%s主oPmgr故障；" % (master_orole_ip,))
    ReliableTest.run_kill_process(master_orole_ip, 'oRole')
    log.info("4、再将节点A %s 业务网%s断开；" % (a_ip, a_eths))
