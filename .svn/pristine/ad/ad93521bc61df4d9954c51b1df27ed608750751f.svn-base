...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-0-2-36 创建一个不存在用户的目录用户的配额
#######################################################
...
def executing_case():
    ...
    rc, check_result = common.create_quota(path=('%s:/%s' % (quota_common.VOLUME_NAME, quota_dir)),
                                           ...
                                           user_or_group_name='quota_no_exist_user')
    common.judge_rc(rc, 0, "create  quota failed", exit_flag=False)
    # 结果检查
    if common.json_loads(check_result)["err_no"] == 0:
        rc, check_result = quota_common.get_all_quota_info()
        common.judge_rc(rc, 0, "get quota failed")
        log.error("11-0-2-36 Failed")
        raise Exception("11-0-2-36 Failed")
    else:
        log.info("11-0-2-36 Succeed")
    return
...
class Quota_Class_11_0_2_36():
    def quota_method_11_0_2_36(self):
