...
def disk_error(arg=0):
    ...
    env_manage_repair_test.Reliable_osan.remove_disk(node_ip=node_ip1, disk_id=disk_phy_id2, disk_usage="DATA")
...
def case():
    ...
    env_manage_repair_test.co2_osan.run_vdb(client_ip=client_ip1, vdb_xml=min_seq_w, jn_jro='jn', output=node_ip1)
    log.info("在步骤3业务进行过程中，将lun写满后，再重新执行,在步骤3业务进行过程中，将同磁盘组多块数据盘故障")
    decorator_func.multi_threads(run_vdb_w, disk_error)
    log.info("故障盘超时后立即开始数据修复...")
    env_manage_repair_test.Reliable_osan.run_down_disk_wait(s_ip=node_ip1,
                                                            timeout=cp('timeout', 'disk_timeout'))  # 设置修复的超时时间

    time.sleep(int(cp('timeout', 'disk_timeout'))/int(cp('timeout', 'unit_time')) + 100)  # 设置等待超时的时间
    log.info(
    ...
    env_manage_repair_test.Reliable_osan.insert_disk(node_ip=node_ip1, disk_id=disk_phy_id2, disk_usage="DATA")
