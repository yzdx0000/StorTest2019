...
# Date: 2018-09-03
# @summary：
#    用户组 硬阈值配额3000，创建3个子目录配额硬阈值为1000，3个子目录下各写1001个文件
# @steps:
#    1.创建用户用户组
...
def case():
    ...
    dir_total_inode = []
    for i in range(3):
        dir_total_inode.append(quota_common.group_total_inodes(quota_common.CLIENT_IP_3, dir1_[i], quota_common.QUOTA_GROUP))
