...
def case():
    ...
    iscsi_login()
    log.info("step2:在主机1上运行vdbench -f min-seq-w-1l.conf -jn,业务过程中将一个数据副本的一块数据盘删除 ...")
    decorator_func.multi_threads(disk_error, run_vdb)

    log.info("step3:移除后触发主动重建,主机端执行vdbench -f min-seq-write.conf -jro校验数据一致性 ...")
