...
def case():
    ...
    log.info("step:2.修改逻辑卷过程中断开网卡")
    threads = []
    t1 = threading.Thread(target=env_manage.update_luns)
    threads.append(t1)
    t2 = threading.Thread(target=env_manage.down_network, args=(node_ip2, data_net))
    threads.append(t2)
    for t in threads:
        t.start()
    for t in threads:
