...
#    nfs目录用户inode数配额混合阈值
# @steps:
#    1、创建ad认证服务器，创建访问分区，启动NAS
#    2、创建共享目录，导出并授权某个用户的导出权限
#    3、针对目录配置inode数为100硬阈值,50软阈值（宽限时间为2分钟），30建议阈值的配额
#    4、通过nfs客户端先写入50个文件，2分钟内再写入50个文件；宽限时间内再次尝试写入，检查配额是否生效
#    5、通过nfs客户端先写入50个文件，2分钟内再写入20个文件；超过2分钟后再次尝试写入，检查配额是否生效
#    6、删除配额及所创建的文件
...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)             # /mnt/volume/nas_test_dir/nas_16_6_0_19
...
def case():
    ...
    '''4> 通过nfs客户端先写入50个文件,2分钟内再写入50个文件，超过2分钟后继续尝试写入'''
    """首先写入50个文件"""
    for i in range(50):
    ...
    common.rm_exe(SYSTEM_IP_2, deleted_file_name)

    rc, stdout = quota_common.delete_one_quota(quota_id)
    common.judge_rc(rc, 0, "del quota info failed")

    '''5> 通过nfs客户端先写入50个文件，2分钟内再写入20个文件；超过2分钟后再次尝试写入'''
    rc, check_result = quota_common.create_one_quota(path=quota_path,
                                                     filenr_quota_cal_type='QUOTA_LIMIT',
                                                     filenr_suggest_threshold=2,
                                                     filenr_soft_threshold=3,
                                                     filenr_grace_time=120,
                                                     filenr_hard_threshold=100)
    common.judge_rc(rc, 0, 'create quota failed !!!')
