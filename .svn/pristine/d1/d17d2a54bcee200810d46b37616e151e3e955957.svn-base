...
# date 2018-04-20
# @summary：
#    创建目录快照，一个客户端修改文件的过程中，断掉另一个客户端的数据网，恢复数据网后，观察快照数据
# @steps:
#    1、部署3节点集群，配比4 2 1，部署三个客户端；
#    2、客户端1在目录/mnt/parastor/snap/下执行vdbench创建文件；
#    3、对目录/mnt/parastor/snap/创建快照；
#    4、断开客户端2的数据网，客户端1在目录/mnt/parastor/snap/下执行vdbench修改文件；
#    5、修改完成后，恢复客户端2的数据网，客户端2在目录/mnt/parastor/.snapshot/下执行vdbench对数据进行校验；
#    6、删除快照；
#    7、检查是否有快照路径入口；
#
# @changelog：
...
SNAP_TRUE_PATH = os.path.join(snap_common.SNAP_PATH, FILE_NAME)              # /mnt/liyao/snap/snap_13_1_3_12
CREATE_SNAP_PATH = os.path.join(snap_common.SNAP_PATH_BASENAME, FILE_NAME)   # /snap/snap_13_1_3_12
...
def case():
    ...
    '''4> 断开客户端2的数据网，客户端1执行vdbench修改文件'''
    eth_name_list, ip_mask_lst = grab_client_global(snap_common.CLIENT_IP_2)
    p1 = Process(target=vdbench_run, args=(SNAP_TRUE_PATH, snap_common.CLIENT_IP_1), kwargs={'run_write': True})
    ...
    vdbench_run(snap_path, snap_common.CLIENT_IP_2, run_check=True)
