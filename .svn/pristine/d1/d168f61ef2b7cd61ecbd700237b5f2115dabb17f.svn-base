...
# date 2018-07-27
# @summary：
#    快照策略更新失败（等待get_events接口完成）
# @steps:
#   1、部署3节点集群环境
#   2、在/mnt/volume1/snap/snap_create_success/目录下创建数据
#   3、对上述目录创建快照策略（参数输入正确，预期成功）
#   4、执行update_snapshot_strategy更新快照策略，输入参数错误（预期更新失败）
#   5、执行get_events查看快照策略更新结果显示
#   6、删除快照策略，清理环境
#
# @changelog：
####################################################################################
...
def case():
    ...
    ''' 2> 在/mnt/volume1/snap/snap_strategy_update_fail/目录下创建数据'''
    for i in range(10):
    ...
    ''' 4> 执行update_snapshot_strategy更新快照策略，输入参数错误（预期更新失败）'''
    rc, stdout = snap_common.update_snapstrategy(0, 'BY_DAY', hours=1, minute=1, expire_time=180)
    if rc != 0:
        log.error('snapshot strategy %s update failed !!!' % snap_strategy_name)
    ...
    code = '0x02060046'
    description = 'updating snapshot strategy'
