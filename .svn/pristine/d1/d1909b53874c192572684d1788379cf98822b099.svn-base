...
def case():
    ...
    log.info("step:2.修改逻辑卷过程中网卡闪断")
    threads = []
    t1 = threading.Thread(target=env_manage.update_luns)
    threads.append(t1)
    t2 = threading.Thread(target=env_manage.com_lh.net_flash_test, args=(node_ip1, subnet_eth))
    threads.append(t2)
    for t in threads:
        t.start()
    for t in threads:
