...
#   执行：
#           1>删除NFS共享
#           2>执行get_events查看删除NFS共享失败的结果显示
#           3>清理工作
# @changelog：
####################################################################################
...
def case():
    ...
    '''1> 删除导出NFS共享目录'''
    '''令导出id不合法，预计删除NFS共享失败'''
    fault_nfs_export_id = nfs_export_id + 1000
    nas_common.delete_nfs_exports(fault_nfs_export_id)

    '''2> 执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)               # 删除NFS共享失败，延时
    code = '0x02030606'                # 删除NFS共享失败对应编号
    description = 'delete_nfs_export'
    '''删除NFS共享失败事件码是否在事件列表中'''
