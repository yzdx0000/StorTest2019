...
3、创建LUN和对应的映射，使同一节点A的4个VIP关联的LUN不同（可以通过指定LUN映射的target，构造两个网口上的两个VIP关联不同数量的LUN，
   如eth2上VIP1和VIP2，VIP1关联1条LUN，VIP2关联1条LUN；eth3上VIP3和VIP4，VIP3关联2条LUN，VIP4关联4条LUN）
...
def case():
    ...
    osan.add_vip_address_pool(s_ip=deploy_ips[0], subnet_id=subnet_id[0], domain_name='sugon.com', vip=vip,
                              ip_failover_policy='IF_CONNECTION_COUNT', rebalance_policy='RB_DISABLED')
    ...
    if rebalance_policy == 'RB_DISABLED' and ip_failover_policy == 'IF_CONNECTION_COUNT':
        log.info("自动均衡策略设置成功.")
    else:
    ...
    osan.lun_map_by_target(s_ip=deploy_ips[0], lun_ids='2', target_id=targets1[1])
    osan.lun_map_by_target(s_ip=deploy_ips[0], lun_ids='3,4', target_id=targets2[0])
    ...
    if sum(luns_list1) == sum(luns_list2) and sum(luns_list1) == 2:
        log.info("自动均衡成功.")
    else:
