...
6、节点B下电超时后上电，启动数据修复，数据修复中，在主机1上执行vdbench -f OLTP.conf -jro比较一致性；
7、节点B数据修复完成，在主机1上执行vdbench -f OLTP.conf -jro比较一致性；
8、比较存储内部数据一致性
:Changerlog:
"""
# testlink case: 1000-32481
import os,sys
import commands
import threading
import random
...
def vdb_jn(pipeout):
    ...
    log.info("5、主机1上业务完成后，在主机1上执行vdbench OLTP.conf -jro比较一致性；")
    osan.run_vdb(client_ips[0], mix_S_align, output=deploy_ips[0], jn_jro="jro")
    log.info("6、节点B下电超时后上电，启动数据修复，数据修复中，在主机1上执行vdbench -f OLTP.conf -jro比较一致性；")
    osan.run_vdb(client_ips[0], mix_S_align, output=deploy_ips[0], jn_jro="jro")
    # 通知故障注入线程，启动两个节点
    ...
    log.info("6、等待系统修复。")
    disk.check_bad_obj()
    log.info("7、节点B数据修复完成，在主机1上执行vdbench -f OLTP.conf -jro比较一致性；")
    osan.run_vdb(client_ips[0], mix_S_align, output=deploy_ips[0], jn_jro="jro")
