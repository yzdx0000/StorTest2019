...
测试步骤：
1）配置访问区，映射逻辑卷到主机
2）映射过程中制造一台日志组非业务节点数据网+日志组业务节点oJmgs进程异常
检查项：
1）访问区配置成功
2）逻辑卷映射成功
'''

# testlink case: 1000-33929
import os
import time
import random
import commands
...
def net_prro_test(net_name):
    log.info("node %s will kill oJmgs process" % (ojmgs_ips[0]))
    ReliableTest.run_kill_process(node_ip=ojmgs_ips[0], process="oJmgs")
    log.info("node %s will down network" % (test_net_ip[0]))
...
def case():
    ...
    check_ip()
    log.info("step:2.杀进程关网卡时创建lun map")
