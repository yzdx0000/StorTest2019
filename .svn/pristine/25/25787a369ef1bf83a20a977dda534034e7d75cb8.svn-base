...
#########################################################
# author: liyao
# 概述：ad认证，创建smb导出（非默认值），授权单个IP（对用户授权）
# 过程：1、创建3节点访问分区，启动服务
#      2、创建本地用户组/用户
#      3、创建默认参数的smb导出并对单个IP进行授权（用户）
...
def create_access_zone():
    ...
    exe_info = nas_common.add_auth_provider_ad(name=ad_name, domain_name=nas_common.AD_DOMAIN_NAME,
                                               ...
                                               username=nas_common.AD_USER_NAME, password=nas_common.AD_PASSWORD,
                                               services_for_unix='RFC2307')
    if exe_info["err_msg"] != "" or exe_info["detail_err_msg"] != "":
        common.except_exit('add auth provider ad failed !!!')
    ad_provider_id = exe_info['result']

    exe_info = nas_common.create_access_zone(nodes_id_str, access_zone_name, auth_provider_id=ad_provider_id)
    if exe_info["err_msg"] != "" or exe_info["detail_err_msg"] != "":
    ...
    return access_zone_id, ad_provider_id


def get_ad_users(user_num):
    '''获取ad认证服务器上的用户'''
    user_name_lst = []
    for i in range(user_num):
        user_name_lst.append(get_config.get_ad_users()[i])
...
def smb_mount_create(mount_vip, mount_export_name, mount_passwd, mount_user):
    ...
    log.info('waiting for 5s')
    time.sleep(5)
    rc = rs.run_keyword(name='create_dir_file', args=(DISK_SYMBOL, 2, 3))
    return rc
...
def case():
    ...
    """1> 创建访问分区"""
    access_zone_id, ad_provider_id = create_access_zone()
    log.info('waiting for 5s')
    time.sleep(5)
    ...
    time.sleep(5)
    user_num = 3
    user_name_lst = get_ad_users(user_num)
    ...
    authorization_ip = str(WIN_AUTH_CLIENT)  # smb导出中对单个IP进行限制
    ...
    auth_num = 3
    if auth_num > int(user_num):   # 控制添加授权的数目不能超过用户总数
        auth_num = user_num
    else:
        pass
    ...
    for i in range(auth_num):
        if i == 0:
            exe_info = nas_common.add_smb_export_auth_clients(export_id=export_id, name=user_name_lst[0],
                                                              user_type='USER', run_as_root=auth_right_lst[0])
            if exe_info["err_msg"] != "" or exe_info["detail_err_msg"] != "":
                log.error('add smb auth client failed !!!')
            auth_id_lst.append(exe_info['result'][0])
            user_name_lst.remove(user_name_lst[0])
        else:
            exe_info = nas_common.add_smb_export_auth_clients(export_id=export_id, name=user_name_lst[0],
    ...
    mount_user = nas_common.AD_DOMAIN_NAME.split('.')[0] + '\\' + random.choice(write_read_users_lst)
