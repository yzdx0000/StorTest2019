...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-3-1-5 【稳定性】配置下发过程中down掉任意节点
#######################################################
...
def down_node_when_create_quota():
    cmd = "echo b > /proc/sysrq-trigger"

    # 注意：down的节点不能是脚本执行的节点
    rc, stdout = common.run_command(quota_common.NOTE_IP_3, cmd)  # rc=255
...
def executing_case():
    ...
    p2 = Process(target=down_node_when_create_quota, args=())
    ...
    if p1.exitcode != 0:
        raise Exception("11-3-1-5 Failed")
    else:
        print "11-3-1-5 Succeed"
    return
...
class Quota_Class_11_3_1_5():
    def quota_method_11_3_1_5(self):
