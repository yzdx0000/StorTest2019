...
# date 2018-05-07
# @summary：
# 16-0-4-120      修改到iNsecure模式
# @steps:
# case1、设置NFS授权客户端的port_constraint为secure；
# case2、修改NFS授权客户端的port_constraint为insecure；
# pscli --command=update_nfs_export_auth_client --id=1 --port_constraint=insecure
# @changelog：
#
#######################################################
import os
...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)  # /mnt/volume1/snap/snap_16_0_4_120
...
def executing_case1():
    ...
    access_zone_name = "access_zone_16_0_4_120"
    node = common.Node()
    ids = node.get_nodes_id()
    print ids
    access_zone_node_id_16_0_4_120 = ','.join(str(p) for p in ids)
    msg1 = nas_common.create_access_zone(node_ids=access_zone_node_id_16_0_4_120, name=access_zone_name)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        common.except_exit('%s create_access_zone failed!!!' % node_ip)
    global access_zone_id_16_0_4_120
    access_zone_id_16_0_4_120 = msg1["result"]

    # 2> 启动NAS
    log.info("\t[ enable_nas ]")
    msg2 = nas_common.enable_nas(access_zone_id=access_zone_id_16_0_4_120, protocol_types="NFS")
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        common.except_exit('%s enable_nas failed!!!' % node_ip)

    # 3> 查看NAS是否按配置启动
    log.info("\t[ get_access_zones ]")
    check_result3 = nas_common.get_access_zones(ids=access_zone_id_16_0_4_120)
    name = check_result3["result"]["access_zones"][0]["name"]
    if name != access_zone_name:
        common.except_exit('%s enable_nas failed!!!' % node_ip)
    global auth_provider_id_16_0_4_120
    auth_provider_id_16_0_4_120 = check_result3["result"]["access_zones"][0]["auth_provider_id"]

    """1> 检查目录并creat file"""
    log.info("\t[ case1-1 create_file ]")
    global nfs_path
    nfs_path = nas_common.ROOT_DIR + "nfs_dir_16_0_4_120"
    global NAS_PATH
    NAS_PATH = get_config.get_one_nas_test_path() + "/nfs_dir_16_0_4_120"
    cmd = "create_file"
    ...
    nfs_export_name = "nfs_exp_test_16_0_4_120"
    global description_nfs
    description_nfs = 'old_export_description'
    cmd = "create_nfs_export"
    check_result1 = nas_common.create_nfs_export(access_zone_id=access_zone_id_16_0_4_120,
    ...
    global nfs_export_id_16_0_4_120
    nfs_export_id_16_0_4_120 = msg1["result"]

    """4> get_ftp_exports"""
    log.info("\t[ case1-4 get_nfs_exports ]")
    cmd = "get_nfs_exports"
    check_result = nas_common.get_nfs_exports(ids=nfs_export_id_16_0_4_120)
    ...
    if access_zone_id != access_zone_id_16_0_4_120:
    ...
    check_result = nas_common.add_nfs_export_auth_clients(export_id=nfs_export_id_16_0_4_120,
    ...
    global nfs_export_clients_id_16_0_4_120
    nfs_export_clients_id_16_0_4_120 = msg["result"][0]

    """7> get_nfs_export_auth_clients"""
    log.info("\t[ case1-7 get_nfs_export_auth_clients ]")
    cmd = "get_nfs_export_auth_clients "
    check_result = nas_common.get_nfs_export_auth_clients(export_ids=nfs_export_id_16_0_4_120)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    msg = check_result
    if msg["err_msg"] != "" or msg["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    ...
    if id != nfs_export_clients_id_16_0_4_120:
        raise Exception('%s id failed!!!' % node_ip)
    if export_id != nfs_export_id_16_0_4_120:
    ...
    auth_clients_mount_dir = "/mnt/nfs_dir_16_0_4_120"
    cmd = "ssh %s ls /mnt/nfs_dir_16_0_4_120" % auth_clients_name
    rc, stdout, stderr = shell.ssh(node_ip, cmd)
    if stdout == "":
        cmd = "ssh %s umount -l /mnt/nfs_dir_16_0_4_120" % auth_clients_name
        rc, stdout, stderr = shell.ssh(node_ip, cmd)
        cmd = "ssh %s rm -rf /mnt/nfs_dir_16_0_4_120" % auth_clients_name
        rc, stdout, stderr = shell.ssh(node_ip, cmd)
        if rc != 0:
            log.error("WARNING: \ncmd = %s\nstdout = %s\nstderr = %s" % (cmd, stdout, stderr))
...
#######################################################
# 2.executing_case2
# @function：修改NFS授权客户端的port_constraint为insecure；
# @parameter：
#       node_ip：执行cmd命令的节点ip
#       cmd：修改NFS授权客户端的port_constraint为insecure；
# @return：
#       check_result —— 执行create命令的字典格式返回值
# @steps:
# 1> 更改nfs客户端
...
def executing_case2():
    ...
    check_result = nas_common.update_nfs_export_auth_client(auth_client_id=nfs_export_clients_id_16_0_4_120,
                                                            port_constraint=auth_clients_port_constraint)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    msg = check_result
    if msg["err_msg"] != "" or msg["detail_err_msg"] != "":
    ...
    """4> 测试insecure权限"""
    log.info("\t[ case2-4 测试insecure权限 ]")
    wait_time1 = random.randint(5, 8)
    time.sleep(wait_time1)
    cmd2 = "file1=`grep %s /etc/exports | head -n 1` " % auth_clients_name + \
           "&& file2=`echo ${file1#*async,}` && echo ${file2%,root_squash*}"
    ...
    cmd1 = "umount -fl /mnt/nfs_dir_16_0_4_120"
    ...
    cmd1 = "rm -rf /mnt/nfs_dir_16_0_4_120"
    rc, stdout, stderr = shell.ssh(auth_clients_name, cmd1)
    print stdout
    if rc != 0:
        log.info("rc = %s" % rc)
...
def clearing_environment():
    ...
    check_result1 = nas_common.delete_nfs_exports(ids=nfs_export_id_16_0_4_120)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
    msg1 = check_result1
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
