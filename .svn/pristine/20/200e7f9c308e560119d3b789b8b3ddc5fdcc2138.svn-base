...
4、业务过程中手动删除一块元数据盘
5、数据重建时，重建目标盘间隔5分钟内反复故障
6、主机端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性
7、恢复故障的目标盘，数据重建和修复完成后，比较内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-33354
import os
import sys
import commands
import threading
...
def disk_fault():
    time.sleep(100)
    log.info("Step4:业务过程中主动将一块共享磁盘删除,触发重建,节点:%s,磁盘:%s" % (fault_node_ip, disk_name))
    env_manage_repair_test.com_disk.remove_disks(disk_ids=disk_id)

def up_down():
    time.sleep(150)
    log.info("Step5:重建过程将该共享盘反复插拔,节点:%s,磁盘:%s" % (fault_node_ip,disk_name))
    for count in range(3):
        log.info("第 %d 次反复插拔该重建目标盘" % (count + 1))
        log.info("将该目标数据磁盘拔出...")
        ...
        time.sleep(30)
    time.sleep(100)
    log.info("Step7:将故障的共享盘恢复,节点:%s,磁盘:%s" % (fault_node_ip,disk_name))
    while True:
