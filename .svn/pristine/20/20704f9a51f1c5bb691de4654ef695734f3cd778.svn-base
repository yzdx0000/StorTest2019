...
def lun_map_case():
    ...
    threads = []
    threads.append(threading.Thread(target=env_manage.create_lun_map, args=(lun_los_ip, lun_id, target_id)))
    threads.append(threading.Thread(target=node_fault, args=(node_ip, os_type)))  # 源节点掉电
    for i in threads:
        i.start()
    for i in threads:
        i.join()
    log.info("step:9.%s 节点开机 节点类型%s,节点ID or IPMI IP: %s" % (node_ip, os_types[0], os_infos[0]))
