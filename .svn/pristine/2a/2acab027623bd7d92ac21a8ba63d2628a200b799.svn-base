...
def executing_case():
    """
        ...
        2、创建共享路径volume:/nas/ftp_dir
        pscli --command=create_file --path=volume:/nas/ftp_dir
        3、在az1上创建ftp共享，输入正确的名称和共享路径；
        pscli --command=create_ftp_export --access_zone_id=x --export_name=ftp_exp_test --export_path=volume:/nas/ftp_dir
        4、查看nfs共享路径配置成功，信息是否与配置信息匹配；
        pscli --command=get_ftp_exports
    :return:
    """
    ...
    # create_file
    create_file_path1 = nas_common.ROOT_DIR + "nas_16-0-4-131_dir"
    create_file_path2 = create_file_path1 + "/" + "dir"
    # create_ftp_export
    export_name = "nas_16_0_4_131_ftp_export_name"
    '''
    ...
    '''
    check_result2 = nas_common.create_file(path=create_file_path1)
