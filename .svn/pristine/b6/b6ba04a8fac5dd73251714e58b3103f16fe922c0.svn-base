...
# -*- coding:utf-8 -*-
#**********************************************#
# 标题：功能测试，用例编号29-0-9-2
# 使用方法：python 29-0-9-xx_wjl.py
# 将账户1的bucket赋予账户2读ACP权限。
# 将账户1的object赋予账户2写ACP权限。
# 作者：王建磊
# 创建时间：2018/09/10
import sys
import os
...
# *******************************************
# 设置账户ACL函数
# 设置ACL，将acc1的bucket赋予acc2读ACP权限。
# 将acc1的object赋予acc2写ACP权限
# *******************************************
...
# ACL验证函数
# 用ak2对桶读ACL操作。
# 用ak2对acc1的对象先修改为READ权限，在下载该对象。
# *******************************************


def confirm_acl(bucket_name, objectname, filepath, ak2, accid1, accid2):
    log.info("使用ak2对acc1的对象ACL修改为READ，验证对象的ACL写ACP权限")
    rc1, opt1 = s3_common.set_object_acl(
        bucket_name, objectname, ak2, accid1, email1, accid2, email2, "READ")
    ...
    log.info("使用ak2对acc1的桶读取ACL操作，验证桶的ACL读ACP权限。")
    rc2, opt2 = s3_common.get_bucket_acl_info(bucket_name, ak2)
    judge_result(rc2, FILE_NAME)
    common.judge_rc(rc2, 0, "%s failed!" % FILE_NAME)
    return md5_d, opt2
