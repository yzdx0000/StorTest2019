...
if FLAG:
    ...
    def dd_thread(arg):
        msg = env_manage_lun_manage.Reliable_osan.dd_command(client_ip1, arg, '8M', '64', need_judge=1)
    ...
    def case():
        ...
        luns = env_manage_lun_manage.osan.ls_scsi_dev(client_ip1)
        log.info("主进程开始执行>>> pid={}".format(os.getpid()))
        threads = []
        results_l = []
        for i in luns:
            t = MyThread(dd_thread, args=(i,))
            threads.append(t)
        for t in threads:
            t.start()
        for t in threads:
            t.join()
            results_l.append(t.get_result())
        log.info("主进程终止")

        log.info(results_l)

        results_str = " ".join(str(i) for i in results_l)
        log.info(results_str)
        env_manage_lun_manage.assert_ins.assertIN("Input/output error", results_str)
