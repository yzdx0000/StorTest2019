...
stor_id_block = env_manage_repair_test.Lun_osan.get_storage__type_id(s_ip=node_ip2, type="SHARED")
...
def disk_error(arg=0):
    ...
    time.sleep(100)  # 等待间隔时间，执行下一步
    env_manage_repair_test.Reliable_osan.insert_disk(node_ip=node_ip1, disk_id=disk_phy_id1, disk_usage="SHARED")
    time.sleep(100)  # 等待间隔时间，执行下一步

    env_manage_repair_test.Reliable_osan.remove_disk(node_ip=node_ip2, disk_id=disk_phy_id2, disk_usage="DATA")
...
def case():
    ...
    iscsi_login()
    log.info(
        "step2:在主机1上运行vdbench -f mix-R-Align.conf -jn；在主机2上运行vdbench -f mix-R.conf,业务过程中将存储池元数据磁盘拔出一块 ...")
    decorator_func.multi_threads(run_vdb1, run_vdb2, disk_error)
    log.info("step3:插回被拔出的磁盘触发修复,修复过程中将修复源数据盘故障,主机端执行vdbench -f mix-R-Align.conf -jro;校验数据一致性 ...")

    env_manage_repair_test.co2_osan.run_vdb(client_ip=client_ip1, vdb_xml=mix_R_Align, jn_jro='jro',
                                            output=node_ip1)
    log.info("step4:数据修复完成系统恢复正常后，比较内部数据一致性 ...")
    env_manage_repair_test.Reliable_osan.insert_disk(node_ip=node_ip2, disk_id=disk_phy_id2, disk_usage="DATA")
