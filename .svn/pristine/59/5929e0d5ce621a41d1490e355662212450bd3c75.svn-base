...
# Date: 2018-09-03
# @summary：
#    用户组 父目录软阈值1000，宽限时间60秒，三个子目录软阈值300，宽限时间60秒，3个子目录分别创建400文件，
#    过期后所有目录无法创建文件
# @steps:
#    1.用户用户组
...
def case():
    ...
    dir_inode = quota_common.group_total_inodes(quota_common.CLIENT_IP_1, dir1_[0], quota_common.QUOTA_GROUP)
    if dir_inode != 400:
        rc_lst[sys._getframe().f_lineno] = 'dir: %s total_inode: %s ' % (dir1_[0], dir_inode)
    dir_inode = quota_common.group_total_inodes(quota_common.CLIENT_IP_2, dir1_[1], quota_common.QUOTA_GROUP)
    if dir_inode != 400:
        rc_lst[sys._getframe().f_lineno] = 'dir: %s total_inode: %s ' % (dir1_[1], dir_inode)
    dir_inode = quota_common.group_total_inodes(quota_common.CLIENT_IP_3, dir1_[2], quota_common.QUOTA_GROUP)
    if dir_inode != 400:
