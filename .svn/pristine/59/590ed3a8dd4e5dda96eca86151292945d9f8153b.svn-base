...
def case():
    ...
    log.info("step:3.vdb运行过程中，修改访问区，主oPmgr 线程异常")
    threads = []
    threads.append(threading.Thread(target=vdb_test))
    threads.append(threading.Thread(target=update_az_pro, args=(az_id1,)))
    for t in threads:
        t.start()
    for t in threads:
        t.join()
