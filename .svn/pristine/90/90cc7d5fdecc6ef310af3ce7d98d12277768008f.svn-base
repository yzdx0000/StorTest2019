...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-0-2-45 修改目录配额规则之改大、改小建议阈值
#######################################################
...
def executing_case():
    ...
    # 验证配额是否生效，预期成功再写入1001个文件
    cmd = "cd %s; for i in {4001..5001}; do touch file_$i; done" % (quota_common.QUOTA_PATH)
    rc, stdout = common.run_command(quota_common.CLIENT_IP_1, cmd)
    if rc != 0:
        log.info("rc = %s" % (rc))
        log.info("WARNING: \ncmd = %s\nstdout = %s\n" % (cmd, stdout))
    ...
    if (check_result1["detail_err_msg"] != "" or
        ...
        rc, check_result = quota_common.get_all_quota_info()
        common.judge_rc(rc, 0, "get quota failed")
        log.error("11-0-2-45 Failed")
        raise Exception("11-0-2-45 Failed")
    else:
        log.info("11-0-2-45 Succeed")
    return
...
class Quota_Class_11_0_2_45():
    def quota_method_11_0_2_45(self):
