...
#   执行：
#           1>修改SMB客户端授权
#           2>执行get_events查看修改SMB客户端授权失败的结果显示
#           3>清理工作
# @changelog：
####################################################################################
...
smb_path = nas_common.ROOT_DIR + "SMB_" + FILE_NAME                     # parastor:/nas/SMB_events_5_3_3_64
...
def case():
    ...
    '''1>修改SMB客户端授权'''
    '''客户端id不合法，预计修改失败'''
    fault_auth_clients_id = auth_clients_id + 1000
    nas_common.update_smb_export_auth_client(auth_client_id=fault_auth_clients_id,
                                             run_as_root="false",
                                             permission_level="ro")
    '''2> 执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)               # 修改SMB客户端授权失败，延时
    code = '0x02030710'                # 修改SMB客户端授权失败对应编号
    description = 'update_smb_export_auth_client_failed'
    '''修改SMB客户端授权失败事件码是否在事件列表中'''
