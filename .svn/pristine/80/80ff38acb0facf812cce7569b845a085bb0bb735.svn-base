...
# date 2018-12-6
# @summary：
#    bucket创建的同时，创建bucket的ACL
# @steps:
#    1、创建两个账户，一个所有者账户，一个目标账户；
#    2、检查账户是否存在
#    3、创建证书；
#    4、创建桶，给所有者账户创建桶
#    5、创建文件，并给所有者账户上传对象
#    6、
#    7、目标账号下创建桶，同时设置桶的ACL权限
#    8、检查环境
#    9、清理环境
#
# @changelog：
...
def case():
    ...
    log.info("6> 目标账户下创建桶，同时设置桶的ACL权限")
    ACL_TYPE = "FULL_CONTROL"
    bucket_name_2 = FILE_NAME + '_bucket_2'
    bucket_name_2 = bucket_name_2.replace('_', '-')
    threads = []
    thread1 = MyThread(s3_common.add_bucket, args=(bucket_name_2, certificate_id_2, ))
    thread2 = MyThread(s3_common.set_bucket_acl, args=(bucket_name_2, certificate_id_2, account_id_2,
                                          ACCOUNT_EMAIL_2, account_id, ACCOUNT_EMAIL, ACL_TYPE, ))
