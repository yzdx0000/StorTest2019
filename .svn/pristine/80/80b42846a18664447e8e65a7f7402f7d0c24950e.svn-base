...
4、在步骤3中的业务运行过程中，将所有节点数据网出现20%丢包；
...
for node_ip in deploy_ips:
    ...
    node_eth_list[node_ip] = data_eth_list

def up_down():
    time.sleep(100)
    for node_ip in deploy_ips:
        log.info("数据网络注入故障前的延迟及丢包情况,节点:%s" % node_ip)
        data_eth_list = node_eth_list[node_ip]
        phy_eth_ip = reliable.get_ip_by_eth(node_ip, data_eth_list[0])
        reliable.check_ping(fault_ip=node_ip, fault_eth_ip=phy_eth_ip)
    log.info("Step4:业务读写过程中,将集群所有日志节点数据网增加10%的丢包率")
    thread_list = []
    for node_ip in deploy_ips:
        data_eths = node_eth_list[node_ip]
        for data_eth in data_eths:
            cmd = ("ssh %s 'tc qdisc add dev %s root netem loss 20%%'" % (node_ip, data_eth))
            thread_list.append(threading.Thread(target=os.system, args=(cmd,)))

    log.info("检查网络异常节点管理通路是否正常")
    for node_ip in deploy_ips:
        osan.get_nodes(node_ip)
    ...
    for node_ip in deploy_ips:
        log.info("数据网络注入故障后的丢包情况,节点:%s" % node_ip)
        data_eth_list = node_eth_list[node_ip]
        phy_eth_ip = reliable.get_ip_by_eth(node_ip, data_eth_list[0])
        reliable.check_ping(fault_ip=node_ip, fault_eth_ip=phy_eth_ip, limit_loss=20)
    ...
    log.info("Step6:等待业务结束后,将所有节点数据网络恢复正常")
    for node_ip in deploy_ips:
        data_eths = node_eth_list[node_ip]
        for data_eth in data_eths:
            cmd = ("ssh %s 'tc qdisc del dev %s root netem loss 20%%'" % (node_ip, data_eth))
            log.info(cmd)
            rc = os.system(cmd)
            if rc == 0:
                log.info("数据网 %s 恢复成功,节点:%s" % (data_eth, node_ip))
