...
# Date  :2018-8-10
'''
测试内容：添加VIP池过程中oPmgr异常
测试步骤：
1）配置节点池，配置存储池
2）配置访问区
3）创建业务子网，创建过程中分别制造主ojmgs和主oPmgr故障，从进程接管业务，创建成功
检查项：
1）节点池，存储池创建成功
2）访问区创建成功
3）业务子网创建成功
...
def case():
    log.info("step:1.创建访问区创建svip")
    env_manage.create_access()
    env_manage.create_subnet()
    log.info("step:2.创建业务子网，kill进程oJmgs和pmgr")
    threads = []
    t1 = threading.Thread(target=env_manage.create_vip)
    threads.append(t1)
