...
io_eth_list, a_extra = error.get_ioip_info(node_id=fault_node_id)
log.info(io_eth_list)
...
def iscsi_login():
    ...
    mix_R = env_manage_repair_test.co2_osan.gen_vdb_xml(max_range=cp("vdbench", "range"),
                                                        ...
                                                        rdpct=rdpct1, offset=int(cp("vdbench", "offset")))
...
def network_error():
    decorator_func.timer(int(cp("wait_time", "remove_disk")) + 30)
    for i in io_eth_list:
