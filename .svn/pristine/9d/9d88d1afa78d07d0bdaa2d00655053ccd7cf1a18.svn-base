...
def up_down():
    log.info("故障点:将日志节点A的数据网闪断,free日志节点接管")
    time.sleep(100)
    log.info("Step4:业务过程中,将日志节点A的数据网闪断,日志节点A:%s" % los_node_ip_A)
    reliable.net_flash_test(node_ip=los_node_ip_A, netname=fault_eth_list_A)

    log.info("Step6:数据修复完成系统恢复正常后,比较内部数据一致性")
