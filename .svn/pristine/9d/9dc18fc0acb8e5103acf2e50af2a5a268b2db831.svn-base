...
# date 2018-11-22
# @summary：
#    posix客户端权限测试：对文件进行chown操作，检验文件权限；原P200测试用例23-0-8-7。
# @steps:
...
def case():
    ...
    cmd = "ssh %s \"cd %s;chown %s:%s %s\"" % (
        client_ip, posix_file_location, user_name_list[1], group_list[1], testfile)
    rc = common.command(cmd)
    ...
    cmd_u1 = "ssh %s \"su %s -c 'cd %s;echo %s >> %s'\"" % (
        client_ip, user_name_list[0], posix_file_location, echo_content, testfile)
    ...
    cmd_u2 = "ssh %s \"su %s -c 'cd %s;echo %s >> %s'\"" % (
        client_ip, user_name_list[1], posix_file_location, echo_content, testfile)
