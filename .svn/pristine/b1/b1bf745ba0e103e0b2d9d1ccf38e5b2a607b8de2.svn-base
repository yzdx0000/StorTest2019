...
4、在步骤3中的业务运行过程中，将A节点关机，将节点B一个业务网线闪断10秒；
...
def up_down(pipein):
    time.sleep(10)
    log.info("4、在步骤3中的业务运行过程中，将A节点关机，将节点B一个业务网线闪断10秒；")
    log.info("Begin shutdown node:%s." % (a_ip, ))
    m_info = error.down_node(fault_ip=a_ip)
    b_eth = []
    b_eth.append(random.choice(b_eths))
    log.info("Begin down io net %s through %s." % (b_eth, b_extra[0]))
    ReliableTest.run_down_net(b_extra[0], b_eth)
    time.sleep(10)
    log.info("Begin up io net %s through %s." % (b_eth, b_extra[0]))
