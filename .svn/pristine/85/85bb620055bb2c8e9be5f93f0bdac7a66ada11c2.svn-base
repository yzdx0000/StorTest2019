...
def lun_map_case():
    ...
    log.info("step:7.创建lun map 过程中关闭lun los 目的节点网卡")
    subnet_eth = env_manage.com_lh.get_vip_eth_name(node_ip)
    threads = []
    threads.append(threading.Thread(target=env_manage.create_lun_map, args=(lun_los_ip, lun_id, target_id)))
    threads.append(threading.Thread(target=date_net_fault, args=(node_ip, subnet_eth)))  # 目的节点断网
    for i in threads:
        i.start()
    for i in threads:
        i.join()
    log.info("step:9.%s 节点网卡 %s 恢复" % (node_ip, subnet_eth))
    env_manage.up_network(node_ip, subnet_eth)
