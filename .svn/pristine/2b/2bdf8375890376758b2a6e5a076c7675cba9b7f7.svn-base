...
error.rel_check_before_run(file_name, node_num=4)
# 生成vdb配置文件
    ...
    for key in node_pool_info.keys():
        ...
        for s_ip in deploy_ips:
            if ReliableTest.check_ping(s_ip):
                lun_osan.update_node_pool(s_ip, node_ids, node_pool_id, node_pool_name, err=False)
                break
        break
...
def down_node(nodeid):
    ...
    if nodeid in nodes:
        nodes.remove(nodeid)
    s_node_id = random.choice(nodes)
    data_eths_a, avail_ips_a = error.get_dataip_info(node_id=nodeid)
    data_eths_b, avail_ips_b = error.get_dataip_info(node_id=s_node_id)
    log.info("5、将非均衡源节点 %s 和目标节点 %s 数据网闪断." % (avail_ips_a[0], avail_ips_b[0]))
    log.info("Begin down net : %s through %s." % (data_eths_a, avail_ips_a[0]))
    ReliableTest.run_down_net(avail_ips_a[0], data_eths_a)
    log.info("Begin down net : %s through %s." % (data_eths_b, avail_ips_b[0]))
    ReliableTest.run_down_net(avail_ips_b[0], data_eths_b)
    time.sleep(300)
    log.info("Begin up net : %s through %s." % (data_eths_a, avail_ips_a[0]))
    ReliableTest.run_up_net(avail_ips_a[0], data_eths_a)
    log.info("Begin up net : %s through %s." % (data_eths_b, avail_ips_b[0]))
    ReliableTest.run_up_net(avail_ips_b[0], data_eths_b)
