...
def cancel_disk_error():
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    decorator_func.timer(int(cp("wait_time", "remove_disk")) + 20)
    env_manage_repair_test.com_disk.cancel_delete_disk(disk_id=disk_id)
...
def case():
    ...
    iscsi_login()
    log.info("在主机1上运行vdbench -f min-seq-write.conf -jn；业务过程中将一个节点的数据盘删除； ...")
    decorator_func.multi_threads(disk_error, run_vdb, cancel_disk_error)
