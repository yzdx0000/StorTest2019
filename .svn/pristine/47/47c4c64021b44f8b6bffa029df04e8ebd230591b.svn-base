...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-0-1-13 目录逻辑空间配额混合阈值，建议阈值最大
#######################################################
...
def executing_case():
    ...
    # 针对目录配置逻辑空间为2G硬阈值、3G软阈值（宽限天数为1天）、4G建议阈值的配额
    ...
    if check_result["err_no"] == 0:
        ...
        log.error("11-0-1-13 Failed")
        raise Exception("11-0-1-13 Failed")
    else:
        log.info("11-0-1-13 Succeed")
    return
...
class Quota_Class_11_0_1_13():
    def quota_method_11_0_1_13(self):
