...
def case():
    ...
    log.info("2> 创建父目录配额,硬阈值100 等待配额状态为work")
    ...
    log.info("3> 创建子目录FILENAME/nesting1_1配额,硬阈值50,等待配额状态为work")
    rc, pscli_info = quota_common.create_one_quota(create_quota_dir1_1,
                                                   filenr_hard_threshold=50,
                                                   filenr_quota_cal_type=quota_common.CAL_TYPE_LIMIT)
    common.judge_rc(rc, 0, "create  quota failed")
    ...
    log.info("4> 子目录创建25文件，预期total_inode=25")
    log.info("写入数据前：做chkmulti检测")
    quota_common.chkmult_client_all_dir_on_path(dir1_1)
    quota_common.creating_files(quota_common.CLIENT_IP_1, dir1_1, 25, 1, 'a', quota_id=quota_id_lst[1])
    ...
    for i in range(25):
        cmd = "cd %s;ln -s file_%s_a_%s soft_%s" % (dir1_1, quota_common.CLIENT_IP_1, i+26, i+26)
        rc, stdout = common.run_command(quota_common.NOTE_IP_1,cmd)
        common.judge_rc(rc, 0, "soft_ln failed!")

    log.info("6> 子目录中再写文件失败，预计子目录中inode个数是50")
