...
# date 2018-01-19
#@summary：
#    创建单个目录快照，移动目录，执行revert，目录回滚到之前的内容，移出去的目录是否删除
#@steps:
#    1、部署3个客户端；
#    2、在目录/mnt/parastor/snap/下创建子目录test_dir1和子文件test_file1；
#    3、对目录/mnt/parastor/snap/创建快照a1；
#    4、客户端1 mv目录/mnt/parastor/snap/test_dir1/ 为/mnt/parastor/snap/snap_dir/test_dir1/；
#    5、对快照a1进行revert；
#    6、3个客户端检查/mnt/parastor/snap/的属性和内容；
#    7、客户端2 mv目录/mnt/parastor/snap/test_dir1/为/mnt/parastor/snap1/test_dir1/；
#    8、对快照a1进行revert；
...
def case():
    ...
    # 创建子目录
    dir_path_2 = os.path.join(dir_path_1, 'test_dir_1_1')
    cmd = 'mkdir %s' % dir_path_2
    common.run_command(snap_common.CLIENT_IP_1, cmd)

    dir_path_3 = os.path.join(dir_path_1, 'test_dir_2_1')
    cmd = 'mkdir %s' % dir_path_3
    common.run_command(snap_common.CLIENT_IP_1, cmd)

    dir_path_4 = os.path.join(SNAP_TRUE_PATH, 'test_dir_2')
    cmd = 'mkdir %s' % dir_path_4
    common.run_command(snap_common.CLIENT_IP_1, cmd)
    ...
    # 4> 转移目录
    cmd = 'mv %s %s' % (dir_path_2, dir_path_3)
    common.run_command(snap_common.CLIENT_IP_1, cmd)
    ...
    if rc != 0:
        log.error("%s is not exist!!!" % dir_path_2)
        raise Exception("%s is not exist!!!" % dir_path_2)
    dir_new_path_1 = os.path.join(dir_path_3, 'test_dir_1_1')
    cmd = 'ls %s' % dir_new_path_1
    rc, stdout = common.run_command(snap_common.CLIENT_IP_2, cmd)
    if rc == 0:
        log.error("%s is exist!!!" % dir_new_path_1)
        raise Exception("%s is exist!!!" % dir_new_path_1)

    # 7> 转移目录
    cmd = 'mv %s %s' % (dir_path_2, dir_path_4)
    common.run_command(snap_common.CLIENT_IP_1, cmd)
    ...
    rc, stdout = common.run_command(snap_common.CLIENT_IP_2, cmd)
    if rc == 0:
        log.error("%s is exist!!!" % dir_path_2)
        raise Exception("%s is exist!!!" % dir_path_2)
