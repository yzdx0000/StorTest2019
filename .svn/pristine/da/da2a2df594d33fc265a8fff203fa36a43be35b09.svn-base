...
#   4、执行update_client_auth，输入参数错误，预期更新失败
#   5、执行get_events命令，查看结果显示是否正确
#   6、删除授权
#   7、删除上述存储卷
#
# @changelog：
...
def case():
    ...
    '''3> 执行create_client_auth添加客户端授权，输入参数正确，预期添加成功'''
    client_ip_str = str(snap_common.CLIENT_IP_1)
    rc, json_info = obj_volume.get_all_volumes()
    ...
    for volume in volumes_info:
        ...
        log.error('get new volume id failed !!!')
        raise Exception('get new volume id failed !!!')
    ...
    '''4> 执行update_client_auth'''
    client_ips_list = [snap_common.CLIENT_IP_1, snap_common.CLIENT_IP_3]
    client_ips_trans = ','.join(client_ips_list)
    rc, target_auth_id = obj_client_auth.get_all_client_auth_ids(new_volume_id)
    common.judge_rc(rc, 0, 'get client auth id')

    rc = obj_client_auth.update_client_auth(target_auth_id[0], client_ips_trans, 0)
    if 0 != rc:
        log.error('update client auth failed !!!')
    ...
    code = '0x02060064'
    description = 'update client auth'
