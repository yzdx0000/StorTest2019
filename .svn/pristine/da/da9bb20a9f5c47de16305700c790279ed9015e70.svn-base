...
# date 2018-12-10
# @summary：
# x-x-x-x     用户/用户组，使用已有本地认证服务器
# @steps:
# 1.使用本地认证创建第一个访问分区，启动NAS，创建subnet，vip池，创建本地用户，验证FTP 功能
# 2.使用第一个访问分区的本地认证，作为ldap认证创建第二个访问分区，启动NAS，创建subnet，vip池，获取用户与步骤1对比，
#   验证FTP 功能
# @changelog：
#
#######################################################
...
def executing_case1():
    ...
    if num > 1:    # 本测试案例至少需要2个节点
        ...
        #
        # """验证用户/用户组"""
        # ldap_user_name_list = get_auth_users_list(ldap_auth_provider_id)
        # ldap_group_name_list = get_auth_groups_list(ldap_auth_provider_id)
        # if user_name_list != ldap_user_name_list:
        #     log.error("user_name_list=%s, ldap_user_name_list=%s, they are different"
        #               % (user_name_list, ldap_user_name_list))
        #     common.except_exit('user_name_list != ldap_user_name_list!!!')
        # if group_name_list != ldap_group_name_list:
        #     log.error("group_name_list=%s, ldap_group_name_list=%s, they are different"
        #               % (group_name_list, ldap_group_name_list))
        #     common.except_exit('group_name_list != ldap_group_name_list!!!')
        #
        # """启动NAS"""
        # enable_nas(access_zone_id=access_zone_id)
        # """创建subnet"""
        ...
        #
        # log.info("\t[ 创建vip地址池 ]")
        # vip_domain_name = nas_common.VIP_2_DOMAIN_NAME
        # a = nas_common.SUBNET_SVIP.split('.')
        # b = nas_common.SUBNET_SVIP.split('.')[-1]
        # c = str(int(b) + int(node_id_num) + 3)
        # d = a[: -1] + c.split(" ")
        # vip_vip_addresses_1 = ".".join(d)     # vip地址池的第一个值
        # c = str(int(b) + int(node_id_num) + 3 + len(node_id))
        # d = c.split(" ")
        # vip_vip_addresses_2 = ".".join(d)  # vip地址池的最后个值
        # vip_vip_addresses = '"%s-%s"' % (vip_vip_addresses_1, vip_vip_addresses_2)
        # vip_supported_protocol = "NAS"
        ...
        # ftp_ip = node.get_node_ip_by_id(node_id=ftp_id)
        # local_user = random.choice(user_name_list)
        # ftp_export(access_zone_id=access_zone_id, node_ip=ftp_ip, local_user=local_user, d=1)
    else:
