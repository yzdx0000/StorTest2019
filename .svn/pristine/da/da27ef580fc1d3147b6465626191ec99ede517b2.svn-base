...
def executing_case():
    ...
    wrong_standby_ip_addresses = "%s,1.1.1.1" % nas_common.NIS_IP_ADDRESSES

    check_result1 = nas_common.add_auth_provider_nis(name="nas_16_0_0_79_nis_auth_name",
                                                     domain_name=nas_common.NIS_DOMAIN_NAME,
                                                     ip_addresses=wrong_standby_ip_addresses)
    ...
    if cmp(auth_provider,{
                ...
                "key": int("%s" % result),
                "name": "nas_16_0_0_79_nis_auth_name",
                "type": "NIS",
                "version": 0
            }):
        log.error(("%s Failed") % FILE_NAME)
