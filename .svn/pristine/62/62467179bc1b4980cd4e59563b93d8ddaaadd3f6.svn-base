...
def case():
    ...
    node_pool_id = osan.create_node_pool(s_ip=node_ip1, node_ids="{}".format(node_ids_list[0]),
                                         replica_num=1,
                                         stripe_width=1,
                                         disk_parity_num=0,
                                         node_parity_num=0,
    ...
    msg = osan.create_node_pool(s_ip=node_ip1, node_ids="{}".format(node_ids_list[0]),
                                         replica_num=1,
                                         stripe_width=1,
                                         disk_parity_num=0,
                                         node_parity_num=0,
                                         name="nodepool2",need_judge=1)
    env_manage_lun_manage.assert_ins.assertNotequal(msg, '', 'judge failed')

def main():
    env_manage_lun_manage.revert_env()
    # error.rel_check_before_run(file_name, jnl_rep=3, free_jnl_num=0, node_num=3)
    env_manage_lun_manage.init_env(target="init")
    case()  # 用例步骤
