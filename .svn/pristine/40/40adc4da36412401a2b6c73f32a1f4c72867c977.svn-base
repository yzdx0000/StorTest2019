...
#   执行：
#           1>删除SMB共享
#           2>执行get_events查看删除SMB共享失败的结果显示
#           3>清理工作
# @changelog：
####################################################################################
...
smb_path = nas_common.ROOT_DIR + "SMB_" + FILE_NAME  # parastor:/nas/SMB_events_5_3_3_60
...
def case():
    ...
    nas_common.delete_smb_exports(fault_smb_export_id)
    '''2> 执行get_events查看结果显示'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)  # 删除SMB共享失败，延时
    code = '0x02030706'   # 删除SMB共享失败对应编号
    description = 'delete_smb_exports_failed'
    '''删除SMB共享失败事件码是否在事件列表中'''
