    ...
    for s_ip in deploy_ips:
        if ReliableTest.check_ping(s_ip):
            for data_disk in data_disks:
                disk.expand_disk_2_storage_pool(s_ip=s_ip, stor_id=2, disk_id=data_disk, err=False)
            break
...
def down_net(nodeid):
    ...
    :return: None
    """
    data_eths, avail_ips = error.get_dataip_info(node_id=nodeid)
    log.info("5、将均衡源节点A %s 数据网闪断." % (avail_ips[0],))
    log.info("Begin down net %s through %s." % (data_eths, avail_ips[0]))
    ReliableTest.run_down_net(avail_ips[0], data_eths)
    time.sleep(40)
    log.info("Begin up net %s through %s." % (data_eths, avail_ips[0]))
