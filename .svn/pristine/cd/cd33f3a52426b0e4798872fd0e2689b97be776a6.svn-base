...
4、在步骤3中的业务运行过程中，将B节点关机，将C节点oJnlr线程故障；
...
def up_down(pipein):
    time.sleep(10)
    log.info("4、在步骤3中的业务运行过程中，将B节点关机，将C节点oJnlr线程故障；")
    log.info("Begin shutdown node:%s." % (a_ip, ))
    m_info = error.down_node(fault_ip=a_ip)
    log.info("On node %s kill oJnl/oPara" % (b_ip,))
    ReliableTest.run_kill_process(b_ip, 'oPara')
    # 监控到匿名管道中写入数据后，退出循环
    while True:
