...
def up_down():
    ...
    for data_eth_name in fault_eth_list_A:
        reliable.network_test(fault_io_ip_A, data_eth_name, 'down')
    time.sleep(60)
    log.info("Step5:在故障异常上报后，将B非日志节点数据网络故障,节点:%s" % (non_free_ip))
    for data_eth_name in fault_eth_list_B:
        reliable.network_test(non_free_ip, data_eth_name, 'down')

    time.sleep(100)
    log.info("Step6:先故障节点A数据网恢复,节点:%s" % (fault_io_ip_A,))
    for data_eth_name in fault_eth_list_A:
        reliable.network_test(fault_io_ip_A, data_eth_name, 'up')
    time.sleep(60)
    log.info("Step6:再恢复非日志节点B数据网恢复,节点:%s" % (fault_io_ip_A,))
    for data_eth_name in fault_eth_list_B:
