...
# date 2018-11-22
# @summary：
#    posix客户端权限测试：对文件进行chattr +a操作，检验文件权限；原P200测试用例23-0-8-10。
# @steps:
...
def case():
    ...
    log.info("3-2> 修改文件的attr, chattr +a")
    cmd = "ssh %s \"cd %s;chattr +a %s\"" % (
    ...
    log.info("3-5> 修改文件的attr, chattr -a")
    cmd = "ssh %s \"cd %s;chattr -a %s\"" % (
        client_ip, posix_file_location, testfile)
    rc = common.command(cmd)
    common.judge_rc(rc, 0, "chattr failed")
