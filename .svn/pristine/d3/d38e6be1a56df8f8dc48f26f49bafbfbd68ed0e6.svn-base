...
:Description:
            ...
            4、在步骤3中的业务运行过程中，将主oPmgr进程异常；
            5、从oPmgr接管主oPmgr服务，检查主oPmgr进程状态
            6、主机1上业务完成后，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
            7、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32965
import os
import sys
import commands
import threading
...
log.info("Get fault process: master oRole")

def up_down():
    time.sleep(100)
    log.info("Step4:将节点主oRole进程杀死,节点: %s." %(m_oRole_ip))
    ReliableTest.run_kill_process(m_oRole_ip,  'oRole')

    time.sleep(300)
    oRole_ip = reliable.get_master_oRole(deploy_ips[0])
    log.info("Step5:检查节点新主oRole进程状态,节点: %s." % (oRole_ip))
    reliable.check_process_stat(oRole_ip,'oRole')
