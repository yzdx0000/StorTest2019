...
# -*- coding:utf-8 -*-
#**********************************************#
# 标题：功能测试，用例编号29-0-17-66
# 使用方法：python 29-0-17-66_wjl.py
# 在http模式下，并发拷贝对象过程中开启认证开关
# 作者：王建磊
# 创建时间：2018/10/10
#**********************************************#
import sys
...
def copy_obj(
    ...
    for i in range(obj_num):
        ...
        CanonicalizedResource = "/" + bucketname2 + "/" + objectname2
        string = "PUT" + "\n" + "" + "\n" + "" + "\n" + "" + "\n" + \
            CanonicalizedHeaders + "\n" + CanonicalizedResource
        sig = create_sig(sk, string)
        rc, opt = s3_common.cp_object(
            bucketname2, objectname2, ak, bucketname, objectname, sig)
...
# 并发拷贝函数
def mul_procs_copy_obj(bucketname2, bucketname, objectname, ak, sk, obj_num):
    global process
