...
def executing_case():
    ...
    # create_file
    create_file_path1 = nas_common.ROOT_DIR + "nas_16-0-4-135_dir_1"
    create_file_path2 = create_file_path1
    # export_name重复性检查【注：ftp只能导出一个，不存在重命名情况】
    duplicate_export_name = "nas_16_0_4_135_ftp_export_name"
    ...
    check_result4 = nas_common.create_ftp_export(access_zone_id=access_zone_id,
                                                 user_name=nas_common.AD_USER_2,
                                                 export_path=create_file_path2)

    exe_info = nas_common.get_ftp_exports()
    if exe_info['result']['total'] != 2:
        common.except_exit('create ftp with the same path failed !!!')
