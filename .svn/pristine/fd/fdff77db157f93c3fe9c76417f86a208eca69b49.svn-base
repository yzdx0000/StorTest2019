...
def case():
    ...
    log.info("存储池容量为{}".format(storage_pool_size))

    global lun_size1
    global lun_id
    global vdb_maxdata
    thin_lun_num = 5  # thin  lun个数
    thick_lun_num = 5
    thin_lun_total = int(cp("reserve", "thin_lun_reserve")) * thin_lun_num
    thick_lun_size = int((storage_pool_size - thin_lun_total) / float(cp("reserve", "multiple")) / thick_lun_num)
    vdb_maxdata = int(thin_lun_total * 2 / 1024 / 1024)
    ...
    for i in range(5):
        env_manage_lun_manage.osan.create_lun(s_ip=node_ip1, lun_type="THIN",
                                              lun_name='LUN_THIN{}'.format(i), total_bytes=5 * storage_pool_size,
                                              stor_pool_id=storage_pool_id, acc_zone_id=az_id)

        env_manage_lun_manage.osan.create_lun(s_ip=node_ip1, lun_type="THICK",
                                              lun_name='LUN_THICK{}'.format(i), total_bytes=thick_lun_size,
                                              stor_pool_id=storage_pool_id, acc_zone_id=az_id)
...
def run_vdb1_thread(arg=0):
    ...
    env_manage_repair_test.co2_osan.run_vdb(client_ip=client_ip1, vdb_xml=mix_R_Align, output=node_ip1,
                                            whether_change_xml="N",need_judge=1)


def run_vdb2_thread(arg=0):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    iscsi_login()
    env_manage_repair_test.co2_osan.run_vdb(client_ip=client_ip2, vdb_xml=mix_R_Align2, output=node_ip1,
                                            whether_change_xml="N",time=1500,need_judge=1)
...
def main():
    ...
    case()  # 用例步骤
    decorator_func.multi_threads(run_vdb1_thread, run_vdb2_thread)
