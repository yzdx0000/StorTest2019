...
def run_vdb1():
    ...
    vdb_file = env_cache.gen_vdb_xml(lun=lun_name[:1], xfersize="(4k,50,16k,50)", rdpct="100", thread=8, rhpct=0,
                                     seekpct=0)
...
def run_vdb2():
    lun_name = env_cache.osan.ls_scsi_dev(client_ip=client_ip1)
    vdb_file = env_cache.gen_vdb_xml(lun=lun_name[1:2], xfersize="(4k,50,16k,50)", rdpct="100", thread=8, rhpct=0,
                                     seekpct=0)
    res = env_cache.run_vdb(vdb_file=vdb_file, c_ip=client_ip1, x_size="4k")
    return res
