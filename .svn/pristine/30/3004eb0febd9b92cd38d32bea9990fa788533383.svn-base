...
#   执行：
#           1>创建FTP共享
#           2>执行get_events查看创建FTP共享成功的结果显示
#           3>清理工作
# @changelog：
####################################################################################
...
ftp_path = nas_common.ROOT_DIR + "FTP_" + FILE_NAME  # parastor:/nas/FTP_events_5_3_3_69
...
def case():
    ...
    time.sleep(delay_time)  # 创建FTP共享成功，延时
    code = '0x02030801'   # 创建FTP共享成功对应编号
    description = 'create_ftp_export'
    '''创建FTP共享成功事件码是否在事件列表中'''
    event_common.check_events_result(start_time, code, description)

    '''3> 清理工作'''
    '''3.1> 清理导出'''
    check_result = nas_common.delete_ftp_exports(ids=ftp_export_id)
    if check_result["detail_err_msg"] != "":
