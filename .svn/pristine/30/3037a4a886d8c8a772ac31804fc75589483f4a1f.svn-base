...
def vdb_test():
    ...
    env_manage.com2_osan.run_vdb(client_ip=client_ip1, vdb_xml=vdb_file, output=node_ip1, time=300)
...
def update_az_osan():
    time.sleep(30)
    log.info("will update access san and kill process ")
    threads = []
    ...
    for t in threads:
        t.join()
    log.info("update access and kill process finished")


def run_vdb_case():
    threads = []
    threads.append(threading.Thread(target=vdb_test))
    threads.append(threading.Thread(target=update_az_osan))
    for t in threads:
        t.start()
    for t in threads:
        t.join()
...
def case():
    ...
    log.info("step:3.运行vdb过程中，update访问区，主oPmgr节点掉电")
    run_vdb_case()
