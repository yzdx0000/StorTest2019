...
4、业务过程中拔出二块数据盘后，触发被动重建
5、数据重建时，重建源节点数据网闪断10秒
...
def fault_disk():
    ...
    reliable.run_down_disk_wait(fault_node_ip,'10000')

def up_down():
    time.sleep(150)
    log.info("Step6:数据重建过程中,将该重建源节点数据网闪断,节点:%s" % (fault_node_ip1))
    reliable.net_flash_test(fault_node_ip1,fault_eth_list)
    ...
    while True:
        ...
        if disk_state == 'DISK_STATE_ZOMBIE':
            env_manage_repair_test.com_disk.remove_disks_asyn(disk_id1)
