...
for i in range(0, len(lun1)):
    j = i + 1
    if i == 0:
        mix_O_1 = osan.vdb_write(sd="sd" + str(j), lun=lun1[i], wd="wd" + str(j), xfersize="8k", seekpct=100, rdpct=0,
                                 skew=20)
    elif i == 1:
        mix_O_1 = osan.vdb_write(sd="sd" + str(j), lun=lun1[i], wd="wd" + str(j), xfersize="4k", seekpct=100, rdpct=0,
                                 skew=45)
    elif i == 2:
        mix_O_1 = osan.vdb_write(sd="sd" + str(j), lun=lun1[i], wd="wd" + str(j), xfersize="8k", seekpct=0, rdpct=10,
                                 skew=15)
    elif i == 3:
        mix_O_1 = osan.vdb_write(sd="sd" + str(j), lun=lun1[i], wd="wd" + str(j), xfersize="4k", seekpct=100, rdpct=0,
                                 skew=10)
    elif i == 4:
        mix_O_1 = osan.vdb_write(sd="sd" + str(j), lun=lun1[i], wd="wd" + str(j), xfersize="4k", seekpct=0, rdpct=0,
                                 skew=10)
cmd = ("mv %s %s.1" % (mix_O_1, mix_O_1))
commands.getstatusoutput(cmd)
mix_O_1 = mix_O_1 + ".1"
mix_O_2 = osan.vdb_write(sd="default")
for i in range(0, len(lun2)):
    j = i + 1
    if i == 0:
        mix_O_2 = osan.vdb_write(sd="sd" + str(j), lun=lun2[i], wd="wd" + str(j), xfersize="8k", seekpct=100, rdpct=0,
                                 skew=20)
    elif i == 1:
        mix_O_2 = osan.vdb_write(sd="sd" + str(j), lun=lun2[i], wd="wd" + str(j), xfersize="4k", seekpct=100, rdpct=0,
                                 skew=45)
    elif i == 2:
        mix_O_2 = osan.vdb_write(sd="sd" + str(j), lun=lun2[i], wd="wd" + str(j), xfersize="8k", seekpct=0, rdpct=0,
                                 skew=15)
    elif i == 3:
        mix_O_2 = osan.vdb_write(sd="sd" + str(j), lun=lun2[i], wd="wd" + str(j), xfersize="4k", seekpct=100, rdpct=0,
                                 skew=10)
    elif i == 4:
        mix_O_2 = osan.vdb_write(sd="sd" + str(j), lun=lun2[i], wd="wd" + str(j), xfersize="4k", seekpct=0, rdpct=0,
                                 skew=10)
...
disk_phyid1 = disk.get_disk_phyid_by_name(s_ip=fault_ip, disk_name=disk_name[0].split('\n'))
disk_phyid2 = disk.get_disk_phyid_by_name(s_ip=fault_ip, disk_name=disk_name[1].split('\n'))
disk_id1 = disk.get_diskid_by_name(s_ip=deploy_ips[0],node_id=node_id,disk_name=disk_name[0])
disk_id2 = disk.get_diskid_by_name(s_ip=deploy_ips[0],node_id=node_id,disk_name=disk_name[1])
disk_uuid = disk.get_disk_uuid_by_name(s_ip=fault_ip, node_id=node_id, disk_name=disk_name[1])
log.info(disk_phyid1)
log.info(disk_phyid2)
log.info(disk_id1)
log.info(disk_id2)
log.info(disk_uuid)
...
def up_down(pipeout):
    ...
    time.sleep(100)
    log.info("Begin add disk:%s %s." % (fault_ip, disk_phyid1[0]))
    ReliableTest.insert_disk(fault_ip, disk_phyid1[0], "share")
    time.sleep(230)
    log.info("Begin add disk:%s %s." % (fault_ip, disk_phyid2[0]))
    ReliableTest.insert_disk(fault_ip, disk_phyid2[0], "share")
    disk_id = disk_id2
...
def vdb_run():
    ...
    osan.run_vdb(client_ips[1], mix_O_2, output=deploy_ips[0], time=1200)
