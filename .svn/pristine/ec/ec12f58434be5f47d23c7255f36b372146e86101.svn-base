...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-0-1-17_sync 目录逻辑空间配额混合阈值，建议阈值先生效，
#         软阈值其次生效，限期内硬阈值也生效，dd中有oflag=sync
#######################################################
...
def executing_case():
    ...
    '''
    # 写入3G文件后开始计时
    quota_common.creating_files_with_oflag_sync(quota_common.CLIENT_IP_1, quota_common.QUOTA_PATH,
                                                3072, 1, "a")

    # 在1天以内再写入1G文件
    quota_common.creating_files_with_oflag_sync(quota_common.CLIENT_IP_2, quota_common.QUOTA_PATH,
                                                1024, 1, "b")
    ...
    if total_file_size != quota_common.FILE_SIZE_4G:
        ...
        log.error("11-0-1-17_sync Failed")
        raise Exception("11-0-1-17_sync Failed")
    else:
        log.info("11-0-1-17_sync Succeed")
    return
...
class Quota_Class_11_0_1_17_sync():
    def quota_method_11_0_1_17_sync(self):
