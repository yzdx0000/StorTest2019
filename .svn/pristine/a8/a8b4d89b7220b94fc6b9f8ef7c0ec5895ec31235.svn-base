...
if len(lun1) < 5:
    mix_S_align = osan.gen_vdb_xml(lun=lun1, xfersize="(4k,100)", seekpct=30, rdpct=0)
else:
...
def up_down(pipein):
    ...
    b_id = disk.get_node_id_by_ip(removed_ip=a_ip, n_ip=b_ip)
    b_eths, b_extra = error.get_ioip_info(node_id=b_id)
    log.info("5、节点A的日志接管节点B %s 业务网故障；" % (b_ip,))
    log.info("Begin down io net %s through %s." % (b_eths, b_extra[0]))
    ...
    log.info("Begin up io net %s through %s." % (b_eths, b_extra[0]))
