...
# date 2018-05-07
# @summary：
# 16-0-4-111      anongid，边界值测试
# @steps:
# case1、设置NFS授权客户端的gid为2147483647（32位int类型的最大值）；
# pscli --command=add_nfs_export_auth_clients --export_id=1 --name=10.20.104.43 --permission_level=rw --anongid=2147483647
# case2、设置NFS授权客户端的gid为2147483648；
# pscli --command=add_nfs_export_auth_clients --export_id=1 --name=10.20.104.43 --permission_level=rw --anongid=2147483648
# @changelog：
#
#######################################################
import os
...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)  # /mnt/volume1/snap/snap_16_0_4_111
...
#######################################################
# 1.executing_case1
# @function：设置NFS授权客户端的gid为2147483647；
# @parameter：
#       node_ip：执行cmd命令的节点ip
#       cmd：设置NFS授权客户端的gid为2147483647；
# @return：
...
def executing_case1():
    ...
    access_zone_name = "access_zone_16_0_4_111"
    node = common.Node()
    ids = node.get_nodes_id()
    print ids
    access_zone_node_id_16_0_4_111 = ','.join(str(p) for p in ids)
    msg1 = nas_common.create_access_zone(node_ids=access_zone_node_id_16_0_4_111, name=access_zone_name)
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        common.except_exit('%s create_access_zone failed!!!' % node_ip)
    global access_zone_id_16_0_4_111
    access_zone_id_16_0_4_111 = msg1["result"]

    # 2> 启动NAS
    log.info("\t[ enable_nas ]")
    msg2 = nas_common.enable_nas(access_zone_id=access_zone_id_16_0_4_111, protocol_types="NFS")
    if msg2["err_msg"] != "" or msg2["detail_err_msg"] != "":
        common.except_exit('%s enable_nas failed!!!' % node_ip)

    # 3> 查看NAS是否按配置启动
    log.info("\t[ get_access_zones ]")
    check_result3 = nas_common.get_access_zones(ids=access_zone_id_16_0_4_111)
    name = check_result3["result"]["access_zones"][0]["name"]
    if name != access_zone_name:
        common.except_exit('%s enable_nas failed!!!' % node_ip)
    global auth_provider_id_16_0_4_111
    auth_provider_id_16_0_4_111 = check_result3["result"]["access_zones"][0]["auth_provider_id"]

    """1> 检查目录并creat file"""
    log.info("\t[ case1-1 create_file ]")
    global nfs_path
    nfs_path = nas_common.ROOT_DIR + "nfs_dir_16_0_4_111"
    global NAS_PATH_1
    NAS_PATH_1 = get_config.get_one_nas_test_path() + "/nfs_dir_16_0_4_111"
    cmd = "create_file"
    ...
    nfs_export_name = "nfs_exp_test_16_0_4_111"
    global description_nfs
    description_nfs = 'old_export_description'
    cmd = "create_nfs_export "
    check_result1 = nas_common.create_nfs_export(access_zone_id=access_zone_id_16_0_4_111,
    ...
    global nfs_export_id_16_0_4_111
    nfs_export_id_16_0_4_111 = msg1["result"]

    """4> get_ftp_exports"""
    log.info("\t[ case1-4 get_nfs_exports ]")
    cmd = "get_nfs_exports"
    check_result = nas_common.get_nfs_exports(ids=nfs_export_id_16_0_4_111)
    ...
    if access_zone_id != access_zone_id_16_0_4_111:
    ...
    auth_clients_anongid = '2147483647'
    cmd = "add_nfs_export_auth_clients "
    check_result = nas_common.add_nfs_export_auth_clients(export_id=nfs_export_id_16_0_4_111,
    ...
    global nfs_export_clients_id_16_0_4_111
    nfs_export_clients_id_16_0_4_111 = msg["result"][0]

    """7> get_nfs_export_auth_clients"""
    log.info("\t[ case1-7 get_nfs_export_auth_clients ]")
    cmd = "get_nfs_export_auth_clients "
    check_result = nas_common.get_nfs_export_auth_clients(export_ids=nfs_export_id_16_0_4_111)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    msg = check_result
    if msg["err_msg"] != "" or msg["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    ...
    auth_clients_mount_dir = "/mnt/nfs_dir_16_0_4_111"
    ...
    """11> 测试anongid权限"""
    auth_clients_name = nas_common.NFS_1_CLIENT_IP
    log.info("\t[ case1-11 测试anongid权限 ]")
    cmd1 = "file1=`grep %s /etc/exports | head -n 1` " % auth_clients_name + \
           "&& file2=`echo ${file1#*anonuid=65534,}` && echo ${file2%,fsid*}"
    rc, stdout, stderr = shell.ssh(node_ip, cmd1)
    print stdout
    if rc != 0 or stdout != 'anongid=2147483647\n':
        log.info("rc = %s" % rc)
        log.error("WARNING: \ncmd = %s\nstdout = %s\nstderr = %s" % (cmd1, stdout, stderr))
        raise Exception('%s 测试anongid权限 failed!!!' % node_ip)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd1, stdout))

    """12> 客户端umount共享路径"""
    log.info("\t[ case1-12 客户端umount共享路径 ]")
    cmd1 = "umount -fl /mnt/nfs_dir_16_0_4_111"
    ...
    cmd1 = "rm -rf /mnt/nfs_dir_16_0_4_111"
    rc, stdout, stderr = shell.ssh(auth_clients_name, cmd1)
    print stdout
    if rc != 0:
        log.info("rc = %s" % rc)
...
#######################################################
# 2.executing_case2
# @function：设置NFS授权客户端的gid为2147483648；
# @parameter：
#       node_ip：执行cmd命令的节点ip
#       cmd：设置NFS授权客户端的gid为2147483648；
# @return：
#       check_result —— 执行create命令的字典格式返回值
# @steps:
# 1> 检查目录并creat file
...
def executing_case2():
    ...
    global nfs_path
    nfs_path = nas_common.ROOT_DIR + "nfs_dir_16_0_4_111_2"
    global NAS_PATH_2
    NAS_PATH_2 = get_config.get_one_nas_test_path() + "/nfs_dir_16_0_4_111_2"
    cmd = "create_file "
    check_result6 = nas_common.create_file(path=nfs_path, posix_permission="rwxr-xr-x")
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result6))
    msg6 = check_result6
    ...
    global nfs_export_name
    nfs_export_name = "nfs_exp_test_16_0_4_111_2"
    global description_nfs
    description_nfs = 'old_export_description'
    cmd = "create_nfs_export "
    check_result1 = nas_common.create_nfs_export(access_zone_id=access_zone_id_16_0_4_111,
    ...
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
        raise Exception('%s create_nfs_export failed!!!' % node_ip)
    global nfs_export_id_16_0_4_111_2
    nfs_export_id_16_0_4_111_2 = msg1["result"]

    """4> get_ftp_exports"""
    log.info("\t[ case2-4 get_nfs_exports ]")
    cmd = "get_nfs_exports "
    check_result = nas_common.get_nfs_exports(ids=nfs_export_id_16_0_4_111_2)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    msg = check_result
    if msg["err_msg"] != "" or msg["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result))
    ...
    auth_clients_write_mode = 'async'
    global auth_clients_anongid
    auth_clients_anongid = '2147483648'
    cmd = "add_nfs_export_auth_clients "
    check_result = nas_common.add_nfs_export_auth_clients(export_id=nfs_export_id_16_0_4_111_2,
                                                          name=auth_clients_name,
                                                          permission_level=auth_clients_permission_level,
                                                          write_mode=auth_clients_write_mode,
                                                          anongid=auth_clients_anongid)
...
def clearing_environment():
    ...
    check_result1 = nas_common.delete_nfs_exports(ids=nfs_export_id_16_0_4_111)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
    msg1 = check_result1
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
    ...
    log.info("\t[ 删除export 2]")
    cmd = "delete_nfs_exports" 
    check_result1 = nas_common.delete_nfs_exports(ids=nfs_export_id_16_0_4_111_2)
    log.info('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
    msg1 = check_result1
    if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
        log.error('node_ip = %s, cmd = %s, check_result = %s' % (node_ip, cmd, check_result1))
