...
4、先将oJob节点掉电，立即拔出二块数据盘；
5、在主机1上运行vdbench -f mix-R-Align.conf -jro;
6、节点上电 恢复，数据修复完成系统恢复正常后，比较内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-33351
import os
import sys
import commands
import threading
...
job_node_id = reliable.get_matid(deploy_ips[0])
job_node_ip = node.get_node_ip_by_id(job_node_id)
deploy_ls.remove(job_node_ip)
...
def disk_fault():
    time.sleep(120)
    log.info("Step4:业务过程中主动将两块数据磁盘拔出,触发被动重建,节点:%s,磁盘:%s,%s" % (fault_node_ip, disk_name, disk_name1))
...
def up_down():
    time.sleep(300)
    log.info("Step5:数据盘重建开始前,将oJob节点掉电,节点:%s" % (job_node_ip))
    vm_id = error.down_node(job_node_ip)
    time.sleep(50)
    while True:
        log.info("等待故障节点重新启动,节点:%s" % job_node_ip)
        error.up_node(vm_id)
        rc = reliable.get_os_status(job_node_ip)    # 检测重启后机器状态，是否已开机
        time.sleep(30)
        if 0 == rc:
            log.info("故障节点启动成功,节点:%s" % job_node_ip)
            break
        else:
            log.info("等待故障节点启动,节点:%s" % job_node_ip)
    ...
    while True:
        ...
        if disk_state == 'DISK_STATE_ZOMBIE':
            ...
            env_manage_repair_test.Reliable_osan.add_disks(fault_node_ip, fault_node_id, disk_uuid, "DATA",
                                                           stor_id_block)
            break
        time.sleep(15)
