...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-0-2-56 修改目录用户组配额规则之改大、改小宽限天数
#######################################################
...
#    time.sleep(3600)
    ...
    # （2）1天之后，马上修改目录用户组配额规则宽限天数为2天，验证配额是否生效【临时：--filenr_grace_time=120】
    rc, check_result2 = quota_common.update_one_quota(id=quota_id,
                                                      filenr_quota_cal_type='QUOTA_LIMIT',
                                                      filenr_soft_threshold=1000,
                                                      filenr_grace_time=180)
    ...
    # (3)修改宽限天数为1天，删除配额目录用户组中所有文件【临时：--filenr_grace_time=60】
    ...
    common.rm_exe(quota_common.NOTE_IP_1, os.path.join(quota_common.QUOTA_PATH, '*'))

    # 预期成功写入1000个文件触发宽限时间启动
    quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_1, quota_common.QUOTA_PATH,
                                                            1000, 1, "f", quota_common.QUOTA_USER)
    ...
    total_inodes3_2 = quota_common.group_total_inodes(quota_common.NOTE_IP_1, quota_common.QUOTA_PATH,
                                                      quota_common.QUOTA_GROUP)
    ...
    total_inodes3_3 = quota_common.user_total_inodes(quota_common.NOTE_IP_1, quota_common.QUOTA_PATH,
    ...
    if (check_result1["detail_err_msg"] != "" or
        ...
        log.error("11-0-2-56 Failed")
        raise Exception("11-0-2-56 Failed")
    else:
        log.info("11-0-2-56 Succeed")
    return
...
class Quota_Class_11_0_2_56():
    def quota_method_11_0_2_56(self):
