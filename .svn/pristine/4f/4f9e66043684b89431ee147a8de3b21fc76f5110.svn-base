# -*- coding:utf-8 _*-
"""
测试内容:三副本下数据2副本所有对象损坏后写该数据

步骤:
1、在节点A创建LUN1-lUN6
2、将逻辑卷映射至主机1上lun1-lun6
3、在主机1上运行vdbench -f min-seq-write.conf -jn；
4、业务完成后将存储池中两块（两个节点上的）硬盘拔出；
5、在主机1上运行vdbench -f min-seq-read.conf；
...
def disk_error(arg=1):
    ...
    share_disk, data_disk = env_manage_repair_test.Reliable_osan.get_share_monopoly_disk_ids(s_ip=node_ip1,
                                                                                             node_id=1)  # 获取节点1的所有共享盘和数据盘
    disk_ids1 = []
    for data_disk in data_disk:
        disk_id = env_manage_repair_test.Reliable_osan.get_physicalid_by_name(node_ip1,
                                                                              data_disk)  # 根据上一步获取的盘名获取节点1数据盘的物理ID
        disk_ids1.append(disk_id)
    log.info(disk_ids1)
    diskid1 = random.choice(disk_ids1)
    env_manage_repair_test.Reliable_osan.remove_disk(node_ip1, disk_id=diskid1, disk_usage="DATA")

    share_disk, data_disk = env_manage_repair_test.Reliable_osan.get_share_monopoly_disk_ids(s_ip=node_ip1,
                                                                                             node_id=2)  # 获取节点2的所有共享盘和数据盘
    disk_ids2 = []
    for data_disk in data_disk:
        disk_id = env_manage_repair_test.Reliable_osan.get_physicalid_by_name(node_ip2,
                                                                              data_disk)  # 根据上一步获取的盘名获取节点2数据盘的物理ID
        disk_ids2.append(disk_id)
    log.info(disk_ids2)
    diskid2 = random.choice(disk_ids2)
    env_manage_repair_test.Reliable_osan.remove_disk(node_ip2, disk_id=diskid2, disk_usage="DATA")


def disk_insert(arg=1):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    env_manage_repair_test.Reliable_osan.insert_disk(node_ip=node_ip1, disk_id=diskid1, disk_usage="DATA")
...
def case():
    ...
    log.info("step3:业务完成后将存储池中两块（两个节点上的）硬盘拔出...")
    disk_error(1)
    log.info(
        "step4:在主机1上运行vdbench -f min-seq-w-1l.conf -jn,插回故障盘开始修复,修复过程中执行主机端执行vdbench -f min-seq-w-1l.conf -jro校验数据一致性(更改磁盘超时时间,以x1000.conf配置为准) ...")
    env_manage_repair_test.co2_osan.run_vdb(client_ip=client_ip1, vdb_xml=min_seq_w, jn_jro='jn', output=node_ip1)
