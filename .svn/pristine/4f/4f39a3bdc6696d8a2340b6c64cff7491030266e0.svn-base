...
# date 2018-07-27
# @summary：
#    删除回滚快照成功（等待get_events接口完成）
# @steps:
#   1、部署3节点集群环境
#   2、在/mnt/volume1/snap/snap_revert_success/目录下创建数据
#   3、对/mnt/volume1/snap/snap_revert_success/创建快照（参数输入正确，预期成功）
#   4、修改上述目录下的内容，执行revert_snapshot回滚快照（预期成功）
#   5、执行delete_revert_snapshot，输入参数正确（预期成功）
#   6、执行get_events查看删除回滚操作显示信息是否正确
#   7、删除快照，清理环境
#
# @changelog：
####################################################################################
...
def case():
    ...
    if rc != 0:
        ...
        raise Exception('snapshot %s revert failed !!!' % snap_name)

    ''' 5> 执行delete_revert_snapshot，输入参数正确（预期成功）'''
    rc, json_info = snap_common.get_revert_snapshot()
    stdout = json_info
    revert_snapshots = stdout['result']['revertSnapshots']

    revert_snap_id = 0
    for revert_snapshot in revert_snapshots:
        if revert_snapshot['snapshot_id'] == snap_id:
            revert_snap_id = revert_snapshot['revert_id']
            break
    rc, stdout = snap_common.delete_revert_snapshot_by_id(revert_snap_id)
    if rc != 0:
        log.error('delete revert snapshot %s failed !!!' % snap_name)
        raise Exception('delete revert snapshot %s failed !!!' % snap_name)

    ''' 6> 执行get_events查看回滚操作显示信息是否正确'''
    delay_time = event_common.DELAY_TIME
    log.info('waiting for %s' % delay_time)
    time.sleep(delay_time)

    code = '0x02060049'
    description = 'deleting revert snapshot'
