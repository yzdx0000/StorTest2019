...
def case():
    ...
    log.info("step:6.运行vdbench停止访问区2的iSCSI,再次启动访问区2iSCSI过程中，oPmgr线程异常")
    threads = []
    threads.append(threading.Thread(target=vdb_test))
    threads.append(threading.Thread(target=os_down_osan, args=(az_id2)))
    for t in threads:
        t.start()
    for t in threads:
        t.join()
