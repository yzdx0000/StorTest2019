...
def case():
    log.info("******************* case start **********************")
    log.info("step:1.check xstor the number of node")
    node_nums1 = check_nodes()
    check_new_node()
    log.info("start get node number :%s" % (node_nums1))
    log.info("step:2.add a new node to xstor when create lun")
    threads = []
    t1 = threading.Thread(target=env_manage.create_luns)
    threads.append(t1)
    ...
    for t in threads:
        t.join()
    log.info("step:3.after a new node. check xstor the number of node")
    node_nums2 = check_nodes()
    log.info("end add get the node number :%s" % (node_nums2))
