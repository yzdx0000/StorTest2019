...
4、在步骤3中的业务运行过程中，将日志节点A节点业务网延迟增大到100ms；
...
io_eth_list = error.get_io_eth(fault_node_id)
phy_eth_ip = reliable.get_ip_by_eth(fault_node_ip, io_eth_list[0])

def up_down():
    time.sleep(100)
    log.info("业务网络注入故障前的延迟情况,节点:%s" % fault_node_ip)
    reliable.check_ping(fault_ip=fault_node_ip, fault_eth_ip=phy_eth_ip)
    log.info("Step4:业务读写过程中,将日志节点业务网延迟增大,节点:%s" % fault_node_ip)
    thread_list = []
    for io_eth in io_eth_list:
        cmd = ("ssh %s 'tc qdisc add dev %s root netem delay 100ms'" % (fault_node_ip, io_eth))
        log.info(cmd)
    ...
    time.sleep(10)
    log.info("业务网络注入故障后延迟情况,节点:%s" % fault_node_ip)
    reliable.check_ping(fault_ip=fault_node_ip, fault_eth_ip=phy_eth_ip, limit_delay=100)
    time.sleep(100)
    ...
    for io_eth in io_eth_list:
        cmd = ("ssh %s 'tc qdisc del dev %s root netem delay 100ms'" % (fault_node_ip, io_eth))
        log.info(cmd)
        rc = os.system(cmd)
        if rc == 0:
            log.info("业务网 %s 恢复成功,节点:%s" % (io_eth, fault_node_ip))
