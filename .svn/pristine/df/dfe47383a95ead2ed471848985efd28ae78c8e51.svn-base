...
4、在步骤3中的业务运行过程中，将节点A业务网断开；
6、检查业务运行状态，管理管收到异常上报后，断开节点B（业务迁移到的节点）业务网；
5、主机1上业务完成后，在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
6、比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32878
import os
import sys
import commands
import threading
...
def up_down():
    ...
    time.sleep(100)
    log.info("Step4:在步骤3中的业务运行过程中，将A节点业务网络故障,业务节点:%s" % fault_io_ip_A)
    for io_eth_name in io_eth_list_A:
        reliable.network_test(fault_io_ip_A, io_eth_name, 'down')
    time.sleep(100)
    fault_io_ip_B = osan.get_node_by_vip(fault_vip_A)
    fault_io_id_B = node.get_node_id_by_ip(fault_io_ip_B)
    io_eth_list_B = error.get_io_eth(fault_io_id_B)
    log.info("Step5:A节点故障异常上报后，将B节点(业务迁移到的节点)络故障,业务节点:%s" % fault_io_ip_B)
    for io_eth_name in io_eth_list_B:
        reliable.network_test(fault_io_ip_B, io_eth_name, 'down')

    time.sleep(100)
    log.info("Step6:将故障节点业务网恢复,业务节点:%s,%s" % (fault_io_ip_A, fault_io_ip_B))
    for io_eth_name in io_eth_list_A:
