...
4、在步骤3中的业务运行过程中，将B关机，将节点c数据网故障；
...
none_id_ids.remove(a_id)
if len(none_id_ids) == 0:
    none_id_ids = nodes
    if a_id in none_id_ids:
        none_id_ids.remove(a_id)
b_id = random.choice(none_id_ids)
...
def up_down(pipein):
    time.sleep(10)
    log.info("4、在步骤3中的业务运行过程中，将B关机，将节点c数据网故障；")
