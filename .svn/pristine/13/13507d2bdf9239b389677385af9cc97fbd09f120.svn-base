...
def disk_error(arg=1):
    ...
    for d_id in disk_ids:
        c_ips, n_id, d_name = disk.get_nodeinfo_by_diskid(s_ip=node_ip1, disk_id=d_id)
    ...
    for diskid in disk_phyid_info.keys():
        ...
        disk.delete_disk(s_ip=node_ip1, disk_id=diskid)
        time.sleep(30)
        # 添加磁盘到集群
        disk.add_disk(s_ip=node_ip1, uuid=disk_uuid, usage='DATA', node_id=disk_info[diskid])
        # 添加磁盘到原存储池
        disk.expand_disk_2_storage_pool_by_uuid(s_ip=node_ip1, node_id=disk_info[diskid], uuid=disk_uuid,
