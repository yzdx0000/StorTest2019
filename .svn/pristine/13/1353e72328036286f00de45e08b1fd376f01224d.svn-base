...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-3-1-4 【稳定性】配置下发过程中kill掉主oPara进程
#                 （QuotaMGR在oPara进程中）
#######################################################
...
def executing_case():
    ...
    p2 = Process(target=kill_master_process_when_create_quota, args=(alive, "oPara", ))
    ...
    if p1.exitcode != 0:
        raise Exception("11-3-1-4 Failed")
    else:
        print "11-3-1-4 Succeed"
    return
...
class Quota_Class_11_3_1_4():
    def quota_method_11_3_1_4(self):
