...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：16-0-x-x 开启home目录，alternative Data Streams打开
#######################################################
...
def executing_case():
    """
        1、设置home目录后，再设置enable_alternative_datasource参数；
        pscli --command=update_smb_global_config --id=1 --home_dir="parastor:/homes" --enable_alternative_datasource=true
        pscli --command=update_smb_global_config --id=1 --home_dir="parastor:/homes" --enable_alternative_datasource=false
    :return:
    """
    ...
    create_file_path = nas_common.ROOT_DIR + "homes-nas_16-0-4-63_dir"
    ...
    if allow_create_ntfs_acl != True \
            or enable_alternative_datasource != True \
            or enable_change_notify != True \
            or enable_dos_attributes != True \
            or enable_guest != False \
            or enable_ntfs_acl != True \
