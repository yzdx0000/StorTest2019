...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-0-1-71 目录用户组逻辑空间配额软阈值之chgrp方式
#######################################################
...
def executing_case():
    ...
    # （2）之后写500M属主为quota_other_group的文件，预期写入成功
    quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_1, quota_common.QUOTA_PATH,
                                                            soft_val, 1, "d", quota_common.QUOTA_OTHER_USER)
   
    # （3）最后尝试写1G属主为quota_group的文件，预期写入成功
    quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_2, quota_common.QUOTA_PATH,
                                                            soft_val_2, 1, "e", quota_common.QUOTA_USER)
...
#    time.sleep(86400)
    ...
    # （2）之后写500M属主为quota_other_group的文件，预期写入成功
    quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_1, quota_common.QUOTA_PATH,
                                                            soft_val, 1, "g", quota_common.QUOTA_OTHER_USER)
    
    # （3）最后尝试写500M属主为quota_group的文件，预期写入失败
    quota_common.creating_files_by_designated_user_or_group(quota_common.CLIENT_IP_2, quota_common.QUOTA_PATH,
                                                            soft_val, 1, "h", quota_common.QUOTA_USER)
    ...
    if total_file_size != quota_common.FILE_SIZE_1M * (soft_val_2 + 2048):
        ...
        log.error("11-0-1-71 Failed")
        raise Exception("11-0-1-71 Failed")
    else:
        log.info("11-0-1-71 Succeed")
    return
...
def preparing_environment():
    ...
    # 针对目录用户组配置逻辑空间为2G软阈值，宽限天数为1天的配额【临时：--logical_grace_time=60s】
...
class Quota_Class_11_0_1_71():
    def quota_method_11_0_1_71(self):
