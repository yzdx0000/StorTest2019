...
6、主机下发业务中，从访问区中同时删除步骤5中的LUN对应节点，且添加一个新节点

检查项:
6、同时删除节点和添加节点成功，访问区有4节点；业务正常
"""
# testlink case: 1000-34213
import os, sys
import time
import utils_path
...
def update_access_zone(arg=3):
    ...
    env_manage_lun_manage.osan.update_access_zone(s_ip=node_ip1,
                                                  node_ids="{},{},{}".format(node_ids_list[1], node_ids_list[3],
                                                                             node_ids_list[4]),
...
def add_vip():
    vip = osan.get_vip_address_pools()
    osan.add_vip_address_pool(s_ip=node_ip1, subnet_id=sub_id1,
                              domain_name=cp("add_vip_address_pool", "domain_name"),
                              vip="")
