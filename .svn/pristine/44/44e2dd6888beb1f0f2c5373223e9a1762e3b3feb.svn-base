...
#print strip
for i in range(0,9):
    osan.create_lun(s_ip=deploy_ips[0],lun_name="lun_"+str(i),stor_pool_id=storid[1],acc_zone_id=acc_zone_ids,replica_num="3",node_parity_num='2')
...
for sip in deploy_ips:
    cmd = ("ssh %s \"sh /home/zt_dws/otrc.sh\""
           %(sip))
    (res, final) = commands.getstatusoutput(cmd)
    if res != 0:
