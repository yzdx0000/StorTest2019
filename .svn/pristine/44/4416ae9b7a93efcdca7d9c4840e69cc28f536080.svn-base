...
def os_down():
    opara_jnl_id = env_manage.com_lh.get_opara_jnl()
    node_ip = env_manage.deploy_ips[int(opara_jnl_id)-1]
    # node_ip = env_manage.com_lh.get_node_id_by_ip(opara_jnl_id)
    ...
    log.info("Down jnl oPara node finished. Get the node type: %s ,info: %s" % (os_type, info))
    return
...
def update_access():
    log.info("will update access node 1,2")
...
def case():
    ...
    log.info("step:4.停止scsi过程中，主oPmgr节点掉电")
    threads = []
    ...
    for t in threads:
        t.join()
    log.info("step:5.check access number of node")
