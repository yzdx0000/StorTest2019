...
def up_down(pipein):
    ...
    while True:
        ...
        if 'pull' in line:
            break
    time.sleep(60)
    log.info("\033[1;31;40m在业务运行过程中，同一节点同时拔出两块数据盘；\033[0m")
    disk_name_dict, node_id_lst = this.get_disk_nodeid_uuid_usage(is_data_disk=True, disk_num_each_node=2)
