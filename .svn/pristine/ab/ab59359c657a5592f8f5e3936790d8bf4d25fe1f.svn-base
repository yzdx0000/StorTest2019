...
4、在步骤3中的业务运行过程中，同时将节点A 和节点B数据网断开
5、等待30秒后同时将节点A和节点B数据网恢复；
6、主机1上执行vdbench -f mix-R-Align.conf -jro比较一致性；
7、节点A上数据修复完成后，再次在主机1上执行vdbench -f mix-S-Align.conf -jro比较一致性；
8、比较存储内部数据一致性。"

:Changerlog:
"""
# testlink case: 1000-32680
import os
import sys
import commands
import threading
...
def up_down():
    ...
    time.sleep(30)
    log.info("Step5:将故障节点数据网同时恢复,节点:%s,%s" % (fault_io_id_A, jnl_node_ip))
    thread_list1 = []
    for data_eth_name in fault_eth_list_A:
        thread_list1.append(threading.Thread(target=reliable.network_test, args=(fault_io_ip_A, data_eth_name, 'up')))
    for data_eth_name in fault_eth_list_B:
        thread_list1.append(threading.Thread(target=reliable.network_test, args=(jnl_node_ip, data_eth_name, 'up')))
