...
# date 2018-08-18
# @summary：
# 16_0_2_19     5节点，其中一个节点oJmgs服务异常
# @steps:
# 0> 检查环境是否大于等于五个节点，大于等于五节点可以执行该脚本
# 1> 制造一个节点处于oJmgs服务异常状态
# 2> 创建访问分区
# 3> 恢复环境
# @changelog：
#
...
def executing_case1():
    ...
    if len(nodes) >= 5:
        """1> 制造一个节点处于oJmgs服务异常状态"""
        log.info("\t[1 制造一个节点处于oJmgs服务异常状态]")
        """获取故障节点的id"""
        ...
        # common.judge_rc(rc, 0, "stop oJmgs failed")
        time.sleep(5)
        """"检查oJmgs进程是否被停止"""
        cmd2 = "ps afx | grep /home/parastor/bin/oJmgs | grep -v grep | awk '{print $3}'"
        rc, stdout = common.run_command(node_ip_1, cmd2)
        common.judge_rc(rc, 0, "check oJmgs failed")
        if stdout.find("Tl") == -1:
            common.except_exit("检查oJmgs进程是否被停止失败")

        """ 2> 创建访问分区 """
        log.info("\t[2 create_access_zone ]")
        access_zone_name = "access_zone_16_0_2_19"
        node = common.Node()
        outtext = node.get_nodes()
        nodes = outtext['result']['nodes']
        ids = []
        ...
        print ids
        access_zone_node_id_16_0_2_19 = ','.join(str(p) for p in ids)
        msg1 = nas_common.create_access_zone(node_ids=access_zone_node_id_16_0_2_19, name=access_zone_name)
        if msg1["err_msg"] != "" or msg1["detail_err_msg"] != "":
            common.except_exit("有节点在oJmgs服务异常状态时，创建访问分区与预期不符，请检查节点状态与访问分区状态")
        ...
        rc, stdout = common.run_command(node_ip_1, cmd1)
        common.judge_rc(rc, 0, "start oJmgs failed")
        time.sleep(10)
        """"检查oJmgs进程是否恢复正常"""
        cmd2 = "ps afx | grep /home/parastor/bin/oJmgs | grep -v grep | awk '{print $3}'"
        rc, stdout = common.run_command(node_ip_1, cmd2)
        common.judge_rc(rc, 0, "check oJmgs failed")
        if stdout.find("Sl") == -1:
            common.except_exit("恢复oJmgs进程失败")
        """检查节点服务是否正常"""
