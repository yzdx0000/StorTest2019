...
def executing_case():
    ...
    4、修改domain_name；
    pscli --command=update_auth_provider_ad --id=x --domain_name=new_domain_name.com
    :return:
    """
    log.info("（2）executing_case")

    # for add_auth_provider_ad
    auth_provider_ad_name = "nas_16_0_7_2_ad_auth_name"
    ...
    # node_ids = "1,2,3"
    access_zone_name = "nas_16_0_7_2_access_zone_name"
    ...
    if check_result4["detail_err_msg"].find("Restart authserv failed") == -1:
        log.error(("%s Failed") % FILE_NAME)
        raise Exception(("%s Failed") % FILE_NAME)
