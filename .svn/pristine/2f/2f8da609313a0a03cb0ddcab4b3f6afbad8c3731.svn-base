...
#   pscli --command=add_vip_address_pool --subnet_id=x --domain_name=xxxx.com --vip_addresses=x.x.x.11-20
#   --supported_protocol=NAS --allocation_method=STATIC
#   4、通过pscli --command=get_vip_address_pools查看与配置的信息是否匹配；
#   5、命令行通过IP addr观察VIP是否绑定到3个节点到对应的网卡上（相同网段的网卡）；
#   6、通过命令行修改vip地址池；
#   pscli --command=update_vip_address_pool --id=x --load_balance_policy=LB_CONNECTION_COUNT
...
def executing_case():
    ...
    access_zone_name = 'nas_16_0_3_62_access_zone_name'
    # for create_subnet
    subnet_name = 'nas_16_0_3_62_subnet_name'
    ...
    load_balance_policy = nas_common.BALANCE_LB_CONNECTION_COUNT
