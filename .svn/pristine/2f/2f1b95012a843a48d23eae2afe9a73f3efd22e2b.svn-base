...
def case():
    ...
    """函数主体"""
    log.info("1> 清理环境完成，访问区节点全部被非访问区节点替换")
    ...
    nodes_replace = random.sample(nodes_update, replace_lenth)
    log.info("3> 使用update_access_zone命令选取节点{}完全替换访问区节点{}".format(nodes_replace, node_id_choosen))
    ...
    log.info("4> 检查访问区内所有节点{}的服务状态是否正常".format(node_id_update_param))
