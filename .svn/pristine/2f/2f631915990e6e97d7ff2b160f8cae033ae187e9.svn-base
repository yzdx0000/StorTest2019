...
def up_down():
    ...
    time.sleep(200)
    zk_ip = reliable.get_master_zk(deploy_ips)
    log.info("Step5:检查新主节点zk进程状态,节点: %s." % (zk_ip))
    state1 = ReliableTest.check_process(zk_ip,'zk')
    if state1 == True:
        log.info("新主节点zk进程状态正常,节点: %s" %(zk_ip))
    log.info("Step5:检查故障节点zk进程状态,节点: %s." % (m_zk_ip))
    state2 = ReliableTest.check_process(m_zk_ip,'zk')
    if state2 == True:
        log.info("故障节点zk进程状态正常,节点: %s" %(m_zk_ip))
