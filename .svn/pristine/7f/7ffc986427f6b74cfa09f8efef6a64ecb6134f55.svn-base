...
:Description:
            ...
            6、节点D上电，数据修复中，在主机1上执行vdbench -f mix-S-align.conf -jro比较一致性；
            7、节点D数据修复完成，在主机1上执行vdbench -f mix-S.conf -jro比较一致性；
            8、比较存储内部数据一致性
:Changerlog:
"""
# testlink case: 1000-32472
import os,sys
import commands
import threading
import random
...
def vdb_jn(pipein):
    ...
    log.info("Step7:节点D数据修复完成，在主机1上执行vdbench -f mix-S.conf -jro比较一致性；")
    osan.run_vdb(client_ips[0], mix_S_align, output=deploy_ips[0], jn_jro="jro")
