...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：11-0-2-47 修改目录配额规则之改变配额类型
#######################################################
...
def executing_case():
    ...
    cmd = "cd %s; for i in {2001..7000}; do touch file_$i; done" % (quota_common.QUOTA_PATH)
    rc, stdout = common.run_command(quota_common.CLIENT_IP_1, cmd)
    if rc != 0:
        log.info("rc = %s" % (rc))
        log.info("WARNING: \ncmd = %s\nstdout = %s\n" % (cmd, stdout))
    ...
    # 验证配额功能，已达到配额，无法继续写入
    cmd = "cd %s; for i in {9001..9002}; do touch file_$i; done" % (quota_common.QUOTA_PATH)
    rc, stdout = common.run_command(quota_common.CLIENT_IP_1, cmd)
    if rc != 0:
        log.info("rc = %s" % (rc))
        log.info("WARNING: \ncmd = %s\nstdout = %s\n" % (cmd, stdout))
    ...
    if (check_result1["detail_err_msg"] != "" or
        ...
        rc, check_result = quota_common.get_all_quota_info()
        common.judge_rc(rc, 0, "get quota failed")
        log.error("11-0-2-47 Failed")
        raise Exception("11-0-2-47 Failed")
    else:
        log.info("11-0-2-47 Succeed")
    return
...
class Quota_Class_11_0_2_47():
    def quota_method_11_0_2_47(self):
