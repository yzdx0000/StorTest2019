...
4、步骤5，节点下电后，系统剩余可用容量不变，可预留容量不变，所有卷的可用容量减少，10条精简卷创建成功
5、步骤6，节点恢复后，存储池可用容量恢复，卷可用容量恢复
"""
...
def create_lun_thin(arg=1):
    ...
    if 0.95 * (storage_pool_size - 10 * thick_lun_size - 10 * (
            ...
            storage_pool_size - 10 * thick_lun_size - 10 * (
            int(cp("reserve", "thin_lun_reserve")) * 1024 * (1024 + 20))):
        pass
...
def map_lun():
    ...
    lun_ids = random.sample(env_manage_lun_manage.Reliable_osan.get_thin_lun(node_ip1), 5)
    for i in lun_ids:
