...
#######################################################
# 脚本作者：姜晓光
# 脚本说明：16-0-x-x 修改create_ntfs_acl配置
#######################################################
...
def executing_case():
    """
        1、修改smb共享的allow_create_ntfs_acl配置；
        pscli --command=update_smb_export --id=x --enable_ntfs_acl=true --allow_create_ntfs_acl=true
        pscli --command=update_smb_export --id=x --enable_ntfs_acl=true --allow_create_ntfs_acl=false
    :return:
    """
    ...
    create_file_path = nas_common.ROOT_DIR + "nas_16-0-4-49_dir"
    export_name = "nas_16_0_4_49_smb_export_name"
