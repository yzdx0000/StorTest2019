...
4、在步骤3中的业务运行过程中，将A节点数据网故障，将节点B oRole进程异常；
...
def up_down(pipein):
    ...
    log.info("On node %s kill oRole." % (b_ip,))
    ReliableTest.run_kill_process(b_ip, 'oRole')  # 先杀进程防止在故障一个节点
