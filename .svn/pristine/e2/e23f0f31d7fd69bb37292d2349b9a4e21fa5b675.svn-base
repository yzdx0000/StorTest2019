...
# date 2018-11-22
# @summary：
#    posix客户端权限测试：对目录进行chown操作，检验目录权限；原P200测试用例23-0-8-8。
# @steps:
...
def case():
    ...
    log.info("6> 测试主体：对目录进行chown操作，检验目录权限")
    log.info("6-1> 创建测试目录并设置权限755")
    testdir = 'dir_' + FILE_NAME
    cmd = "ssh %s \"su %s -c 'cd %s;mkdir %s;chmod 755 %s'\" " % (
    ...
    log.info("6-2> 修改目录的属主")
    cmd = "ssh %s \"cd %s;chown %s:%s %s\"" % (
        client_ip, posix_file_location, user_name_list[1], group_list[1], testdir)
    rc = common.command(cmd)
    common.judge_rc(rc, 0, "chown failed")
    log.info("6-3> 分别使用用户%s和%s对目录进行创建子文件操作" %
             (user_name_list[0], user_name_list[1]))
    cmd_u1 = "ssh %s \"su %s -c 'cd %s;touch %s/%s'\"" % (
        client_ip, user_name_list[0], posix_file_location, testdir, 'file_' + user_name_list[0])
    rc = common.command(cmd_u1)
    common.judge_rc_unequal(
        rc, 0, "user %s, touch file success, the acl dosen't take effect" %
        user_name_list[0])
    cmd_u2 = "ssh %s \"su %s -c 'cd %s;touch %s/%s'\"" % (
        client_ip, user_name_list[1], posix_file_location, testdir, 'file_' + user_name_list[1])
    rc = common.command(cmd_u2)
    common.judge_rc(
        rc,
        0,
        "user %s, touch file failed, the acl dosen't take effect" %
        user_name_list[1])
