...
# date 2018-11-19
# @summary：
# x-x-x-x     删除访问分区（被导出协议引用，NAS启动状态下）
# @steps:
# 1.创建ad，ldap，ldap-pdc，nis认证
# 2.随机选取认证服务创建访问分区，随机选取NFS,FTP,SMB中的服务启动NAS
# 3.如果协议中有NFS，创建NFS导出并验证功能；如果协议中有SMB, FTP，创建相应的导出
# 4.删除访问分区
# @changelog：
#
#######################################################
...
def executing_case1():
    ...
    for i in range(1, run_times + 1):
        ...
        if "NFS" in protocol_types:
            nfs_export(access_zone_id=access_zone_id)
        if "FTP" in protocol_types:
            ftp_export(access_zone_id=access_zone_id)
        if "SMB" in protocol_types:
            smb_export(access_zone_id=access_zone_id)
        ...
        if msg1["err_msg"] != "ILLEGAL_OPERATION" and msg1["detail_err_msg"].find("exports") == -1:
            common.except_exit("delete_access_zone Failed")
    return


def nfs_export(access_zone_id):
    log.info("\t[ 创建nfs导出 ]")
    """创建目录"""
...
def ftp_export(access_zone_id):
    log.info("\t[ 创建ftp导出 ]")
    """创建目录"""
    log.info("\t[ create_file ]")
    ftp_path = nas_common.ROOT_DIR + "ftp_dir_%s" % access_zone_id
...
def smb_export(access_zone_id):
    log.info("\t[ 创建smb导出 ]")
    """创建目录"""
    log.info("\t[ create_file ]")
    smb_path = nas_common.ROOT_DIR + "smb_dir_%s" % access_zone_id
    ...
    export_name = "smb_export_%s" % access_zone_id
