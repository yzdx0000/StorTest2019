...
#    4、通过nfs客户端先写入1000M文件，2分钟内再写入1000M文件；宽限时间内再次尝试写入，检查配额是否生效
#    5、通过nfs客户端先写入1000M文件，2分钟内再写入500M文件；超过宽限时间后再次尝试写入，检查配额是否生效
#    6、删除配额及所创建的文件
...
def case():
    ...
    '''4> 通过nfs客户端先写入1000M文件,2分钟内再写入1000M文件；随后宽限时间内继续尝试写入'''
    for i in range(10):
        file_name = 'test' + str(i)
        file_path = os.path.join(local_path, file_name)
        cmd = 'dd if=%s of=%s bs=1M count=100' % (snap_common.get_system_disk(auth_name_ip), file_path)
        common.run_command(auth_name_ip, cmd)

    for i in range(10):
        file_name = 'test_include' + str(i)
        file_path = os.path.join(local_path, file_name)
        cmd = 'dd if=%s of=%s bs=1M count=100' % (snap_common.get_system_disk(auth_name_ip), file_path)
        common.run_command(auth_name_ip, cmd)
    ...
    cmd = 'dd if=%s of=%s bs=1M count=100' % (snap_common.get_system_disk(auth_name_ip), file_create_again_path)
    rc, stdout = common.run_command(auth_name_ip, cmd)
    common.judge_rc_unequal(rc, 0, 'quota did not work well !!!')

    """清理配额及文件"""
    deleted_file_name = os.path.join(local_path, 'test*')
    common.rm_exe(common.SYSTEM_IP, deleted_file_name)

    rc, stdout = quota_common.delete_one_quota(quota_id)
    common.judge_rc(rc, 0, "delete quota failed")

    '''5> 通过nfs客户端先写入1000M文件，2分钟内再写入500M文件；超过2分钟后再次尝试写入'''
    rc, check_result = quota_common.create_one_quota(path=quota_path,
                                                     logical_quota_cal_type='QUOTA_LIMIT',
                                                     logical_suggest_threshold=524288000,
                                                     logical_soft_threshold=1048576000,
    ...
    for i in range(5):
        ...
        rc, stdout = common.run_command(auth_name_ip, cmd)
        if rc != 0:
            log.error('soft_threshold quota did not work well !!!')
            raise Exception('soft_threshold quota did not work well !!!')
