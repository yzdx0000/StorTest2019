...
# -*- coding:utf-8 -*-
#**********************************************#
# 标题：功能测试，用例编号29-0-17-24
# 使用方法：python 29-0-17-24_wjl.py
# 在http模式下，设置桶的acl，关闭验证开关，原指令校验认证。日志打印在屏幕上。
# 作者：王建磊
# 创建时间：2018/03/20
#**********************************************#
import sys
...
# 完全权限验证
    ...
    log.info("7> 普通方式验证完全权限")
    log.info("******验证完全权限，使用ak2验证写权限,start.******")
    create_file(FILE_NAME, 4)
    rc, opt_normal1 = s3_common.add_object(
        bucket_name1, object_name1 + "www", FILE_NAME, ak2)
    common.run_command_shot_time("rm %s" % FILE_NAME)
    log.info("******验证完全权限，使用ak2验证写权限,end.******")

    log.info("******验证完全权限，使用ak2验证读ACP权限,start.******")
    rc, opt_normal2 = s3_common.get_bucket_acl_info(bucket_name1, ak2)
    log.info("******验证完全权限，使用ak2验证读ACP权限,start.******")

    if "403" in opt_normal1 and "403" in opt_normal2:
        pass
    else:
