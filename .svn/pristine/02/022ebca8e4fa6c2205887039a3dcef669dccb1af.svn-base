...
def run_kill():
    rc = env_manage.com_lh.get_cmd_status(node_ip1, cmd_name="create_access_zone")
    if rc == 0:
...
def case():
    log.info("step:1.创建访问区过程中kill 进程")
    threads = []
    t1 = threading.Thread(target=run_kill)
    threads.append(t1)
    t2 = threading.Thread(target=env_manage.create_access, args=(node_ip1, '1,2,3', "accesszone1"))
    threads.append(t2)
    for t in threads:
        t.start()
    for t in threads:
