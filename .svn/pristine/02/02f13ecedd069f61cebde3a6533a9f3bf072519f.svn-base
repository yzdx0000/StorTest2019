...
#   pscli --command=add_vip_address_pool --subnet_id=x --domain_name=xxxx.com
#   --vip_addresses=x.x.x.11-20 --supported_protocol=NAS --allocation_method=STATIC
#   4、通过pscli --command=get_vip_address_pools查看与配置的信息是否匹配；
#   5、命令行通过IP addr观察VIP是否绑定到3个节点到对应的网卡上（相同网段的网卡）；
#   6、输入不存在的id；
#   pscli --command=delete_vip_address_pool --id=x
#   7、通过pscli --command=get_vip_address_pools查看是否删除；
# @changelog：
#   None
######################################################
...
def executing_case():
    ...
    access_zone_name = 'nas_16_0_3_71_access_zone_name'
    # for create_subnet
    subnet_name = 'nas_16_0_3_71_subnet_name'
    ...
    # for delete_vip_address_pool
    not_exist_vip_address_pool_id = 250
    # for delete_subnet
    not_exist_subnet_id = 250
    ...
    """删除vip地址池"""
    msg = nas_common.delete_vip_address_pool(vip_address_pool_id=not_exist_vip_address_pool_id)
    if msg['detail_err_msg'].find('Can not find ip address pool') == -1:
        raise Exception('%s Failed' % FILE_NAME)

    """删除子网"""
    msg = nas_common.delete_subnet(subnet_id=not_exist_subnet_id)
    if msg['detail_err_msg'].find('Can not find subnet by id') == -1:
        raise Exception('%s Failed' % FILE_NAME)
