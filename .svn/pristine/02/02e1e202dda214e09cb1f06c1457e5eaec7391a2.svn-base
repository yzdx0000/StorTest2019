...
# date 2018-07-26
# @summary：
#    创建快照失败（等待get_events接口完成）
# @steps:
#   1、部署3节点集群环境
#   2、在/mnt/volume1/snap/snap_create_success/目录下创建数据
#   3、对不存在的目录/mnt/volume1/snap/snap_create_success/data/创建快照（预期失败）
...
def case():
    ...
    ''' 3> 对不存在的目录创建快照（参数输入错误，预期失败）'''
    snap_name = FILE_NAME + '_snapshot1'
    path = snap_common.VOLUME_NAME + ':/' + os.path.join(CREATE_EVENT_PATH, 'not_exist')   # path不存在，快照创建失败
    rc, stdout = snap_common.create_snapshot(snap_name, path)
    if rc != 0:
        log.info('snapshot %s create failed !!!' % snap_name)
