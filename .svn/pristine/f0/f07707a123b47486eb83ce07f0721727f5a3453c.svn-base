...
#    nfs目录inode数配额软阈值
# @steps:
#    1、创建ad访问服务器，创建访问分区，启动NAS
#    2、创建共享目录，导出并授权某个用户的导出权限
#    3、针对目录配置inode数为100的软阈值配额，宽限时间为2分钟
#    4、通过nfs客户端先写入100个文件后开始计时，2分钟之内再次写入100个文件
#    5、2分钟后尝试继续写入文件，检查配额是否生效
#    6、删除配额及所创建的文件
...
NAS_TRUE_PATH = os.path.join(nas_common.NAS_PATH, FILE_NAME)             # /mnt/volume/nas_test_dir/nas_16_6_0_17
...
def case():
    ...
    '''3> 针对根目录配置inode数为100的软阈值配额，宽限时间为2分钟'''
    ...
    '''4> 通过nfs客户端先写入100个文件,2分钟内再写入100个文件'''
    """首次写入100个文件"""
    for i in range(100):
