...
fault_ip = env_manage_lun_manage.Reliable_osan.get_master_oRole(env_manage_repair_test.deploy_ips[0])  # 获取oPmgr主进程节点ip
env_manage_repair_test.deploy_ips.remove(fault_ip)
...
fault_node_id3 = env_manage_repair_test.break_down.get_node_id_by_ip(n_ip=fault_ip)
...
def process_thread(arg=1):
    log.info('Run task %s (%s)...' % (sys._getframe().f_code.co_name, os.getpid()))
    decorator_func.timer(230, 25, log_interval=0)
...
def case():
    ...
    iscsi_login()
    decorator_func.multi_threads(run_vdb1, run_vdb2, os_error,process_thread)
