...
:Description:
            ...
            6、节点D上电恢复后，日志迁移到节点D过程中时，节点B重启；
            7、节点B上电恢复，数据修复过程中在主机1上执行vdbench -f mix-R-align.conf -jro比较一致性；
            8、节点A上电恢复，数据修复完成后，比较存储内部数据一致性。
:Changerlog:
"""
# testlink case: 1000-32327
import os,sys
import commands
import threading
import random
...
def up_down(pipeout):
    ...
    #将free日志节点D重启
    log.info("Step5:将free日志节点D重启 : %s." % (fault_node_d_ip))
    tmp_d = error.down_node(fault_ip=fault_node_d_ip)
    tmp_d = re.sub('\r','',tmp_d)
    #将free日志节点D上电
    time.sleep(40)
    error.up_node(node_info=tmp_d)
    common.check_ip(fault_node_d_ip)
    error.check_host(fault_node_d_ip)
    log.info("Step6:将节点B重启 : %s." % (fault_node_b_ip))
    tmp_b = error.down_node(fault_ip=fault_node_b_ip)
    tmp_b = re.sub('\r','',tmp_b)
    #将日志节点B上电
    time.sleep(40)
    error.up_node(node_info=tmp_b)
    common.check_ip(fault_node_b_ip)
    error.check_host(fault_node_b_ip)
