...
for share_disk in share_disk:
    disk_phy_id = env_manage_repair_test.Reliable_osan.get_physicalid_by_name(fault_node_ip, share_disk)
...
def up_down():
    time.sleep(100)
    log.info("Step4:业务过程中将存储中的一块共享磁盘拔出,节点:%s,磁盘:%s" % (fault_node_ip, disk_name))
    env_manage_repair_test.Reliable_osan.remove_disk(fault_node_ip, disk_phy_id, "SHARED")
    time.sleep(100)
    log.info("Step5:数据修复过程中,将两个oStor进程反复故障,节点:%s" % (fault_node_ip))
    for count in range(3):
        log.info("第 %d 次故障两个节点的oStor进程" % (count + 1))
        log.info("Step5:将故障节点oStor进程杀死,节点: %s,%s." %(fault_node_ip, fault_node_ip1))
        ReliableTest.run_kill_process(fault_node_ip, 'oStor')
        ReliableTest.run_kill_process(fault_node_ip1, 'oStor')
        time.sleep(100)
        while True:
            log.info("Step6:检查故障节点oStor进程状态,节点: %s,%s." % (fault_node_ip,fault_node_ip1))
            state = reliable.check_process_stat(fault_node_ip,'oStor')
            state1 = reliable.check_process_stat(fault_node_ip1, 'oStor')
            if state == 'OK' and state1 == 'OK':
                break
            time.sleep(20)

    time.sleep(100)
    log.info("Step7:将拔出的盘在超时内恢复,节点:%s,磁盘:%s" % (fault_node_ip, disk_name))
