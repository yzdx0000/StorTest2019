...
# date 2018-12-6
# @summary：
#    bucket创建的同时，创建bucket,(两条一样的命令同时发送)
# @steps:
#    1、创建账户；
#    2、检查账户是否存在
#    3、创建证书；
...
def case():
    ...
    threads = [
        MyThread(s3_common.add_bucket, args=(bucket_name, certificate_id,))
        for i in range(10)
    ]
    # thread1 = MyThread(s3_common.add_bucket, args=(bucket_name, certificate_id, ))
    # thread2 = MyThread(s3_common.add_bucket, args=(bucket_name, certificate_id, ))
    #
    ...
    log.info("create same bucket at same time!")
