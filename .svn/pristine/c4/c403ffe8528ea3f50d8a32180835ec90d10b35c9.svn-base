...
def case():
    ...
    [env_manage_lun_manage.osan.create_host_group(node_ip1, "hg_name_{}".format(i)) for i in range(10)]
    for i in env_manage_lun_manage.osan.get_host_groups(node_ip1):
        env_manage_lun_manage.osan.add_host(node_ip1, "h_name{}".format(i), i)
        env_manage_lun_manage.osan.add_host(node_ip1, "h_name{}".format(i + 20), i)

    [env_manage_lun_manage.osan.update_host_group(node_ip1, hg_id, name="hg_name_update{}".format(hg_id))
     for hg_id in random.sample(env_manage_lun_manage.osan.get_host_groups(node_ip1), 5)]

    msg = env_manage_lun_manage.osan.delete_host_groups(node_ip1,
                                                        env_manage_lun_manage.osan.get_host_groups(node_ip1)[-1],
                                                        need_judge=1)
